...
class IPAChangeConf:
    ...
    def getSectionLine(self, section):
        ...
        return self._dump_line(
            self.sectnamdel[0], section, self.sectnamdel[1], self.deol
        )
    ...
    def dump(self, options, level=0):
        ...
        for o in options:
            if o["type"] == "section":
                output.append(
                    self._dump_line(self.sectnamdel[0], o["name"], self.sectnamdel[1])
                )
            ...
            if o["type"] == "subsection":
                output.append(
                    self._dump_line(
                        self.indent[level], o["name"], self.dassign, self.subsectdel[0]
                    )
                )
                output.append(self.dump(o["value"], (level + 1)))
                output.append(self._dump_line(self.indent[level], self.subsectdel[1]))
                continue
            if o["type"] == "option":
                ...
                output.append(
                    self._dump_line(self.indent[level], o["name"], delim, o["value"])
                )
                continue
            if o["type"] == "comment":
    ...
    def parseLine(self, line):
        ...
        o.update(
            {"name": parts[0].strip(), "type": "option", "value": parts[1].rstrip()}
        )
        return o
    ...
    def findOpts(self, opts, type, name, exclude_sections=False):
        ...
        for o in opts:
            if o["type"] == type and o["name"] == name:
                return (num, o)
            if exclude_sections and (
                o["type"] == "section" or o["type"] == "subsection"
            ):
                return (num, None)
            num += 1
        return (num, None)
    ...
    def commentOpts(self, inopts, level=0):
        ...
        for o in inopts:
            if o["type"] == "section":
                ...
                val = self._dump_line(
                    self.dcomment, self.sectnamdel[0], o["name"], self.sectnamdel[1]
                )
            ...
            if o["type"] == "option":
                ...
                if delim not in self.assign:
                    val = self._dump_line(
                        self.indent[level], o["name"], delim, o["value"]
                    )
                opts.append({"name": "comment", "type": "comment", "value": val})
                continue
            if o["type"] == "comment":
        ...
        for o in oldopts:
            ...
            if o["type"] == "option":
                ...
                if no["action"] == "comment" or no["action"] == "remove":
                    if no["value"] is not None and o["value"] is not no["value"]:
                        opts.append(o)
                        continue
                    if no["action"] == "comment":
                        value = self._dump_line(
                            self.dcomment, o["name"], self.dassign, o["value"]
                        )
                        opts.append(
                            {"name": "comment", "type": "comment", "value": value}
                        )
                    continue
                if no["action"] == "set":
                    opts.append(no)
    ...
    def parse(self, f):
        ...
        for line in f:
            ...
            if value:
                ...
                fatheropts.append(
                    {"name": subsection, "type": "subsection", "value": subsectopts}
                )
                subsection = None
                curopts = fatheropts
                continue
    ...
    def setOption(name, value):
        return {"name": name, "type": "option", "action": "set", "value": value}

    @staticmethod
    def rmOption(name):
        return {"name": name, "type": "option", "action": "remove", "value": None}

    @staticmethod
    def setSection(name, options):
        return {"name": name, "type": "section", "action": "set", "value": options}
