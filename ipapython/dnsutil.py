...
class DNSZoneAlreadyExists(dns.exception.DNSException):
    supp_kwargs = {"zone", "ns"}
    fmt = u"DNS zone {zone} already exists in DNS " "and is handled by server(s): {ns}"
...
class DNSName(dns.name.Name):
    ...
    def choose_relativity(self, origin=None, relativize=True):
        return DNSName(
            super(DNSName, self).choose_relativity(origin=origin, relativize=relativize)
        )
...
def assert_absolute_dnsname(name):
    ...
    assert isinstance(
        name, DNSName
    ), "name must be DNSName instance, " "got '%s'" % type(name)
...
def has_empty_zone_addresses(hostname):
    ...
    return any(inside_auto_empty_zone(DNSName(ip.reverse_dns)) for ip in ip_addresses)
...
def check_zone_overlap(zone, raise_on_error=True):
    ...
    if containing_zone == zone:
        try:
            ...
            logger.debug("Failed to resolve nameserver(s) for domain %s: %s", zone, e)
            ns = []
