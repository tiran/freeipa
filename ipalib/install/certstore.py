...
def clean_old_config(ldap, base_dn, dn, config_ipa, config_compat):
    ...
    try:
        result, _truncated = ldap.find_entries(
            base_dn=DN(("cn", "certificates"), ("cn", "ipa"), ("cn", "etc"), base_dn),
...
def add_ca_cert(
    ...
    """
    container_dn = DN(("cn", "certificates"), ("cn", "ipa"), ("cn", "etc"), base_dn)
...
def get_ca_certs(ldap, base_dn, compat_realm, compat_ipa_ca, filter_subject=None):
    """
    ...
    if filter_subject is not None:
        ...
        filter_subject = [str(subj).replace("\\;", "\\3b") for subj in filter_subject]
...
def put_ca_cert_nss(
    ldap, base_dn, cert, nickname, trust_flags, config_ipa=False, config_compat=False
):
    """
    Add or update entry for a CA certificate in the certificate store.
    ...
    put_ca_cert(
        ldap, base_dn, cert, nickname, trusted, ext_key_usage, config_ipa, config_compat
    )


def get_ca_certs_nss(ldap, base_dn, compat_realm, compat_ipa_ca, filter_subject=None):
    """
    ...
    certs = get_ca_certs(
        ldap, base_dn, compat_realm, compat_ipa_ca, filter_subject=filter_subject
    )
