...
class _SchemaPlugin:
    ...
    def _create_default_from(self, api, name, keys):
        ...
        def get_default(*args):
            ...
            result = api.Command.command_defaults(
                unicode(cmd_name), params=[unicode(name)], kw=kw,
            )["result"]
    ...
    def _create_param(self, api, schema):
        ...
        for key, value in schema.items():
            if key in (
                ...
                kwargs[key] = value
            elif key in ("cli_metavar", "cli_name"):
                ...
                kwargs["default_from"] = self._create_default_from(api, name, keys)
    ...
    def _create_class(self, api, schema):
        ...
        class_dict["takes_params"] = tuple(
            self._create_param(api, s) for s in schema.get("params", [])
        )
...
class _SchemaCommandPlugin(_SchemaPlugin):
    ...
    def _create_class(self, api, schema):
        name, bases, class_dict = super(_SchemaCommandPlugin, self)._create_class(
            api, schema
        )
...
class Schema:
        ...
        for namespace in ("commands", "topics"):
            ...
            for member_schema in schema[namespace].values():
                ...
                if "doc" in member_schema:
                    topic["summary"] = member_schema["doc"].split("\n\n", 1)[0].strip()
    ...
    def _write_schema(self, fingerprint):
        ...
        with tempfile.NamedTemporaryFile(
            "wb", prefix=fingerprint, dir=self._DIR, delete=False
        ) as f:
            try:
    ...
    def _write_schema_data(self, fileobj):
        with zipfile.ZipFile(fileobj, "w", zipfile.ZIP_DEFLATED) as schema:
            ...
            schema.writestr(
                "_help", json.dumps(self._help, default=json_default).encode("utf-8")
            )
