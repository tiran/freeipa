...
class cert_request(CertRetrieveOverride):
    takes_options = CertRetrieveOverride.takes_options + (
        ...
        Str(
            "password_file?",
            label=_("File containing a password for the private key or database"),
        ),
        Str(
            "csr_profile_id?",
            label=_(
                "Name of CSR generation profile (if not the same as" " profile_id)"
            ),
        ),
    )
        ...
        for option in super(cert_show, self).get_options():
            ...
            if option.name == "certificate_out":
                ...
                option = option.clone_rename(
                    "out", cli_name="certificate_out", deprecated_cli_aliases={"out"},
                )
            yield option
...
class cert_remove_hold(MethodOverride):
    has_output_params = (
        Flag("unrevoked", label=_("Unrevoked"),),
        Str("error_string", label=_("Error"),),
    )
