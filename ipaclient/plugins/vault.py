...
def encrypt(data, symmetric_key=None, public_key=None):
    ...
    if symmetric_key is not None:
        ...
        public_key_obj = load_pem_public_key(data=public_key, backend=default_backend())
...
def decrypt(data, symmetric_key=None, private_key=None):
    ...
    if symmetric_key is not None:
        ...
        try:
            ...
            raise errors.AuthenticationError(message=_("Invalid credentials"))
...
class vault_add(Local):
    ...
    takes_options = (
        Str("password?", cli_name="password", doc=_("Vault password"),),
    ...
    def __NO_CLI_getter(cls):
        return api.Command.get_plugin("vault_add_internal") is _fake_vault_add_internal
    ...
    def forward(self, *args, **options):
        ...
        if vault_type != u"symmetric" and (password or password_file):
            raise errors.MutuallyExclusiveError(
                reason=_("Password can be specified only for " "symmetric vault")
            )

        if vault_type != u"asymmetric" and (public_key or public_key_file):
            raise errors.MutuallyExclusiveError(
                reason=_("Public key can be specified only for " "asymmetric vault")
            )
        ...
        if vault_type == u"standard":
            ...
            if password and password_file:
                ...
                password = validated_read(
                    "password-file", password_file, encoding="utf-8"
                )
                password = password.rstrip("\n")

            else:
                password = self.api.Backend.textui.prompt_password("New password")
            ...
            if public_key and public_key_file:
                ...
                public_key = validated_read(
                    "public-key-file", public_key_file, mode="rb"
                )
                ...
                raise errors.ValidationError(
                    name="ipavaultpublickey", error=_("Missing vault public key")
                )
...
class vault_mod(Local):
    ...
    takes_options = (
        Flag("change_password?", doc=_("Change password"),),
        Str("old_password?", cli_name="old_password", doc=_("Old vault password"),),
        Str(  # TODO: use File parameter
            "old_password_file?",
            cli_name="old_password_file",
            doc=_("File containing the old vault password"),
        ),
        Str("new_password?", cli_name="new_password", doc=_("New vault password"),),
        Str(  # TODO: use File parameter
            "new_password_file?",
            cli_name="new_password_file",
            doc=_("File containing the new vault password"),
        ),
        Bytes("private_key?", cli_name="private_key", doc=_("Old vault private key"),),
        Str(  # TODO: use File parameter
            "private_key_file?",
    ...
    @classmethod
    def __NO_CLI_getter(cls):
        return api.Command.get_plugin("vault_mod_internal") is _fake_vault_mod_internal
        ...
        if vault_type:
            ...
            if vault_type == u"standard":
                ...
                if new_public_key and new_public_key_file:
                    ...
                    new_public_key = validated_read(
                        "public_key_file", new_public_key_file, mode="rb"
                    )
...
class _TransportCertCache:
    def __init__(self):
        self._dirname = os.path.join(USER_CACHE_PATH, "ipa", "kra-transport-certs")
    ...
    def store_cert(self, domain, transport_cert):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                dir=self._dirname, delete=False, mode="wb"
            ) as f:
                try:
                    f.write(pem)
...
class vaultconfig_show(MethodOverride):
    def forward(self, *args, **options):
        ...
        _transport_cert_cache.store_cert(self.api.env.domain, transport_cert)
...
class ModVaultData(Local):
    ...
    def _do_internal(self, algo, transport_cert, raise_unexpected, *args, **options):
        public_key = transport_cert.public_key()

        # wrap session key with transport certificate
        wrapped_session_key = public_key.encrypt(algo.key, padding.PKCS1v15())
        ...
        except (errors.InternalError, errors.ExecutionError, errors.GenericError):
    ...
    def internal(self, algo, *args, **options):
        ...
        if transport_cert is not None:
            result = self._do_internal(algo, transport_cert, False, *args, **options)
...
class vault_archive(ModVaultData):
    ...
    takes_options = (
        Bytes("data?", doc=_("Binary data to archive"),),
        Str(  # TODO: use File parameter
            "in?", doc=_("File containing data to archive"),
        ),
        Str("password?", cli_name="password", doc=_("Vault password"),),
        Str(  # TODO: use File parameter
            "password_file?",
        ...
        Flag("override_password?", doc=_("Override existing password"),),
    )

    @classmethod
    def __NO_CLI_getter(cls):
        return (
            api.Command.get_plugin("vault_archive_internal")
            is _fake_vault_archive_internal
        )
    ...
    def get_options(self):
        for option in self.api.Command.vault_archive_internal.options():
            if option.name not in ("nonce", "session_key", "vault_data", "version"):
                yield option
    ...
    def forward(self, *args, **options):
        ...
        if vault_type == u"standard":
            ...
            raise errors.ValidationError(
                name="vault_type", error=_("Invalid vault type")
            )

        vault_data = {"data": base64.b64encode(data).decode("utf-8")}
        if encrypted_key:
            vault_data[u"encrypted_key"] = base64.b64encode(encrypted_key).decode(
                "utf-8"
            )
...
class vault_retrieve(ModVaultData):
    ...
    takes_options = (
        ...
        Bytes("private_key?", cli_name="private_key", doc=_("Vault private key"),),
        Str(  # TODO: use File parameter
            "private_key_file?",
            cli_name="private_key_file",
    ...
    has_output_params = (Bytes("data", label=_("Data"),),)

    @classmethod
    def __NO_CLI_getter(cls):
        return (
            api.Command.get_plugin("vault_retrieve_internal")
            is _fake_vault_retrieve_internal
        )
    ...
    def forward(self, *args, **options):
        ...
        vault_data = self._unwrap_response(
            algo, response["result"]["nonce"], response["result"]["vault_data"]
        )
        del algo
        ...
        if vault_type == u"standard":
            ...
            if private_key and private_key_file:
                ...
                private_key = validated_read(
                    "private-key-file", private_key_file, mode="rb"
                )
