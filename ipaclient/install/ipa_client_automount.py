...
def parse_options():
    ...
    parser.add_option(
        "--location", dest="location", default="default", help="Automount location",
    )
    parser.add_option(
        "-S",
        "--no-sssd",
...
def configure_xml(fstore):
    ...
    try:
        ...
        tree = etree.ElementTree(element=etree.Element("autofs_ldap_sasl_conf"))
...
def configure_autofs_sssd(fstore, statestore, autodiscover, options):
    ...
    try:
        ...
        logger.info("Configure autofs support manually in /etc/sssd/sssd.conf.")
...
def configure_autofs(fstore, statestore, autodiscover, server, options):
    ...
    search_base = str(
        DN(("cn", options.location), api.env.container_automount, api.env.basedn,)
    )
    replacevars = {
    ...
    if statestore.has_state("autofs"):
        ...
        if sssd:
            try:
                ...
                logger.debug("Unable to restore SSSD configuration: %s", str(e))
