...
def remove_file(filename):
    ...
    try:
        ...
        logger.error(
            "Please remove %s manually, as it can cause "
            "subsequent installation to fail.",
            filename,
        )
...
def restore_state(service, statestore):
    ...
    if running:
        try:
            ...
            logger.warning("Failed to restart the %s daemon", service.service_name)
    ...
    files_to_check = [
        {
            ...
            "mandatory": [paths.LDAP_CONF, paths.NSS_LDAP_CONF, paths.LIBNSS_LDAP_CONF],
...
def check_ldap_conf(conf=paths.OPENLDAP_LDAP_CONF, error_rval=CLIENT_INSTALL_ERROR):
...
def configure_nsswitch_database(
    fstore, database, services, preserve=True, append=True, default_value=()
):
    """
    ...
    return tasks.configure_nsswitch_database(
        fstore, database, services, preserve, append, default_value
    )
...
def configure_ipa_conf(fstore, cli_basedn, cli_realm, cli_domain, cli_server, hostname):
    ...
    defopts = [
        ...
        ipaconf.setOption(
            "xmlrpc_uri",
            "https://{}/ipa/xml".format(ipautil.format_netloc(cli_server[0])),
        ),
        ipaconf.setOption("enable_ra", "True"),
    ]

    opts.extend(
        [ipaconf.setSection("global", defopts), ipaconf.emptyLine(),]
    )
...
def configure_ldap_conf(
    fstore, cli_basedn, cli_realm, cli_domain, cli_server, dnsok, options, files
):
    ...
    opts = [
        ...
        ldapconf.setOption(
            "nss_base_passwd",
            "{dn}{suffix}".format(
                dn=DN(("cn", "users"), ("cn", "accounts"), cli_basedn), suffix="?sub"
            ),
        ),
        ldapconf.setOption(
            "nss_base_group",
            "{dn}{suffix}".format(
                dn=DN(("cn", "groups"), ("cn", "accounts"), cli_basedn), suffix="?sub"
            ),
        ),
...
def configure_nslcd_conf(
    ...
    opts = [
        ...
        nslcdconf.emptyLine(),
        nslcdconf.setOption(
            "base passwd", str(DN(("cn", "users"), ("cn", "accounts"), cli_basedn))
        ),
        nslcdconf.setOption(
            "base group", str(DN(("cn", "groups"), ("cn", "accounts"), cli_basedn))
        ),
    ...
    if nslcd.is_installed():
        ...
        logger.debug(
            "%s daemon is not installed, skip configuration", nslcd.service_name
        )
        return (0, None, None)
...
def configure_openldap_conf(fstore, cli_basedn, cli_server):
    ...
    opts = [
        ...
        ldapconf.emptyLine(),
        {
            "name": "comment",
            "type": "comment",
            "value": " We do not want to break your existing configuration, " "hence:",
        },
        ...
        },
        {
            "name": "comment",
            "type": "comment",
            "value": '   "# CONF_NAME modified by IPA" added to the line ' "above.",
        },
        {
            "name": "comment",
            "type": "comment",
        ...
        },
        {
            "name": "comment",
            "type": "comment",
            "value": " existing configuration for these options and " "uncomment the",
        },
        {
            "name": "comment",
            "type": "comment",
...
def hardcode_ldap_server(cli_server):
    ...
    logger.info(
        "Changed configuration of /etc/ldap.conf to use " "hardcoded server name: %s",
        cli_server[0],
    )
...
def configure_krb5_conf(
    ...
    if kernel_keyring.is_persistent_keyring_supported():
        ...
        libopts.append(
            krbconf.setOption("default_ccache_name", "KEYRING:persistent:%{uid}")
        )

    opts.extend([krbconf.setSection("libdefaults", libopts), krbconf.emptyLine()])
        ...
        for server in cli_server:
            kropts.extend(
                [
                    ...
                    krbconf.setOption("master_kdc", ipautil.format_netloc(server, 88)),
                    krbconf.setOption(
                        "admin_server", ipautil.format_netloc(server, 749)
                    ),
                    krbconf.setOption(
                        "kpasswd_server", ipautil.format_netloc(server, 464)
                    ),
                ]
            )
    ...
    kropts.append(
        krbconf.setOption("pkinit_anchors", "FILE:%s" % paths.KDC_CA_BUNDLE_PEM)
    )
    kropts.append(krbconf.setOption("pkinit_pool", "FILE:%s" % paths.CA_BUNDLE_PEM))
    ...
    opts.extend([krbconf.setSection("domain_realm", dropts), krbconf.emptyLine()])
...
def configure_certmonger(
    ...
    if not ca_enabled:
        logger.warning(
            "An RA is not configured on the server. " "Not requesting host certificate."
        )
        return
    ...
    try:
        ...
        logger.warning("Automatic certificate management will not be available")
...
def configure_sssd_conf(
    fstore, cli_realm, cli_domain, cli_server, options, client_domain, client_hostname
):
    try:
        sssdconfig = SSSDConfig.SSSDConfig()
        sssdconfig.import_config()
        ...
        if os.path.exists(paths.SSSD_CONF) and options.preserve_sssd:
            ...
            logger.error("SSSD config exists but cannot be parsed: %s", str(e))
            logger.error("Was instructed to preserve existing SSSD config")
            logger.info(
                "Correct errors in /etc/sssd/sssd.conf and re-run " "installation"
            )
            return 1
    ...
    if (options.conf_ssh and os.path.isfile(paths.SSH_CONFIG)) or (
        ...
        try:
            ...
            logger.error("Unable to activate the SSH service in SSSD config.")
            logger.info(
                "Please make sure you have SSSD built with SSH support " "installed."
            )
            logger.info("Configure SSH support manually in /etc/sssd/sssd.conf.")
    ...
    if options.conf_sudo:
        ...
        try:
            ...
            logger.error("Unable to activate the SUDO service in SSSD config.")
...
def sssd_enable_service(sssdconfig, name):
    try:
        ...
        logger.error("Unable to activate the '%s' service in SSSD config.", name)
        logger.info(
            "Please make sure you have SSSD built with %s support " "installed.", name
        )
        logger.info("Configure %s support manually in /etc/sssd/sssd.conf.", name)
        return None
...
def configure_automount(options):
    ...
    args = [paths.IPA_CLIENT_AUTOMOUNT, "--debug", "-U", "--location", options.location]
...
def configure_nisdomain(options, domain, statestore):
    ...
    if os.path.exists(paths.BIN_NISDOMAINNAME):
        try:
            result = ipautil.run([paths.BIN_NISDOMAINNAME], capture_output=True)
    ...
    statestore.backup_state(
        "domainname", "enabled", services.knownservices.domainname.is_enabled()
    )
...
def verify_dns_update(fqdn, ips):
    ...
    for record_type in [dns.rdatatype.A, dns.rdatatype.AAAA]:
        logger.debug(
            "DNS resolver: Query: %s IN %s", fqdn, dns.rdatatype.to_text(record_type)
        )
        try:
    ...
    if missing_ips:
        logger.warning(
            "Missing A/AAAA record(s) for host %s: %s.", fqdn, ", ".join(missing_ips)
        )
    if extra_ips:
        logger.warning(
            "Extra A/AAAA record(s) for host %s: %s.", fqdn, ", ".join(extra_ips)
        )
    if missing_reverse:
        logger.warning(
            "Missing reverse record(s) for address(es): %s.", ", ".join(missing_reverse)
        )
    if wrong_reverse:
        ...
        for ip in wrong_reverse:
            for target in wrong_reverse[ip]:
                logger.warning(
                    "%s is pointing to %s instead of %s", ip, target, fqdn_name
                )
...
def client_dns(server, hostname, options):
    ...
    try:
        ...
        logger.warning("Hostname (%s) does not have A/AAAA record.", hostname)
        dns_ok = False
    except errors.DNSResolverError as ex:
        logger.warning("DNS resolution for hostname %s failed: %s", hostname, ex)
        dns_ok = False

    if (
        options.dns_updates
        or options.all_ip_addresses
        or options.ip_addresses
        or not dns_ok
    ):
...
def get_ca_certs_from_http(url, warn=True):
    ...
    if warn:
        logger.warning("Downloading the CA certificate via HTTP, " "this is INSECURE")
...
def validate_new_ca_certs(existing_ca_certs, new_ca_certs, ask, override=False):
    if existing_ca_certs is None:
        logger.info("%s", cert_summary("Successfully retrieved CA cert", new_ca_certs))
        return
    ...
    if existing_ca_certs > new_ca_certs:
        logger.warning(
            ...
            "local certificate available at %s",
            paths.IPA_CA_CRT,
        )
        logger.warning("%s", cert_summary("Existing CA cert:", existing_ca_certs))
        logger.warning("%s", cert_summary("Retrieved CA cert:", new_ca_certs))
        if override:
            ...
            "certificate retrieved from the IPA server?",
            True,
        ):
            raise errors.CertificateInvalidError(name="Retrieved CA")
    else:
        logger.debug("Existing CA cert and Retrieved CA cert are identical")
...
def get_ca_certs(fstore, options, server, basedn, realm):
    ...
    def http_url():
        return urlunparse(
            ("http", ipautil.format_netloc(server), "/ipa/config/ca.crt", "", "", "")
        )
    ...
    if options.ca_cert_file:
        ...
        if os.path.exists(paths.IPA_CA_CRT):
            if os.path.isfile(paths.IPA_CA_CRT):
                ...
                raise errors.FileError(
                    reason=u"Existing ca cert '%s' is "
        ...
        if otp_auth:
            if existing_ca_certs:
                ...
                validate_new_ca_certs(existing_ca_certs, ca_certs, False, override)
        else:
            ...
            try:
                ...
                if existing_ca_certs:
                    ...
                    raise errors.NoCertificateError(
                        message=u"HTTP " "certificate download requires --force"
                    )
                else:
                    try:
        ...
        if ca_certs is None and existing_ca_certs is None:
            raise errors.InternalError(
                u"expected CA cert file '%s' to " u"exist, but it's absent" % ca_file
            )
        ...
        try:
            ...
            if os.path.exists(ca_file):
                try:
                    ...
                    logger.error("Failed to remove '%s': %s", ca_file, e2)
            raise errors.FileError(
                reason=u"cannot write certificate file '%s': %s" % (ca_file, e)
            )
    ...
    try:
        ...
        raise errors.FileError(
            reason=u"Unable set permissions on ca "
...
def configure_firefox(options, statestore, domain):
    try:
        ...
        if options.firefox_dir is not None:
            pref_path = os.path.join(options.firefox_dir, FIREFOX_PREFERENCES_REL_PATH)
            ...
            if os.path.isfile(paths.FIREFOX):
                ...
                for path in [paths.LIB_FIREFOX, paths.LIB64_FIREFOX]:
                    pref_path = os.path.join(path, FIREFOX_PREFERENCES_REL_PATH)
                    if os.path.isdir(pref_path):
                        preferences_dir = pref_path
                        break
            else:
                logger.error("Firefox configuration skipped (Firefox not found).")
                return
        ...
        if preferences_dir is not None:
            ...
            logger.debug("Firefox preferences directory found '%s'.", preferences_dir)
            ...
            logger.debug("Firefox trusted uris will be set as '.%s' domain.", domain)
            logger.debug(
                "Firefox configuration will be stored in '%s' file.", preferences_fname
            )
            ...
            try:
                ...
                logger.debug(
                    "An error occured during creating preferences file: %s.", e
                )
...
def purge_host_keytab(realm):
    try:
        ipautil.run([paths.IPA_RMKEYTAB, "-k", paths.KRB5_KEYTAB, "-r", realm])
    except CalledProcessError as e:
        if e.returncode not in (3, 5):
            # 3 - Unable to open keytab
            # 5 - Principal name or realm not found in keytab
            logger.error(
                "Error trying to clean keytab: " "/usr/sbin/ipa-rmkeytab returned %s",
                e.returncode,
            )
    else:
        logger.info("Removed old keys for realm %s from %s", realm, paths.KRB5_KEYTAB)
...
def install_check(options):
    ...
    try:
        ...
        raise ScriptError("invalid hostname: {}".format(e), rval=CLIENT_INSTALL_ERROR)
    ...
    if ret in (discovery.NO_LDAP_SERVER, discovery.NOT_IPA_SERVER) or not ds.domain:
        ...
        if options.domain:
            ...
            logger.info("DNS discovery failed to determine your DNS domain")
            cli_domain = user_input(
                "Provide the domain name of your IPA server (ex: example.com)",
                allow_empty=False,
            )
            cli_domain_source = "Provided interactively"
            logger.debug("will use interactively provided domain: %s", cli_domain)
        ret = ds.search(
            domain=cli_domain,
            servers=options.server,
            hostname=hostname,
    ...
    if ret in (discovery.NO_LDAP_SERVER, discovery.NOT_IPA_SERVER) or not ds.server:
        ...
        if options.server:
            ...
            logger.debug("will use interactively provided server: %s", cli_server[0])
        ret = ds.search(
            domain=cli_domain,
            servers=cli_server,
            hostname=hostname,
    ...
    if dnsok and not cli_kdc:
        logger.error(
            "DNS domain '%s' is not configured for automatic " "KDC address lookup.",
            ds.realm.lower(),
        )
...
def update_ipa_nssdb():
    ...
    for nickname, trust_flags in (
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to add %s to %s: %s" % (nickname, ipa_db.secdir, e)
            )
    ...
    for nickname, trust_flags in ipa_db.list_certs():
        while sys_db.has_nickname(nickname):
            try:
                ...
                raise RuntimeError(
                    "Failed to remove %s from %s: %s" % (nickname, sys_db.secdir, e)
                )
...
def sync_time(ntp_servers, ntp_pool, fstore, statestore):
    ...
    if ntp_servers or ntp_pool:
        configured = timeconf.configure_chrony(
            ntp_servers, ntp_pool, fstore, statestore
        )
    else:
        logger.warning(
            "No SRV records of NTP servers found and no NTP server "
...
def restore_time_sync(statestore, fstore):
    if statestore.has_state("chrony"):
        ...
        try:
            ...
            logger.debug("Configuration file %s was not restored.", paths.CHRONY_CONF)
...
def _install(options):
    ...
    if not options.unattended:
        if (
            options.principal is None
            and options.password is None
            and options.prompt_password is False
            and options.keytab is None
        ):
            options.principal = user_input(
                "User authorized to enroll " "computers", allow_empty=False
            )
            logger.debug("will use principal provided as option: %s", options.principal)
    ...
    if not options.on_master:
        ...
        try:
            ...
            if options.principal is not None:
                ...
                if options.password is not None:
                    ...
                    if not options.unattended:
                        ...
                        if not stdin:
                            raise ScriptError(
                                "Password must be provided for {}.".format(principal),
                                rval=CLIENT_INSTALL_ERROR,
                            )
                    else:
                        if sys.stdin.isatty():
                ...
                try:
                    kinit_password(principal, stdin, ccache_name, config=krb_name)
                except RuntimeError as e:
                    print_port_conf_info()
                    raise ScriptError(
                ...
                if os.path.exists(options.keytab):
                    ...
                    raise ScriptError(
                        "Keytab file could not be found: {}".format(options.keytab),
                        rval=CLIENT_INSTALL_ERROR,
                    )
                ...
                if not password:
                    raise ScriptError(
                        "Password must be provided.", rval=CLIENT_INSTALL_ERROR
                    )
                join_args.append("-w")
            ...
            try:
                ...
                get_ca_certs(fstore, options, cli_server[0], cli_basedn, cli_realm)
            ...
            # Now join the domain
            result = run(
                join_args, raiseonerr=False, env=env, nolog=nolog, capture_error=True
            )
    ...
    with certdb.NSSDatabase() as tmp_db:
        ...
        if "config_loaded" not in api.env:
            raise ScriptError(
                "Failed to initialize IPA API.", rval=CLIENT_INSTALL_ERROR
            )
        ...
        try:
            ...
            for i, cert in enumerate(ca_certs):
                tmp_db.add_cert(
                    cert, "CA certificate %d" % (i + 1), certdb.EXTERNAL_CA_TRUST_FLAGS
                )
        except CalledProcessError:
            raise ScriptError(
                "Failed to add CA to temporary NSS database.", rval=CLIENT_INSTALL_ERROR
            )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Target IPA server has a lower version than the enrolled " "client"
                )
                logger.warning(
                ...
                logger.warning("Second connect with delegate=True also failed: %s", e2)
                raise ScriptError(
    ...
    # Use the RPC directly so older servers are supported
    try:
        result = api.Backend.rpcclient.forward("ca_is_enabled", version=u"2.107",)
        ca_enabled = result["result"]
    except (errors.CommandError, errors.NetworkError):
        result = api.Backend.rpcclient.forward("env", server=True, version=u"2.0",)
    ...
    try:
        result = api.Backend.rpcclient.forward(
            "config_show", raw=True, version=u"2.0"  # so that servroles are not queried
        )
    except Exception as e:
    ...
    try:
        ca_certs = get_certs_from_ldap(cli_server[0], cli_basedn, cli_realm, ca_enabled)
    ...
    if not options.no_ac:
        ...
        if not options.sssd:
            for configurer in [configure_ldap_conf, configure_nslcd_conf]:
                ...
                if conf:
                    logger.info(
                        "%s configured using configuration file(s) %s", conf, filenames
                    )
        ...
        if not options.on_master:
            ...
            if not found:
                ...
                try:
                    ...
                    logger.error(
                        "Adding hardcoded server name to " "/etc/ldap.conf failed: %s",
                        str(e),
                    )
    ...
    if not options.no_nisdomain:
        configure_nisdomain(options=options, domain=cli_domain, statestore=statestore)
...
def uninstall_check(options):
    ...
    if not is_ipa_client_installed(fstore):
        ...
        raise ScriptError("IPA client is not configured on this system.", rval=rval)

    server_fstore = sysrestore.FileStore(paths.SYSRESTORE)
    if server_fstore.has_files() and not options.on_master:
        logger.error("IPA client is configured as a part of IPA server on this system.")
...
def uninstall(options):
    ...
    try:
        ...
        if e.returncode != CLIENT_NOT_CONFIGURED:
            logger.error("Unconfigured automount client failed: %s", str(e))
    ...
    if ipa_db.has_nickname("Local IPA host"):
        try:
            certmonger.stop_tracking(paths.IPA_NSSDB_DIR, nickname="Local IPA host")
        except RuntimeError as e:
            logger.error(
                "%s failed to stop tracking certificate: %s", cmonger.service_name, e
            )
    ...
    if os.path.exists(paths.IPA_DEFAULT_CONF):
        logger.info("Removing Kerberos service principals from /etc/krb5.keytab")
        try:
            parser = RawConfigParser()
            parser.read(paths.IPA_DEFAULT_CONF)
            ...
            if err.returncode != 5:
                ...
                # and can be ignored
                logger.error(
                    "Failed to remove Kerberos service principals: %s", str(err)
                )
        except Exception as e:
            logger.error("Failed to remove Kerberos service principals: %s", str(e))
    ...
    try:
        tasks.restore_pre_ipa_client_configuration(
    ...
    if was_sssd_installed and was_sssd_configured:
        ...
        try:
            ...
            logger.debug("Error while restoring pre-IPA /etc/sssd/sssd.conf.")
        ...
        logger.info("IPA domain removed from current one, restarting SSSD service")
        sssd = services.service("sssd", api)
        try:
            sssd.restart()
        except CalledProcessError:
    ...
    # than IPA are configured in sssd.conf - make sure config file is removed
        ...
        not was_sssd_installed
        and not was_sssd_configured
        ...
        try:
            ...
            logger.debug(
                "Error while moving /etc/sssd/sssd.conf to %s", paths.SSSD_CONF_DELETED
            )
    ...
    for service in (nscd, nslcd):
        if service.is_installed():
            ...
            logger.info(
                "%s daemon is not installed, skip configuration", service.service_name
            )
        ...
        if preferences_fname is not None:
            if os.path.isfile(preferences_fname):
                try:
                    ...
                    logger.warning(
                        "'%s' could not be removed: %s.", preferences_fname, str(e)
                    )
                    logger.warning(
    ...
    if fstore.has_files():
        logger.error(
            "Some files have not been restored, see %s", paths.SYSRESTORE_INDEX
        )
    has_state = False
    ...
    if not options.unattended:
        logger.info("The original nsswitch.conf configuration has been restored.")
        logger.info("You may need to restart services or reboot the machine.")
        if not options.on_master:
...
@group
class ClientInstallInterface(
    hostname_.HostNameInstallInterface,
    ...
    * ipa-replica-install
    ...
    host_password = knob(str, None, sensitive=True,)
    ...
    mkhomedir = knob(
        None, description="create home directories for users on their first login",
    )
    mkhomedir = enroll_only(mkhomedir)

    force_join = knob(
        None, description="Force client enrollment even if already enrolled",
    )
    ...
    ntp_servers = knob(
        ...
        typing.List[str],
        None,
        description="ntp server to use. This option can be used multiple " "times",
        cli_names="--ntp-server",
        cli_metavar="NTP_SERVER",
    )
    ntp_servers = enroll_only(ntp_servers)

    ntp_pool = knob(str, None, description="ntp server pool to use",)
    ntp_pool = enroll_only(ntp_pool)

    no_ntp = knob(None, description="do not configure ntp", cli_names=[None, "-N"],)
    ...
    nisdomain = knob(str, None, description="NIS domain name",)
    nisdomain = enroll_only(nisdomain)

    no_nisdomain = knob(None, description="do not configure NIS domain name",)
    no_nisdomain = enroll_only(no_nisdomain)

    ssh_trust_dns = knob(
        None, description="configure OpenSSH client to trust DNS SSHFP records",
    )
    ssh_trust_dns = enroll_only(ssh_trust_dns)

    no_ssh = knob(None, description="do not configure OpenSSH client",)
    no_ssh = enroll_only(no_ssh)

    no_sshd = knob(None, description="do not configure OpenSSH server",)
    no_sshd = enroll_only(no_sshd)

    no_sudo = knob(None, description="do not configure SSSD as data source for sudo",)
    no_sudo = enroll_only(no_sudo)

    no_dns_sshfp = knob(
        None, description="do not automatically create DNS SSHFP records",
    )
    no_dns_sshfp = enroll_only(no_dns_sshfp)

    kinit_attempts = knob(
        int, 5, description="number of attempts to obtain host TGT (defaults to 5).",
    )
    ...
    request_cert = knob(
        None, deprecated=True, description="request certificate for the machine",
    )
...
class ClientInstall(ClientInstallInterface, automount.AutomountInstallInterface):
    """
    Client installer
    """
    ...
    ca_cert_files = extend_knob(ClientInstallInterface.ca_cert_files,)
    ...
    force = knob(
        None, description="force setting of LDAP/Kerberos conf", cli_names=[None, "-f"],
    )

    on_master = False

    configure_firefox = knob(
        None, description="configure Firefox to use IPA domain credentials",
    )
