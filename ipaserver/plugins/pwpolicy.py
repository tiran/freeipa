...
class cosentry(LDAPObject):
    __doc__ = _("Class of Service object used for linking policies with" " groups")
    NO_CLI = True
    ...
    def check_priority_uniqueness(self, *keys, **options):
        if options.get("cospriority") is not None:
            entries = self.methods.find(cospriority=options["cospriority"])["result"]
            if len(entries) > 0:
                ...
                raise errors.ValidationError(
                    ...
                    % {"prio": options["cospriority"], "gname": group_name,},
                )
...
class pwpolicy(LDAPObject):
    ...
    def validate_lifetime(self, entry_attrs, add=False, *keys):
        ...
        if not add:  # then read existing entry
            existing_entry = self.api.Command.pwpolicy_show(keys[-1], all=True,)[
                "result"
            ]
    ...
    def add_cospriority(self, entry, pwpolicy_name, rights=True):
        if pwpolicy_name and pwpolicy_name != global_policy_name:
            cos_entry = self.api.Command.cosentry_show(
                pwpolicy_name, rights=rights, all=rights
            )["result"]
            if cos_entry.get("cospriority") is not None:
                entry["cospriority"] = cos_entry["cospriority"]
                if rights:
                    entry["attributelevelrights"]["cospriority"] = cos_entry[
...
class pwpolicy_del(LDAPDelete):
    ...
    def get_args(self):
        yield self.obj.primary_key.clone(attribute=True, required=True, multivalue=True)
...
class pwpolicy_mod(LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if options.get("cospriority") is not None:
            if keys[-1] is None:
                raise errors.ValidationError(
                    name="priority", error=_("priority cannot be set on global policy")
                )
            try:
...
class pwpolicy_show(LDAPRetrieve):
    ...
    def pre_callback(self, ldap, dn, attrs_list, *keys, **options):
        ...
        if options.get("user") is not None:
            user_entry = self.api.Command.user_show(options["user"], all=True)["result"]
