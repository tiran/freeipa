...
class automember_add_condition(LDAPUpdate):
    ...
    has_output = (
        ...
        output.value,
        output.Output(
            "failed", type=dict, doc=_("Conditions that could not be added"),
        ),
        output.Output("completed", type=int, doc=_("Number of conditions added"),),
    )
                ...
                try:
                    ...
                    for regex in old_entry.keys():
                        ...
                        if len(duplicate) > 0:
                            ...
                            entry_attrs[regex] = (
                                list(entry_attrs[regex]) + old_entry[regex]
                            )
                except errors.NotFound:
...
class automember_remove_condition(LDAPUpdate):
    ...
    has_output = (
        ...
        output.value,
        output.Output(
            "failed", type=dict, doc=_("Conditions that could not be removed"),
        ),
        output.Output("completed", type=int, doc=_("Number of conditions removed"),),
    )
...
class automember_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d rules matched", "%(count)d rules matched", 0)
...
class automember_default_group_set(LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        dn = DN(("cn", options["type"]), api.env.container_automember, api.env.basedn)
        entry_attrs["automemberdefaultgroup"] = self.obj.dn_exists(
            options["type"], options["automemberdefaultgroup"]
        )
        return dn
...
class automember_default_group_remove(LDAPUpdate):
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        if "automemberdefaultgroup" not in entry_attrs:
            entry_attrs["automemberdefaultgroup"] = unicode(
...
class automember_task(Object):
    takes_params = (
        DNParam("dn", label=_("Task DN"), doc=_("DN of the started task"),),
    )
...
class automember_rebuild(Method):
    ...
    takes_options = (
        group_type[0].clone(
            required=False, label=_("Rebuild membership for all members of a grouping")
        ),
        Str(
            "users*", label=_("Users"), doc=_("Rebuild membership for specified users"),
        ),
        Str(
            "hosts*", label=_("Hosts"), doc=_("Rebuild membership for specified hosts"),
        ),
        Flag(
            "no_wait?",
            default=False,
    ...
    def validate(self, **kw):
        ...
        if not (gtype or users or hosts):
            raise errors.MutuallyExclusiveError(
                reason=_(
                    "at least one of options: type, users, hosts must be " "specified"
                )
            )
    ...
    def execute(self, *keys, **options):
        ...
        types = {
            "group": ("user", "users", DN(api.env.container_user, api.env.basedn)),
            "hostgroup": ("host", "hosts", DN(api.env.container_host, api.env.basedn)),
        }
        ...
        if names:
            ...
            search_filter = ldap.make_filter_from_attr(
                obj.primary_key.name, names, rules=ldap.MATCH_ANY
            )
        else:
        ...
        return dict(
            result=result, summary=unicode(summary), value=pkey_to_value(None, options)
        )
...
class automember_find_orphans(LDAPSearch):
    ...
    takes_options = group_type + (
        Flag("remove?", doc=_("Remove orphan automember rules"),),
    )
