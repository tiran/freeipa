...
from ipalib import _, ngettext, api, Str, DNSNameParam, output, messages
...
class location(LDAPObject):
    ...
    managed_permissions = {
        "System: Read IPA Locations": {
            ...
            "ipapermdefaultattr": {"objectclass", "idnsname", "description",},
    ...
    takes_params = (
        ...
        Str("description?", label=_("Description"), doc=_("IPA Location description"),),
        Str(
            "servers_server*",
            label=_("Servers"),
        ...
        if not options.get("force"):
            ...
            if location_member:
                raise DependentEntry(
                    label=_("IPA Server"), key=keys[-1], dependent=location_member
                )
...
class location_find(LDAPSearch):
    ...
    msg_summary = ngettext(
        "%(count)d IPA location matched", "%(count)d IPA locations matched", 0
    )
    ...
    def execute(self, *keys, **options):
        ...
        if not options.get("raw"):
            ...
            servers = self.api.Command.server_find(
                in_location=keys[0], no_members=False
            )["result"]
            for server in servers:
                ...
                servers_additional_info[s_name] = {
                    "cn": server["cn"],
                    "ipaserviceweight": server.get("ipaserviceweight", [u"100"]),
                }

                s_roles = server.get("enabled_role_servrole", ())
                if s_roles:
                    servers_additional_info[s_name]["enabled_role_servrole"] = s_roles
            ...
            for server in servers_additional_info.values():
                if weight_sum == 0:
                    ...
                    val = int(server["ipaserviceweight"][0]) * dividend / weight_sum
            ...
            if not dns_servers and servers_additional_info:
                self.add_message(messages.LocationWithoutDNSServer(location=keys[0]))
