...
def deprecated(attribute):
    raise errors.ValidationError(
        name=attribute, error=_("this option has been deprecated.")
    )
...
class sudorule(LDAPObject):
    ...
    takes_params = (
        Str("cn", cli_name="sudorule_name", label=_("Rule name"), primary_key=True,),
    ...
    def check_order_uniqueness(self, *keys, **options):
        if options.get("sudoorder") is not None:
            entries = self.methods.find(sudoorder=options["sudoorder"])["result"]
            ...
            if len(entries) > 0:
                ...
                raise errors.ValidationError(
                    ...
                    % {"order": options["sudoorder"], "rule": rule_name,},
                )
...
class sudorule_mod(LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        error = _(
            "%(type)s category cannot be set to 'all' "
        ...
        category_info = [
            ...
            (
                "ipasudorunasusercategory",
                ["ipasudorunas", "ipasudorunasextuser", "ipasudorunasextusergroup"],
        ...
        for category, member_attrs, error in category_info:
            any_member_attrs_set = any(attr in _entry_attrs for attr in member_attrs)
    ...
    def pre_callback(self, ldap, dn, found, not_found, *keys, **options):
        ...
        if is_all(_entry_attrs, "cmdcategory"):
            raise errors.MutuallyExclusiveError(
                reason=_("commands cannot be added when command " "category='all'")
            )
...
class sudorule_add_runasuser(LDAPAddMember):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if any(
            (
                is_all(_entry_attrs, "ipasudorunasusercategory"),
            ...
            raise errors.MutuallyExclusiveError(
                reason=_(
                    "users cannot be added when runAs user or runAs "
                    "group category='all'"
                )
            )
            ...
            for name in options["user"]:
                if not check_validity(name):
                    raise errors.ValidationError(
                        name="runas-user",
                        error=unicode(
                            _("RunAsUser does not accept " "'%(name)s' as a user name")
                        )
                        % dict(name=name),
                    )
            ...
            for name in options["group"]:
                if not check_validity(name):
                    raise errors.ValidationError(
                        name="runas-user",
                        error=unicode(
                            _("RunAsUser does not accept " "'%(name)s' as a group name")
                        )
                        % dict(name=name),
                    )
...
class sudorule_add_runasgroup(LDAPAddMember):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        try:
            ...
            raise self.obj.handle_not_found(*keys)
        if is_all(_entry_attrs, "ipasudorunasusercategory") or is_all(
            _entry_attrs, "ipasudorunasgroupcategory"
        ):
            raise errors.MutuallyExclusiveError(
                reason=_(
                    "users cannot be added when runAs user or runAs "
...
class sudorule_add_option(LDAPQuery):
    ...
    takes_options = (Str("ipasudoopt", cli_name="sudooption", label=_("Sudo Option"),),)
    ...
    def execute(self, cn, **options):
        ...
        try:
            if options["ipasudoopt"] not in entry_attrs["ipasudoopt"]:
                entry_attrs.setdefault("ipasudoopt", []).append(options["ipasudoopt"])
            else:
    ...
    def execute(self, cn, **options):
        ...
        try:
            if options["ipasudoopt"] in entry_attrs["ipasudoopt"]:
                entry_attrs.setdefault("ipasudoopt", []).remove(options["ipasudoopt"])
                ldap.update_entry(entry_attrs)
            else:
                raise errors.AttrValueNotFound(
                    attr="ipasudoopt", value=options["ipasudoopt"]
                )
        except ValueError:
            pass
        except KeyError:
