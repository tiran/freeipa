...
class role(LDAPObject):
    ...
    takes_params = (
        Str("cn", cli_name="name", label=_("Role name"), primary_key=True,),
        Str(
            "description?",
            cli_name="desc",
            label=_("Description"),
...
class role_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d role matched", "%(count)d roles matched", 0)
...
class role_add_privilege(LDAPAddReverseMember):
    ...
    has_output = (
        ...
        output.Output("completed", type=int, doc=_("Number of privileges added"),),
    )
...
class role_remove_privilege(LDAPRemoveReverseMember):
    ...
    has_output = (
        output.Entry("result"),
        output.Output("failed", type=dict, doc=_("Members that could not be added"),),
        output.Output("completed", type=int, doc=_("Number of privileges removed"),),
    )
