...
class permission(baseldap.LDAPObject):
    ...
    takes_params = (
        (
            ...
            Str(
                ...
                doc=_(
                    "All target filters, including those implied by "
                    "type and memberof"
                ),
            ),
            DNParam(
                ...
                label=_("Target DN"),
                doc=_(
                    "Optional DN to apply the permission to "
            ...
            DNParam(
                ...
                doc=_(
                    "Optional DN subtree where an entry can be moved to "
                    "(must be in the subtree, but may not yet exist)"
                ),
            ),
            DNParam(
                ...
                doc=_(
                    "Optional DN subtree from where an entry can be moved "
                    "(must be in the subtree, but may not yet exist)"
                ),
            ),
            Str(
        ...
        for flag in flags:
            if flag not in KNOWN_FLAGS:
                raise errors.ACIError(
                    info=_(
                        "Permission with unknown flag %s may not be "
                        "modified or removed"
                    )
                    % flag
                )
        ...
        for targetfilter in ipapermtargetfilter:
            ...
            if match:
                try:
                    ...
                    continue
                groups_dn = DN(self.api.Object.group.container_dn, self.api.env.basedn)
    ...
    def postprocess_result(self, entry, options):
        ...
        :param options:
            ...
            ``pkey_only``, ``version``.
        """
        old_client = not client_has_capability(options["version"], "permissions2")
        ...
        if not options.get("raw") and not options.get("pkey_only"):
            ...
            if ipapermtarget:
                ...
                if (
                    dn[1:]
                    == DN(self.api.Object.group.container_dn, self.api.env.basedn)[:]
                    and dn[0].attr == "cn"
                    and dn[0].value != "*"
                ):
        ...
        if rights:
            ...
            if "ipapermtargetfilter" in rights:
                ...
                rights["type"] = "".join(
                    sorted(type_rights, key=rights["ipapermtargetfilter"].index)
                )
        ...
        if options.get("raw"):
            ...
            if (
                not options.get("all")
                and not entry.get("ipapermexcludedattr")
                and not entry.get("ipapermdefaultattr")
            ):
    ...
    def get_effective_attrs(self, entry):
        ...
        if "read" in entry.get("ipapermright", ()) and "objectclass" in (
            x.lower() for x in attrs
        ):
    ...
    def make_aci(self, entry):
        ...
        ipapermtarget = entry.single_value.get("ipapermtarget")
        if ipapermtarget:
            aci_parts.append('(target = "%s")' % "ldap:///%s" % ipapermtarget)

        # target_to
        ipapermtargetto = entry.single_value.get("ipapermtargetto")
        if ipapermtargetto:
            aci_parts.append('(target_to = "%s")' % "ldap:///%s" % ipapermtargetto)

        # target_from
        ipapermtargetfrom = entry.single_value.get("ipapermtargetfrom")
        if ipapermtargetfrom:
            aci_parts.append('(target_from = "%s")' % "ldap:///%s" % ipapermtargetfrom)

        # targetfilter
        ipapermtargetfilter = entry.get("ipapermtargetfilter")
        if ipapermtargetfilter:
            assert all(
                f.startswith("(") and f.endswith(")") for f in ipapermtargetfilter
            )
        ...
        ipapermbindruletype = entry.single_value.get(
        ...
        aci_parts.append(
            '(version 3.0;acl "permission:%s";allow (%s) %s;)'
    ...
    def add_aci(self, permission_entry):
        ...
        location = permission_entry.single_value.get(
            "ipapermlocation", self.api.env.basedn
        )
    ...
    def _get_aci_entry_and_string(
        self, permission_entry, name=None, notfound_ok=False, cached_acientry=None
    ):
        ...
        if (
            cached_acientry
            and cached_acientry.dn == location
            and "aci" in cached_acientry
        ):
        ...
        raise errors.NotFound(
            reason=_("The ACI for permission %(name)s was not found " "in %(dn)s ")
            % {"name": name, "dn": location}
        )

    def upgrade_permission(
        self, entry, target_entry=None, output_only=False, cached_acientry=None
    ):
        ...
        if "target" in aci.target:
            target_entry.single_value["ipapermtarget"] = DN(
                strip_ldap_prefix(aci.target["target"]["expression"])
            )
        ...
        if not ACI(new_acistring).isequal(aci):
            raise ValueError(
                "Cannot convert ACI, %r != %r" % (new_acistring, acistring)
            )
    ...
    def preprocess_options(
        self, options, return_filter_ops=False, merge_targetfilter=False
    ):
        ...
        if "extratargetfilter" in options:
            if "ipapermtargetfilter" in options:
                raise errors.ValidationError(
                    ...
                    error=_(
                        "cannot specify full target filter "
        ...
        if options.get("subtree"):
            ...
            try:
                ...
                raise errors.ValidationError(
                    name="subtree", error='does not start with "ldap:///"'
                )
        ...
        for old_name, new_name in _DEPRECATED_OPTION_ALIASES.items():
            if old_name in options:
                if client_has_capability(options["version"], "permissions2"):
                    raise errors.ValidationError(
                        name=old_name, error=_("option was renamed; use %s") % new_name
                    )
                if new_name in options:
                    raise errors.ValidationError(
                        name=old_name,
                        error=(
                            _("Cannot use %(old_name)s with %(new_name)s")
        ...
        if "memberof" in options:
            ...
            for group in memberof or ():
                try:
                    ...
                    raise errors.NotFound(reason=_("%s: group not found") % group)
        ...
        if "targetgroup" in options:
            ...
            if targetgroup:
                ...
                try:
                    ...
                    raise errors.NotFound(reason=_("%s: group not found") % targetgroup)
        ...
        if return_filter_ops:
            ...
            options["ipapermtargetfilter"] = (
                list(options.get("ipapermtargetfilter") or []) + filter_ops["add"]
            )
    ...
    def validate_permission(self, entry):
            ...
            try:
                ldap.find_entries(
                    filter=ldap.combine_filters(
                        entry["ipapermtargetfilter"], rules="&"
                    ),
                ...
                raise errors.ValidationError(
                    name="ipapermtargetfilter", error=_("Bad search filter")
                )
        ...
        if not any(v for a in needed_attrs for v in (entry.get(a) or ())):
            raise errors.ValidationError(
                name="target",
                error=_(
                    "there must be at least one target entry specifier "
...
class permission_add_noaci(baseldap.LDAPCreate):
    ...
    takes_options = (_ipapermissiontype_param,)
    ...
    def pre_callback(self, ldap, dn, entry, attrs_list, *keys, **options):
        ...
        entry["objectclass"] = [
            oc for oc in entry["objectclass"] if oc.lower() != "ipapermissionv2"
        ]
        return dn
    ...
    def pre_callback(self, ldap, dn, entry, attrs_list, *keys, **options):
        ...
        if "attrs" in options:
            if "ipapermincludedattr" in options:
                raise errors.ValidationError(
                    name="attrs",
                    error=_("attrs and included attributes are " "mutually exclusive"),
                )
    ...
    def pre_callback(self, ldap, dn, *keys, **options):
        ...
        if not options.get("force"):
            ...
            if entry.get("ipapermdefaultattr"):
                raise errors.ACIError(info=_("cannot delete managed permissions"))
...
class permission_mod(baseldap.LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry, attrs_list, *keys, **options):
        ...
        if "MANAGED" in old_entry.get("ipapermissiontype", ()):
            for option_name in sorted(options):
                if option_name == "rename":
                    raise errors.ValidationError(
                        name=option_name, error=_("cannot rename managed permissions")
                    )
                ...
                if (
                    option.attribute
                    and not allow_mod
                    or option_name == "extratargetfilter"
                ):
                    raise errors.ValidationError(
                        name=option_name,
        ...
        if "attrs" in options:
            if any(
                a in options for a in ("ipapermincludedattr", "ipapermexcludedattr")
            ):
                raise errors.ValidationError(
                    name="attrs",
                    error=_(
                        "attrs and included/excluded attributes are "
                        "mutually exclusive"
                    ),
                )
        ...
        if options.get("ipapermbindruletype") and old_entry.get("member"):
            raise errors.ValidationError(
                ...
                error=_(
                    "cannot set bindtype for a permission that is "
        ...
        for key, value in old_entry.items():
            if (
                key not in options
                and key != "cn"
                and key not in self.obj.attribute_members
            ):
        ...
        if "extratargetfilter" in options:
            ...
            entry["ipapermtargetfilter"] = list(
                options["extratargetfilter"] or []
        ...
        new_filters = set(
            filt
            for filt in (entry.get("ipapermtargetfilter") or [])
            if filt not in old_implicit_filters
            or not any(rem.match(filt) for rem in removes)
        )
        ...
        old_location = old_entry.single_value.get(
            "ipapermlocation", self.api.env.basedn
        )
    ...
    def _revert_aci(self):
        ...
        if old_aci_info:
            ...
            if old_aci_string:
                logger.warning("Reverting ACI on %s to %s", entry.dn, old_aci_string)
...
class permission_find(baseldap.LDAPSearch):
    ...
    def pre_callback(self, ldap, filters, attrs_list, base_dn, scope, *args, **options):
        if "attrs" in options and "ipapermincludedattr" in options:
            raise errors.ValidationError(
                name="attrs",
                error=_(
                    "attrs and included/excluded attributes are " "mutually exclusive"
                ),
            )
    ...
    def post_callback(self, ldap, entries, truncated, *args, **options):
        ...
        attribute_options = [
            o for o in options if (o in self.options and self.options[o].attribute)
        ]
        ...
        if not truncated:
            max_entries = options.get("sizelimit", self.api.Backend.ldap2.size_limit)

            if max_entries > 0:
                # should we get more entries than current sizelimit, fail
                assert len(entries) <= max_entries

            filters = ["(objectclass=ipaPermission)", "(!(ipaPermissionType=V2))"]
            if "name" in options:
                filters.append(
                    ldap.make_filter_from_attr("cn", options["name"], exact=False)
                )
            ...
            for entry in legacy_entries:
                ...
                self.obj.upgrade_permission(
                    entry, output_only=True, cached_acientry=root_entry
                )
                ...
                for opt in attribute_options:
                    ...
                    if not value:
                        break
                    if not all(
                        any(str(ov).lower() in str(v).lower() for v in value)
                        for ov in optval
                    ):
                        break
                else:
