...
from .baseldap import LDAPCreate, LDAPQuery, DN
...
class stageuser(baseuser):
    ...
    managed_permissions = {
        ...
        "System: Read Stage User password": {
            ...
            "ipapermdefaultattr": {"userPassword", "krbPrincipalKey",},
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if "givenname" not in entry_attrs:
            raise errors.RequirementError(
                name="givenname", error=_("givenname is required")
            )
...
class stageuser_find(baseuser_find):
    ...
    def pre_callback(self, ldap, filter, attrs_list, base_dn, scope, *keys, **options):
        ...
        container_filter = ldap.make_filter_from_attr("objectclass", "posixaccount")
        # provisioning system can create non posixaccount stage user
        # but then they have to create inetOrgPerson stage user
        stagefilter = filter.replace(
            container_filter, "(|%s(objectclass=inetOrgPerson))" % container_filter
        )
        logger.debug("stageuser_find: pre_callback new filter=%s ", stagefilter)
...
class stageuser_activate(LDAPQuery):
    ...
    searched_operational_attributes = [
        "uidNumber",
        "gidNumber",
        "nsAccountLock",
        "ipauniqueid",
    ...
    def _check_validy(self, dn, entry):
        if dn[0].attr != "uid":
            raise errors.ValidationError(
                name=self.obj.primary_key.cli_name, error=_("Entry RDN is not 'uid'"),
            )
        ...
        for value in entry_from[attr]:
            ...
            if (isinstance(v, str) and v in ("", None)) or (
                ...
                try:
                    ...
                    logger.debug("merge %s: [no_print %s]", attr, v.__class__.__name__)
                ...
                try:
                    ...
                    logger.debug("Add %s: [no_print %s]", attr, v.__class__.__name__)
    ...
    def __value_2_add(self, args, options, attr, value):
        ...
        if ldap.has_dn_syntax(attr):
            if attr.lower() in self.preserved_DN_syntax_attrs:
                ...
                try:
                    self._exc_wrapper(args, options, ldap.get_entry)(value, ["dn"])
                    return value
    ...
    def execute(self, *args, **options):
        ...
        try:
            self._exc_wrapper(args, options, ldap.get_entry)(active_dn, ["dn"])
        ...
        if (
            "description" in new_entry_attrs
            and NO_UPG_MAGIC in new_entry_attrs["description"]
        ):
        ...
        result["summary"] = unicode(_("Stage user %s activated" % staging_dn[0].value))
...
class stageuser_add_certmapdata(baseuser_add_certmapdata):
    __doc__ = _("Add one or more certificate mappings to the stage user" " entry.")


@register()
class stageuser_remove_certmapdata(baseuser_remove_certmapdata):
    __doc__ = _("Remove one or more certificate mappings from the stage user" " entry.")
