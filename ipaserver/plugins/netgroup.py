...
output_params = (
    Str("memberuser_user?", label="Member User",),
    Str("memberuser_group?", label="Member Group",),
    Str("memberhost_host?", label=_("Member Host"),),
    Str("memberhost_hostgroup?", label="Member Hostgroup",),
)
        ...
        try:
            ...
            if "mepManagedEntry" in netgroup.get("objectclass", []):
                raise errors.DuplicateEntry(
                    message=unicode(self.msg_collision % keys[-1])
                )
            else:
...
class netgroup_find(LDAPSearch):
    ...
    takes_options = LDAPSearch.takes_options + (
        Flag("private", exclude="webui", flags=["no_option", "no_output"],),
        Flag(
            "managed",
            cli_name="managed",
