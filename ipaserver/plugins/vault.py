...
vault_options = (
    ...
    Flag("shared?", doc=_("Shared vault"),),
    Str("username?", cli_name="user", doc=_("Username of the user vault"),),
)
...
class vaultcontainer(LDAPObject):
    ...
    managed_permissions = {
        "System: Read Vault Containers": {
            ...
            "ipapermdefaultattr": {"objectclass", "cn", "description", "owner",},
        ...
        "System: Modify Vault Containers": {
            ...
            "ipapermtarget": DN(api.env.container_vault, api.env.basedn),
            "ipapermright": {"write"},
            "ipapermdefaultattr": {"objectclass", "cn", "description",},
            "default_privileges": {"Vault Administrators"},
        },
        "System: Manage Vault Container Ownership": {
            ...
            "ipapermtarget": DN(api.env.container_vault, api.env.basedn),
            "ipapermright": {"write"},
            "ipapermdefaultattr": {"owner",},
            "default_privileges": {"Vault Administrators"},
        },
    }

    takes_params = (
        Str("owner_user?", label=_("Owner users"),),
        Str("owner_group?", label=_("Owner groups"),),
        Str("owner_service?", label=_("Owner services"),),
        Str("owner?", label=_("Failed owners"),),
        Str("service?", label=_("Vault service"), flags={"virtual_attribute"},),
        Flag("shared?", label=_("Shared vault"), flags={"virtual_attribute"},),
        Str("username?", label=_("Vault user"), flags={"virtual_attribute"},),
    )
    ...
    def get_dn(self, *keys, **options):
        ...
        if count > 1:
            raise errors.MutuallyExclusiveError(
                reason=_(
                    "Service, shared and user options "
        ...
        if not count:
            ...
            if principal.is_host:
                raise errors.NotImplementedError(reason=_("Host is not supported"))
...
class vaultcontainer_show(LDAPRetrieve):
    ...
    def pre_callback(self, ldap, dn, attrs_list, *keys, **options):
        ...
        if not self.api.Command.kra_is_enabled()["result"]:
            raise errors.InvocationError(format=_("KRA service is not enabled"))
...
class vaultcontainer_add_owner(VaultModMember, LDAPAddMember):
    ...
    has_output = (
        output.Entry("result"),
        output.Output("failed", type=dict, doc=_("Owners that could not be added"),),
        output.Output("completed", type=int, doc=_("Number of owners added"),),
    )
...
class vaultcontainer_remove_owner(VaultModMember, LDAPRemoveMember):
    ...
    has_output = (
        output.Entry("result"),
        output.Output("failed", type=dict, doc=_("Owners that could not be removed"),),
        output.Output("completed", type=int, doc=_("Number of owners removed"),),
    )
...
class vault_find(LDAPSearch):
    ...
    takes_options = (
        LDAPSearch.takes_options
        + vault_options
        + (
            Flag("services?", doc=_("List all service vaults"),),
            Flag("users?", doc=_("List all user vaults"),),
        )
    )

    has_output_params = LDAPSearch.has_output_params

    msg_summary = ngettext("%(count)d vault matched", "%(count)d vaults matched", 0,)
    ...
    def pre_callback(self, ldap, filter, attrs_list, base_dn, scope, *args, **options):
        ...
        if options.get("users") or options.get("services"):
            ...
            if count > 1:
                raise errors.MutuallyExclusiveError(
                    reason=_(
                        "Service(s), shared, and user(s) options "
                        + "cannot be specified simultaneously"
                    )
                )

            scope = ldap.SCOPE_SUBTREE
            container_dn = DN(self.obj.container_dn, self.api.env.basedn)
...
class vaultconfig(Object):
    ...
    takes_params = (
        Bytes("transport_cert", label=_("Transport Certificate"),),
        Str(
            "kra_server_server*",
...
class vaultconfig_show(Retrieve):
    ...
    takes_options = (
        Str("transport_out?", doc=_("Output file to store the transport certificate"),),
    )
...
class vault_archive_internal(PKQuery):
    ...
    takes_options = vault_options + (
        Bytes("session_key", doc=_("Session key wrapped with transport certificate"),),
        Bytes("vault_data", doc=_("Vault data encrypted with session key"),),
        Bytes("nonce", doc=_("Nonce"),),
    )
    ...
    def execute(self, *args, **options):
        ...
        with self.api.Backend.kra.get_client() as kra_client:
            ...
            key = kra_client.keys.retrieve_key(
                key_info.get_key_id(), wrapped_session_key
            )
        ...
        response = {
            "value": args[-1],
            "result": {"vault_data": key.encrypted_data, "nonce": key.nonce_data,},
        }
