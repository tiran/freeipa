...
from ipaserver.topology import (
    ...
    get_topology_connection_errors,
...
def validate_domain_level(api):
    ...
    if current < DOMAIN_LEVEL_1:
        raise errors.InvalidDomainLevelError(
            reason=_("Topology management requires minimum domain level {0} ").format(
                DOMAIN_LEVEL_1
            )
        )
...
class topologysegment(LDAPObject):
    ...
    takes_params = (
        Str(
            ...
            default_from=lambda iparepltoposegmentleftnode, iparepltoposegmentrightnode: "%s-to-%s"
        ...
        StrEnum(
            ...
            doc=_(
                "Whether a replication agreement is active, meaning whether "
    ...
    def validate_nodes(self, ldap, dn, entry_attrs, suffix):
        ...
        masters = self.api.Command.server_find("", sizelimit=0, no_members=False)[
            "result"
        ]
        ...
        if leftnode == rightnode:
            raise errors.ValidationError(
                name=key, error=_("left node and right node must not be the same")
            )
        ...
        if leftnode not in suffix_m_hostnames:
            raise errors.ValidationError(
                name="leftnode",
                error=_(
                    "left node ({host}) does not support " "suffix '{suff}'"
        ...
        if rightnode not in suffix_m_hostnames:
            raise errors.ValidationError(
                name="rightnode",
                error=_(
                    "right node ({host}) does not support " "suffix '{suff}'"
...
class topologysegment_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d segment matched", "%(count)d segments matched", 0)
...
class topologysegment_reinitialize(LDAPQuery):
    __doc__ = _(
        "Request a full re-initialization of the node "
    ...
    takes_options = (
        Flag("left?", doc=_("Initialize left node"), default=False,),
        Flag("right?", doc=_("Initialize right node"), default=False,),
        Flag(
            "stop?",
    ...
    def execute(self, *keys, **options):
        ...
        if stop:
            action = u"stop"
            msg = _(
                'Stopping of replication refresh for segment: "' '%(pkey)s" requested.'
            )
...
class topologysuffix(LDAPObject):
    ...
    takes_params = (
        Str("cn", cli_name="name", primary_key=True, label=_("Suffix name"),),
        DNParam(
            "iparepltopoconfroot",
            cli_name="suffix_dn",
...
class topologysuffix_find(LDAPSearch):
    ...
    msg_summary = ngettext(
        "%(count)d topology suffix matched", "%(count)d topology suffixes matched", 0
    )
...
"""
    ...
    def execute(self, *keys, **options):
        ...
        segments = self.api.Command.topologysegment_find(keys[0], sizelimit=0)["result"]
