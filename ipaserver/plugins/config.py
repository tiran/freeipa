...
class config(LDAPObject):
    ...
    def gather_trusted_domains(self):
        ...
        for forest_name in [a["cn"][0] for a in ad_forests]:
            forest_domains = command.trustdomain_find(forest_name, sizelimit=0)[
                "result"
            ]
            ...
            trusted_domains.update(
                {
                    ...
                    for dom in forest_domains
                    if "domain_enabled" in dom
                }
            )
...
class config_mod(LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if kw:
            ...
            for (k, v) in kw.items():
                ...
                attributes = [
                    field.strip().lower() for field in entry_attrs[k].split(",")
                ]
        ...
        for (attr, obj) in (
            ("ipauserobjectclasses", "user"),
            ("ipagroupobjectclasses", "group"),
        ):
            if attr in entry_attrs:
                if not entry_attrs[attr]:
                    raise errors.ValidationError(name=attr, error=_("May not be empty"))
                objectclasses = list(
                    set(entry_attrs[attr]).union(
                        self.api.Object[obj].possible_objectclasses
                    )
                )
                new_allowed_attrs = ldap.get_allowed_attributes(
                    objectclasses, raise_on_unknown=True
                )
                checked_attrs = self.api.Object[obj].default_attributes
                if self.api.Object[obj].uuid_attribute:
                    checked_attrs = checked_attrs + [
                        self.api.Object[obj].uuid_attribute
                    ]
                for obj_attr in checked_attrs:
                    ...
                    if obj_attr in OPERATIONAL_ATTRIBUTES:
                        continue
                    if (
                        obj_attr in self.api.Object[obj].params
                        and "virtual_attribute"
                        in self.api.Object[obj].params[obj_attr].flags
                    ):
                    ...
                    if obj_attr not in new_allowed_attrs:
                        raise errors.ValidationError(
                            name=attr,
                            error=_(
                                "%(obj)s default attribute %(attr)s would not be allowed!"
                            )
                            % dict(obj=obj, attr=obj_attr),
                        )

        if (
            "ipaselinuxusermapdefault" in entry_attrs
            or "ipaselinuxusermaporder" in entry_attrs
        ):
            config = None
            ...
            if "ipaselinuxusermapdefault" in entry_attrs:
                ...
                if defaultuser is not None:
                    ...
                    if error_message:
                        raise errors.ValidationError(
                            name="ipaselinuxusermapdefault", error=error_message
                        )
            ...
            if "ipaselinuxusermaporder" in entry_attrs:
                ...
                for user in userlist:
                    ...
                    if error_message:
                        error_message = _(
                            "SELinux user '%(user)s' is not " "valid: %(error)s"
                        ) % dict(user=user, error=error_message)
                        raise errors.ValidationError(
                            name="ipaselinuxusermaporder", error=error_message
                        )
            else:
                if not config:
                    config = ldap.get_ipa_config()
    ...
    def exc_callback(self, keys, options, exc, call_func, *call_args, **call_kwargs):
        if (
            ...
            and "ca_renewal_master_server" in options
        ):
            return
