...
class sudocmd(LDAPObject):
    ...
    takes_params = (
        Str("sudocmd", cli_name="command", label=_("Sudo Command"), primary_key=True,),
        Str(
            "description?",
            cli_name="desc",
            label=_("Description"),
    ...
    def pre_callback(self, ldap, dn, *keys, **options):
        ...
        try:
            entries, _truncated = ldap.find_entries(
                filter, ["cn"], base_dn=DN(api.env.container_sudorule, api.env.basedn)
            )
        except errors.NotFound:
            pass
        else:
        ...
        if dependent_sudorules:
            raise errors.DependentEntry(
                key=keys[0], label="sudorule", dependent=", ".join(dependent_sudorules)
            )
        return dn
