...
group_output_params = (
    Str("membermanager_group", label="Membership managed by groups",),
    Str("membermanager_user", label="Membership managed by users",),
...
class group(LDAPObject):
    ...
    managed_permissions = {
        ...
        "System: Modify Group Membership": {
            "ipapermright": {"write"},
            "ipapermtargetfilter": ["(objectclass=ipausergroup)", "(!(cn=admins))",],
            ...
            "default_privileges": {"Group Administrators", "Modify Group membership"},
        },
        "System: Modify External Group Membership": {
            "ipapermright": {"write"},
            "ipapermtargetfilter": ["(objectclass=ipaexternalgroup)",],
            "ipapermdefaultattr": {"ipaexternalmember"},
            "default_privileges": {"Group Administrators", "Modify Group membership"},
        },
        ...
        "System: Read Group Compat Tree": {
            ...
            "ipapermright": {"read", "search", "compare"},
            "ipapermdefaultattr": {"objectclass", "cn", "memberuid", "gidnumber",},
        },
        "System: Read Group Views Compat Tree": {
            ...
            "ipapermlocation": api.env.basedn,
            "ipapermtarget": DN(
                "cn=groups", "cn=*", "cn=views", "cn=compat", api.env.basedn
            ),
            "ipapermright": {"read", "search", "compare"},
            "ipapermdefaultattr": {"objectclass", "cn", "memberuid", "gidnumber",},
        },
        ...
        if options["external"]:
            ...
            if "gidnumber" in options:
                raise errors.MutuallyExclusiveError(
                    reason=_("gid cannot be set for external group")
                )
    ...
    def pre_callback(self, ldap, dn, *keys, **options):
        ...
        if keys[0] in PROTECTED_GROUPS:
            raise errors.ProtectedEntryError(
                label=_(u"group"), key=keys[0], reason=_(u"privileged group")
            )
    ...
    def post_callback(self, ldap, dn, *keys, **options):
        ...
        try:
            ...
            logger.warning(
                ...
                "not an issue and can be ignored.",
                keys[-1],
            )
...
class group_mod(LDAPUpdate):
    ...
    takes_options = LDAPUpdate.takes_options + (
        Flag("posix", cli_name="posix", doc=_("change to a POSIX group"),),
        Flag(
            "external",
            cli_name="external",
    ...
    def pre_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        if "rename" in options or "cn" in entry_attrs:
            if is_protected_group:
                raise errors.ProtectedEntryError(
                    label=u"group", key=keys[-1], reason=u"Cannot be renamed"
                )
...
class group_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d group matched", "%(count)d groups matched", 0)

    takes_options = LDAPSearch.takes_options + (
        Flag("private", cli_name="private", doc=_("search for private groups"),),
        Flag("posix", cli_name="posix", doc=_("search for POSIX groups"),),
        Flag(
            "external",
            cli_name="external",
            doc=_(
        ...
        Flag("nonposix", cli_name="nonposix", doc=_("search for non-POSIX groups"),),
    )
...
class group_show(LDAPRetrieve):
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        assert isinstance(dn, DN)
        if (
            "ipaexternalmember" in entry_attrs
            and len(entry_attrs["ipaexternalmember"]) > 0
            and "trust_resolve" in self.Command
...
class group_add_member(LDAPAddMember):
    ...
    def post_callback(self, ldap, completed, failed, dn, entry_attrs, *keys, **options):
        ...
        if "ipaexternalmember" in options:
            ...
            result = add_external_post_callback(
                ldap,
                dn,
                entry_attrs,
                failed=failed,
    ...
    def post_callback(self, ldap, completed, failed, dn, entry_attrs, *keys, **options):
        ...
        if "ipaexternalmember" in options:
            ...
            if "member" in failed and "group" in failed["member"]:
                restore = failed["member"]["group"]
            failed["member"]["group"] = list((id, id) for id in sids)
            result = remove_external_post_callback(
                ldap,
                dn,
                entry_attrs,
                failed=failed,
...
class group_detach(LDAPQuery):
    ...
    def execute(self, *keys, **options):
        ...
        if (
            not ldap.can_write(group_dn, "objectclass")
            or not ldap.can_write(group_dn, "mepManagedBy")
            and is_managed
        ):
...
class group_remove_member_manager(LDAPRemoveMember):
    ...
    has_output_params = LDAPRemoveMember.has_output_params + group_output_params
    member_attributes = ["membermanager"]
