...
host_output_params = (
    Flag("has_keytab", label=_("Keytab"),),
    Str("managedby_host", label="Managed by",),
    Str("managing_host", label="Managing",),
    ...
    Str("ipaallowedtoperform_read_keys", label=_("Failed allowed to retrieve keytab"),),
    Str("ipaallowedtoperform_write_keys", label=_("Failed allowed to create keytab"),),
)
...
class host(LDAPObject):
    ...
    attribute_members = {
        ...
        "managedby": ["host"],
        "managing": ["host"],
        "memberofindirect": ["hostgroup", "netgroup", "role", "hbacrule", "sudorule"],
    ...
    managed_permissions = {
        ...
        "System: Manage Host Keytab": {
            ...
            "ipapermtargetfilter": [
                "(objectclass=ipahost)",
                "(!(memberOf=%s))"
                % DN("cn=ipaservers", api.env.container_hostgroup, api.env.basedn),
            ],
        ...
        "System: Read Host Compat Tree": {
            ...
            "ipapermright": {"read", "search", "compare"},
            "ipapermdefaultattr": {"objectclass", "cn", "macaddress",},
        },
    }
    ...
    takes_params = (
        ...
        Str(
            ...
            pattern_errmsg=(
                "Must be of the form HH:HH:HH:HH:HH:HH, where "
        ...
        Str("ipaassignedidview?", label=_("Assigned ID View"), flags=["no_option"],),
        StrEnum(
            "krbprincipalauthind*",
            cli_name="auth_ind",
...
class host_add(LDAPCreate):
    ...
    takes_options = LDAPCreate.takes_options + (
        Flag("force", label=_("Force"), doc=_("force host name even if not in DNS"),),
        Flag("no_reverse", doc=_("skip reverse DNS detection"),),
        Str(
            "ip_address?",
            validate_ipaddr,
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if "ipamaxhostnamelength" in config:
            ...
            if len(keys[-1]) > maxlen:
                raise errors.ValidationError(
                    ...
                    error=_("can be at most %(len)d characters" % dict(len=maxlen)),
                )
        ...
        if not entry_attrs.get("userpassword", False) and not options.get(
            "random", False
        ):
...
class host_del(LDAPDelete):
    ...
    takes_options = LDAPDelete.takes_options + (
        Flag(
            "updatedns?",
            doc=_(
                "Remove A, AAAA, SSHFP and PTR records of the host(s) "
                "managed by IPA DNS"
            ),
            default=False,
        ),
    ...
    def pre_callback(self, ldap, dn, *keys, **options):
        ...
        while truncated:
            try:
                ...
                for entry_attrs in services:
                    principal = kerberos.Principal(entry_attrs["krbprincipalname"][0])
        ...
        if updatedns:
            ...
            rec_removed = False
            try:
                record = api.Command["dnsrecord_show"](zone, relative_hostname)[
                    "result"
                ]
            except errors.NotFound:
                pass
                ...
                for attr in ("arecord", "aaaarecord"):
                    for val in record.get(attr, []):
                        rec_removed = remove_ptr_rec(val, fqdn_dnsname) or rec_removed
                try:
            ...
            if not rec_removed:
                self.add_message(
                    messages.FailedToRemoveHostDNSRecords(
                        host=fqdn, reason=_("No A, AAAA, SSHFP or PTR records found.")
                    )
                )
...
class host_mod(LDAPUpdate):
    ...
    takes_options = LDAPUpdate.takes_options + (
        Flag("updatedns?", doc=_("Update DNS entries"), default=False,),
    )
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if options.get("userpassword") or options.get("random"):
            ...
            if not entry["has_password"] and entry["has_keytab"]:
                raise errors.ValidationError(
                    name="password", error=_("Password cannot be set on enrolled host.")
                )
        ...
        if "krbprincipalname" in entry_attrs:
            entry_attrs_old = ldap.get_entry(dn, ["objectclass", "krbprincipalname"])
            ...
            if "krbprincipalaux" not in (item.lower() for item in obj_classes):
        ...
        if "usercertificate" in options and ca_is_enabled:
            ...
            for cert in removed_certs:
                rm_certs = api.Command.cert_find(certificate=cert, host=keys)["result"]
        ...
        if "krbticketflags" in entry_attrs:
            ...
            if "krbticketpolicyaux" not in (
                item.lower() for item in entry_attrs["objectclass"]
            ):
                entry_attrs["objectclass"].append("krbticketpolicyaux")
...
class host_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d host matched", "%(count)d hosts matched", 0)
    ...
    def pre_callback(self, ldap, filter, attrs_list, base_dn, scope, *args, **options):
        ...
        if "man_host" in options or "not_man_host" in options:
            ...
            for target_hosts, filter_op in (
                (hosts, ldap.MATCH_ANY),
                (not_hosts, ldap.MATCH_NONE),
            ):
                hosts_avas = [DN(host)[0][0] for host in target_hosts]
                hosts_filters = [
                    ldap.make_filter_from_attr(ava.attr, ava.value)
                    for ava in hosts_avas
                ]
                hosts_filter = ldap.combine_filters(hosts_filters, filter_op)

                filter = ldap.combine_filters((filter, hosts_filter), ldap.MATCH_ALL)
    ...
    def post_callback(self, ldap, entries, truncated, *args, **options):
        ...
        for entry_attrs in entries:
            ...
            try:
                ...
                self.add_message(
                    messages.CertificateInvalid(subject=hostname, reason=e,)
                )
...
class host_show(LDAPRetrieve):
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        try:
            ...
            self.add_message(messages.CertificateInvalid(subject=hostname, reason=e,))
            del entry_attrs["usercertificate"]
...
class host_allow_retrieve_keytab(LDAPAddMember):
    __doc__ = _(
        "Allow users, groups, hosts or host groups to retrieve a keytab"
        " of this host."
    )
...
class host_disallow_retrieve_keytab(LDAPRemoveMember):
    __doc__ = _(
        "Disallow users, groups, hosts or host groups to retrieve a "
        "keytab of this host."
    )
    member_attributes = ["ipaallowedtoperform_read_keys"]
    has_output_params = LDAPRemoveMember.has_output_params + host_output_params
...
class host_disallow_create_keytab(LDAPRemoveMember):
    __doc__ = _(
        "Disallow users, groups, hosts or host groups to create a "
        "keytab of this host."
    )
    member_attributes = ["ipaallowedtoperform_write_keys"]
    has_output_params = LDAPRemoveMember.has_output_params + host_output_params
...
class host_remove_cert(LDAPRemoveAttributeViaOption):
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        for cert in options.get("usercertificate", []):
            revoke_certs(api.Command.cert_find(certificate=cert, host=keys)["result"])
