...
class caacl(LDAPObject):
    ...
    takes_params = (
        Str("cn", cli_name="name", label=_("ACL name"), primary_key=True,),
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if is_all(options, "ipacacategory") and "ipamemberca" in entry_attrs:
            raise errors.MutuallyExclusiveError(
                reason=_(
                    "CA category cannot be set to 'all' " "while there are allowed CAs"
                )
            )
        if (
            is_all(options, "ipacertprofilecategory")
...
class caacl_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d CA ACL matched", "%(count)d CA ACLs matched", 0)
...
class caacl_add_user(LDAPAddMember):
    ...
    member_count_out = (_("%i user or group added."), _("%i users or groups added."))
...
class caacl_add_profile(LDAPAddMember):
    ...
    def pre_callback(self, ldap, dn, found, not_found, *keys, **options):
        ...
        if is_all(entry_attrs, "ipacertprofilecategory"):
            raise errors.MutuallyExclusiveError(
                reason=_("profiles cannot be added when profile category='all'")
            )
        return dn
...
class caacl_add_ca(LDAPAddMember):
    ...
    def pre_callback(self, ldap, dn, found, not_found, *keys, **options):
        ...
        if is_all(entry_attrs, "ipacacategory"):
            raise errors.MutuallyExclusiveError(
                reason=_("CAs cannot be added when CA category='all'")
            )
        return dn
