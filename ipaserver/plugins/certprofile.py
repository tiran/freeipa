...
class certprofile(LDAPObject):
    ...
    managed_permissions = {
        ...
        "System: Modify Certificate Profile": {
            "ipapermright": {"write"},
            "ipapermdefaultattr": {"cn", "description", "ipacertprofilestoreissued",},
            "replaces": [
...
class certprofile_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d profile matched", "%(count)d profiles matched", 0)
...
class certprofile_show(LDAPRetrieve):
    ...
    takes_options = LDAPRetrieve.takes_options + (
        Str("out?", doc=_("Write profile configuration to file"),),
    )
    ...
    def pre_callback(self, ldap, dn, entry, entry_attrs, *keys, **options):
        ...
        if len(matches) == 0:
            ...
            raise errors.ValidationError(
                name="file",
                error=_(
                    "Profile data specifies profileId multiple times: " "%(values)s"
                )
                % dict(values=matches),
            )
        ...
        if "file" in options:
            ...
            if not ldap.can_write(dn, "ipacertprofilestoreissued"):
                raise errors.ACIError(
                    info=_("Insufficient privilege to modify a certificate profile.")
                )
    ...
    def execute(self, *keys, **options):
        try:
            ...
            if "file" in options:
                ...
                return self.api.Command.certprofile_show(keys[0], version=API_VERSION)
            else:
