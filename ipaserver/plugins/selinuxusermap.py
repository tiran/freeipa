...
def validate_selinuxuser(ugettext, user):
    ...
    if not regex_name.match(name):
        return _("Invalid SELinux user name, must match {}").format(SELINUX_USER_REGEX)
    ...
    if not mls or not _validate_level(mls, regex_mls, SELINUX_MLS_MAX):
        return _(
            "Invalid MLS value, must match {mls}, where max level " "{mls_max}"
        ).format(mls=SELINUX_MLS_REGEX, mls_max=SELINUX_MLS_MAX)

    if mcs and not _validate_level(mcs, regex_mcs, SELINUX_MCS_MAX):
        return _(
            "Invalid MCS value, must match {mcs}, where max category " "{mcs_max}"
        ).format(mcs=SELINUX_MCS_REGEX, mcs_max=SELINUX_MCS_MAX)
...
def validate_selinuxuser_inlist(ldap, user):
    ...
    if len(item) != 1:
        raise errors.NotFound(
            reason=_("SELinux user map list not " "found in configuration")
        )
    userlist = item[0].split("$")
    if user not in userlist:
        raise errors.NotFound(
            reason=_("SELinux user %(user)s not found in " "ordering list (in config)")
            % dict(user=user)
        )
    ...
    def _normalize_seealso(self, seealso):
        ...
        try:
            ...
            try:
                ...
                raise errors.NotFound(
                    reason=_("HBAC rule %(rule)s not found") % dict(rule=seealso)
                )
...
class selinuxusermap_mod(LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        def is_to_be_deleted(x):
            return (x in _entry_attrs and x in entry_attrs) and entry_attrs[x] is None
        ...
        def is_to_be_set(x):
            return (
                (x in _entry_attrs and _entry_attrs[x] is not None)
                or (x in entry_attrs and entry_attrs[x] is not None)
            ) and not is_to_be_deleted(x)

        are_local_members_to_be_set = any(
            is_to_be_set(attr)
            for attr in ("usercategory", "hostcategory", "memberuser", "memberhost")
        )
...
class selinuxusermap_find(LDAPSearch):
    ...
    def execute(self, *args, **options):
        ...
        if options.get("seealso"):
            ...
            try:
                hbac = api.Command["hbacrule_show"](hbacrule, all=True)["result"]
                dn = hbac["dn"]
            except errors.NotFound:
