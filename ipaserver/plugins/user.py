...
def check_protected_member(user, protected_group_name=u"admins"):
    ...
    if enabled_users == [user]:
        raise errors.LastMemberError(
            key=user, label=_(u"group"), container=protected_group_name
        )
...
class user(baseuser):
    ...
    managed_permissions = {
        ...
        "System: Add User to default group": {
            ...
            "ipapermtarget": DN("cn=ipausers", api.env.container_group, api.env.basedn),
        ...
        "System: Read User Views Compat Tree": {
            ...
            "ipapermlocation": api.env.basedn,
            "ipapermtarget": DN(
                "cn=users", "cn=*", "cn=views", "cn=compat", api.env.basedn
            ),
            "ipapermright": {"read", "search", "compare"},
            "ipapermdefaultattr": {
                "objectclass",
...
class user_add(baseuser_add):
    ...
    takes_options = LDAPCreate.takes_options + (
        Flag(
            "noprivate", cli_name="noprivate", doc=_("Don't create user private group"),
        ),
    )
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if not client_has_capability(options["version"], "optional_uid_params"):
        ...
        entry_attrs.setdefault(
            "krbprincipalname", "%s@%s" % (entry_attrs["uid"], api.env.realm)
        )
        ...
        if "userclass" in entry_attrs and "ipauser" not in entry_attrs["objectclass"]:
            entry_attrs["objectclass"].append("ipauser")

        if (
            "ipauserauthtype" in entry_attrs
            and "ipauserauthtypeclass" not in entry_attrs["objectclass"]
        ):
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        if (
            (options.get("noprivate", False) or not ldap.has_upg())
            and "description" in newentry
            and NO_UPG_MAGIC in newentry["description"]
        ):
...
class user_del(baseuser_del):
    ...
    takes_options = baseuser_del.takes_options + (Bool("preserve?", exclude="cli",),)
    ...
    def _preserve_user(self, pkey, delete_container, **options):
        ...
        if dn.endswith(delete_container):
            raise errors.ExecutionError(_("%s: user is already preserved" % pkey))
        # Check that this value is a Active user
        try:
            original_entry_attrs = self._exc_wrapper(pkey, options, ldap.get_entry)(
                dn, ["dn"]
            )
        except errors.NotFound:
        ...
        self._exc_wrapper(pkey, options, ldap.move_entry)(dn, delete_dn, del_old=True)
        ...
        entry_attrs = self._exc_wrapper(pkey, options, ldap.get_entry)(
            delete_dn, attrs_to_restore
        )

        restoreAttr = False
        for attr in attrs_to_restore:
            if (attr.lower() in original_entry_attrs) and not (
                attr.lower() in entry_attrs
            ):
                restoreAttr = True
    ...
    def pre_callback(self, ldap, dn, *keys, **options):
        ...
        results = self.api.Command.otptoken_find(ipatokenowner=owner, no_members=False)[
            "result"
        ]
...
class user_find(baseuser_find):
    ...
    msg_summary = ngettext("%(count)d user matched", "%(count)d users matched", 0)
    ...
    def pre_callback(self, ldap, filter, attrs_list, base_dn, scope, *keys, **options):
        ...
        self.pre_common_callback(
            ldap, filter, attrs_list, base_dn, scope, *keys, **options
        )
...
class user_undel(LDAPQuery):
    ...
    def execute(self, *keys, **options):
        ...
        self._exc_wrapper(keys, options, ldap.move_entry)(
            delete_dn, active_dn, del_old=True
        )
...
class user_stage(LDAPMultiQuery):
    ...
    def execute(self, *keys, **options):
            ...
            try:
                ...
                if certmapdata:
                    self.api.Command.stageuser_add_certmapdata(
                        *single_keys, ipacertmapdata=certmapdata
                    )
                try:
        ...
        return dict(
            result=dict(failed=pkey_to_value(failed, options),),
        ...
        for host in masters:
            if host == api.env.host:
                ...
                try:
                    ...
                    logger.error(
                        "user_status: Connecting to %s failed with " "%s", host, str(e)
                    )
                    newresult = {"dn": dn}
                    newresult["server"] = _("%(host)s failed: %(error)s") % dict(
                ...
                if not options.get("raw", False):
                    for attr in ["krblastsuccessfulauth", "krblastfailedauth"]:
                        try:
                            ...
                            logger.debug("time conversion failed with %s", str(e))
