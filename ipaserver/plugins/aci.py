...
_type_map = {
    ...
    "service": "ldap:///"
    + str(DN(("krbprincipalname", "*"), api.env.container_service, api.env.basedn)),
    "netgroup": "ldap:///"
    + str(DN(("ipauniqueid", "*"), api.env.container_netgroup, api.env.basedn)),
    "dnsrecord": "ldap:///"
    + str(DN(("idnsname", "*"), api.env.container_dns, api.env.basedn)),
}

_valid_permissions_values = [u"read", u"write", u"add", u"delete", u"all"]

_valid_prefix_values = (u"permission", u"delegation", u"selfservice", u"none")
...
def _make_aci(ldap, current, aciname, kw):
    ...
    if valid["type"] + valid["filter"] + valid["subtree"] + valid["targetgroup"] > 1:
        raise errors.ValidationError(
            name="target",
            error=_("type, filter, subtree and targetgroup are mutually exclusive"),
        )

    if "aciprefix" not in kw:
        raise errors.ValidationError(
            name="aciprefix", error=_("ACI prefix is required")
        )
    ...
    if valid["filter"] + valid["memberof"] > 1:
        raise errors.ValidationError(
            name="target", error=_("filter and memberof are mutually exclusive")
        )
    ...
    if group + permission + selfaci > 1:
        raise errors.ValidationError(
            name="target", error=_("group, permission and self are mutually exclusive")
        )
    elif group + permission + selfaci == 0:
        raise errors.ValidationError(
            name="target", error=_("One of group, permission or self is required")
        )
    ...
    try:
        ...
        raise errors.ValidationError(
            name="target", error=_("Syntax Error: %(error)s") % dict(error=str(e))
        )
    ...
    if groupdn == "self":
        ...
        if len(groupdn) and groupdn[0].attr == "cn":
            ...
            try:
                ...
                if test:
                    dn = DN(
                        ("cn", "test"), api.env.container_permission, api.env.basedn
                    )
...
_prefix_option = StrEnum(
    "aciprefix",
    cli_name="prefix",
    label=_("ACI prefix"),
    doc=_(
        "Prefix used to distinguish ACI types "
...
Search for ACIs.
    ...
    takes_options = (
        _prefix_option.clone_rename("aciprefix?", required=False),
...
class aci_show(crud.Retrieve):
    ...
    takes_options = (
        _prefix_option,
        DNParam("location?", label=_("Location of the ACI"),),
    )
...
class aci_rename(crud.Update):
    ...
    takes_options = (
        _prefix_option,
        Str("newname", doc=_("New ACI name"),),
    )
    ...
    def execute(self, aciname, **kw):
        ...
        if kw.get("raw", False):
            ...
            result = _aci_to_kw(ldap, newaci)
        return dict(result=result, value=pkey_to_value(kw["newname"], kw),)
