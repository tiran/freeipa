...
class ca_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d CA matched", "%(count)d CAs matched", 0)
...
class ca_show(LDAPRetrieve):
    ...
    takes_options = LDAPRetrieve.takes_options + (_chain_flag,)
...
class ca_add(LDAPCreate):
    ...
    takes_options = LDAPCreate.takes_options + (_chain_flag,)
    ...
    def pre_callback(self, ldap, dn, entry, entry_attrs, *keys, **options):
        ...
        dn_attrs = {
            ava.attr.lower() for rdn in options["ipacasubjectdn"] for ava in rdn
        }
        x509_attrs = {attr.lower() for attr in six.viewvalues(ATTR_NAME_BY_OID)}
...
class ca_del(LDAPDelete):
    ...
    def pre_callback(self, ldap, dn, *keys, **options):
        ...
        if not ldap.can_delete(dn):
            raise errors.ACIError(info=_("Insufficient privilege to delete a CA."))

        if keys[0] == IPA_CA_CN:
            raise errors.ProtectedEntryError(
                label=_("CA"), key=keys[0], reason=_("IPA CA cannot be deleted")
            )
...
class ca_mod(LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if "rename" in options or "cn" in entry_attrs:
            if keys[0] == IPA_CA_CN:
                raise errors.ProtectedEntryError(
                    label=_("CA"), key=keys[0], reason=u"IPA CA cannot be renamed"
                )
...
class CAQuery(LDAPQuery):
    ...
    def execute(self, cn, **options):
        ...
        if not self.api.Backend.ldap2.can_write(ca_obj["dn"], "description"):
            raise errors.ACIError(info=_("Insufficient privilege to modify a CA."))
...
class ca_disable(CAQuery):
    ...
    def execute(self, cn, **options):
        if cn == IPA_CA_CN:
            raise errors.ProtectedEntryError(
                label=_("CA"), key=cn, reason=_("IPA CA cannot be disabled")
            )
