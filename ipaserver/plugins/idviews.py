...
from ipalib.util import normalize_sshpubkey, validate_sshpubkey, convert_sshpubkey_post
...
fallback_to_ldap_option = Flag(
    ...
    doc=_(
        "Allow falling back to AD DC LDAP when resolving AD "
...
class idview(LDAPObject):
    ...
    takes_params = (
        Str("cn", cli_name="name", label=_("ID View Name"), primary_key=True,),
...
class idview_add(LDAPCreate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if "ipadomainresolutionorder" in entry_attrs:
            add_missing_object_class(
                ldap, u"ipanameresolutiondata", dn, entry_attrs, update=False
            )
        return dn
...
class idview_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d ID View matched", "%(count)d ID Views matched", 0)
...
class idview_show(LDAPRetrieve):
    ...
    def show_id_overrides(self, dn, entry_attrs):
        ...
        for objectclass, obj_type in [
            ("ipaUserOverride", "user"),
            ...
            try:
                ...
                for override in overrides:
                    ...
                    try:
                        name = resolve_anchor_to_object_name(ldap, obj_type, anchor)
    ...
    def enumerate_hosts(self, dn, entry_attrs):
        ...
        try:
            ...
            entry_attrs["appliedtohosts"] = [host.single_value["cn"] for host in hosts]
        except errors.NotFound:
            pass
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        if (options.get("show_hosts") or options.get("all")) and not options.get("raw"):
        ...
        for host in hosts_to_apply:
            try:
                ...
                try:
                    ...
                    failed["host"].append(
                        (host, unicode(_("ID View cannot be applied to IPA master")))
                    )
                    continue
                host_dn = api.Object["host"].get_dn_if_exists(host)

                host_entry = ldap.get_entry(host_dn, attrs_list=["ipaassignedidview"])
                ...
                failed["host"].append((host, unicode(_("ID View already applied"))))
            except errors.NotFound:
...
class idview_apply(baseidview_apply):
    __doc__ = _(
        "Applies ID View to specified hosts or current members of "
        "specified hostgroups. If any other ID View is applied to "
        "the host, it is overridden."
    )

    member_count_out = (
        _("ID View applied to %i host."),
    ...
    has_output = (
        output.summary,
        output.Output(
            "succeeded", type=dict, doc=_("Hosts that this ID View was applied to."),
        ),
        output.Output(
            "failed",
            type=dict,
        ...
        output.Output(
            "completed", type=int, doc=_("Number of hosts the ID View was applied to:"),
        ),
    )
...
class idview_unapply(baseidview_apply):
    __doc__ = _(
        "Clears ID View from specified hosts or current members of "
        "specified hostgroups."
    )

    member_count_out = (
        _("ID View cleared from %i host."),
    ...
    takes_options = (
        ...
        Str(
            ...
            cli_name="hostgroups",
            doc=_(
                "Hostgroups whose hosts should have ID Views cleared. Note "
    ...
    has_output = (
        output.summary,
        output.Output(
            "succeeded", type=dict, doc=_("Hosts that ID View was cleared from."),
        ),
        output.Output(
            "failed",
            type=dict,
...
def verify_trusted_domain_object_type(validator, desired_type, name_or_sid):
    ...
    if object_type == desired_type:
        ...
        raise errors.ConversionError(
            name="identifier",
            error=_(
                "You are trying to reference a magic private group "
...
def resolve_object_to_anchor(ldap, obj_type, obj, fallback_to_ldap):
    ...
    try:
        entry = ldap.get_entry(
            api.Object[obj_type].get_dn(obj), attrs_list=["ipaUniqueID", "objectClass"]
        )
        ...
        required_objectclass = {"user": "posixaccount", "group": "ipausergroup",}[
            obj_type
        ]

        if not api.Object[obj_type].has_objectclass(
            entry["objectclass"], required_objectclass
        ):
            raise errors.ValidationError(
                name=_("IPA object"),
                error=_(
                    "system IPA objects (e.g. system groups, user "
...
def resolve_anchor_to_object_name(ldap, obj_type, anchor):
    ...
    raise errors.NotFound(
        reason=_("Anchor '%(anchor)s' could not be resolved.") % dict(anchor=anchor)
    )
...
def remove_ipaobject_overrides(ldap, api, dn):
    ...
    override_filter = "(ipaanchoruuid=:IPA:{0}:{1})".format(api.env.domain, object_uuid)
    try:
    ...
    def filter_for_anchor(self, ldap, filter, options, obj_type):
        ...
        try:
            ...
            return ldap.make_filter(
                {"objectClass": self.object_class, "ipaanchoruuid": resolved_anchor,},
...
class baseidoverride_mod(LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        if "rename" in options:
            raise errors.ValidationError(
                name=_("ID override"), error=_("ID overrides cannot be renamed")
            )
...
class baseidoverride_find(LDAPSearch):
    ...
    msg_summary = ngettext(
        "%(count)d ID override matched", "%(count)d ID overrides matched", 0
    )
...
class idoverrideuser(baseidoverride):
    ...
    takes_params = baseidoverride.takes_params + (
        ...
        Str("gecos?", label=_("GECOS"),),
        Int("gidnumber?", label=_("GID"), doc=_("Group ID Number"), minvalue=1,),
        Str("homedirectory?", cli_name="homedir", label=_("Home directory"),),
        Str("loginshell?", cli_name="shell", label=_("Login shell"),),
...
class idoverrideuser_add_cert(LDAPAddAttributeViaOption):
    ...
    takes_options = LDAPAddAttributeViaOption.takes_options + (fallback_to_ldap_option,)
...
class idoverrideuser_add(baseidoverride_add):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        dn = super(idoverrideuser_add, self).pre_callback(
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        dn = super(idoverrideuser_add, self).post_callback(
...
class idoverrideuser_mod(baseidoverride_mod):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        dn = super(idoverrideuser_mod, self).pre_callback(
            ldap, dn, entry_attrs, attrs_list, *keys, **options
        )
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        dn = super(idoverrideuser_mod, self).post_callback(
            ldap, dn, entry_attrs, *keys, **options
        )
        convert_sshpubkey_post(entry_attrs)
        self.obj.convert_usercertificate_post(entry_attrs, **options)
...
class idoverrideuser_find(baseidoverride_find):
    ...
    msg_summary = ngettext(
        "%(count)d User ID override matched", "%(count)d User ID overrides matched", 0
    )
...
class idoverrideuser_show(baseidoverride_show):
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        dn = super(idoverrideuser_show, self).post_callback(
            ldap, dn, entry_attrs, *keys, **options
        )
        convert_sshpubkey_post(entry_attrs)
        self.obj.convert_usercertificate_post(entry_attrs, **options)
...
class idoverridegroup_find(baseidoverride_find):
    ...
    msg_summary = ngettext(
        "%(count)d Group ID override matched", "%(count)d Group ID overrides matched", 0
    )
