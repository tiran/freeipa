...
class server_role(Object):
    ...
    default_attributes = ["role", "status"]
...
class server_role_find(Search):
    ...
    msg_summary = ngettext(
        "%(count)s server role matched", "%(count)s server roles matched", 0
    )
    takes_options = Search.takes_options + (
        ...
        Flag("include_master", doc=_("Include IPA master entries"),),
    )
    ...
    def execute(self, *keys, **options):
        ...
        if options.get("include_master") or role_name == "IPA master":
            ...
            result = [r for r in role_status if r[u"role_servrole"] != "IPA master"]
