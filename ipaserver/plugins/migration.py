...
_krb_failed_msg = _(
    "Unable to determine if Kerberos principal %s already exists. Use 'ipa user-mod' to set it manually."
)
_grp_err_msg = _(
    "Failed to add user to the default group. Use 'ipa group-add-member' to add manually."
)
...
_supported_scopes = {
    u"base": SCOPE_BASE,
    u"onelevel": SCOPE_ONELEVEL,
    u"subtree": SCOPE_SUBTREE,
}
...
def _pre_migrate_user(ldap, pkey, dn, entry_attrs, failed, config, ctx, **kwargs):
    ...
    if "gidnumber" not in entry_attrs:
        raise errors.NotFound(
            reason=_("%(user)s is not a POSIX user") % dict(user=pkey)
        )
    else:
        ...
        if entry_attrs["gidnumber"][0] in invalid_gids:
            ...
            try:
                ...
                logger.warning(
                    "Search limit exceeded searching for GID %s",
                    entry_attrs["gidnumber"][0],
                )
    ...
    for attr in entry_attrs.keys():
        if ldap.has_dn_syntax(attr):
            for ind, value in enumerate(entry_attrs[attr]):
                ...
                logger.debug("converting DN value %s for %s in %s", value, attr, dn)
                rdnval = remote_entry[primary_key][0].lower()
                entry_attrs[attr][ind] = DN(
                    (primary_key, rdnval), container, api.env.basedn
                )
...
def _update_default_group(ldap, ctx, force):
    ...
    if force or migrate_cnt % 100 == 0:
        ...
        try:
            ...
            logger.error(
                "Adding new members to default group failed: %s \n" "members: %s",
                e,
                ",".join(member_dns),
            )
        ...
        logger.info("Adding %d users to group%s duration %s", len(member_dns), mode, d)
...
def _pre_migrate_group(ldap, pkey, dn, entry_attrs, failed, config, ctx, **kwargs):
    def convert_members_rfc2307bis(member_attr, search_bases, overwrite=False):
        ...
        for m in entry_attrs[member_attr]:
            ...
            if m.endswith(search_bases["user"]):
                ...
                m = DN(
                    (api.Object.user.primary_key.name, rdnval),
                ...
                m = DN(
                    (api.Object.group.primary_key.name, rdnval),
                    api.env.container_group,
                    api.env.basedn,
                )
            else:
                logger.error("entry %s does not belong into any known " "container", m)
                continue
    ...
    def convert_members_rfc2307(member_attr):
        ...
        for m in entry_attrs[member_attr]:
            memberdn = DN(
                (api.Object.user.primary_key.name, m),
                api.env.container_user,
                api.env.basedn,
            )
...
def _group_exc_callback(ldap, dn, entry_attrs, exc, options):
    ...
    if isinstance(exc, errors.DuplicateEntry):
        if (
            options.get("groupoverwritegid", False)
            and entry_attrs.get("gidnumber") is not None
        ):
            try:
            ...
            not options.get("groupoverwritegid", False)
            and entry_attrs.get("gidnumber") is not None
        ):
            msg = unicode(exc)
...
class migrate_ds(Command):
    ...
    takes_options = (
        ...
        Flag(
            ...
            doc=_(
                "When migrating a group already existing in IPA domain overwrite the "
        ...
        StrEnum(
            ...
            doc=_(
                "LDAP search scope for users and groups: base, "
    ...
    has_output = (
        ...
        output.Output(
            "enabled", type=bool, doc=_("False if migration mode was disabled."),
        ),
        output.Output(
            "compat",
            type=bool,
...
migration process might be incomplete\n"""
    ...
    def get_options(self):
        ...
        for ldap_obj_name in self.migrate_objects:
            ...
            yield Str(
                "%s*" % name, cli_name=name, doc=doc, default=tuple(), autofill=True
            )
        ...
        for ldap_obj_name in self.migrate_order:
            ...
            for entry_attrs in entries:
                ...
                if migrate_cnt > 0 and migrate_cnt % 100 == 0:
                    logger.info(
                        "%d %ss migrated. %s elapsed.",
                        migrate_cnt,
                        ldap_obj_name,
                        total_dur,
                    )
                logger.debug(
                    "%d %ss migrated, duration: %s (total %s)",
                    migrate_cnt,
                    ldap_obj_name,
                    d,
                    total_dur,
    ...
    def execute(self, ldapuri, bindpw, **options):
        ...
        # migrate!
        (migrated, failed) = self.migrate(ldap, config, ds_ldap, ds_base_dn, options)
