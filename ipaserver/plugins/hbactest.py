...
class hbactest(Command):
    ...
    has_output = (
        ...
        output.Output(
            "error", (list, tuple, type(None)), _("Non-existent or invalid rules")
        ),
        output.Output("value", bool, _("Result of simulation"), ["no_display"]),
    )

    takes_options = (
        Str("user", cli_name="user", label=_("User name"), primary_key=True,),
        Str(
            "sourcehost?",
            deprecated=True,
            cli_name="srchost",
        ...
        Str("targethost", cli_name="host", label=_("Target host"),),
        Str("service", cli_name="service", label=_("Service"),),
        Str(
            "rules*",
            cli_name="rules",
    ...
    def execute(self, *args, **options):
        ...
        if options["user"] != u"all":
            ...
            if is_valid_sid or "domain" in components or "flatname" in components:
                ...
                (
                    user_sid,
                    group_sids,
                ) = domain_validator.get_trusted_domain_user_and_groups(options["user"])
                ...
                try:
                    ...
                    search_result = self.api.Command.user_show(request.user.name)[
                        "result"
                    ]
        ...
        if options["service"] != u"all":
            try:
                ...
                service_result = self.api.Command.hbacsvc_show(request.service.name)[
                    "result"
                ]
        ...
        if options["targethost"] != u"all":
            try:
                ...
                tgthost_result = self.api.Command.host_show(request.targethost.name)[
                    "result"
                ]
            ...
            for ipa_rule in rules:
                try:
                    ...
                    if code == pyhbac.HBAC_EVAL_ERROR:
                        ...
                        logger.info(
                            'Native IPA HBAC rule "%s" parsing error: ' "%s",
                            rule_name,
