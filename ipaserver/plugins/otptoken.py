...
def _convert_owner(userobj, entry_attrs, options):
    if "ipatokenowner" in entry_attrs and not options.get("raw", False):
        entry_attrs["ipatokenowner"] = [
            userobj.get_primary_key_from_dn(o) for o in entry_attrs["ipatokenowner"]
        ]
...
class otptoken_add(LDAPCreate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if not _check_interval(
            options.get("ipatokennotbefore", None),
            options.get("ipatokennotafter", None),
        ):
            raise ValidationError(
                name="not_after", error="is before the validity start"
            )
        ...
        if owner is not None:
            try:
                issuer = ldap.get_entry(owner, ["krbprincipalname"])[
                    "krbprincipalname"
                ][0]
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        return super(otptoken_add, self).post_callback(
...
class otptoken_mod(LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if not _check_interval(notbefore, notafter):
            if notafter_set:
                ...
                raise ValidationError(
                    name="not_before", error="is after the validity end"
                )
        _normalize_owner(self.api.Object.user, entry_attrs)
        ...
        if "ipatokenowner" in entry_attrs and "managedby" not in entry_attrs:
            ...
            prev_entry = ldap.get_entry(dn, attrs_list=["ipatokenowner", "managedby"])
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        _set_token_type(entry_attrs, **options)
        _convert_owner(self.api.Object.user, entry_attrs, options)
        return super(otptoken_mod, self).post_callback(
            ldap, dn, entry_attrs, *keys, **options
        )
...
class otptoken_find(LDAPSearch):
    ...
    def pre_callback(self, ldap, filters, attrs_list, *args, **kwargs):
        ...
        filters = filters.replace(
            "(objectclass=ipatoken)", "(objectclass=ipatoken%s)" % type
        )

        attrs_list.append("objectclass")
        return super(otptoken_find, self).pre_callback(
            ldap, filters, attrs_list, *args, **kwargs
        )
    ...
    def post_callback(self, ldap, entries, truncated, *args, **options):
        ...
        return super(otptoken_find, self).post_callback(
...
class otptoken_show(LDAPRetrieve):
    ...
    def pre_callback(self, ldap, dn, attrs_list, *keys, **options):
        attrs_list.append("objectclass")
        return super(otptoken_show, self).pre_callback(
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        _set_token_type(entry_attrs, **options)
        _convert_owner(self.api.Object.user, entry_attrs, options)
        return super(otptoken_show, self).post_callback(
            ldap, dn, entry_attrs, *keys, **options
        )
