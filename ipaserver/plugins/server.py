...
class server(LDAPObject):
    ...
    takes_params = (
        ...
        Str(
            "iparepltopomanagedsuffix*", flags={"no_create", "no_update", "no_search"},
        ),
        Str(
    ...
    def _get_suffixes(self):
        suffixes = self.api.Command.topologysuffix_find(all=True, raw=True,)["result"]
    ...
    def get_enabled_roles(self, entry_attrs, **options):
        ...
        enabled_roles = self.api.Command.server_role_find(
            server_server=entry_attrs["cn"][0], status=ENABLED, include_master=True,
        )["result"]
...
class server_mod(LDAPUpdate):
    ...
    def args_options_2_entry(self, *args, **options):
        kw = super(server_mod, self).args_options_2_entry(*args, **options)
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        if "ipalocation" in entry_attrs or "ipaserviceweight" in entry_attrs:
            ...
            entry_attrs["objectclass"] = server_entry["objectclass"] + [
                "ipalocationmember"
            ]
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        if "ipalocation_location" or "ipaserviceweight" in options:
            self.add_message(
                messages.ServiceRestartRequired(
                    service=services.service("named", api).systemd_name, server=keys[0],
                )
            )
        ...
        if ipalocation:
            servers_in_loc = self.api.Command.server_find(
                in_location=ipalocation, no_members=False
            )["result"]
...
class server_find(LDAPSearch):
    ...
    msg_summary = ngettext(
        "%(count)d IPA server matched", "%(count)d IPA servers matched", 0
    )

    member_attributes = ["iparepltopomanagedsuffix", "ipalocation", "role"]

    def args_options_2_entry(self, *args, **options):
        kw = super(server_find, self).args_options_2_entry(*args, **options)
        self.obj.normalize_location(kw, **options)
        return kw
    ...
    def _get_enabled_servrole_filter(self, ldap, servroles):
        ...
        return ldap.make_filter_from_attr(
            "cn", list(enabled_masters), rules=ldap.MATCH_ANY
        )
    ...
    def pre_callback(self, ldap, filters, attrs_list, base_dn, scope, *args, **options):
        ...
        if included or excluded:
            ...
            if included:
                ...
                filters = ldap.combine_filters((filters, filter), ldap.MATCH_ALL)
        ...
        if options.get("servrole", []):
            ...
            filters = ldap.combine_filters((filters, servrole_filter), ldap.MATCH_ALL)
...
class server_del(LDAPDelete):
        ...
        if self.api.Command.ca_is_enabled()["result"]:
            try:
                ...
                if kra_servers == [hostname]:
                    handler(
                        _(
                            "Deleting this server is not allowed as it would "
        ...
        if ignore_last_of_role:
            self.add_message(
                messages.ServerRemovalWarning(
                    message=_("Ignoring these warnings and proceeding with " "removal")
                )
            )
    ...
    def _remove_server_principal_references(self, master):
        ...
        for (dn, member_principal) in (
            ...
            try:
                ...
                self.add_message(
                    messages.ServerRemovalWarning(
                        message=_(
                            "Failed to clean memberPrincipal "
                            "%(principal)s from s4u2proxy entry %(dn)s: "
                            "%(err)s"
                        )
                        % dict(
                            principal=(member_principal.decode("utf-8")), dn=dn, err=e
                        )
                    )
                )
        ...
        try:
            ...
            entries = conn.get_entries(etc_basedn, ldap.SCOPE_SUBTREE, filter=filter)
            if len(entries) != 0:
                for entry in entries:
        ...
        try:
            ...
            self.add_message(
                messages.ServerRemovalWarning(
                    message=_(
                        "Failed to remove server %(master)s from server "
                        "list: %(err)s"
                    )
                    % dict(master=master, err=e)
                )
    ...
    def _remove_server_custodia_keys(self, ldap, master):
        ...
        try:
            entries = conn.get_entries(
                custodia_subtree, ldap.SCOPE_SUBTREE, filter=member_filter
            )
            for entry in entries:
                conn.delete_entry(entry)
        except errors.NotFound:
            ...
            self.add_message(
                messages.ServerRemovalWarning(
                    message=_(
                        "Failed to clean up Custodia keys for " "%(master)s: %(err)s"
                    )
                    % dict(master=master, err=e)
                )
            )
    ...
    def _remove_server_host_services(self, ldap, master):
        ...
        try:
            ...
            if master == self.api.env.host:
                filter = (
                    "(&(krbprincipalname=*/{}@{})"
                    "(!(krbprincipalname=ldap/*)))".format(master, self.api.env.realm)
                )
            else:
                filter = "(krbprincipalname=*/{}@{})".format(master, self.api.env.realm)
            ...
            self.add_message(
                messages.ServerRemovalWarning(
                    message=_("Failed to cleanup server principals/keys: " "%(err)s")
                    % dict(err=e)
                )
            )
    ...
    def _cleanup_server_dns_records(self, hostname, **options):
        ...
        try:
            bindinstance.remove_master_dns_records(hostname, self.api.env.realm)
            dnskeysyncinstance.remove_replica_public_keys(hostname)
        except Exception as e:
            self.add_message(
                messages.ServerRemovalWarning(
                    message=_("Failed to cleanup %(hostname)s DNS entries: " "%(err)s")
                    % dict(hostname=hostname, err=e)
                )
            )

            self.add_message(
                messages.ServerRemovalWarning(
                    message=_("You may need to manually remove them from the " "tree")
                )
            )
    ...
    def pre_callback(self, ldap, dn, *keys, **options):
        ...
        if options.get("force", False):
            self.add_message(
                messages.ServerRemovalWarning(
                    message=_("Forcing removal of %(hostname)s") % dict(hostname=pkey)
                )
            )

        # check the topology errors before and after removal
        self.context.topology_connectivity = topology.TopologyConnectivity(self.api)
        ...
        if options.get("ignore_topology_disconnect", False):
            ...
            self._check_topology_connectivity(self.context.topology_connectivity, pkey)
    ...
    def exc_callback(self, keys, options, exc, call_func, *call_args, **call_kwargs):
        if (
            ...
            and isinstance(exc, errors.NotFound)
    ...
    def _check_deleted_segments(self, hostname, topology_connectivity, starting_host):
        def wait_for_segment_removal(
            hostname, master_cns, suffix_name, orig_errors, new_errors
        ):
            i = 0
            while True:
                ...
                if orig_errors or new_errors:
                    ...
                    left = [
                        s
                        for s in left
                        if s["iparepltoposegmentrightnode"][0] in can_contact_me
                    ]
                    right = [
                        s
                        for s in right
                ...
                if not left and not right:
                    self.add_message(
                        messages.ServerRemovalInfo(message=_("Agreements deleted"))
                    )
                    return
                time.sleep(2)
                if i == 2:  # taking too long, something is wrong, report
                    logger.info("Waiting for removal of replication agreements")
                if i > 90:
                    ...
                    for s in left:
                        self.add_message(
                            messages.ServerRemovalWarning(message=u"  %s" % s["cn"][0])
                        )
                    for s in right:
                        self.add_message(
                            messages.ServerRemovalWarning(message=u"  %s" % s["cn"][0])
        ...
        new_errors = topology_connectivity.errors_after_master_removal(hostname)
        ...
        for suffix_name in topology_graphs:
            ...
            if hostname not in suffix_members:
                ...
                # were not deleted.
                logger.info(
                    "Skipping replication agreement deletion check for " "suffix '%s'",
                    suffix_name,
                )
                continue

            logger.info("Checking for deleted segments in suffix '%s", suffix_name)
...
class server_conncheck(crud.PKQuery):
    ...
    def execute(self, *keys, **options):
        ...
        result = dict(result=(ret == 0), value=keys[-2],)
...
class server_state(crud.PKQuery):
    ...
    def _check_hide_server(self, fqdn):
        ...
        for key, name in checks:
            ...
            if values == [fqdn]:  # fqdn is the only entry
                err.append(
                    _("Cannot hide last enabled %(name)s server.") % {"name": name}
                )
        if err:
            raise errors.ValidationError(name=fqdn, error=" ".join(str(e) for e in err))
    ...
    def execute(self, *keys, **options):
        ...
        roles = self.api.Command.server_role_find(
            server_server=fqdn, status=from_status, include_master=True,
        )["result"]
