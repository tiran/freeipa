...
def normalize_principal(value):
    ...
    try:
        ...
        raise errors.ValidationError(name="principal", reason=_("Malformed principal"))
    ...
    for cert in certs:
        ...
        try:
            api.Command["cert_revoke"](
                cert["serial_number"], cacn=cert["cacn"], revocation_reason=4,
            )
...
def set_certificate_attrs(entry_attrs):
    ...
    entry_attrs["valid_not_before"] = x509.format_datetime(cert.not_valid_before)
    ...
    try:
        ...
        if principal.service_name in service_types:
            raise errors.ValidationError(
                name="principal",
                error=_("This principal is required by the IPA master"),
            )
...
def set_kerberos_attrs(entry_attrs, options):
    ...
    try:
        ticket_flags = entry_attrs.single_value.get(
            "krbticketflags", _ticket_flags_default
        )
        ticket_flags = int(ticket_flags)
    except ValueError:
        return
...
class service(LDAPObject):
    ...
    def populate_krbcanonicalname(self, entry_attrs, options):
        ...
        entry_attrs.setdefault("krbcanonicalname", entry_attrs["krbprincipalname"])
...
class service_add(LDAPCreate):
    ...
    takes_options = LDAPCreate.takes_options + (
        ...
        Flag(
            ...
            doc=_(
                "force service to be created even when host "
...
class service_add_smb(LDAPCreate):
    ...
    smb_takes_args = (
        Str(
            ...
            flags={"virtual_attribute", "no_display", "no_update", "no_search"},
        ),
        Str(
            "ipantflatname?",
    ...
    def get_options(self):
        ...
        """
        excluded = ("ipakrbauthzdata", "krbprincipalauthind", "ipakrbrequirespreauth")
        for arg in self.takes_options:
            yield arg
        for arg in super(service_add_smb, self).get_options():
            check = all(
                [
                    arg not in self.takes_options,
                    not arg.primary_key,
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        try:
            ...
            raise errors.NotFound(
                reason=_("The host '%s' does not exist to add a service to.") % hostname
            )
        ...
        dn = DN(
            ("krbprincipalname", entry_attrs["krbcanonicalname"]),
        ...
        for o in ("ipakrbprincipal", "ipaidobject", "krbprincipalaux", "posixaccount"):
            if o not in entry_attrs["objectclass"]:
                entry_attrs["objectclass"].append(o)

        entry_attrs["uid"] = ["/".join(kerberos.Principal(smbprincipal).components)]
        ...
        if "usercertificate" in options and ca_is_enabled:
            ...
            for cert in removed_certs:
                rm_certs = api.Command.cert_find(
                    certificate=cert.public_bytes(x509.Encoding.DER), service=keys
                )["result"]
...
class service_find(LDAPSearch):
    ...
    msg_summary = ngettext("%(count)d service matched", "%(count)d services matched", 0)
    member_attributes = ["managedby"]
...
class service_show(LDAPRetrieve):
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        try:
            ...
            self.add_message(messages.CertificateInvalid(subject=principal, reason=e,))
            logger.error("Invalid certificate: %s", e)
            del entry_attrs["usercertificate"]
...
class service_allow_create_keytab(LDAPAddMember):
    __doc__ = _(
        "Allow users, groups, hosts or host groups to create a keytab "
        "of this service."
    )
...
class service_remove_cert(LDAPRemoveAttributeViaOption):
    ...
    def post_callback(self, ldap, dn, entry_attrs, *keys, **options):
        ...
        for cert in options.get("usercertificate", []):
            revoke_certs(
                api.Command.cert_find(certificate=cert, service=keys)["result"]
            )
