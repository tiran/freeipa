...
from ipapython.certdb import IPA_CA_TRUST_FLAGS, EXTERNAL_CA_TRUST_FLAGS, TrustFlags
...
def get_ds_instances():
    ...
    for basename in os.listdir(dirsrv_instance_dir):
        ...
        if os.path.isdir(pathname):
            ...
            if basename.startswith(DS_INSTANCE_PREFIX) and not basename.endswith(
                ".removed"
            ):
                # Strip off prefix
...
class DsInstance(service.Service):
    ...
    def __common_setup(self):
        ...
        self.step(
            "configuring replication version plugin", self.__config_version_module
        )
        ...
        self.step(
            "configure new location for managed entries", self.__repoint_managed_entries
        )
        self.step("configure dirsrv ccache and keytab", self.configure_systemd_ipa_env)
    ...
    def __common_post_setup(self):
        ...
        self.step("configuring Posix uid/gid generation", self.__config_uidgid_gen)
    ...
    def create_instance(
        ...
        self.step(
            "adding sasl mappings to the directory", self.__configure_sasl_mappings
        )
    ...
    def create_replica(
        ...
        self.step(
            "prevent time skew after initial replication", self.replica_manage_time_skew
        )
        self.step(
            "adding sasl mappings to the directory", self.__configure_sasl_mappings
        )
        ...
        self.step(
            "setting Auto Member configuration", self.__add_replica_automember_config
        )
    ...
    def __setup_replica(self):
        ...
        that the bind method for the replication differs between domain levels:
            ...
            * in domain level 1, GSSAPI using admin/privileged host credentials
              ...
              stage)
        """
        replication.enable_replication_version_checking(self.realm, self.dm_password)

        repl, bind_dn, bind_pw = self._get_replication_manager()
        repl.setup_promote_replication(
            self.master_fqdn, r_binddn=bind_dn, r_bindpw=bind_pw, cacert=self.ca_file
        )
        ...
        try:
            ...
            for r in res:
                try:
                    ...
                    logger.critical("Error during SASL mapping removal: %s", e)
                    raise
        except Exception as e:
    ...
    def __setup_sub_dict(self):
        ...
        self.sub_dict = dict(
            ...
            EXCLUDES="(objectclass=*) $ EXCLUDE " + " ".join(replication.EXCLUDES),
    ...
    def __create_instance(self):
        ...
        userroot = {"cn": "userRoot", "nsslapd-suffix": self.suffix.ldap_text()}
        ...
        inst.ldapi_socket = paths.SLAPD_INSTANCE_SOCKET_TEMPLATE % (self.serverid)
        ...
        try:
            ...
            ipadomain.create(
                properties={"dc": self.realm.split(".")[0].lower(), "info": "IPA V2.0",}
            )
        finally:
            inst.close()
    ...
    def __update_dse_ldif(self):
        ...
        dse_filename = os.path.join(
            paths.ETC_DIRSRV_SLAPD_INSTANCE_TEMPLATE % self.serverid, "dse.ldif"
        )

        with tempfile.NamedTemporaryFile(mode="w", delete=False) as new_dse_ldif:
    ...
    def __add_default_schemas(self):
        ...
        for schema_fname in IPA_SCHEMA_FILES:
            ...
            shutil.copyfile(
                os.path.join(paths.USR_SHARE_IPA_DIR, schema_fname), target_fname
            )
        ...
        try:
            ...
            shutil.copyfile(
                os.path.join(paths.USR_SHARE_IPA_DIR, "05rfc2247.ldif"), target_fname
            )
    ...
    def stop(self, instance_name="", capture_output=True):
        ...
        super(DsInstance, self).stop(instance_name, capture_output=capture_output)
    ...
    def restart(self, instance_name="", capture_output=True, wait=True):
        ...
        try:
            ...
            if not is_ds_running(instance_name):
                logger.critical(
                    "Failed to restart the directory server. "
            ...
            # TODO: roll back here?
            logger.critical(
                "Failed to restart the directory server (%s). "
                "See the installation log for details.",
                e,
            )
        api.Backend.ldap2.connect()
    ...
    def init_memberof(self):
        ...
        dn = DN(
            ("cn", "IPA install %s" % self.sub_dict["TIME"]),
            ("cn", "memberof task"),
            ("cn", "tasks"),
            ("cn", "config"),
        )
    ...
    def configure_systemd_ipa_env(self):
        ...
        template = os.path.join(paths.USR_SHARE_IPA_DIR, "ds-ipa-env.conf.template")
        sub_dict = dict(
            KRB5_KTNAME=paths.DS_KEYTAB, KRB5CCNAME=paths.TMP_KRB5CC % pent.pw_uid
        )
        conf = ipautil.template_file(template, sub_dict)

        destfile = paths.SLAPD_INSTANCE_SYSTEMD_IPA_ENV_TEMPLATE % (self.serverid)
        ...
        if os.path.isfile(paths.SYSCONFIG_DIRSRV):
            ...
            ipautil.config_replace_variables(
                paths.SYSCONFIG_DIRSRV, removevars={"KRB5_KTNAME", "KRB5CCNAME"}
            )
    ...
    def __enable_ssl(self):
        ...
        try:
            ...
            logger.debug("cn=encryption,cn=config is already properly configured")
    ...
    def __upload_ca_cert(self):
        ...
        for nickname in nicknames:
            ...
            certstore.put_ca_cert_nss(
                conn, self.suffix, cert, nickname, trust_flags[nickname]
            )
    ...
    def __setup_s4u2proxy(self):
        def __add_principal(last_cn, principal, self):
            dn = DN(("cn", last_cn), ("cn", "s4u2proxy"), ("cn", "etc"), self.suffix)

            value = "{principal}/{fqdn}@{realm}".format(
                fqdn=self.fqdn, realm=self.realm, principal=principal
            )
    ...
    def __certmap_conf(self):
        ...
        sysupgrade.set_upgrade_state(
            "certmap.conf", "subject_base", str(self.subject_base)
        )
    ...
    def change_admin_password(self, password):
        ...
        with tempfile.NamedTemporaryFile(
            "w", dir=dir_ipa
        ) as dmpwdfile, tempfile.NamedTemporaryFile("w", dir=dir_ipa) as admpwdfile:
            ...
            try:
                env = {
                    "LDAPTLS_CACERTDIR": os.path.dirname(paths.IPA_CA_CRT),
    ...
    def uninstall(self):
        ...
        if serverid is not None:
            ...
            try:
                ...
                logger.error(
                    "Failed to remove DS instance. You may "
                    "need to remove instance data manually"
                )

        else:
            logger.error(
                "Failed to remove DS instance. No serverid present "
                "in sysrestore file."
            )
        ...
        if serverid is None:
            # Remove scripts dir
            scripts = paths.VAR_LIB_DIRSRV_INSTANCE_SCRIPTS_TEMPLATE % (serverid)
            ipautil.rmtree(scripts)

            # remove systemd unit file
            unitfile = paths.SLAPD_INSTANCE_SYSTEMD_IPA_ENV_TEMPLATE % (serverid)
        ...
        for ds_instance in get_ds_instances():
            try:
                ...
                logger.error("Unable to restart DS instance %s: %s", ds_instance, e)
    ...
    def get_server_cert_nickname(self, serverid=None):
        ...
        if serverid is not None:
            ...
            with open(config_file, "r") as in_file:
                parser = upgradeinstance.GetEntryFromLDIF(in_file, entries_dn=[rsa_dn])
                parser.parse()
                try:
                    ...
                    logger.error(
                        "Unable to find server cert nickname in %s", config_file
                    )
    ...
    def start_tracking_certificates(self, serverid):
        ...
        if dsdb.is_ipa_issued_cert(api, nickname):
            ...
            logger.debug(
                "Will not track DS server certificate %s as it is " "not issued by IPA",
                nickname,
            )
    ...
    def add_ca_cert(self, cacert_fname, cacert_name=""):
        ...
        dirname = config_dirname(ipaldap.realm_to_serverid(self.realm))
        certdb = certs.CertDB(
            self.realm,
            nssdir=dirname,
            subject_base=self.subject_base,
        ...
        try:
            ...
            logger.critical(
                "Error importing CA cert file named [%s]: %s", cacert_fname, str(e)
            )
            status = False
    ...
    def find_subject_base(self):
        ...
        """
        logger.debug("Trying to find certificate subject base in sysupgrade")
        subject_base = sysupgrade.get_upgrade_state("certmap.conf", "subject_base")

        if subject_base:
            logger.debug(
                "Found certificate subject base in sysupgrade: %s", subject_base
            )
            return subject_base

        logger.debug("Unable to find certificate subject base in sysupgrade")
        logger.debug("Trying to find certificate subject base in DS")
        ...
        if not ds_is_running:
            try:
                ...
                logger.error(
                    "Cannot start DS to find certificate " "subject base: %s", e
                )
        ...
        if ds_is_running:
            try:
                ...
                logger.debug("Found certificate subject base in DS: %s", subject_base)
            except errors.PublicError as e:
                logger.error(
                    "Cannot connect to DS to find certificate " "subject base: %s", e
                )
...
def write_certmap_conf(realm, ca_subject):
    ...
    shutil.copyfile(
        os.path.join(paths.USR_SHARE_IPA_DIR, "certmap.conf.template"), certmap_filename
    )
