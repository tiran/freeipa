...
class RemoveRUVParser(ldif.LDIFParser):
    ...
    def handle(self, dn, entry):
        ...
        if (
            objectclass
            and nsuniqueid
            and b"nstombstone" in objectclass
            and b"ffffffff-ffffffff-ffffffff-ffffffff" in nsuniqueid
        ):
...
class Restore(admintool.AdminTool):
    ...
    FILES_TO_BE_CLEARED = [paths.HTTPD_NSS_CONF]
    ...
    def add_options(cls, parser):
        ...
        parser.add_option(
            "-p", "--password", dest="password", help="Directory Manager password"
        )
        parser.add_option(
            "--gpg-keyring", dest="gpg_keyring", help=optparse.SUPPRESS_HELP
        )
        parser.add_option(
            "--data",
            dest="data_only",
        ...
        parser.add_option(
            ...
            action="store_true",
            default=False,
            help="Do not restore log files from the backup",
        )
        parser.add_option(
            "-U",
            "--unattended",
    ...
    def run(self):
        ...
        if restore_type == "FULL":
            ...
            if options.instance:
                instance_dir = (
                    paths.VAR_LIB_SLAPD_INSTANCE_DIR_TEMPLATE % options.instance
                )
            ...
            for instance, backend in itertools.product(self.instances, self.backends):
                db_dir = paths.SLAPD_INSTANCE_DB_DIR_TEMPLATE % (instance, backend)
                if os.path.exists(db_dir):
                    break
            else:
                raise admintool.ScriptError(
        ...
        logger.info(
            "Performing %s restore from %s backup", restore_type, self.backup_type
        )

        if self.backup_host != FQDN:
            raise admintool.ScriptError(
                "Host name %s does not match backup name %s" % (FQDN, self.backup_host)
            )
        ...
        if self.backup_ipa_version != str(version.VERSION):
            logger.warning(
                ...
                "Data is version %s.\n"
                "Server is running %s.",
                self.backup_ipa_version,
                str(version.VERSION),
            )
            if not options.unattended and not user_input("Continue to restore?", False):
            ...
            if "DNS" in self.backup_services:
                if not os.path.isfile(paths.IPA_DNS_INSTALL):
                    raise admintool.ScriptError(
                        ...
                        "'{}' package. Please install the package and "
                        "run ipa-restore again.".format(constants.IPA_DNS_PACKAGE_NAME)
                    )
        ...
        try:
            ...
            logger.info("Each master will individually need to be re-initialized or")
            logger.info("re-created from this one. The replication agreements on")
            logger.info("masters running IPA 3.1 or earlier will need to be manually")
    ...
    def get_connection(self):
        ...
        try:
            ...
            raise admintool.ScriptError("Unable to bind to LDAP server: %s" % e)
        return self._conn
    ...
    def disable_agreements(self):
        ...
        try:
            ...
            logger.error(
                "Unable to get connection, skipping disabling " "agreements: %s", e
            )
            return
        ...
        for master in masters:
            ...
            try:
                repl = ReplicationManager(api.env.realm, master, self.dirman_password)
            except Exception as e:
                logger.critical("Unable to disable agreement on %s: %s", master, e)
                continue

            master_dn = DN(("cn", master), api.env.container_masters, api.env.basedn)
            try:
                services = repl.conn.get_entries(master_dn, repl.conn.SCOPE_ONELEVEL)
            ...
            hosts = [rep.single_value.get("nsds5replicahost") for rep in host_entries]

            for host in hosts:
                logger.info("Disabling replication agreement on %s to %s", master, host)
                repl.disable_agreement(host)

            if "CA" in services_cns:
                try:
                    repl = get_cs_replication_manager(
                        api.env.realm, master, self.dirman_password
                    )
                except Exception as e:
                    logger.critical("Unable to disable agreement on %s: %s", master, e)
                    continue
                ...
                host_entries = repl.find_ipa_replication_agreements()
                hosts = [
                    rep.single_value.get("nsds5replicahost") for rep in host_entries
                ]
                for host in hosts:
                    logger.info(
                        "Disabling CA replication agreement on %s to " "%s",
                        master,
                        host,
                    )
    ...
    def ldif2db(self, instance, backend, online=True):
        ...
        if online:
            ...
            args = [paths.DSCTL, instance, "ldif2db", backend, ldiffile]
            result = run(args, raiseonerr=False)
            if result.returncode != 0:
    ...
    def bak2db(self, instance, backend, online=True):
        ...
        if online:
            ...
            args = [paths.DSCTL, instance, "bak2db", os.path.join(self.dir, instance)]
            result = run(args, raiseonerr=False)
            if result.returncode != 0:
    ...
    def restore_default_conf(self):
        ...
        if result.returncode != 0:
            logger.critical(
                "Restoring %s failed: %s", paths.IPA_DEFAULT_CONF, result.error_log
            )
        os.chdir(cwd)
    ...
    def extract_backup(self):
        ...
        if not os.path.exists(filename):
            if not os.path.exists(filename + ".gpg"):
                raise admintool.ScriptError(
                    "Unable to find backup file in %s" % self.backup_dir
                )
            else:
        ...
        args = ["tar", "--xattrs", "--selinux", "-xzf", filename, "."]
        run(args)
    ...
    def __create_dogtag_log_dirs(self):
        ...
        dirs = []
        # dogtag 10
        if os.path.exists(paths.VAR_LIB_PKI_TOMCAT_DIR) and not os.path.exists(
        ...
        try:
            ...
            logger.debug(
                "No %s user exists, skipping CA directory creation", constants.PKI_USER
            )
            return
    ...
    def init_api(self, **overrides):
        ...
        if os.path.exists(paths.IPA_DEFAULT_CONF):
            ...
            if not services.knownservices.dirsrv.is_running(instance_name):
                raise admintool.ScriptError("directory server instance is not running")
            try:
                ReplicationManager(api.env.realm, api.env.host, self.dirman_password)
