...
def _find_dnssec_enabled_zones(conn):
    ...
    try:
        ...
        return [
            entry.single_value["idnsname"] for entry in entries if "idnsname" in entry
        ]
...
def _disable_dnssec():
    ...
    ods_exporter.ldap_remove_service_container(
        "DNSKeyExporter", api.env.host, api.env.basedn
    )
...
def install_check(standalone, api, replica, options, hostname):
    ...
    if not os.path.isfile(paths.IPA_DNS_INSTALL):
        raise RuntimeError(
            "Integrated DNS requires '%s' package" % constants.IPA_DNS_PACKAGE_NAME
        )
    ...
    if (
        options.dnssec_master
        and not options.unattended
        and not ipautil.user_input(
            "Do you want to setup this IPA server as DNSSEC key master?", False
        )
    ):
        sys.exit("Aborted")
    elif (
        options.disable_dnssec_master
        and not options.unattended
        and not ipautil.user_input(
    ...
    if options.disable_dnssec_master:
        ...
        if options.kasp_db_file:
            ...
            if not dnskeysyncd.is_installed():
                raise RuntimeError(
                    "ipa-dnskeysyncd is not configured on this "
            ...
            try:
                ...
                logger.debug("%s", e)
                raise RuntimeError(
                    "This IPA server cannot be promoted to "
    ...
    if options.forwarders:
        if not options.no_dnssec_validation and not bindinstance.check_forwarders(
            options.forwarders
        ):
    ...
    reverse_zones = bindinstance.check_reverse_zones(
        ip_addresses,
        options.reverse_zones,
        options,
        reverse_zones_unattended_check,
        search_reverse_zones,
    )
...
def install(standalone, replica, options, api=api):
    ...
    if options.dnssec_master:
        ...
        ods.create_instance(
            api.env.host, api.env.realm, kasp_db_file=options.kasp_db_file
        )
    elif options.disable_dnssec_master:
        _disable_dnssec()
...
def uninstall_check(options):
    ...
    if api.env.host in masters:
        print(
            "This server is active DNSSEC key master. Uninstall could break your DNS system."
        )
        if not (
            options.unattended
            or user_input(
...
class DNSInstallInterface(hostname.HostNameInstallInterface):
    ...
    * ipa-dns-install
    ...
    allow_zone_overlap = knob(
        None, description="Create DNS zone even if it already exists",
    )
    ...
    no_reverse = knob(None, description="Do not create new reverse DNS zone",)
    no_reverse = prepare_only(no_reverse)

    auto_reverse = knob(None, description="Create necessary reverse zones",)
    ...
    forwarders = knob(
        ...
        None,
        description=("Add a DNS forwarder. This option can be used multiple " "times"),
        cli_names="--forwarder",
    )
    forwarders = enroll_only(forwarders)

    no_forwarders = knob(
        None, description="Do not add any DNS forwarders, use root servers instead",
    )
    no_forwarders = enroll_only(no_forwarders)

    auto_forwarders = knob(
        None, description="Use DNS forwarders configured in /etc/resolv.conf",
    )
    ...
    no_dnssec_validation = knob(None, description="Disable DNSSEC validation",)
