...
def upgrade_file(sub_dict, filename, template, add=False):
    ...
    if old == 0:
        ...
        logger.warning(
            "%s is now managed by IPA. It will be "
...
def check_certs():
    ...
    if not os.path.exists(paths.CA_CRT):
        ...
        if os.path.exists(ca_file):
            ...
            logger.error(
                "You should place a copy of the CA certificate in "
...
def cleanup_adtrust(fstore):
    ...
    logger.info("[Checking for deprecated backups of Samba " "configuration files]")
...
def cleanup_dogtag():
    ...
    for system in subsystems:
        logger.debug("Cleaning up after pkispawn for the %s subsystem", system)
...
def upgrade_adtrust_config():
    ...
    logger.info(
        "[Update 'max smbd processes' in Samba configuration "
    ...
    try:
        ...
        while True:
            ...
            value = directivesetter.get_directive(
                paths.CA_CS_CFG_PATH, directive, separator="="
            )
            if not value:
                logger.error(
                    "Unable to find profile subsystem in %s", paths.CA_CS_CFG_PATH
                )
                return False
            if value != "profile":
                i = i + 1
        ...
        logger.error(
            'Cannot read CA configuration file "%s": %s', paths.CA_CS_CFG_PATH, e
        )
        return False
...
def ca_ensure_lightweight_cas_container(ca):
    logger.info("[Ensuring Lightweight CAs container exists in Dogtag " "database]")
...
def ca_add_default_ocsp_uri(ca):
    ...
    value = directivesetter.get_directive(
        paths.CA_CS_CFG_PATH, "ca.defaultOcspUri", separator="="
    )
    if value:
...
def ca_disable_publish_cert(ca):
    ...
    value = directivesetter.get_directive(
        paths.CA_CS_CFG_PATH, "ca.publish.cert.enable", separator="="
    )
    if value:
        return False  # already set; restart not needed
...
def named_remove_deprecated_options():
    ...
    try:
        ...
        logger.error("Cannot modify DNS configuration in %s: %s", paths.NAMED_CONF, e)
    ...
    logger.debug(
        "The following configuration options have been removed: %s",
...
def named_add_ipa_ext_conf_file():
    ...
    return bindinstance.named_add_ext_conf_file(
        paths.NAMED_CUSTOM_CFG_SRC, paths.NAMED_CUSTOM_CONFIG
    )
...
def named_set_minimum_connections():
    ...
    try:
        ...
        logger.debug(
            "Cannot retrieve connections option from %s: %s", paths.NAMED_CONF, e
        )
        return changed
    ...
    try:
        ...
        if connections is not None and connections < minimum_connections:
            try:
                ...
                logger.error("Cannot update connections in %s: %s", paths.NAMED_CONF, e)
            else:
                changed = True
...
def named_update_gssapi_configuration():
    ...
    try:
        ...
        logger.error(
            "Cannot retrieve tkey-gssapi-keytab option from %s: %s", paths.NAMED_CONF, e
        )
        return False
    else:
        if gssapi_keytab:
    ...
    try:
        tkey_credential = bindinstance.named_conf_get_directive(
            "tkey-gssapi-credential", bindinstance.NAMED_SECTION_OPTIONS
        )
        tkey_domain = bindinstance.named_conf_get_directive(
            "tkey-domain", bindinstance.NAMED_SECTION_OPTIONS
        )
    except IOError as e:
        logger.error(
            "Cannot retrieve tkey-gssapi-credential option from %s: " "%s",
            paths.NAMED_CONF,
            e,
        )
        return False
    ...
    if not tkey_credential or not tkey_domain:
        logger.error(
            "Either tkey-gssapi-credential or tkey-domain is missing "
            "in %s. Skip update.",
            paths.NAMED_CONF,
        )
        return False
    ...
    try:
        bindinstance.named_conf_set_directive(
            "tkey-gssapi-credential", None, bindinstance.NAMED_SECTION_OPTIONS
        )
        bindinstance.named_conf_set_directive(
            "tkey-domain", None, bindinstance.NAMED_SECTION_OPTIONS
        )
        bindinstance.named_conf_set_directive(
            "tkey-gssapi-keytab", paths.NAMED_KEYTAB, bindinstance.NAMED_SECTION_OPTIONS
        )
    except IOError as e:
        logger.error(
            "Cannot update GSSAPI configuration in %s: %s", paths.NAMED_CONF, e
        )
        return False
    else:
...
def named_update_pid_file():
    ...
    try:
        ...
        logger.error("Cannot retrieve pid-file option from %s: %s", paths.NAMED_CONF, e)
        return False
    else:
        if pid_file:
    ...
    try:
        ...
        logger.error(
            "Cannot update pid-file configuration in %s: %s", paths.NAMED_CONF, e
        )
        return False
    else:
...
def named_validate_dnssec():
    ...
    if (
        not sysupgrade.get_upgrade_state("named.conf", "dnssec_validation_upgraded")
        and bindinstance.named_conf_get_directive(
            "dnssec-validation", bindinstance.NAMED_SECTION_OPTIONS, str_val=False
        )
        is None
    ):
        ...
        try:
            ...
            logger.error(
                "Cannot update dnssec-validate configuration in %s: " "%s",
                paths.NAMED_CONF,
                e,
            )
            return False
    else:
        logger.debug("dnssec-validate already configured in %s", paths.NAMED_CONF)
...
def named_bindkey_file_option():
    ...
    try:
        ...
        logger.error(
            "Cannot retrieve bindkey-file option from %s: %s", paths.NAMED_CONF, e
        )
        return False
    else:
        if bindkey_file:
    ...
    try:
        ...
        logger.error(
            "Cannot update bindkeys-file configuration in %s: %s", paths.NAMED_CONF, e
        )
        return False
...
def named_managed_keys_dir_option():
    ...
    try:
        managed_keys = bindinstance.named_conf_get_directive(
            "managed-keys-directory", bindinstance.NAMED_SECTION_OPTIONS
        )
    except IOError as e:
        logger.error(
            "Cannot retrieve managed-keys-directory option from %s: " "%s",
            paths.NAMED_CONF,
            e,
        )
        return False
        ...
        if managed_keys:
            logger.debug("managed_keys_directory configuration already " "updated")
            sysupgrade.set_upgrade_state(
                "named.conf", "managed-keys-directory_updated", True
            )
            return False
    ...
    try:
        ...
        logger.error(
            "Cannot update managed-keys-directory configuration in " "%s: %s",
            paths.NAMED_CONF,
            e,
        )
        return False
...
def named_root_key_include():
    ...
    try:
        ...
        logger.error("Cannot check root key include in %s: %s", paths.NAMED_CONF, e)
        return False
    else:
        if root_key:
    ...
    try:
        ...
        logger.error(
            "Cannot update named root key include in %s: %s", paths.NAMED_CONF, e
        )
        return False
...
def named_update_global_forwarder_policy():
    ...
    logger.info(
        "[Checking global forwarding policy in named.conf "
    ...
    sysupgrade.set_upgrade_state(
        "named.conf", "forward_policy_conflict_with_empty_zones_handled", True
    )
    try:
    ...
    if (
        bindinstance.named_conf_get_directive(
            "forward", section=bindinstance.NAMED_SECTION_OPTIONS, str_val=False
        )
        == "only"
    ):
        return False
    ...
    logger.info(
        "Global forward policy in named.conf will "
        'be changed to "only" to avoid conflicts with '
        "automatic empty zones"
    )
    bindinstance.named_conf_set_directive(
        "forward", "only", section=bindinstance.NAMED_SECTION_OPTIONS, str_val=False
    )
    return True
...
def certificate_renewal_update(ca, kra, ds, http):
    ...
    for nick, profile in dogtag_reqs:
        req = {
            ...
            "cert-postsave-command": (template % 'renew_ca_cert "{}"'.format(nick)),
        ...
        requests.append(
            {
                ...
                "ca-name": "IPA",
                "cert-postsave-command": "%s %s"
                % (template % "restart_dirsrv", serverid),
            }
        )
...
def http_certificate_ensure_ipa_ca_dnsname(http):
    ...
    try:
        ...
        if certs.is_ipa_issued_cert(api, cert):
            request_id = certmonger.get_request_id({"cert-file": paths.HTTPD_CERT_FILE})
            if request_id is None:
                # shouldn't happen
...
def copy_crl_file(old_path, new_path=None):
    ...
    if os.path.islink(old_path):
        ...
        logger.debug("copy_crl_file: Create symlink %s -> %s", new_path, realpath)
...
def migrate_crl_publish_dir(ca):
    ...
    try:
        ...
        crl_files = sorted(crl_files_unsorted, key=lambda f: os.path.islink(f))
        for f in crl_files:
            try:
                copy_crl_file(f)
    ...
    try:
        ...
        logger.error(
            'Cannot update CA configuration file "%s": %s', paths.CA_CS_CFG_PATH, e
        )
        return False
    sysupgrade.set_upgrade_state("dogtag", "moved_crl_publish_dir", True)
    logger.info(
        "CRL publish directory has been migrated, " "request pki-tomcat restart"
    )
    return True
...
def find_subject_base():
    ...
    logger.error(
        "Unable to determine certificate subject base. "
...
def uninstall_dogtag_9(ds, http):
    ...
    dn = DN(
        ("cn", "CA"), ("cn", api.env.host), api.env.container_masters, api.env.basedn
    )
    try:
    ...
    ca = dogtaginstance.DogtagInstance(
        api.env.realm, "CA", "certificate server", nss_db=paths.VAR_LIB_PKI_CA_ALIAS_DIR
    )
...
def fix_schema_file_syntax():
    ...
    with open(filename) as file:
        for line in file:
            line = line.strip("\n")
            if (
                line.startswith("objectClasses:")
                and "NAME 'idnsRecord'" in line
                and line.count("(") == 2
                and line.count(")") == 1
            ):
    ...
    with open(filename) as file:
        for line in file:
            line = line.strip("\n")
            if line.startswith("objectClasses:") and "NAME 'ipaSudoRule'" in line:
...
def sssd_update():
    ...
    if flag:
        logger.debug(
            "%s has ok_to_auth_as_delegate, allow Apache to access IFP", services
        )
...
def remove_ds_ra_cert(subject_base):
    ...
    dbdir = dsinstance.config_dirname(ipaldap.realm_to_serverid(api.env.realm))
...
def setup_pkinit(krb):
    ...
    try:
        ...
        expr = '{}[count(pkinit_identity)=1][pkinit_identity="{}"]'.format(path, value)
...
def ntpd_cleanup(fqdn, fstore):
    ...
    for ntpd_file in [paths.NTP_CONF, paths.NTP_STEP_TICKERS, paths.SYSCONFIG_NTPD]:
        try:
...
def update_replica_config(db_suffix):
    dn = DN(
        ("cn", "replica"), ("cn", db_suffix), ("cn", "mapping tree"), ("cn", "config")
    )
    try:
...
def migrate_to_authselect():
    ...
    try:
        ...
        raise RuntimeError("Failed to migrate to authselect profile: %s" % e, 1)
...
def add_systemd_user_hbac():
    ...
    try:
        api.Command.hbacsvc_add(
            service, description=u"pam_systemd and systemd user@.service"
        )
    ...
    try:
        api.Command.hbacrule_add(
            rule,
            description=(
                u"Allow pam_systemd to run user@.service to create "
        ...
        api.Command.hbacrule_add_service(rule, hbacsvc=(service,))
...
def upgrade_configuration():
    ...
    if ca.is_configured() and not os.path.exists(
        os.path.join(paths.PKI_TOMCAT_ALIAS_DIR, "pwdfile.txt")
    ):
    ...
    with installutils.stopped_service("pki-tomcatd", "pki-tomcat"):
        ...
        if kra.is_installed():
            ...
            kra.backup_config()
            value = directivesetter.get_directive(
                paths.KRA_CS_CFG_PATH, "kra.ephemeralRequests", separator="="
            )
    ...
    if fstore.has_file(removed_sysconfig_file):
        logger.info("Restoring %s as it is no longer required", removed_sysconfig_file)
    ...
    simple_service_list = ((otpdinstance.OtpdInstance(), "OTPD"),)
...
def upgrade_check(options):
    ...
    if not options.skip_version_check:
        ...
        try:
            ...
            installutils.UpgradePlatformError,
            installutils.UpgradeDataNewerVersionError,
        ) as e:
            raise RuntimeError("Unable to execute IPA upgrade: %s" % e, 1)
...
def upgrade():
    realm = api.env.realm
    schema_files = [
        os.path.join(paths.USR_SHARE_IPA_DIR, f) for f in dsinstance.ALL_SCHEMA_FILES
    ]

    schema_files.extend(
        dsinstance.get_all_external_schema_files(paths.EXTERNAL_SCHEMA_DIR)
    )
    ...
    try:
        ...
        raise RuntimeError("Bad syntax detected in upgrade file(s).", 1)
    except RuntimeError:
