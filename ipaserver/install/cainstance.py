...
def check_ports():
    ...
    """
    return all([ipautil.check_port_bindable(8443), ipautil.check_port_bindable(8080)])
...
def get_preop_pin(instance_root, instance_name):
    ...
    if preop_pin is None:
        raise RuntimeError(
            "Unable to find preop.pin in %s. Is your CA already configured?" % filename
        )

    return preop_pin


def import_pkcs12(input_file, input_passwd, cert_database, cert_passwd):
    ipautil.run(
        [
            paths.PK12UTIL,
            "-d",
...
class CAInstance(DogtagInstance):
    ...
    def __init__(self, realm=None, host_name=None, custodia=None):
        ...
        self.keytab = os.path.join(paths.PKI_TOMCAT, self.service_prefix + ".keytab")
    ...
    def configure_instance(
        ...
        self.ca_subject = ca_subject or installutils.default_ca_subject_dn(
            self.subject_base
        )
        ...
        if not ra_only:
            ...
            if promote:
                self.step("Finalize replication settings", self.finalize_replica_config)
        ...
        if self.external != 1:
            ...
            if not ra_only:
                self.step(
                    "setting audit signing renewal to 2 years", self.set_audit_renewal
                )
                self.step("restarting certificate server", self.restart_instance)
                if not self.clone:
                    self.step("publishing the CA certificate", self.__export_ca_chain)
                    self.step(
                        "adding RA agent as a trusted user", self.__create_ca_agent
                    )
                ...
                self.step(
                    "Ensure lightweight CAs container exists",
                    ensure_lightweight_cas_container,
                )
                if self.clone and not promote:
                    self.step(
                        "Ensuring backward compatibility", self.__dogtag10_migration
                    )
                ...
                if not promote:
                    if self.clone:
                        ...
                        self.step(
                            "exposing CA instance on LDAP", self.__expose_ca_in_ldap
                        )
                    ...
                    self.step(
                        "importing IPA certificate profiles", import_included_profiles
                    )
    ...
    def __spawn_instance(self):
        ...
        """
        cfg = dict(pki_ds_secure_connection=self.use_ldaps)
        ...
        if self.external == 1:
            cfg.update(
                pki_external=True, pki_ca_signing_csr_path=self.csr_file,
            )
        ...
        with tempfile.NamedTemporaryFile("w") as f:
            ...
            DogtagInstance.spawn_instance(self, f.name, nolog_list=nolog_list)
        ...
        if self.external == 1:
            ...
            print(
                "%s --external-cert-file=/path/to/signed_certificate --external-cert-file=/path/to/external_ca_certificate"
                % sys.argv[0]
            )
            sys.exit(0)
        else:
            if config.getboolean(self.subsystem, "pki_backup_keys", fallback=True):
                shutil.move(paths.CA_BACKUP_KEYS_P12, paths.CACERT_P12)
    ...
    def __update_topology(self):
        ld = ldapupdate.LDAPUpdate(
            ldapi=True, sub_dict={"SUFFIX": api.env.basedn, "FQDN": self.fqdn,}
        )
    ...
    def __disable_nonce(self):
        ...
        update_result = installutils.update_file(
            self.config, "ca.enableNonces=true", "ca.enableNonces=false"
        )
        if update_result != 0:
    ...
    def __export_ca_chain(self):
        ...
        for path in [paths.IPA_CA_CRT, paths.KDC_CA_BUNDLE_PEM, paths.CA_BUNDLE_PEM]:
    ...
    def __request_ra_certificate(self):
        ...
        certs.install_pem_from_p12(
            paths.DOGTAG_ADMIN_P12, self.dm_password, agent_cert.name
        )
        certs.install_key_from_p12(
            paths.DOGTAG_ADMIN_P12, self.dm_password, agent_key.name
        )
        ...
        agent_args = [
            ...
            chain_file.name,
            "--ee-url",
            "http://%s:8080/ca/ee/ca/" % self.fqdn,
            "--agent-url",
        ...
        try:
            ...
            self.ra_cert = x509.load_certificate_from_file(paths.RA_AGENT_PEM)
        finally:
            # we can restore the helper parameters
            certmonger.modify_ca_helper(ipalib.constants.RENEWAL_CA_NAME, old_helper)
    ...
    def __enable_crl_publish(self):
        ...
        with directivesetter.DirectiveSetter(
            self.config, quotes=False, separator="="
        ) as ds:
            ...
            ds.set("ca.publish.rule.instance.LdapUserCertRule.enable", "false")
    ...
    def uninstall(self):
        ...
        if os.path.exists(paths.PKI_CA_PUBLISH_DIR):
            try:
                ...
                logger.warning("Error while removing CRL publish " "directory: %s", e)
    ...
    def unconfigure_certmonger_renewal_guard(self):
        ...
        if helper:
            ...
            if path:
                ...
                ca_iface.Set(
                    "org.fedorahosted.certmonger.ca", "external-helper", helper
                )
    ...
    def configure_agent_renewal(self):
        try:
            ...
            logger.error("certmonger failed to start tracking certificate: %s", e)
    ...
    def stop_tracking_certificates(self):
        ...
        try:
            ...
            logger.error("certmonger failed to stop tracking certificate: %s", e)
    ...
    def set_audit_renewal(self):
        ...
        logger.debug("caSignedLogCert.cfg profile validity range is %s", cert_range)
        if cert_range == "180":
            directivesetter.set_directive(
                paths.CASIGNEDLOGCERT_CFG,
                "policyset.caLogSigningSet.2.default.params.range",
            ...
            logger.debug("updated caSignedLogCert.cfg profile validity range to 720")
            return True
        return False
    ...
    def is_renewal_master(self, fqdn=None):
        ...
        dn = DN(("cn", "CA"), ("cn", fqdn), api.env.container_masters, api.env.basedn)
    ...
    def set_renewal_master(self, fqdn=None):
        ...
        for entry in entries:
            ...
            entry["ipaConfigString"] = [
                x for x in entry["ipaConfigString"] if x.lower() != "carenewalmaster"
            ]
    ...
    def update_cert_config(self, nickname, cert):
        ...
        if nickname in directives:
            super(CAInstance, self).update_cert_cs_cfg(directives[nickname], cert)
    ...
    def __create_ds_db(self):
        ...
        entry = api.Backend.ldap2.make_entry(
            dn, objectclass=["top", "extensibleObject", "nsMappingTree"], cn=[suffix],
        )
    ...
    def __setup_lightweight_ca_key_retrieval_custodia(self):
        ...
        ensure_entry(
            custodia_basedn, objectclass=["top", "nsContainer"], cn=["custodia"],
        )
        ensure_entry(
    ...
    def add_lightweight_ca_tracking_requests(self):
        try:
            ...
            logger.warning("Did not find any lightweight CAs; nothing to track")

    def __dogtag10_migration(self):
        ld = ldapupdate.LDAPUpdate(
            ldapi=True, sub_dict={"SUFFIX": api.env.basedn, "FQDN": self.fqdn,}
        )
        ld.update([os.path.join(paths.UPDATES_DIR, "50-dogtag10-migration.update")])
    ...
    def is_crlgen_enabled(self):
        ...
        try:
            ...
            raise RuntimeError("Unable to read {}".format(self.config))
...
def __update_entry_from_cert(make_filter, make_entry, cert):
    ...
    while attempts < 10:
        ...
        try:
            ...
            for entry in entries:
                syslog.syslog(syslog.LOG_NOTICE, "Updating entry %s" % str(entry.dn))
...
def get_ca_renewal_nickname(subject_base, ca_subject_dn, sdn):
    ...
    nickname_by_subject_dn = {
        ...
        DN("CN=KRA Transport Certificate", subject_base): "transportCert cert-pki-kra",
        DN("CN=KRA Storage Certificate", subject_base): "storageCert cert-pki-kra",
...
def __get_profile_config(profile_id):
    ...
    profile_upg_filename = "/usr/share/ipa/profiles/{}.UPGRADE.cfg".format(profile_id)
...
def import_included_profiles():
    ...
    ensure_entry(
        DN(("cn", "ca"), api.env.basedn), objectclass=["top", "nsContainer"], cn=["ca"],
    )
    ensure_entry(
    ...
    for (profile_id, desc, store_issued) in dogtag.INCLUDED_PROFILES:
        dn = DN(("cn", profile_id), api.env.container_certprofile, api.env.basedn)
        try:
            conn.get_entry(dn)
...
def repair_profile_caIPAserviceCert():
    ...
    if need_repair:
        logger.debug(
            ...
            "incorrect version; begin repair.",
            profile_id,
        )
...
def migrate_profiles_to_ldap():
    ...
    for profile_id in profile_ids:
        match = re.search(
            r"^profile\.{}\.config=(\S*)".format(profile_id), cs_cfg, re.MULTILINE
        )
        if match is None:
        ...
        match = re.search(
            r"^profile\.{}\.class_id=(\S*)".format(profile_id), cs_cfg, re.MULTILINE
        )
        if match is None:
...
def _create_dogtag_profile(profile_id, profile_data, overwrite):
    with api.Backend.ra_certprofile as profile_api:
        ...
        try:
            ...
            logger.debug("Profile '%s' successfully migrated to LDAP", profile_id)
        ...
        try:
            ...
            logger.debug(
                "Failed to enable profile '%s' " "(it is probably already enabled)",
                profile_id,
            )
...
def ensure_default_caacl():
    ...
    if not api.Command.caacl_find()["result"]:
        api.Command.caacl_add(
            u"hosts_services_caIPAserviceCert",
            hostcategory=u"all",
            servicecategory=u"all",
        )
        api.Command.caacl_add_profile(
            u"hosts_services_caIPAserviceCert", certprofile=(u"caIPAserviceCert",)
        )
...
def add_lightweight_ca_tracking_requests(lwcas):
    ...
    for entry in lwcas:
        ...
        nickname = "{} {}".format(ipalib.constants.IPA_CA_NICKNAME, entry["ipacaid"][0])
        criteria = {
        ...
        if request_id is None:
            try:
                ...
                logger.debug(
                    "Lightweight CA renewal: " 'added tracking request for "%s"',
                    nickname,
                )
            except RuntimeError as e:
                logger.error(
                    "Lightweight CA renewal: Certmonger failed to "
                    "start tracking certificate: %s",
                    e,
                )
        else:
            logger.debug(
                "Lightweight CA renewal: " 'already tracking certificate "%s"', nickname
            )
