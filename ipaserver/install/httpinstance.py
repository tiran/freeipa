...
class HTTPInstance(service.Service):
    ...
    def create_instance(
        ...
        self.step("configuring mod_ssl certificate paths", self.configure_mod_ssl_certs)
        self.step("setting mod_ssl protocol list", self.set_mod_ssl_protocol)
        ...
        self.step("clean up any existing httpd ccaches", self.remove_httpd_ccaches)
    ...
    def configure_selinux_for_httpd(self):
        try:
            tasks.set_selinux_booleans(
                constants.SELINUX_BOOLEAN_HTTPD, self.backup_state
            )
    ...
    def get_mod_nss_nickname(self):
        cert = directivesetter.get_directive(paths.HTTPD_NSS_CONF, "NSSNickname")
        nickname = directivesetter.unquote_directive_value(cert, quote_char="'")
        return nickname
    ...
    def __add_include(self):
        ...
        if (
            installutils.update_file(
                ...
                "</VirtualHost>",
                "Include {path}\n"
                "</VirtualHost>".format(path=paths.HTTPD_IPA_REWRITE_CONF),
            )
            != 0
        ):
            self.print_msg(
                "Adding Include conf.d/ipa-rewrite to "
    ...
    def configure_certmonger_renewal_guard(self):
        ...
        try:
            ...
            if path:
                ...
                helper = ca_iface.Get(
                    "org.fedorahosted.certmonger.ca", "external-helper"
                )
                if helper:
    ...
    def __setup_ssl(self):
        ...
        if self.pkcs12_info:
            p12_certs, p12_priv_keys = certs.pkcs12_to_certkeys(*self.pkcs12_info)
            keys_dict = {k.public_key().public_numbers(): k for k in p12_priv_keys}
            certs_keys = [
                (c, keys_dict.get(c.public_key().public_numbers())) for c in p12_certs
            ]
            server_certs_keys = [(c, k) for c, k in certs_keys if k is not None]
            ...
            x509.write_pem_private_key(
                server_certs_keys[0][1], paths.HTTPD_KEY_FILE, passwd=pkey_passwd
            )
    ...
    def configure_mod_ssl_certs(self):
        ...
        directivesetter.set_directive(
            paths.HTTPD_SSL_SITE_CONF, "SSLCACertificateFile", paths.IPA_CA_CRT, False
        )
        # set SSLVerifyDepth for external CA installations
        directivesetter.set_directive(
            paths.HTTPD_SSL_CONF, "SSLVerifyDepth", MOD_SSL_VERIFY_DEPTH, quotes=False
        )
    ...
    def uninstall(self):
        ...
        # NSS DB backups
        remove_files.extend(glob.glob(os.path.join(paths.HTTPD_ALIAS_DIR, "*.ipasave")))
        ...
        try:
            ...
            if e.errno not in {errno.ENOENT, errno.ENOTEMPTY}:
                logger.error(
                    "Failed to remove directory %s", paths.SYSTEMD_SYSTEM_HTTPD_D_DIR
                )

        # Restore SELinux boolean states
        boolean_states = {
            name: self.restore_state(name) for name in constants.SELINUX_BOOLEAN_HTTPD
        }
        try:
    ...
    def start_tracking_certificates(self):
        ...
        if certs.is_ipa_issued_cert(api, cert):
            ...
            logger.debug(
                "Will not track HTTP server cert %s as it is not " "issued by IPA",
                cert.subject,
            )
    ...
    def request_service_keytab(self):
        ...
        if self.master_fqdn is not None:
            ...
            with ipaldap.LDAPClient(
                ldap_uri, start_tls=not self.promote, cacert=paths.IPA_CA_CRT
            ) as remote_ldap:
                if self.promote:
                    remote_ldap.gssapi_bind()
                else:
                    remote_ldap.simple_bind(ipaldap.DIRMAN_DN, self.dm_password)
                replication.wait_for_entry(
                    remote_ldap, service_dn, timeout=api.env.replication_wait_timeout
                )
    ...
    def migrate_to_mod_ssl(self):
        ...
        with tempfile.NamedTemporaryFile() as temp:
            ...
            certs.install_pem_from_p12(temp.name, pk12_password, paths.HTTPD_CERT_FILE)

            passwd_fname = paths.HTTPD_PASSWD_FILE_FMT.format(host=api.env.host)
            with open(passwd_fname, "wb") as passwd_file:
                os.fchmod(passwd_file.fileno(), 0o600)
                passwd_file.write(ipautil.ipa_generate_password().encode("utf-8"))
