...
class update_dna_shared_config(Updater):
    def execute(self, **options):
        ...
        try:
            ...
            logger.error("Could not find DNA plugin entry: %s", dna_config_base)
            return False, ()

        try:
            entry = conn.get_entry(dna_config_base)
        except errors.NotFound:
            logger.error("Could not find DNA config entry: %s", dna_config_base)
            return False, ()
        ...
        if sharedcfgdn is not None:
            ...
            logger.error(
                "Could not find DNA shared config DN in entry: %s", dna_config_base
            )
            return False, ()
        ...
        for _i in range(0, max_wait + 1):
            ...
            logger.error(
                "Could not get dnaHostname entries in %s seconds", max_wait * 2
            )
            return False, ()
