...
class DNSUpdater(Updater):
    ...
    def version_update_needed(self, target_version):
        ...
        try:
            return (
                int(
                    self.api.Command["dnsconfig_show"](all=True)["result"][
                        "ipadnsversion"
                    ][0]
                )
                < target_version
    ...
    def ldif_writer(self):
        if not self._ldif_writer:
            logger.info(
                "Original zones will be saved in LDIF format in " "%s file",
                self.backup_path,
            )
...
class update_ipaconfigstring_dnsversion_to_ipadnsversion(Updater):
    ...
    def execute(self, **options):
        ...
        if "ipadnscontainer" in [o.lower() for o in container_entry["objectclass"]]:
        ...
        for config_option in container_entry.get("ipaConfigString", []):
            matched = re.match(
                r"^DNSVersion\s+(?P<version>\d+)$", config_option, flags=re.I
            )
            if matched:
...
class update_dns_limits(Updater):
    ...
    def execute(self, **options):
        ...
        dns_service_dn = DN(
            ("krbprincipalname", dns_principal),
        ...
        try:
            ...
            logger.debug(
                "DNS: service %s not found, no need to update limits", dns_service_dn
            )
            return False, []
        ...
        logger.debug("DNS: limits for service %s will be updated", dns_service_dn)
...
class update_master_to_dnsforwardzones(DNSUpdater):
    ...
    def execute(self, **options):
        ...
        try:
            # raw values are required to store into ldif
            zones = self.api.Command.dnszone_find(all=True, raw=True, sizelimit=0)[
                "result"
            ]
        except errors.NotFound:
            pass
        ...
        for zone in zones:
            if (
                zone.get("idnsforwardpolicy", [u"first"])[0] == u"none"
        ...
        if zones_to_transform:
            logger.info(
                "Zones with specified forwarders with policy "
            ...
            for zone in zones_to_transform:
                try:
                    ...
                    logger.error(
                        "Unable to create backup for zone, " "terminating zone upgrade"
                    )
                ...
                try:
                    ...
                    logger.error(
                        "Transform to forwardzone terminated: "
                ...
                try:
                    kw = {
                        ...
                        "idnsforwardpolicy": zone.get("idnsforwardpolicy", [u"first"])[
                            0
                        ],
                ...
                if "managedBy" in zone:
                    try:
                        ...
                        logger.error("%s", traceback.format_exc())
                        logger.info(
                            "Zone %s was transformed to forward zone "
                        ...
                        if zone["idnsname"][0] in self.saved_zone_to_privilege:
                            privileges = [
                                dn[0].value
                                for dn in self.saved_zone_to_privilege[
                                    zone["idnsname"][0]
                                ]
                            ]
                            try:
                                self.api.Command["permission_add_member"](
                                    perm_name, privilege=privileges
                                )
                            except Exception:
                                logger.error(
                                    "Unable to restore privileges "
                ...
                logger.debug(
                    "Zone %s was sucessfully transformed to forward " "zone",
                    zone["idnsname"][0],
                )
...
class update_dnsforward_emptyzones(DNSUpdater):
    ...
    def update_zones(self):
        try:
            fwzones = self.api.Command.dnsforwardzone_find(all=True, raw=True)["result"]
        except errors.NotFound:
        ...
        for zone in fwzones:
            ...
            if not logged_once:
                logger.info(
                    "Forward policy for zones conflicting with "
            ...
            try:
                ...
                logger.error(
                    "Unable to create backup for zone %s, " "terminating zone upgrade",
                    zone["idnsname"][0],
                )
                logger.error("%s", traceback.format_exc())
                continue
            ...
            try:
                self.api.Command["dnsforwardzone_mod"](
                    zone["idnsname"][0], idnsforwardpolicy=u"only"
                )
            except Exception as e:
                logger.error(
                    "Forward policy update for zone %s failed " "(%s)",
                    zone["idnsname"][0],
                    e,
                )
                logger.error("%s", traceback.format_exc())
            ...
            logger.debug(
                "Zone %s was sucessfully modified to use forward " 'policy "only"',
                zone["idnsname"][0],
            )

    def update_global_ldap_forwarder(self):
        config = self.api.Command["dnsconfig_show"](all=True, raw=True)["result"]
        if config.get("idnsforwardpolicy", [u"first"])[0] == u"first" and config.get(
            "idnsforwarders", []
        ):
            logger.info(
                "Global forward policy in LDAP for all servers will "
    ...
    def execute(self, **options):
        ...
        logger.debug(
            "Updating forwarding policies in LDAP "
        ...
        try:
            ...
            logger.error(
                "Skipping update of global DNS forwarder in LDAP: "
...
class update_dnsserver_configuration_into_ldap(DNSUpdater):
    ...
    def execute(self, **options):
        ...
        try:
            ...
            logger.debug("DNS server configuration already exists " "in LDAP database")
        else:
            logger.debug(
                "DNS server configuration has been sucessfully "
