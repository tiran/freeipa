...
class update_idrange_type(Updater):
    ...
    def execute(self, **options):
        ...
        logger.debug("update_idrange_type: search for ID ranges with no " "type set")
        ...
        while True:
            ...
            try:
                (entries, truncated) = ldap.find_entries(
                    search_filter, ["objectclass"], base_dn, time_limit=0, size_limit=0
                )

            except errors.NotFound:
                logger.debug(
                    "update_idrange_type: no ID range without " "type set found"
                )
                return False, []

            except errors.ExecutionError as e:
                logger.error(
                    "update_idrange_type: cannot retrieve list "
                    "of ranges with no type set: %s",
                    e,
                )
                return False, []
            ...
            logger.debug(
                "update_idrange_type: found %d " "idranges to update, truncated: %s",
                len(entries),
                truncated,
            )
            ...
            for entry in entries:
                objectclasses = [o.lower() for o in entry.get("objectclass", [])]
                ...
                if "ipatrustedaddomainrange" in objectclasses:
                    ...
                    logger.error(
                        "update_idrange_type: could not detect "
                        "range type for entry: %s",
                        str(entry.dn),
                    )
                    logger.error(
                        "update_idrange_type: ID range type set "
                        "to 'unknown' for entry: %s",
                        str(entry.dn),
                    )
                ...
                try:
                    ...
                    logger.debug(
                        "update_idrange_type: cannot " "update idrange type: %s", e
                    )
                    error = True
            ...
            if error:
                ...
                logger.debug(
                    "update_idrange_type: all affected idranges " "were assigned types"
                )
                return False, []
...
class update_idrange_baserid(Updater):
    ...
    def execute(self, **options):
        ...
        try:
            ...
            logger.debug(
                "update_idrange_baserid: no AD domain "
            ...
            logger.error(
                "update_idrange_baserid: cannot retrieve "
                "list of affected ranges: %s",
                e,
            )
            return False, []

        logger.debug(
            "update_idrange_baserid: found %d " "idranges possible to update",
            len(entries),
        )
        ...
        for entry in entries:
            ...
            try:
                ...
                logger.debug("update_idrange_type: cannot " "update idrange: %s", e)
                error = True

        if error:
            logger.error(
                "update_idrange_baserid: error(s) "
                "detected during idrange baserid update"
            )
        else:
            # All affected entries updated, exit the loop
            logger.debug("update_idrange_baserid: all affected " "idranges updated")
