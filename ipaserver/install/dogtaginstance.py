...
def get_security_domain():
    ...
    """
    connection = PKIConnection(protocol="https", hostname=api.env.ca_host, port="8443")
...
class DogtagInstance(service.Service):
    ...
    def spawn_instance(self, cfg_file, nolog_list=()):
        ...
        with open(cfg_file) as f:
            logger.debug(
                ...
                cfg_file,
                ipautil.nolog_replace(f.read(), nolog_list),
            )
    ...
    def clean_pkispawn_files(self):
        ...
        client_dir = os.path.join("/root/.dogtag/pki-tomcat/", self.subsystem.lower())
    ...
    def enable_client_auth_to_db(self):
        ...
        if self.token_name != INTERNAL_TOKEN:
            ...
            sub_system_nickname = "{}:{}".format(self.token_name, sub_system_nickname)
        ...
        with stopped_service("pki-tomcatd", "pki-tomcat"):
            ...
            directivesetter.set_directive(
                paths.PKI_TOMCAT_PASSWORD_CONF, "internaldb", None, separator="="
            )
    ...
    def uninstall(self):
        ...
        try:
            ipautil.run([paths.PKIDESTROY, "-i", "pki-tomcat", "-s", self.subsystem])
        except ipautil.CalledProcessError as e:
            logger.critical("failed to uninstall %s instance %s", self.subsystem, e)

    def __is_newer_tomcat_version(self, default=None):
        try:
            result = ipautil.run([paths.BIN_TOMCAT, "version"], capture_output=True)
            sn = re.search(
                r"Server number:\s+([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)", result.output
            )
            if sn is None:
                logger.info(
                    "tomcat version cannot be parsed, " "default to pre-%s", default
                )
                return False
            ...
            logger.info(
                "failed to discover tomcat version, " "default to pre-%s, error: %s",
                default,
                str(e),
            )
        return False
    ...
    def http_proxy(self):
        ...
        template_filename = os.path.join(
            paths.USR_SHARE_IPA_DIR, "ipa-pki-proxy.conf.template"
        )
        sub_dict = dict(
            DOGTAG_PORT=8009,
    ...
    def __get_pin(self, token_name=INTERNAL_TOKEN):
        try:
            ...
            logger.debug("Unable to determine PIN for the Dogtag instance: %s", e)
    ...
    def stop_tracking_certificates(self, stop_certmonger=True):
        ...
        """
        logger.debug(
            "Configuring certmonger to stop tracking system certificates " "for %s",
            self.subsystem,
        )
    ...
    def update_cert_cs_cfg(self, directive, cert):
        ...
        with stopped_service("pki-tomcatd", "pki-tomcat"):
            directivesetter.set_directive(
                ...
                (
                    base64.b64encode(cert.public_bytes(x509.Encoding.DER)).decode(
                        "ascii"
                    )
                ),
                quotes=False,
    ...
    def handle_setup_error(self, e):
        logger.critical("Failed to configure %s instance: %s", self.subsystem, e)
        logger.critical(
    ...
    def setup_admin(self):
        ...
        logger.debug(
            ...
            api.env.replication_wait_timeout,
            self.admin_dn,
            master_conn,
        )
        ...
        while time.time() < deadline:
            ...
            raise errors.NotFound(
                reason="{} did not replicate to {}".format(self.admin_dn, master_conn)
            )
    ...
    def _configure_clone(
        self, subsystem_config, security_domain_hostname, clone_pkcs12_path
    ):
        subsystem_config.update(
            ...
            pki_clone_uri="https://%s" % ipautil.format_netloc(self.master_host, 443),
        )
...
class PKIIniLoader:
    ...
    ipaca_customize = os.path.join(paths.USR_SHARE_IPA_DIR, "ipaca_customize.ini")
    ...
    def verify_pki_config_override(cls, filename):
        ...
        * ini file must not override immutable settings
        ...
        try:
            ...
            raise ValueError("Invalid config '{}': {}".format(filename, e))
    ...
    def create_spawn_config(self, subsystem_config):
        ...
        immutable_settings = {
            k: v for k, v in cfgtpl.items(section_name) if k in immutable_keys
        }
        ...
        # verify again
        self._verify_immutable(cfgtpl, immutable_settings, self.pki_config_override)
