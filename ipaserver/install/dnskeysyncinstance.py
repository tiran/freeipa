...
def dnssec_container_exists(suffix):
    ...
    return api.Backend.ldap2.entry_exists(DN(("cn", "sec"), ("cn", "dns"), suffix))
...
class DNSKeySyncInstance(service.Service):
    ...
    def set_dyndb_ldap_workdir_permissions(self):
        ...
        os.chown(paths.BIND_LDAP_DNS_IPA_WORKDIR, self.named_uid, self.named_gid)

    def remove_replica_public_keys(self, replica_fqdn):
        ldap = api.Backend.ldap2
        dn_base = DN(("cn", "keys"), ("cn", "sec"), ("cn", "dns"), api.env.basedn)
        keylabel = (
            replica_keylabel_template
            % DNSName(replica_fqdn).make_absolute().canonicalize().ToASCII()
        )
    ...
    def create_instance(self, fqdn, realm_name):
        ...
        self.step(
            "setting up bind-dyndb-ldap working directory",
    ...
    def __setup_softhsm(self):
        ...
        for sysconfig in [paths.SYSCONFIG_NAMED, paths.SYSCONFIG_IPA_DNSKEYSYNCD]:
        ...
        if (
            token_dir_exists
            and os.path.exists(paths.DNSSEC_SOFTHSM_PIN)
        ...
        # remove old tokens
        if token_dir_exists:
            logger.debug("Removing old tokens directory %s", paths.DNSSEC_TOKENS_DIR)
    ...
    def __setup_replica_keys(self):
        keylabel = (
            replica_keylabel_template
            % DNSName(self.fqdn).make_absolute().canonicalize().ToASCII()
        )
        ...
        try:
            ...
            while True:
                ...
                pub_keys = p11.find_keys(
                    _ipap11helper.KEY_CLASS_PUBLIC_KEY, label=keylabel, id=key_id
                )
                if pub_keys:
                    # key with id exists
                    continue

                priv_keys = p11.find_keys(
                    _ipap11helper.KEY_CLASS_PRIVATE_KEY, label=keylabel, id=key_id
                )
                if not priv_keys:
            ...
            logger.debug("Storing replica public key to LDAP, %s", replica_pubkey_dn)
            ...
            replica_pub_keys = p11.find_keys(
                _ipap11helper.KEY_CLASS_PUBLIC_KEY, label=keylabel, cka_wrap=True
            )
            # old keys in softHSM
    ...
    def __enable(self):
        try:
            self.ldap_configure(
                "DNSKeySync", self.fqdn, None, self.suffix, self.extra_config
            )
    ...
    def uninstall(self):
        ...
        try:
            ...
            if e.errno != errno.ENOENT:
                logger.exception("Failed to remove %s", paths.DNSSEC_TOKENS_DIR)
