...
class common_smart_card_auth_config(Advice):
    ...
    def check_and_set_ca_cert_paths(self):
        ...
        with self.log.for_loop(
            single_ca_path_variable, "${}".format(ca_paths_variable)
        ):
            self.log.exit_on_predicate(
    ...
    def update_ipa_ca_certificate_store(self):
        self.log.exit_on_failed_command(
            "ipa-certupdate", ["Failed to update IPA CA certificate database"]
        )
...
class config_server_for_smart_card_auth(common_smart_card_auth_config):
    ...
    def resolve_ipaca_records(self):
        ...
        self.log.comment(
            "make sure bind-utils are installed so that we can "
            "dig for ipa-ca records"
        )
        self.log.install_packages(["bind-utils"], ["Failed to install bind-utils"])

        self.log.comment(
            "make sure ipa-ca records are resolvable, "
            "otherwise error out and instruct"
        )
        self.log.comment("the user to update the DNS infrastructure")
        self.log.command(
            "ipaca_records=$(dig +short " "ipa-ca.{})".format(ipa_domain_name)
        )
    ...
    def enable_ssl_ocsp(self):
        ...
        self.log.comment("if it is absent, append it to the end of VirtualHost section")
        ...
        self.log.commands_on_predicate(
            ...
            commands_to_run_when_false=[
                self._interpolate_ocsp_directive_file_into_command(
                    r"sed -i.ipabkp '/<\/VirtualHost>/i {directive} on' " r"{filename}"
                )
            ],
        )

    def _interpolate_ocsp_directive_file_into_command(self, fmt_line):
        return self._format_command(fmt_line, self.ssl_ocsp_directive, self.ssl_conf)
    ...
    def restart_httpd(self):
        ...
        self.log.command("systemctl restart {}".format(self.httpd_service_name))
    ...
    def check_and_enable_pkinit(self):
        ...
        with self.log.if_branch("ipa-pkinit-manage status | grep -q 'enabled'"):
...
class config_client_for_smart_card_auth(common_smart_card_auth_config):
    ...
    def check_and_remove_pam_pkcs11(self):
        self.log.remove_package("pam_pkcs11", ["Could not remove pam_pkcs11 package"])
    ...
    def start_enable_smartcard_daemon(self):
        self.log.command(
            ...
            "&& systemctl enable {service} {socket}".format(
                service=self.smart_card_service_file, socket=self.smart_card_socket
            )
        )
    ...
    def add_pkcs11_module_to_systemwide_db(self):
        ...
        self.log.commands_on_predicate(
            "modutil -dbdir {} -list | grep -q {}".format(nssdb, module_name),
            ['echo "{} PKCS#11 module already configured"'.format(module_name)],
            commands_to_run_when_false=[
    ...
    def run_authselect_to_configure_smart_card_auth(self):
        ...
        self.log.comment(
            "Use either authselect or authconfig to enable " "Smart Card authentication"
        )
        self.log.commands_on_predicate(
        ...
        self.log.exit_on_failed_command(
            "$AUTHCMD", ["Failed to configure Smart Card authentication in SSSD"]
        )
