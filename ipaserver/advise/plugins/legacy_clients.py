...
CACERTDIR_REHASH_URL = "https://pagure.io/authconfig/raw/master/f/" "cacertdir_rehash"
...
class config_base_legacy_client(Advice):
    ...
    def configure_ca_cert(self):
        ...
        self.log.command(
            "curl http://%s/ipa/config/ca.crt -o "
        ...
        self.log.command(' curl "%s" -o cacertdir_rehash ;' % CACERTDIR_REHASH_URL)
    ...
    def configure_and_start_sssd(self):
        ...
        template = os.path.join(
            paths.USR_SHARE_IPA_DIR, "advise", "legacy", "sssd.conf.template"
        )
...
class config_redhat_sssd_before_1_9(config_base_legacy_client):
    ...
    description = (
        "Instructions for configuring a system with an old version "
    ...
    def get_info(self):
        ...
        self.log.command("authconfig --updateall --enablesssd " "--enablesssdauth\n")

        self.configure_and_start_sssd()

    def configure_ca_cert(self):
        self.log.comment(
            "NOTE: IPA certificate uses the SHA-256 hash "
...
class config_generic_linux_sssd_before_1_9(config_base_legacy_client):
    ...
    def get_info(self):
        ...
        with open(
            os.path.join(
                paths.USR_SHARE_IPA_DIR, "advise", "legacy", "pam.conf.sssd.template"
            )
        ) as fd:
            pam_conf = fd.read()
        ...
        self.log.comment(
            "Configure nsswitch.conf. Append sss to the lines "
        ...
        self.log.comment(
            "Configure PAM. Configuring the PAM stack differs on "
    ...
    def configure_ca_cert(self):
        ...
        self.log.comment(
            "Configure ldap.conf. Set the value of "
...
class config_redhat_nss_pam_ldapd(config_base_legacy_client):
    ...
    description = (
        "Instructions for configuring a system with nss-pam-ldapd "
    ...
    def get_info(self):
        ...
        self.log.comment("Install required packages via yum")
        self.log.command(
            "yum install -y curl openssl nss-pam-ldapd pam_ldap " "authconfig\n"
        )
        ...
        self.log.command(
            "authconfig --updateall --enableldap --enableldaptls "
...
class config_generic_linux_nss_pam_ldapd(config_base_legacy_client):
    ...
    def get_info(self):
        ...
        self.log.command(
            "apt-get -y install curl openssl libnss-ldapd " "libpam-ldapd nslcd\n"
        )

        self.configure_ca_cert()

        self.log.comment(
            "Configure nsswitch.conf. Append ldap to the lines "
            "beginning with passwd and group. "
        )
...
class config_freebsd_nss_pam_ldapd(config_base_legacy_client):
    ...
    description = (
        "Instructions for configuring a FreeBSD system with " "nss-pam-ldapd. "
    )
    ...
    def get_info(self):
        ...
        with open(
            os.path.join(
                paths.USR_SHARE_IPA_DIR, "advise", "legacy", "pam_conf_sshd.template"
            )
        ) as fd:
            pam_conf = fd.read()
        ...
        self.log.command(
            "sed -i '' -e 's/^passwd:/passwd: files ldap/' " "/etc/nsswitch.conf"
        )
        self.log.command(
            "sed -i '' -e 's/^group:/group: files ldap/' " "/etc/nsswitch.conf\n"
        )
        ...
        self.log.command(
            'echo \'nslcd_enable="YES"\nnslcd_debug="NO"\' >> ' "/etc/rc.conf"
        )
    ...
    def configure_ca_cert(self, cacrt):
        self.log.comment("Download the CA certificate of the IPA server")
        self.log.command(
            "curl -k https://%s/ipa/config/ca.crt > " "%s" % (api.env.host, cacrt)
        )
...
class config_redhat_nss_ldap(config_base_legacy_client):
    ...
    description = (
        "Instructions for configuring a system with nss-ldap "
        "as a FreeIPA client. This set of instructions is targeted "
        "for platforms that include the authconfig utility, which "
        "are all Red Hat based platforms."
    )
    ...
    def get_info(self):
        ...
        self.log.comment("Install required packages via yum")
        self.log.command("yum install -y curl openssl nss_ldap " "authconfig\n")
