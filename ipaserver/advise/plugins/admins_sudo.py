...
class enable_admins_sudo(Advice):
    ...
    description = (
        "Instructions for enabling HBAC and unauthenticated "
    ...
    def create_hbac_rule(self):
        ...
        self.log.command("ipa hbacrule-add-service --hbacsvcs=sudo admins_sudo")
    ...
    def get_info(self):
        ...
        with self.log.if_branch("ipa hbacrule-show admins_sudo > /dev/null 2>&1"):
            self.log.command("echo HBAC rule admins_sudo already exists")
        with self.log.else_branch():
            self.create_hbac_rule()

        with self.log.if_branch("ipa sudorule-show admins_all > /dev/null 2>&1"):
