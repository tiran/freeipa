...
def get_default_attrs(object_classes):
    ...
    if len(present_clss) <= 0:
        raise ValueError("none of '%s' object classes are supported" % object_classes)
...
class Key(MutableMapping):
    ...
    def __assert_not_deleted(self):
        assert self.entry and not self._delentry, (
            "attempt to use to-be-deleted entry %s detected" % self._delentry.dn
        )
    ...
    def _delete_key(self):
        ...
        """
        assert not self.entry, "Key._delete_key() called before Key.schedule_deletion()"
        assert self._delentry, "Key._delete_key() called more than once"
        logger.debug(
            "deleting key id 0x%s DN %s from LDAP",
...
class MasterKey(Key):
    ...
    def add_wrapped_data(self, data, wrapping_mech, replica_key_id):
        wrapping_key_uri = "pkcs11:id=%s;type=public" % uri_escape(replica_key_id)
...
class LdapKeyDB(AbstractHSM):
    ...
    def _get_key_dict(self, key_type, ldap_filter):
        try:
            objs = self.ldap.get_entries(base_dn=self.base_dn, filter=ldap_filter)
        except ipalib.errors.NotFound:
            return {}
        ...
        for o in objs:
            ...
            if "ipk11id" not in key:
                raise ValueError("key is missing ipk11Id in %s" % key.entry.dn)
            key_id = key["ipk11id"]
            if key_id in keys:
                raise ValueError(
            ...
            if "ipk11label" not in key:
                raise ValueError("key '%s' is missing ipk11Label" % key.entry.dn)
    ...
    def _update_keys(self):
        for cache in [
            self.cache_masterkeys,
            self.cache_replica_pubkeys_wrap,
    ...
    def import_zone_key(
        ...
        new_key.entry["ipaWrappingKey"] = "pkcs11:id=%s;type=secret-key" % uri_escape(
            master_key_id
        )
        ...
        self.ldap.add_entry(new_key.entry)
        logger.debug("imported zone key id: 0x%s", str_hexlify(new_key["ipk11id"]))
