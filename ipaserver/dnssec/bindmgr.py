...
class BINDMgr:
    ...
    def dates2params(self, ldap_attrs):
        ...
        for attr, param in attr2param.items():
            ...
            if attr in ldap_attrs:
                assert len(ldap_attrs[attr]) == 1, (
                    "Timestamp %s is expected to be single-valued" % attr
                )
    ...
    def ldap_event(self, op, uuid, attrs):
        ...
        if op == "add":
            logger.info("Key metadata %s added to zone %s", attrs["dn"], zone)
            zone_keys[uuid] = attrs

        elif op == "del":
            logger.info("Key metadata %s deleted from zone %s", attrs["dn"], zone)
            zone_keys.pop(uuid)

        elif op == "mod":
            logger.info("Key metadata %s updated in zone %s", attrs["dn"], zone)
            zone_keys[uuid] = attrs
    ...
    def sync_zone(self, zone):
        ...
        zone_path = os.path.join(
            paths.BIND_LDAP_DNS_ZONE_WORKDIR, self.get_zone_dir_name(zone)
        )
        try:
    ...
    def sync(self, dnssec_zones):
        ...
        logger.debug(
            "Zones modified but skipped during bindmgr.sync: %s",
