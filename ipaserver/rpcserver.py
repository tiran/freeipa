...
from ipalib.rpc import xml_dumps, xml_loads, json_encode_binary, json_decode_binary
...
class wsgi_dispatch(Executioner, HTTP_Status):
    ...
    def mount(self, app, key):
        ...
        if key in self.__apps:
            raise Exception(
                "%s.mount(): cannot replace %r with %r at %r"
...
class WSGIExecutioner(Executioner):
    ...
    def wsgi_execute(self, environ):
        ...
        if (
            not environ["HTTP_REFERER"].startswith("https://%s/ipa" % self.api.env.host)
        ...
        try:
            ...
            if self.api.env.debug:
                logger.debug(
                    "WSGI wsgi_execute PublicError: %s", traceback.format_exc()
                )
            error = e
    ...
    def marshal(self, result, error, _id=None, version=VERSION_WITHOUT_CAPABILITIES):
...
class jsonserver(WSGIExecutioner, HTTP_Status):
    ...
    def marshal(self, result, error, _id=None, version=VERSION_WITHOUT_CAPABILITIES):
        ...
        dump = json_encode_binary(response, version, pretty_print=self.api.env.debug)
...
class KerberosSession(HTTP_Status):
    ...
    def get_environ_creds(self, environ):
        ...
        creds = get_credentials_if_valid(name=gss_name, ccache_name=ccache_name)
        if not creds:
            logger.debug("ccache expired or invalid, deleting session, need login")
            return None

        return ccache_name

    def finalize_kerberos_acquisition(
        self, who, ccache_name, environ, start_response, headers=None
    ):
        if headers is None:
            headers = []
        ...
        try:
            ...
            return self.unauthorized(
                environ, start_response, str(e), "Authentication failed"
            )
...
class KerberosWSGIExecutioner(WSGIExecutioner, KerberosSession):
    ...
    def __call__(self, environ, start_response):
        ...
        object.__setattr__(
            self, "headers", [("Content-Type", "%s; charset=utf-8" % self.content_type)]
        )
...
class xmlserver(KerberosWSGIExecutioner):
    ...
    def listMethods(self, *params):
        ...
        return tuple(unicode(cmd.name) for cmd in self.api.Command) + tuple(
            unicode(name) for name in self._system_commands
        )
    ...
    def marshal(self, result, error, _id=None, version=VERSION_WITHOUT_CAPABILITIES):
        if error:
            logger.debug("response: %s: %s", error.__class__.__name__, str(error))
            response = Fault(error.errno, error.strerror)
        else:
            if isinstance(result, dict):
                logger.debug("response: entries returned %d", result.get("count", 1))
            response = (result,)
...
class jsonserver_i18n_messages(jsonserver):
    ...
    def __call__(self, environ, start_response):
        ...
        data = read_input(environ)
        unmarshal_data = super(jsonserver_i18n_messages, self).unmarshal(data)
        ...
        response = super(jsonserver_i18n_messages, self).__call__(
            environ, start_response
        )
        return response
...
class login_x509(KerberosLogin):
    ...
    def __call__(self, environ, start_response):
        ...
        if "KRB5CCNAME" not in environ:
            return self.unauthorized(
                environ, start_response, "KRB5CCNAME not set", "Authentication failed"
            )
...
class login_password(Backend, KerberosSession):
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            ...
            return self.unauthorized(environ, start_response, str(e), "user-locked")
    ...
    def kinit(self, principal, password, ccache_name):
        ...
        armor_path = os.path.join(paths.IPA_CCACHES, "armor_{}".format(os.getpid()))

        logger.debug("Obtaining armor in ccache %s", armor_path)

        try:
            kinit_armor(
                armor_path, pkinit_anchors=[paths.KDC_CERT, paths.KDC_CA_BUNDLE_PEM],
            )
        ...
        try:
            ...
            if (
                ...
                raise KrbPrincipalExpired(principal=principal, message=unicode(e))
            elif (
                "kinit: Clients credentials have been revoked "
                "while getting initial credentials"
            ) in str(e):
                raise UserLocked(principal=principal, message=unicode(e))
            raise InvalidSessionPassword(principal=principal, message=unicode(e))
...
class change_password(Backend, HTTP_Status):
    ...
    def __call__(self, environ, start_response):
        ...
        for field in ("user", "old_password", "new_password", "otp"):
            ...
            if value is not None:
                if len(value) == 1:
                    ...
                    return self.bad_request(
                        environ, start_response, "more than one %s parameter" % field
                    )
        ...
        logger.info(
            "WSGI change_password: start password change of user '%s'", data["user"]
        )
        status = HTTP_STATUS_SUCCESS
        response_headers = [("Content-Type", "text/html; charset=utf-8")]
        ...
        try:
            ...
            try:
                conn.modify_password(
                    bind_dn, data["new_password"], data["old_password"], skip_bind=True
                )
        ...
        start_response(status, response_headers)
        output = _success_template % dict(title=str(title), message=str(message))
        return [output.encode("utf-8")]
...
class sync_token(Backend, HTTP_Status):
    ...
    def __call__(self, environ, start_response):
        ...
        conn = ldap2(self.api)
        try:
            conn.connect(bind_dn=bind_dn, bind_pw=data["password"], serverctrls=[rc,])
            result = "ok"
