...
class Test_set_directive_lines(object):
    def test_remove_directive(self):
        lines = directives.set_directive_lines(
...
class Test_get_directive(object):
    def test_get_directive(self, tmpdir):
        ...
        assert "1" == directives.get_directive(
            str(configfile), "foo", separator="="
        )
        assert "2" == directives.get_directive(
            str(configfile), "foobar", separator="="
        )
...
class Test_get_directive_whitespace(object):
    def test_get_directive(self, tmpdir):
        ...
        assert "1" == directives.get_directive(str(configfile), "foo")
        assert "2" == directives.get_directive(str(configfile), "foobar")
...
def test_directivesetter(tempdir):
    ...
    ds = directives.DirectiveSetter(filename)
    ...
    with ds:
        ...
        ds.setitems([("list1", "value1"), ("list2", "value2")])
        ds.setitems({"dict1": "value1", "dict2": "value2"})
    ...
    with directives.DirectiveSetter(filename, True, "=") as ds:
