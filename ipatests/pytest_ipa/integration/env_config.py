...
def config_from_env(env):
    if "IPATEST_YAML_CONFIG" in env:
        try:
            ...
            raise ImportError("%s, please install PyYAML package to fix it" % e)
    ...
    kwargs = {s.name: env.get(s.var_name, s.default) for s in _setting_infos}
    kwargs["domains"] = []
    ...
    domain_index = 1
    while (
        env.get("MASTER_env%s" % domain_index)
        ...
        if env.get("MASTER_env%s" % domain_index):
            ...
            config.domains.append(
                domain_from_env(env, config, domain_index, domain_type="IPA")
            )
        else:
            for domain_type in ("AD", "AD_SUBDOMAIN", "AD_TREEDOMAIN"):
                if env.get("%s_env%s" % (domain_type, domain_index)):
                    config.domains.append(
                        domain_from_env(
                            env, config, domain_index, domain_type=domain_type
                        )
                    )
                    break
        domain_index += 1
    ...
    for domain in config.domains:
        ...
        for role in domain.roles:
            ...
            for i, host in enumerate(hosts, start=1):
                ...
                env["%s%s%s" % (prefix, suffix, env_suffix)] = host.hostname
...
def domain_from_env(env, config, index, domain_type):
    ...
    for role in _roles_from_env(domain, env, env_suffix):
        ...
        for host_index, hostname in enumerate(value.split(), start=1):
            host = host_from_env(env, domain, hostname, role, host_index, index)
...
def host_from_env(env, domain, hostname, role, index, domain_index):
    ip = env.get("BEAKER%s%s_IP_env%s" % (role.upper(), index, domain_index), None)
...
def host_to_env(host, **kwargs):
    ...
    prefix = "" if host.role in host.domain.static_roles else TESTHOST_PREFIX
