...
def prepare_reverse_zone(host, ip):
    ...
    result = host.run_command(["ipa", "dnszone-add", zone], raiseonerr=False)
...
def rpcbind_kadmin_workaround(host):
    ...
    cmd.extend(
        (
            "-o",
            "state",
            "all",  # ports in any state, not just listening
...
def allow_sync_ptr(host):
    kinit_admin(host)
    host.run_command(
        ["ipa", "dnsconfig-mod", "--allow-sync-ptr=true"], raiseonerr=False
    )
...
def backup_file(host, filename):
    if host.transport.file_exists(filename):
        backupname = os.path.join(
            host.config.test_dir, "file_backup", filename.lstrip("/")
        )
        ...
        host.run_command(
            "echo %s >> %s"
            % (ipautil.shell_quote(filename), ipautil.shell_quote(rmname))
        )
...
def host_service_active(host, service):
    res = host.run_command(
        ["systemctl", "is-active", "--quiet", service], raiseonerr=False
    )
...
def set_default_ttl_for_ipa_dns_zone(host, raiseonerr=True):
    args = ["ipa", "dnszone-mod", host.domain.name, "--default-ttl", "1", "--ttl", "1"]
    result = host.run_command(args, raiseonerr=raiseonerr, stdin_text=None)
    if result.returncode != 0:
        logger.info(
            "Failed to set TTL and default TTL for DNS zone %s to 1", host.domain.name
        )
...
def install_master(
    ...
    if setup_dns:
        args.extend(
            ["--setup-dns", "--forwarder", host.config.dns_forwarder, "--auto-reverse"]
        )
    ...
    result = host.run_command(args, raiseonerr=raiseonerr, stdin_text=stdin_text)
    if result.returncode == 0:
...
def master_authoritative_for_client_domain(master, client):
    ...
    result = master.run_command(["ipa", "dnszone-show", zone], raiseonerr=False)
...
def config_host_resolvconf_with_master_data(master, host):
    ...
    """
    content = "search {domain}\nnameserver {master_ip}".format(
        domain=master.domain.name, master_ip=master.ip
    )
...
def install_replica(
    ...
    args = ["ipa-replica-install", "--admin-password", replica.config.admin_password]
    ...
    if promote:  # while promoting we use directory manager password
        ...
        args.extend(
            ["--principal", replica.config.admin_name, "--server", master.hostname]
        )
    ...
    if setup_kra:
        ...
        args.append("--setup-kra")
    if setup_dns:
        args.extend(["--setup-dns", "--forwarder", replica.config.dns_forwarder])
        fw_services.append("dns")
    if setup_adtrust:
        args.append("--setup-adtrust")
        fw_services.append("freeipa-trust")
    ...
    args.extend(["--realm", replica.domain.realm, "--domain", replica.domain.name])
    fw.enable_services(fw_services)

    result = replica.run_command(args, raiseonerr=raiseonerr, stdin_text=stdin_text)
    if result.returncode == 0:
...
def install_client(
    ...
    if not error:
        master.run_command(["ipa", "dnszone-mod", zone, "--dynamic-update=TRUE"])
    if user is None:
...
def install_adtrust(host):
    ...
    host.run_command(["systemctl", "restart", knownservices.named.systemd_name])
...
def disable_dnssec_validation(host):
    ...
    named_conf = re.sub(
        br"dnssec-validation\s*yes;", b"dnssec-validation no;", named_conf
    )
...
def is_subdomain(subdomain, domain):
    ...
    if len(subdomain_unpacked) > len(domain_unpacked):
        ...
        for subdomain_segment, domain_segment in zip(
            subdomain_unpacked, domain_unpacked
        ):
...
def configure_dns_for_trust(master, *ad_hosts):
    ...
    for ad in ad_hosts:
        if is_subdomain(ad.domain.name, master.domain.name):
            ...
            master.run_command(
                ["ipa", "dnszone-mod", master.domain.name, "--allow-transfer", ad.ip]
            )
        else:
...
def configure_windows_dns_for_trust(ad, master):
    ad.run_command(["dnscmd", "/zoneadd", master.domain.name, "/Forwarder", master.ip])
...
def establish_trust_with_ad(master, ad_domain, extra_args=(), shared_secret=None):
    """
    ...
    master.run_command(["kinit", "-kt", paths.HTTP_KEYTAB, "HTTP/%s" % master.hostname])
    ...
    run_repeatedly(
        master,
        ["ipa", "trust-add", "--type", "ad", ad_domain] + extra_args,
...
def remove_trust_info_from_ad(master, ad_domain):
    ...
    master.run_command(
        [
            "rpcclient",
            ad_domain,
            "-U\\Administrator%{}".format(master.config.ad_admin_password),
            "-c",
            "deletetrustdom {}".format(master.domain.name),
        ],
        raiseonerr=False,
    )
...
def setup_sssd_debugging(host):
    ...
    host.run_command(
        ["sed", "-i", "/debug_level = 7/d", paths.SSSD_CONF], raiseonerr=False
    )

    # Add the debug directive to each section
    host.run_command(
        ["sed", "-i", r"/\[*\]/ a\debug_level = 7", paths.SSSD_CONF], raiseonerr=False
    )
...
def remote_sssd_config(host):
    ...
    try:
        ...
        host.run_command(
            ["tar", "cJvf", tmpname, "sssd.api.conf", "sssd.api.d"],
            log_stdout=False,
            cwd="/usr/share/sssd",
        )
        # fetch tar file
...
def clear_sssd_cache(host):
    ...
    host.run_command("find /var/lib/sss/db -name '*.ldb' | " "xargs rm -fv")
...
def sync_time(host, server):
    ...
    host.run_command(
        [
            "chronyd",
            "-q",
            "server {srv} iburst maxdelay 1000".format(srv=server.hostname),
...
def connect_replica(master, replica, domain_level=None, database=DOMAIN_SUFFIX_NAME):
    if domain_level is None:
...
def disconnect_replica(master, replica, domain_level=None, database=DOMAIN_SUFFIX_NAME):
    if domain_level is None:
        domain_level = master.config.domain_level
    check_domain_level(domain_level)
    if domain_level == DOMAIN_LEVEL_0:
...
def kinit_admin(host, raiseonerr=True):
    return host.run_command(
        ["kinit", "admin"], raiseonerr=raiseonerr, stdin_text=host.config.admin_password
    )
...
def uninstall_master(
    ...
    if clean:
        Firewall(host).disable_services(
            ["freeipa-ldap", "freeipa-ldaps", "freeipa-trust", "dns"]
        )

    host.run_command(["pkidestroy", "-s", "CA", "-i", "pki-tomcat"], raiseonerr=False)
    host.run_command(
        [
            "rm",
            "-rf",
    ...
    host.run_command(
        "find /var/lib/sss/keytabs -name '*.keytab' | " "xargs rm -fv", raiseonerr=False
    )
    host.run_command("find /run/ipa -name 'krb5*' | xargs rm -fv", raiseonerr=False)
    if clean:
        unapply_fixes(host)


def uninstall_client(host):
    host.run_command(["ipa-client-install", "--uninstall", "-U"], raiseonerr=False)
    unapply_fixes(host)


@check_arguments_are((0, 2), Host)
def clean_replication_agreement(master, replica, cleanup=False, raiseonerr=True):
    """
...
def create_segment(master, leftnode, rightnode, suffix=DOMAIN_SUFFIX_NAME):
    ...
    if result.returncode == 0:
        return {"leftnode": lefthost, "rightnode": righthost, "name": segment_name}, ""
    else:
...
def destroy_segment(master, segment_name, suffix=DOMAIN_SUFFIX_NAME):
    ...
    kinit_admin(master)
    command = ["ipa", "topologysegment-del", suffix, segment_name]
...
def install_topo(
    ...
    if setup_replica_kras and not setup_replica_cas:
        raise ValueError(
            "Option 'setup_replica_kras' requires " "'setup_replica_cas' set to True"
        )
    replicas = list(replicas)
    installed = {master}
    if not skip_master:
        install_master(master, domain_level=domain_level, setup_kra=setup_replica_kras)
    ...
    for parent, child in get_topo(topo)(master, replicas):
        if child in installed:
            ...
            install_replica(
                parent, child, setup_ca=setup_replica_cas, setup_kra=setup_replica_kras
            )
        installed.add(child)
...
def wait_for_replication(
    ldap, timeout=30, target_status_re=r"^0 |^Error \(0\) ", raise_on_timeout=False
):
    ...
    while True:
        ...
        wrong_statuses = [s for s in statuses if not re.match(target_status_re, s)]
...
def wait_for_cleanallruv_tasks(ldap, timeout=30):
    ...
    for i in range(timeout):
        ...
        try:
            ...
            break
        # Check status
        if all(e.single_value[status_attr].startswith(success_status) for e in entries):
            logger.debug("All cleanallruv tasks finished successfully")
            break
        logger.debug("cleanallruv task in progress, (waited %s/%ss)", i, timeout)
        time.sleep(1)
    else:
...
def add_a_record(master, host):
    ...
    cmd = master.run_command(
        ["ipa", "dnsrecord-show", master.domain.name, host.hostname + "."],
        raiseonerr=False,
    )
...
def ipa_restore(master, backup_path):
    master.run_command(
        ["ipa-restore", "-U", "-p", master.config.dirman_password, backup_path]
    )
    ...
    def wrapped(*args):
        ...
        for host in args[0].replicas:
            ...
            result = args[0].master.run_command(
                ["ipa", "host-del", "--updatedns", host.hostname], raiseonerr=False
            )
            # Workaround for 5627
            if "host not found" in result.stderr_text:
                args[0].master.run_command(
...
def run_certutil(host, args, reqdir, dbtype=None, stdin=None, raiseonerr=True):
    dbdir = reqdir if dbtype is None else "{}:{}".format(dbtype, reqdir)
    new_args = [paths.CERTUTIL, "-d", dbdir]
    new_args.extend(args)
    return host.run_command(new_args, raiseonerr=raiseonerr, stdin_text=stdin)
...
def certutil_fetch_cert(host, reqdir, pwd_file, nickname, token_name=None):
    ...
    if token_name is not None:
        args.extend(["{}:{}".format(token_name, nickname), "-h", token_name])
    else:
        args.append(nickname)
    result = run_certutil(host, args, reqdir)
    return x509.load_pem_x509_certificate(result.stdout_bytes, default_backend())
...
def run_repeatedly(host, command, assert_zero_rc=True, test=None, timeout=30, **kwargs):
    """
    ...
    raise AssertionError(
        "Command: {cmd} repeatedly failed {times} times, "
        "exceeding the timeout of {timeout} seconds.".format(
            cmd=" ".join(command), times=timeout // time_step, timeout=timeout
        )
    )
...
def sign_ca_and_transport(
    ...
    root_ca_name,
    ipa_ca_name,
    root_ca_path_length=None,
    ipa_ca_path_length=1,
    key_size=None,
    root_ca_extensions=(),
):
    """
    ...
    root_ca = external_ca.create_ca(
        path_length=root_ca_path_length, extensions=root_ca_extensions,
    )
    # Sign CSR
...
def user_add(host, login, first="test", last="user", extra_args=(), password=None):
    cmd = ["ipa", "user-add", login, "--first", first, "--last", last]
...
def create_active_user(host, login, password, first="test", last="user", extra_args=()):
...
def wait_for_request(host, request_id, timeout=120):
    for _i in range(0, timeout, 5):
        result = host.run_command(
            "getcert list -i %s | grep status: | awk '{ print $2 }'" % request_id
        )
