...
def is_fips_enabled(host):
    """Check if host has """
    result = host.run_command(["cat", paths.PROC_FIPS_ENABLED], raiseonerr=False)
...
def enable_userspace_fips(host):
    ...
    host.run_command(["mount", "--bind", FIPS_OVERLAY, paths.PROC_FIPS_ENABLED])
    ...
    result = host.run_command(["openssl", "md5", "/dev/null"], raiseonerr=False)
...
def disable_userspace_fips(host):
    ...
    result = host.run_command(["umount", paths.PROC_FIPS_ENABLED], raiseonerr=False)
