...
class TestCLIParsing:
    ...
    def run_command(self, command_name, **kw):
        ...
        try:
            ...
            pytest.skip(
                "%r: Server not available: %r" % (self.__module__, api.env.xmlrpc_uri)
            )
    ...
    def test_group_add_interactive(self):
        with self.fake_stdin("Test group\n"):
            self.check_command(
                "group-add tgroup1", "group_add", cn=u"tgroup1",
            )
    ...
    def test_dnsrecord_del_one_by_one(self):
        ...
        try:
            records = (
                u"1 1 E3B72BA346B90570EED94BE9334E34AA795CED23",
    ...
    def test_dnsrecord_add_ask_for_missing_fields(self):
        ...
        with self.fake_stdin(sshfp_parts[2]):
            self.check_command(
                "dnsrecord-add %s sshfp --sshfp-algorithm=%d "
                "--sshfp-fp-type=%d" % (TEST_ZONE, sshfp_parts[0], sshfp_parts[1]),
                "dnsrecord_add",
                dnszoneidnsname=TEST_ZONE,
                idnsname=u"sshfp",
                # passed via cmdline
    ...
    def test_certfind(self):
        with tempfile.NamedTemporaryFile() as f:
            ...
            f.flush()
            self.check_command(
                "cert_find --file={}".format(f.name), "cert_find", file=goodcert_headers
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ipa-ca-install"], 1, None, b"IPA server is not configured on this system"),
