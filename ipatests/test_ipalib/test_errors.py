...
class test_PluginSubclassError(PrivateExceptionTester):
    ...
    def test_init(self):
        ...
        assert str(inst) == "'bad' not subclass of any base in ('base1', 'base2')"
...
class test_PublicError(PublicExceptionTester):
    ...
    def test_init(self):
        ...
        assert (
            str(e)
            == "%s.format is None yet format=None, message=None" % self.klass.__name__
        )
        ...
        regexp = re.compile(
            "(?ims).*" + "".join("(%s).*" % (x) for x in instructions) + "$"
        )
...
class BaseMessagesTest:
    ...
    def test_public_messages(self):
        ...
        for klass in self.message_list:
            ...
            errno = int(m.group(1))
            assert errno == klass.errno, "docstring=%r but errno=%r in %s" % (
                errno,
                klass.errno,
                klass.__name__,
            )
