...
from ipalib.util import get_pager, create_https_connection, get_proper_tls_version_span
from ipaplatform.constants import constants


@pytest.mark.parametrize(
    "pager,expected_result",
    [
        # Valid values
        ("cat", "/bin/cat"),
        ("/bin/cat", "/bin/cat"),
...
BASE_OPT |= (
    ssl.OP_ALL | ssl.OP_NO_COMPRESSION | ssl.OP_SINGLE_DH_USE | ssl.OP_SINGLE_ECDH_USE
)
TLS_OPT = ssl.OP_NO_SSLv2 | ssl.OP_NO_SSLv3 | ssl.OP_NO_TLSv1 | ssl.OP_NO_TLSv1_1
...
def test_tls_version_span(minver, maxver, opt, expected):
    ...
    conn = create_https_connection(
        "invalid.test", cafile=cafile, tls_version_min=minver, tls_version_max=maxver
    )
