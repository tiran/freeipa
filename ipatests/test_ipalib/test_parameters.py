...
class test_DefaultFrom(ClassChecker):
    ...
    def test_call(self):
        ...
        keys = ("givenname", "sn")
        o = self.cls(callback, *keys)
        kw = dict(givenname="John", sn="Public", hello="world",)
        assert o(**kw) == "JP"
        assert o() is None
...
class test_Param(ClassChecker):
    ...
    def test_init(self):
        ...
        class Subclass1(self.cls):
            kwargs = self.cls.kwargs + (("convert", callable, None),)
        ...
        assert str(e) == "Param('my_param'): takes no such kwargs: 'whatever'"
        e = raises(TypeError, self.cls, "my_param", great="Yes", ape="he is!")
        assert str(e) == "Param('my_param'): takes no such kwargs: 'ape', 'great'"
        ...
        assert str(e) == "%s: cannot have both %s=%r and %s=%r" % (
            "Param('my_param')",
            "include",
            frozenset(["server", "foo"]),
            "exclude",
            frozenset(["client", "bar"]),
        )
    ...
    def test_get_default(self):
        ...
        o = Str("my_str", normalizer=PassThrough(), default=u"Static Default",)
...
class test_Data(ClassChecker):
    ...
    def test_init(self):
        ...
        assert str(e) == "Data('my_data'): minlength == maxlength; use length=7 instead"
...
class test_Bytes(ClassChecker):
    ...
    def test_init(self):
        ...
        assert (
            str(e) == "Bytes('my_bytes'): minlength == maxlength; use length=7 instead"
        )
    ...
    def test_rule_minlength(self):
        ...
        for value in (b"", b"a", b"12"):
            assert_equal(rule(dummy, value), translation % dict(minlength=3))
    ...
    def test_rule_maxlength(self):
        ...
        for value in (b"12345", b"sixsix"):
            assert_equal(rule(dummy, value), translation % dict(maxlength=4))
    ...
    def test_rule_length(self):
        ...
        for value in (b"ab", b"123", b"12345", b"sixsix"):
            assert_equal(
                rule(dummy, value), translation % dict(length=4),
            )
    ...
    def test_rule_pattern(self):
        ...
        for value in (b"Hello!", b"Hello World", utf8_bytes, binary_bytes):
            assert_equal(
                rule(dummy, value), translation % dict(pattern=pat),
            )
...
class EnumChecker(ClassChecker):
    ...
    def test_empty(self):
        ...
        assert_equal(
            str(e),
            "%s('empty_enum'): list of values must not "
    ...
    def test_rules_with_failing_rules(self):
        ...
        for val in self._bad_values:
            assert_equal(
                rule(dummy, val), self._translation % dict(values=self._test_values),
            )
    ...
    def test_one_value(self):
        ...
        for val in self._bad_values:
            assert_equal(
                rule(dummy, val), self._single_value_translation % dict(values=values),
            )
...
class test_Int(ClassChecker):
    ...
    def test_init(self):
        ...
        assert (
            str(e) == "Int('my_number'): minvalue > maxvalue (minvalue=22, maxvalue=15)"
        )
    ...
    def test_rule_minvalue(self):
        ...
        # Test with failing values:
        for value in (-1, 0, 2):
            assert_equal(rule(dummy, value), translation % dict(minvalue=3))
    ...
    def test_rule_maxvalue(self):
        ...
        for value in (5, 99, 1009):
            assert_equal(rule(dummy, value), translation % dict(maxvalue=4))
...
class test_Decimal(ClassChecker):
    ...
    def test_init(self):
        ...
        e = raises(
            ...
            self.cls,
            "my_number",
            minvalue=Decimal("22.5"),
            maxvalue=Decimal("15.1"),
        )
        assert (
            str(e)
    ...
    def test_rule_minvalue(self):
        ...
        for value in (Decimal("-1.2"), Decimal("0.0"), Decimal("3.0")):
            assert_equal(
                rule(dummy, value), translation % dict(minvalue=Decimal("3.1"))
            )
    ...
    def test_rule_maxvalue(self):
        ...
        for value in (Decimal("5.3"), Decimal("99.9")):
            assert_equal(
                rule(dummy, value), translation % dict(maxvalue=Decimal("4.7"))
            )
    ...
    def test_precision(self):
        ...
        for value in (Decimal("0"), Decimal("4.4"), Decimal("4.67")):
            assert_equal(param(value), value)
        ...
        for original, expected in (
            ...
            assert_equal(str(param(original)), expected)
    ...
    def test_numberclass(self):
        ...
        param = self.cls(
            "my_number", exponential=True, numberclass=("-Normal", "+Zero", "+Infinity")
        )
        for value, raises_verror in (
            (Decimal("0"), False),
            (Decimal("-0"), True),
...
class test_AccessTime(ClassChecker):
    ...
    def test_init(self):
        ...
        for value in (
            u"absolute 201012161032 ~ 201012161033",
...
class test_CertificateSigningRequest(ClassChecker):
    ...
    def test_convert(self):
        ...
        for prep_input in (
            ...
            assert csr_object.public_bytes(x509.Encoding.PEM) == self.sample_csr
        ...
        assert isinstance(csr_object, crypto_x509.CertificateSigningRequest)
        assert csr_object.public_bytes(x509.Encoding.DER) == self.sample_der_csr

        # test base64-encoded DER as an input to the convert method
        csr_object = o.convert(self.sample_base64)
        assert isinstance(csr_object, crypto_x509.CertificateSigningRequest)
        assert csr_object.public_bytes(x509.Encoding.DER) == base64.b64decode(
            self.sample_base64
        )
...
class test_DNParam(ClassChecker):
    ...
    def test_convert_scalar(self):
        ...
        good = ["cn=dn,cn=suffix", DN("cn=dn,cn=suffix")]
        for value in good:
