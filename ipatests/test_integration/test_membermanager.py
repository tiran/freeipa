...
class TestMemberManager(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        master.run_command(
            ["ipa", "group-add-member-manager", GROUP1, "--users", USER_MM]
        )
        master.run_command(
            ["ipa", "hostgroup-add-member-manager", HOSTGROUP1, "--users", USER_MM]
        )
        # make indirect group member manager for group and hostgroup
        master.run_command(
            ["ipa", "group-add-member-manager", GROUP1, "--groups", GROUP_INDIRECT]
        )
        master.run_command(
            [
                "ipa",
    ...
    def test_find_by_member_manager(self):
        ...
        result = master.run_command(
            ["ipa", "group-find", "--membermanager-users", USER1], raiseonerr=False
        )
        ...
        result = master.run_command(
            ["ipa", "hostgroup-find", "--membermanager-users", USER1], raiseonerr=False
        )
        assert result.returncode == 1
    ...
    def test_group_member_manager_user(self):
        ...
        master.run_command(["ipa", "group-add-member", GROUP1, "--users", USER1])
        result = master.run_command(["ipa", "group-show", GROUP1])
        assert USER1 in result.stdout_text

        # indirect: add user2 to group
        tasks.kinit_as_user(master, USER_INDIRECT, PASSWORD)
        master.run_command(["ipa", "group-add-member", GROUP1, "--users", USER2])
        # verify
    ...
    def test_group_member_manager_group(self):
        ...
        tasks.kinit_as_user(master, USER_MM, PASSWORD)
        master.run_command(["ipa", "group-add-member", GROUP1, "--groups", GROUP2])
        result = master.run_command(["ipa", "group-show", GROUP1])
    ...
    def test_hostgroup_member_manager_user(self):
        ...
        tasks.kinit_as_user(master, USER_MM, PASSWORD)
        master.run_command(
            ["ipa", "hostgroup-add-member", HOSTGROUP1, "--hosts", master.hostname]
        )
        result = master.run_command(["ipa", "hostgroup-show", HOSTGROUP1])
        assert master.hostname in result.stdout_text
        master.run_command(
            ["ipa", "hostgroup-remove-member", HOSTGROUP1, "--hosts", master.hostname]
        )
        result = master.run_command(["ipa", "hostgroup-show", HOSTGROUP1])
    ...
    def test_hostgroup_member_manager_nopermission(self):
        ...
        tasks.kinit_as_user(master, USER1, PASSWORD)
        result = master.run_command(
            ["ipa", "hostgroup-add-member-manager", HOSTGROUP1, "--users", USER1],
            raiseonerr=False,
        )
        assert result.returncode != 0
        expected = (
