...
class TestSMB(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        tasks.config_host_resolvconf_with_master_data(cls.master, cls.smbclient)
        tasks.install_adtrust(cls.master)
        tasks.configure_dns_for_trust(cls.master, cls.ad)
        tasks.configure_windows_dns_for_trust(cls.ad, cls.master)
        tasks.establish_trust_with_ad(
            cls.master, cls.ad.domain.name, extra_args=["--two-way=true"]
        )

        tasks.create_active_user(
            cls.master, cls.ipa_user1, password=cls.ipa_user1_password
        )
        tasks.create_active_user(
            cls.master, cls.ipa_user2, password=cls.ipa_user2_password
        )
    ...
    def enable_smb_client_dns_lookup_kdc(self):
        ...
        with tasks.FileBackup(smbclient, paths.KRB5_CONF):
            krb5_conf = smbclient.get_file_contents(paths.KRB5_CONF, encoding="utf-8")
    ...
    def samba_share_public(self):
        ...
        with tasks.FileBackup(smbserver, paths.SMB_CONF):
            smb_conf = smbserver.get_file_contents(paths.SMB_CONF, encoding="utf-8")
            ...
            [{name}]
                ...
                browsable=yes
            """.format(
                    name=share_name, path=share_path
                )
            )
    ...
    def smb_sanity_check(self, user, client_mountpoint, share):
        ...
        run_smb_client = partial(
            tasks.run_command_as_user, self.smbclient, user, cwd=client_mountpoint
        )
        run_smb_server = partial(self.smbserver.run_command, cwd=share["server_path"])
        ...
        try:
            ...
            file_contents_at_server = self.smbserver.get_file_contents(
                "{}/{}".format(share["server_path"], test_file_path), encoding="utf-8"
            )
    ...
    def smb_installation_check(self, result):
        ...
        ipa_regexp = domain_regexp_tpl.format(
            domain=re.escape(self.master.domain.name), netbios=self.master.netbios
        )
        ...
        """.format(
            ipa_regexp, ad_regexp
        )
        assert re.search(output_regexp, result.stdout_text, re.VERBOSE | re.DOTALL)
    ...
    def test_samba_uninstallation_without_installation(self):
        res = self.smbserver.run_command(["ipa-client-samba", "--uninstall", "-U"])
        assert res.stdout_text == "Samba domain member is not configured yet\n"

    def test_install_samba(self):
        samba_install_result = self.smbserver.run_command(["ipa-client-samba", "-U"])
        ...
        self.smbserver.run_command(["systemctl", "enable", "--now", "smb", "winbind"])
    ...
    def test_samba_service_listed(self):
        ...
        res = self.master.run_command(["ipa", "service-show", "--raw", service_name])
        expected_output = "krbprincipalname: {}\n".format(service_name)
        assert expected_output in res.stdout_text
        res = self.master.run_command(["ipa", "service-find", "--raw", service_name])
        assert expected_output in res.stdout_text
    ...
    def check_smb_access_at_ipa_client(self, user, password, samba_share):
        ...
        try:
            ...
            tasks.run_command_as_user(
                self.smbclient, user, ["kinit", user], stdin_text=password + "\n"
            )
    ...
    def test_smb_mount_and_access_by_different_users(self, samba_share_public):
        ...
        try:
            self.mount_smb_share(user1, password1, samba_share_public, mount_point)
            tasks.run_command_as_user(self.smbclient, user2, ["kdestroy", "-A"])
            tasks.run_command_as_user(
