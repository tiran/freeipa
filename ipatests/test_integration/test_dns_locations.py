...
IPA_CA_A_REC = ((DNSName(str(IPA_CA_RECORD))),)
...
def resolve_records_from_server(rname, rtype, nameserver):
    ...
    pytest.fail(
        "Query: {} {}, nameserver {} failed due to {}".format(
...
def _gen_expected_srv_rrset(rname, port, servers, ttl=86400):
    rdata_list = [
        "{prio} {weight} {port} {servername}".format(
            prio=prio, weight=weight, port=port, servername=servername.make_absolute()
        )
...
def _gen_expected_a_rrset(rname, servers, ttl=86400):
    return dns.rrset.from_text_list(
        rname, ttl, dns.rdataclass.IN, dns.rdatatype.A, servers
    )
...
class TestDNSLocations(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        tasks.install_replica(
            cls.master, cls.replicas[0], setup_dns=True, setup_ca=False
        )
        tasks.install_replica(
            cls.master, cls.replicas[1], setup_dns=True, setup_ca=True
        )
    ...
    def delete_update_system_records(cls, rnames):
        ...
        for name in rnames:
            cls.master.run_command(
                ["ipa", "dnsrecord-del", str(cls.domain), str(name), "--del-all"]
            )
    ...
    def _test_A_rec_against_server(
        self, server_ip, domain, expected_servers, rec_list=IPA_CA_A_REC
    ):
        for rname in rec_list:
            name_abs = rname.derelativize(domain)
            expected = _gen_expected_a_rrset(name_abs, expected_servers)
            query = resolve_records_from_server(name_abs, "A", server_ip)
    ...
    def _test_SRV_rec_against_server(
        self, server_ip, domain, expected_servers, rec_list=IPA_DEFAULT_MASTER_SRV_REC
    ):
        for rname, port in rec_list:
            name_abs = rname.derelativize(domain)
            expected = _gen_expected_srv_rrset(name_abs, port, expected_servers)
            query = resolve_records_from_server(name_abs, "SRV", server_ip)
    ...
    def test_without_locations(self):
        ...
        for ip in (self.master.ip, self.replicas[0].ip, self.replicas[1].ip):
            self._test_SRV_rec_against_server(ip, self.domain, expected_servers)
    ...
    def test_nsupdate_without_locations(self):
        ...
        self.delete_update_system_records(
            rnames=(r[0] for r in IPA_DEFAULT_MASTER_SRV_REC)
        )
    ...
    def test_two_replicas_in_location(self):
        ...
        domain_paris_loc = (
            DNSName("{}._locations".format(self.LOC_PARIS))
            + DNSName(self.master.domain.name).make_absolute()
        )
    ...
    def test_all_servers_in_location(self):
        ...
        self.master.run_command(
            ["ipa", "server-mod", self.master.hostname, "--location", self.LOC_PARIS]
        )
    ...
    def test_change_weight_relative_zero_0(self):
        ...
        result = self.master.run_command(["ipa", "location-show", self.LOC_PARIS])
    ...
    def test_change_weight_relative_zero_1(self):
        ...
        for hostname in (self.replicas[0].hostname, self.replicas[1].hostname):
            self.master.run_command(
                ["ipa", "server-mod", hostname, "--service-weight", str(new_weight)]
            )
    ...
    def test_change_weight_relative_zero_2(self):
        ...
        self.master.run_command(
            ["ipa", "server-mod", self.master.hostname, "--service-weight", "200"]
        )
        for hostname in (self.replicas[0].hostname, self.replicas[1].hostname):
            self.master.run_command(
                ["ipa", "server-mod", hostname, "--service-weight", str(new_weight)]
    ...
    def test_restore_locations_and_weight(self):
        ...
        for hostname in (
            ...
            self.master.run_command(["ipa", "server-mod", hostname, "--location=" ""])
    ...
    def test_adtrust_system_records(self):
        ...
        time.sleep(5)
        self.delete_update_system_records(
            rnames=(r[0] for r in IPA_DEFAULT_ADTRUST_SRV_REC)
        )
        ...
        for ip in (self.master.ip, self.replicas[0].ip, self.replicas[1].ip):
            self._test_SRV_rec_against_server(
                ip, self.domain, expected_servers, rec_list=IPA_DEFAULT_ADTRUST_SRV_REC
            )
    ...
    def test_remove_replica_with_ca(self):
        ...
        self._test_A_rec_against_server(self.master.ip, self.domain, expected_servers)
