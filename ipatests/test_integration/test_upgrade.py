...
class TestUpgrade(IntegrationTest):
    ...
    def test_invoke_upgrader(self):
        cmd = self.master.run_command(["ipa-server-upgrade"], raiseonerr=False)
        assert (
            "DN: cn=Schema Compatibility,cn=plugins,cn=config does not \
                exists or haven't been updated"
    ...
    def test_double_encoded_cacert(self):
        ...
        try:
            ...
            raise AssertionError("%s contains a double-encoded cert" % entry.dn)
