...
class TestAutounmembership(IntegrationTest):
    ...
    topology = "line"
    dn = DN(("cn", "Auto Membership Plugin"), ("cn", "plugins"), ("cn", "config"))

    def automember_group_add(self, group):
        """Add group/automember entry."""
        cmdlist = [
            ["ipa", "group-add", "--desc=%s" % group, group],
    ...
    def user_automember_mod(self, group, user):
        """Modify user entry."""
        self.master.run_command(
            ["ipa", "user-mod", user, "--setattr=destinationindicator=%s" % group]
        )

    def remove_user_automember(self, user, raiseonerr=True):
        """Delete user entry."""
        self.master.run_command(["ipa", "user-del", user], raiseonerr=raiseonerr)

    def automember_group_cleanup(self, group):
        """Cleanup of automember/group entry."""
        cmdlist = [
            ["ipa", "automember-del", "--type=group", group],
            ["ipa", "group-del", group],
        ]
        for cmd in cmdlist:
    ...
    def automember_hostgroup_add(self, hostgroup):
        ...
        cmdlist = [
            ["ipa", "hostgroup-add", "--desc=%s" % hostgroup, hostgroup],
    ...
    def host_automember_setup(self, hostgroup, host):
        """Add host entry."""
        self.master.run_command(
            ["ipa", "host-add", host, "--location=%s" % hostgroup, "--force"]
        )

    def host_automember_mod(self, hostgroup, host):
        """Modify host entry."""
        self.master.run_command(["ipa", "host-mod", host, "--location=%s" % hostgroup])

    def remove_host_automember(self, host, raiseonerr=True):
        """Delete host entry."""
        self.master.run_command(["ipa", "host-del", host], raiseonerr=raiseonerr)

    def automember_hostgroup_cleanup(self, hostgroup):
        """Cleanup of automember/hostgroup entry."""
        cmdlist = [
            ["ipa", "automember-del", "--type=hostgroup", hostgroup],
    ...
    def test_modify_user_entry_unmembership_disabled(self):
        ...
        try:
            ...
            self.master.run_command(["ipa", "automember-rebuild", "--users=%s" % user2])
    ...
    def test_modify_host_entry_unmembership_disabled(self):
        ...
        try:
            ...
            self.master.run_command(["ipa", "automember-rebuild", "--hosts=%s" % host2])
