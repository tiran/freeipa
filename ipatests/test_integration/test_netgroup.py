...
def three_netgroups(request):
    ...
    for d in test_data:
        request.cls.master.run_command(
            [
                "ipa",
                "user-add",
                d["user"]["login"],
        ...
        request.cls.master.run_command(
            ["ipa", "netgroup-add", d["netgroup"]], raiseonerr=False
        )

        user_opt = "--users={u[login]}".format(u=d["user"])
        request.cls.master.run_command(
            ["ipa", "netgroup-add-member", user_opt, d["netgroup"]], raiseonerr=False
        )
    ...
    def teardown_three_netgroups():
        ...
        for d in test_data:
            request.cls.master.run_command(
                ["ipa", "user-del", d["user"]["login"]], raiseonerr=False
            )

            request.cls.master.run_command(
                ["ipa", "netgroup-del", d["netgroup"]], raiseonerr=False
            )
...
class TestNetgroups(IntegrationTest):
    ...
    def check_users_in_netgroups(self):
        ...
        for d in test_data:
            ...
            result = master.run_command(
                ["getent", "netgroup", d["netgroup"]], raiseonerr=False
            )
            assert result.returncode == 0

            netgroup = "(-,{},{})".format(d["user"]["login"], self.master.domain.name)
    ...
    def prepare_nested_netgroup_hierarchy(self):
        ...
        for d in test_data:
            ...
            self.master.run_command(
                ["ipa", "netgroup-add-member", netgroups_opt, d["netgroup"]]
            )
    ...
    def test_remove_nested_netgroup(self, three_netgroups):
        ...
        result = self.master.run_command(
            ["ipa", "netgroup-remove-member", netgroups_opt, test_data[1]["netgroup"]],
            raiseonerr=False,
        )
        assert result.returncode == 0
        clear_sssd_cache(master)
        ...
        result = self.master.run_command(
            ["ipa", "netgroup-remove-member", netgroups_opt, test_data[2]["netgroup"]],
            raiseonerr=False,
        )
        assert result.returncode == 0
        clear_sssd_cache(master)
