...
def establish_winsync_agreement(master, ad):
    ...
    master.run_command(
        [
            ...
            "--winsync",
            "--binddn",
            "cn=%s,cn=users,%s" % (ad.config.ad_admin_name, ad.domain.basedn),
            "--bindpw",
            ad.config.ad_admin_password,
            "--password",
...
class TestWinsyncMigrate(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        testuser_regex = r"^{0}:\*:(\d+):(\d+):{0}:/home/{0}:{1}$".format(
    ...
    def create_test_objects(cls):
        ...
        for role in [
            ...
            cls.collision_role2,
            cls.collision_role3,
            cls.test_role_with_nonposix_chars,
        ]:
    ...
    def setup_user_memberships(cls, user):
        cls.master.run_command(
            ["ipa", "group-add-member", cls.ipa_group, "--users", user]
        )
        for role in [
            cls.test_role,
            cls.collision_role1,
            ...
            cls.master.run_command(["ipa", "role-add-member", role, "--users", user])
        cls.master.run_command(
            ["ipa", "hbacrule-add-user", cls.test_hbac_rule, "--users", user]
        )
        cls.master.run_command(
            ["ipa", "selinuxusermap-add-user", cls.test_selinux_map, "--users", user]
        )

    def check_replication_agreement_exists(self, server_name, should_exist):
        result = self.master.run_command(["ipa-replica-manage", "list", server_name])
        if should_exist:
            expected_message = "%s: winsync" % self.ad.hostname
        else:
            expected_message = "Cannot find %s in public server list" % server_name
    ...
    def test_preconditions(self):
        ...
        result = self.master.run_command(
            ["ipa", "user-show", self.ad_user], raiseonerr=False
        )
    ...
    def test_migration(self):
        ...
        assert "The ipa-winsync-migrate command was successful" in result.stderr_text
    ...
    def test_user_attributes_preserved(self):
        result = self.master.run_command(["getent", "passwd", self.trust_test_user])
        passwd_template = (
            "{trust_user}:*:{uid}:{gid}:{user}:/home/{domain}/{user}:{shell}"
        )
        expected_result = passwd_template.format(
            user=self.ad_user,
            uid=self.test_user_uid,
            gid=self.test_user_gid,
            domain=self.ad.domain.name,
            trust_user=self.trust_test_user,
    ...
    def test_selinuxusermap_membership_preserved(self):
        ...
        result = self.master.run_command(
            ["ipa", "selinuxusermap-show", self.test_selinux_map]
        )
        assert "User Groups: %s" % wrapper_group in ipa_output_fields(
            result.stdout_text
        )

        result = self.master.run_command(["ipa", "group-show", wrapper_group])
        assert "External member: %s" % self.trust_test_user in ipa_output_fields(
            result.stdout_text
        )

    def test_hbacrule_membership_preserved(self):
        result = self.master.run_command(
            ["ipa", "group-show", "hbacrule_%s_winsync_external" % self.test_hbac_rule]
        )
        output_fields = ipa_output_fields(result.stdout_text)
        assert "External member: %s" % self.trust_test_user in output_fields
    ...
    def test_non_posix_chars_in_group_names_replaced(self):
        ...
        expected_group_name = (
            "role_%s_winsync_external" % self.test_role_with_nonposix_chars_normalized
        )
        assert "Member groups: %s" % expected_group_name in ipa_output_fields(
            result.stdout_text
        )
