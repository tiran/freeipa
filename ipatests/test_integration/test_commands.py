...
class TestIPACommand(IntegrationTest):
    ...
    def pwpolicy_global(self):
        ...
        self.master.run_command(["ipa", "pwpolicy-mod", "--history=5", "--minlife=0"],)
        yield
        self.master.run_command(["ipa", "pwpolicy-mod", "--history=0", "--minlife=1"],)
    ...
    def test_aes_sha_kerberos_enctypes(self):
        ...
        tasks.kinit_admin(self.master)
        dn = DN(
            ("cn", self.master.domain.realm),
            ("cn", "kerberos"),
            realm_to_suffix(self.master.domain.realm),
        )
        result = tasks.ldapsearch_dm(
            self.master, str(dn), ["krbSupportedEncSaltTypes"], scope="base"
        )
    ...
    def test_cert_find_issue7520(self):
        # https://pagure.io/freeipa/issue/7520
        tasks.kinit_admin(self.master)
        subject = "CN=Certificate Authority,O={}".format(self.master.domain.realm)
        ...
        # by base64 cert
        cab64 = self.get_cert_base64(self.master, paths.IPA_CA_CRT)
        result = self.master.run_command(["ipa", "cert-find", "--certificate", cab64])
        assert subject in result.stdout_text
        assert "1 certificate matched" in result.stdout_text
    ...
    def test_change_sysaccount_password_issue7561(self):
        ...
        entry_ldif = textwrap.dedent(
            ...
            nsIdleTimeout: 0
        """
        ).format(base_dn=base_dn, original_passwd=original_passwd, sysuser=sysuser)
        tasks.ldapmodify_dm(master, entry_ldif)

        tasks.ldappasswd_sysaccount_change(sysuser, original_passwd, new_passwd, master)
    ...
    def test_change_sysaccount_pwd_history_issue7181(self, pwpolicy_global):
        ...
        entry_ldif = textwrap.dedent(
            ...
            nsIdleTimeout: 0
        """
        ).format(account_name=sysuser, base_dn=base_dn, original_passwd=original_passwd)
        ...
        tasks.ldappasswd_sysaccount_change(sysuser, original_passwd, new_passwd, master)
        tasks.ldappasswd_sysaccount_change(sysuser, new_passwd, original_passwd, master)
        tasks.ldappasswd_sysaccount_change(sysuser, original_passwd, new_passwd, master)
    ...
    def test_dm_change_user_pwd_history_issue7181(self, pwpolicy_global):
        ...
        try:
            tasks.ldappasswd_user_change(
                user, new_passwd, original_passwd, master, use_dirman=True
            )
    ...
    def test_huge_password(self):
        ...
        entry_ldif = textwrap.dedent(
            ...
            userpassword: {new_passwd}
        """
        ).format(user=user, base_dn=base_dn, new_passwd=huge_password)
        ...
        keytab_file = os.path.join(self.master.config.test_dir, "user.keytab")
        ...
        tasks.kinit_admin(master)
        result = master.run_command(
            ["ipa", "host-add", "--force", hostname, "--password", huge_password],
            raiseonerr=False,
        )
        assert result.returncode != 0
        ...
        result = master.run_command(
            ["ipa", "host-add", "--force", hostname, "--password", original_passwd],
            raiseonerr=False,
        )
        assert result.returncode == 0
    ...
    def test_cleartext_password_httpd_log(self):
        ...
        self.master.run_command(
            ["ipa", "host-add", "--force", hostname, "--password", passwd]
        )

        # remove added host i.e cleanup
        self.master.run_command(["ipa", "host-del", hostname])

        result = self.master.run_command(["grep", hostname, paths.VAR_LOG_HTTPD_ERROR])
    ...
    def test_change_selinuxusermaporder(self):
        ...
        tasks.kinit_admin(self.master)
        result = self.master.run_command(
            ["ipa", "config-mod", "--ipaselinuxusermaporder={}".format(maporder)],
            raiseonerr=False,
        )
        assert result.returncode == 0

        # apply the update
        result = self.master.run_command(["ipa-server-upgrade"], raiseonerr=False)
        assert result.returncode == 0

        # ensure result is the same
        result = self.master.run_command(["ipa", "config-show"], raiseonerr=False)
        assert result.returncode == 0
        assert "SELinux user map order: {}".format(maporder) in result.stdout_text

    def test_ipa_console(self):
        tasks.kinit_admin(self.master)
        result = self.master.run_command(["ipa", "console"], stdin_text="api.env")
    ...
    def test_list_help_topics(self):
        tasks.kinit_admin(self.master)
        result = self.master.run_command(["ipa", "help", "topics"], raiseonerr=False)
        assert result.returncode == 0
    ...
    def test_ssh_key_connection(self, tmpdir):
        ...
        test_user = "test-ssh"
        external_master_hostname = self.master.external_hostname
        ...
        for i in range(40):
            ...
            with open(os.path.join(tmpdir, "ssh_priv_{}".format(i)), "w") as fp:
    ...
    def test_ssh_leak(self):
        ...
        cert_args = list(chain.from_iterable(list(zip(repeat("--certificate"), certs))))
    ...
    def test_sssd_ifp_access_ipaapi(self):
        ...
        # test IFP as ipaapi
        result = self.master.run_command(["sudo", "-u", IPAAPI_USER, "--"] + cmd)
        assert uid in result.stdout_text

    def test_ipa_cacert_manage_install(self):
        # Re-install the IPA CA
        self.master.run_command([paths.IPA_CACERT_MANAGE, "install", paths.IPA_CA_CRT])

        # Test a non-existent file
        result = self.master.run_command(
            [paths.IPA_CACERT_MANAGE, "install", "/run/cert_not_found"],
            raiseonerr=False,
        )
        assert result.returncode == 1
        ...
        for contents in (good_pkcs7,):
            ...
            result = self.master.run_command(
                [paths.IPA_CACERT_MANAGE, "install", filename]
            )

        for contents in (badcert,):
            self.master.put_file_contents(filename, contents)
            result = self.master.run_command(
                [paths.IPA_CACERT_MANAGE, "install", filename], raiseonerr=False
            )
            assert result.returncode == 1
    ...
    def test_hbac_systemd_user(self):
        ...
        tasks.kinit_admin(self.master)
        # check for presence
        self.master.run_command(["ipa", "hbacsvc-show", "systemd-user"])
        result = self.master.run_command(
        ...
        # delete both
        self.master.run_command(["ipa", "hbacrule-del", "allow_systemd-user"])
        self.master.run_command(["ipa", "hbacsvc-del", "systemd-user"])
        ...
        # run upgrade
        result = self.master.run_command(["ipa-server-upgrade"])
        assert "hbac service systemd-user already exists" in result.stderr_text
        assert "Created hbac rule allow_systemd-user" not in result.stderr_text
        result = self.master.run_command(
            ["ipa", "hbacrule-show", "allow_systemd-user"], raiseonerr=False
        )
        assert result.returncode != 0
    ...
    def test_ssh_from_controller(self):
        ...
        with tasks.remote_sssd_config(self.master) as sssd_config:
            sssd_config.edit_domain(self.master.domain, "ldap_deref_threshold", 0)
        try:
            ...
            input_passwd = "Secret123\nSecret123\nSecret123\n"
            self.master.run_command(["kinit", username], stdin_text=input_passwd)

            client = paramiko.SSHClient()
            client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            client.connect(
                self.master.hostname, username=username, password="Secret123"
            )
            client.close()
        finally:
    ...
    def test_user_mod_change_capitalization_issue5879(self):
        ...
        cmd = self.master.run_command(
            ["ipa", "user-mod", ipauser, "--first", modfirst, "--last", modlast]
        )
    ...
    def test_enabled_tls_protocols(self):
        ...
        def is_tls_version_enabled(tls_version):
            res = self.master.run_command(
                [
                    ...
                    "s_client",
                    "-connect",
                    "{}:443".format(self.master.hostname),
                    "-{}".format(tls_version),
                ],
                stdin_text="\n",
                ok_returncode=[0, 1],
        ...
        openssl_cnf = self.master.get_file_contents(
            "/etc/crypto-policies/back-ends/opensslcnf.config", encoding="utf-8"
        )
    ...
    def test_sss_ssh_authorizedkeys(self):
        ...
        tasks.create_active_user(
            self.master, user, passwd, extra_args=["--homedir", "/home/{}".format(user)]
        )
        tasks.kinit_admin(self.master)
        tasks.run_command_as_user(
            self.master, user, ["ssh-keygen", "-N", "", "-f", user_key]
        )
        ssh_pub_key = self.master.get_file_contents(
            "{}.pub".format(user_key), encoding="utf-8"
        )
        openssl_cmd = [
            "openssl",
            "req",
        ...
        try:
            ...
            self.master.run_command(["ipa", "user-mod", user, "--ssh", ssh_pub_key])
            self.master.run_command(
                ["ipa", "user-add-cert", user, "--certificate", cert_b64]
            )
            # clear cache to avoid SSSD to check the user in old lookup
            tasks.clear_sssd_cache(self.master)
            result = self.master.run_command([paths.SSS_SSH_AUTHORIZEDKEYS, user])
    ...
    def test_cacert_manage(self):
        ...
        result = self.master.run_command(
            ["ipa-cacert-manage", "delete"], raiseonerr=False
        )
        assert result.returncode != 0

        # deletion with an unknown nickname
        result = self.master.run_command(
            ["ipa-cacert-manage", "delete", "unknown"], raiseonerr=False
        )
        assert result.returncode != 0
        ...
        result = self.master.run_command(
            ["ipa-cacert-manage", "delete", ipa_ca_nickname], raiseonerr=False
        )
        assert result.returncode != 0
        assert "The IPA CA cannot be removed with this tool" in result.stderr_text
        ...
        result = self.master.run_command(
            ["ipa-cacert-manage", "delete", isrgrootx1_nick], raiseonerr=False
        )
        assert result.returncode != 0
        assert (
            "Verifying '%s' failed. Removing part of the "
            "chain? certutil: certificate is invalid: Peer's "
            "Certificate issuer is not recognized." % isrgrootx1_nick
            in result.stderr_text
        )

        # deletion of a root CA needed by a subCA, with -f option
        result = self.master.run_command(
            ["ipa-cacert-manage", "delete", isrgrootx1_nick, "-f"], raiseonerr=False
        )
        assert result.returncode == 0

        # deletion of a subca
        result = self.master.run_command(
            ["ipa-cacert-manage", "delete", le_x3_nick], raiseonerr=False
        )
        assert result.returncode == 0
