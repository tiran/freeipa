...
class TestUserPermissions(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        cls.master.run_command(["kinit", cls.altadmin], stdin_text=altadmin_kinit)
    ...
    def test_delete_preserve_as_alternate_admin(self):
        ...
        # kinit as altadmin
        self.master.run_command(
            ["kinit", self.altadmin], stdin_text=self.master.config.admin_password
        )
    ...
    def test_selinux_user_optimized(self):
        ...
        client.connect(self.master.hostname, username=testuser, password=password)
        client.close()
...
class TestInstallClientNoAdmin(IntegrationTest):
    ...
    def test_installclient_as_user_admin(self):
        ...
        self.master.run_command(
            ["ipa", "role-add-privilege", "useradmin", "--privileges", "Add Hosts"]
        )
        ...
        result = tasks.run_certutil(
            client, ["-K", "-f", paths.IPA_NSSDB_PWDFILE_TXT], paths.IPA_NSSDB_DIR
        )
        assert "Local IPA host" in result.stdout_text
