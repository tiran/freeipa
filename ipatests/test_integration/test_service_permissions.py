...
class TestServicePermissions(IntegrationTest):
    ...
    def test_service_as_user_admin(self):
        ...
        service_name1 = "testservice1/%s@%s" % (
            self.master.hostname,
            self.master.domain.realm,
        )
        keytab_file1 = os.path.join(self.master.config.test_dir, "testservice_keytab1")
        ...
        self.master.run_command(["kinit", "-k", service_name1, "-t", keytab_file1])
        ...
        self.master.run_command(
            ["ipa", "role-add-member", "User Administrator", "--service", service_name1]
        )
        ...
        self.master.run_command(
            ["ipa", "user-add", "tuser", "--first", "a", "--last", "b", "--random"]
        )
    ...
    def test_service_access(self):
        ...
        service_name2 = "testservice2/%s@%s" % (
            self.master.hostname,
            self.master.domain.realm,
        )

        keytab_file2 = os.path.join(self.master.config.test_dir, "testservice_keytab2")

        # Prepare a service without authentication indicator
        self.master.run_command(["ipa", "service-add", service_name2])

        self.master.run_command(
            ["ipa-getkeytab", "-p", service_name2, "-k", keytab_file2]
        )
        ...
        # Authenticate
        self.master.run_command(["kinit", "-k", service_name2, "-t", keytab_file2])

        # Verify access to service is granted
        result = self.master.run_command(["kvno", service_name2], raiseonerr=False)
        assert result.returncode == 0
        ...
        self.master.run_command(["ipa", "service-mod", service_name2, "--auth-ind=otp"])
