...
from ipatests.pytest_ipa.integration.tasks import assert_error, replicas_cleanup
from ipatests.pytest_ipa.integration.firewall import Firewall
from ipatests.pytest_ipa.integration.env_config import get_global_config
from ipalib.constants import DOMAIN_LEVEL_1, IPA_CA_NICKNAME, CA_SUFFIX_NAME
...
class TestReplicaPromotionLevel1(ReplicaPromotionBase):
    ...
    def test_one_step_install_pwd_and_admin_pwd(self):
        ...
        Firewall(self.replicas[0]).enable_services(["freeipa-ldap", "freeipa-ldaps"])
        expected_err = (
            "--password and --admin-password options are " "mutually exclusive"
        )
...
class TestUnprivilegedUserPermissions(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        cls.master.run_command(
            ["kinit", cls.username], stdin_text=user_kinit_stdin_text
        )
    ...
    def test_replica_promotion_by_unprivileged_user(self):
        ...
        assert_error(result2, "Insufficient privileges to promote the server", 1)

    def test_replica_promotion_after_adding_to_admin_group(self):
        self.master.run_command(
            ["ipa", "group-add-member", "admins", "--users=%s" % self.username]
        )
...
class TestProhibitReplicaUninstallation(IntegrationTest):
    ...
    def test_replica_uninstallation_prohibited(self):
        ...
        assert_error(
            result,
            "Removal of '%s' leads to disconnected"
            " topology" % self.replicas[0].hostname,
            1,
        )
        self.replicas[0].run_command(
            ["ipa-server-install", "--uninstall", "-U", "--ignore-topology-disconnect"]
        )
        Firewall(self.replicas[0]).disable_services(["freeipa-ldap", "freeipa-ldaps"])
...
class TestWrongClientDomain(IntegrationTest):
    ...
    def wrong_client_dom_setup(self, request):
        def fin():
            ...
            tasks.kinit_admin(self.master)
            self.master.run_command(
                ["ipa", "host-del", self.replicas[0].hostname], raiseonerr=False
            )
    ...
    def test_wrong_client_domain(self):
        ...
        result = client.run_command(
            ["ipa-replica-install", "-U", "-w", self.master.config.dirman_password],
            raiseonerr=False,
        )
        assert_error(
            result,
    ...
    def test_client_rollback(self):
        ...
        assert "Unconfigured automount client failed" not in result.stdout_text

        assert "WARNING: Unable to revert" not in result.stdout_text

        assert "An error occurred while removing SSSD" not in result.stdout_text
...
class TestRenewalMaster(IntegrationTest):
    ...
    def test_replica_not_marked_as_renewal_master(self):
        ...
        assert (
            "IPA CA renewal master: %s" % master.hostname in result
        ), "Master hostname not found among CA renewal masters"
        assert (
            "IPA CA renewal master: %s" % replica.hostname not in result
    ...
    def test_manual_renewal_master_transfer(self):
        replica = self.replicas[0]
        replica.run_command(
            ["ipa", "config-mod", "--ca-renewal-master-server", replica.hostname]
        )
        result = self.master.run_command(["ipa", "config-show"]).stdout_text
        assert (
            "IPA CA renewal master: %s" % replica.hostname in result
    ...
    def test_automatic_renewal_master_transfer_ondelete(self):
        ...
        result = self.master.run_command(["ipa", "config-show"]).stdout_text
        assert (
            "IPA CA renewal master: %s" % self.master.hostname in result
        ), "Master hostname not found among CA renewal masters"
...
class TestReplicaInstallWithExistingEntry(IntegrationTest):
    ...
    def test_replica_install_with_existing_entry(self):
        ...
        entry_ldif = textwrap.dedent(
            ...
            changetype: modify
            add: memberPrincipal
            memberPrincipal: ldap/{hostname}@{realm}"""
        ).format(
            base_dn=base_dn,
...
AUTH_ID_RE = re.compile(
    "Authority ID: " "([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})"
)
...
class TestSubCAkeyReplication(IntegrationTest):
    ...
    SERVER_KEY_NICK_FIPS = "NSS FIPS 140-2 Certificate DB:Server-Cert cert-pki-ca"
    EXPECTED_CERTS = {
    ...
    def add_subca(self, host, name, subject):
        result = host.run_command(
            ["ipa", "ca-add", name, "--subject", subject, "--desc", self.SUBCA_DESC,]
        )
    ...
    def check_subca(self, host, name, cert_nick):
        ...
        tasks.run_certutil(host, ["-L", "-n", cert_nick], paths.PKI_TOMCAT_ALIAS_DIR)
        host.run_command(
            [
                paths.CERTUTIL,
                "-d",
    ...
    def check_certdb(self, master, replica):
        ...
        result = master.run_command(["ipa", "ca-find", "--desc", self.SUBCA_DESC])
    ...
    def check_pki_error(self, host):
        pki_log_filename = "{0}.{1}.log".format(
            self.PKI_DEBUG_PATH, time.strftime("%Y-%m-%d")
        )
        pki_debug_log = host.get_file_contents(pki_log_filename, encoding="utf-8")
    ...
    def test_subca_master(self):
        ...
        master_nick = self.add_subca(master, self.SUBCA_MASTER, self.SUBCA_MASTER_CN)
...
def update_etc_hosts(host, ip, old_hostname, new_hostname):
    ...
    """
    # Make a backup
    host.run_command(["/bin/cp", paths.HOSTS, "%s.sav" % paths.HOSTS])
    ...
    new_contents, mods = re.subn(
        pattern, r"\1{}\2".format(new_hostname), contents, flags=re.MULTILINE
    )
    # If it didn't contain any entry for old_hostname, just add new_hostname
    if mods == 0:
        short = new_hostname.split(".", 1)[0]
        new_contents = new_contents + "\n{}\t{} {}\n".format(ip, new_hostname, short)
...
def restore_etc_hosts(host):
    ...
    """
    host.run_command(["/bin/mv", "%s.sav" % paths.HOSTS, paths.HOSTS], raiseonerr=False)
...
class TestReplicaInForwardZone(IntegrationTest):
    ...
    def test_replica_install_in_forward_zone(self):
        ...
        r_new_hostname = "{}.{}".format(r_shortname, self.forwardzone)

        # Update /etc/hosts on the master with an entry for the replica
        # otherwise replica conncheck would fail
        update_etc_hosts(master, replica.ip, replica.hostname, r_new_hostname)
        ...
        update_etc_hosts(replica, replica.ip, replica.hostname, r_new_hostname)

        try:
            # install client with a hostname in the forward zone
            tasks.install_client(
                self.master, replica, extra_args=["--hostname", r_new_hostname]
            )
...
class TestHiddenReplicaPromotion(IntegrationTest):
    ...
    def _check_dnsrecords(self, hosts_expected, hosts_unexpected=()):
        ...
        rset = [(rname, "SRV") for rname, _port in IPA_DEFAULT_MASTER_SRV_REC]
        rset.append((DNSName(IPA_CA_RECORD), "A"))

        for rname, rtype in rset:
            name_abs = rname.derelativize(domain)
            query = resolve_records_from_server(name_abs, rtype, self.master.ip)
            if rtype == "SRV":
    ...
    def _check_server_role(self, host, status, kra=True, dns=True):
        ...
        for role in roles:
            result = self.replicas[0].run_command(
                ["ipa", "server-role-find", "--server", host.hostname, "--role", role]
            )
    ...
    def test_hide_master_fails(self):
        ...
        result = self.master.run_command(
            ["ipa", "server-state", self.master.hostname, "--state=enabled"],
            raiseonerr=False,
        )
        assert result.returncode == 1
        ...
        result = self.master.run_command(
            ["ipa", "server-state", self.master.hostname, "--state=hidden"],
            raiseonerr=False,
        )
        assert result.returncode == 1
        keys = [
    ...
    def test_hidden_replica_promote(self):
        self.replicas[0].run_command(
            ["ipa", "server-state", self.replicas[0].hostname, "--state=enabled"]
        )
    ...
    def test_replica_from_hidden(self):
        ...
        self._check_server_role(self.replicas[0], "hidden")
        tasks.install_replica(
            master=self.replicas[0], replica=self.replicas[1], setup_dns=True
        )
        self._check_server_role(self.replicas[0], "hidden")
        self._check_server_role(self.replicas[1], "enabled", kra=False, dns=False)
        self._check_dnsrecords([self.master, self.replicas[1]], [self.replicas[0]])
        # hide the new replica
        self.replicas[0].run_command(
            ["ipa", "server-state", self.replicas[1].hostname, "--state=hidden"]
        )
        # and establish replication agreements from master
        tasks.connect_replica(
            master=self.master, replica=self.replicas[1],
        )
        tasks.connect_replica(
            master=self.master, replica=self.replicas[1], database=CA_SUFFIX_NAME,
        )
        ...
        tasks.uninstall_replica(
            master=self.replicas[0], replica=self.replicas[1],
        )
    ...
    def test_hidden_replica_automatic_crl(self):
        ...
        # check CRL status
        result = self.replicas[0].run_command(["ipa-crlgen-manage", "status"])
...
class TestHiddenReplicaKRA(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        tasks.install_replica(
            cls.replicas[0], cls.replicas[1], setup_dns=True, setup_kra=False
        )
