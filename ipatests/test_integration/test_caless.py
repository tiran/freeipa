...
def ipa_certs_cleanup(host):
    host.run_command(
        ["certutil", "-d", paths.NSS_DB_DIR, "-D", "-n", "External CA cert"],
        raiseonerr=False,
    )
    # A workaround for https://fedorahosted.org/freeipa/ticket/4639
    result = host.run_command(
        ["certutil", "-L", "-d", paths.HTTPD_ALIAS_DIR], raiseonerr=False
    )
    for rawcert in result.stdout_text.split("\n")[4:-1]:
        cert = rawcert.split("    ")[0]
        host.run_command(
            ["certutil", "-D", "-d", paths.HTTPD_ALIAS_DIR, "-n", cert],
            raiseonerr=False,
        )
...
def replica_install_teardown(func):
    def wrapped(*args):
        try:
            ...
            tasks.clean_replication_agreement(
                master, replica, cleanup=True, raiseonerr=False
            )
            master.run_command(["ipa", "host-del", replica.hostname], raiseonerr=False)
...
class CALessBase(IntegrationTest):
    ...
    def install_server(
        cls,
        host=None,
        http_pkcs12="server.p12",
        dirsrv_pkcs12="server.p12",
        http_pkcs12_exists=True,
        dirsrv_pkcs12_exists=True,
        http_pin=_DEFAULT,
        dirsrv_pin=_DEFAULT,
        pkinit_pin=None,
        root_ca_file="root.pem",
        pkinit_pkcs12_exists=False,
        pkinit_pkcs12="server-kdc.p12",
        unattended=True,
        stdin_text=None,
        extra_args=None,
    ):
        ...
        std_args = [
            "--http-cert-file",
            destname(http_pkcs12),
            "--dirsrv-cert-file",
            destname(dirsrv_pkcs12),
            "--ca-cert-file",
            destname(root_ca_file),
            "--ip-address",
            host.ip,
        ]
        if extra_args:
        ...
        if pkinit_pkcs12_exists:
            ...
            extra_args.extend(["--pkinit-cert-file", destname(pkinit_pkcs12)])
        else:
    ...
    def copy_cert(cls, host, filename):
        host.transport.put_file(
            os.path.join(cls.cert_dir, filename),
    ...
    def prepare_replica(
        ...
        replica=None,
        master=None,
        http_pkcs12="replica.p12",
        dirsrv_pkcs12="replica.p12",
        http_pkcs12_exists=True,
        dirsrv_pkcs12_exists=True,
        http_pin=_DEFAULT,
        dirsrv_pin=_DEFAULT,
    ...
    def verify_installation(self):
        ...
        logger.debug(
            "Expected /etc/ipa/ca.crt contents:\n%s", expected_cacrt.decode("utf-8")
        )
        expected_cacrt = x509.load_unknown_x509_certificate(expected_cacrt)
        logger.debug(
            "Expected CA cert:\n%r", expected_cacrt.public_bytes(x509.Encoding.PEM)
        )
        for host in [self.master] + self.replicas:
            ...
            entry = ldap.get_entry(
                DN(("cn", "CACert"), ("cn", "ipa"), ("cn", "etc"), host.domain.basedn)
            )
            cert_from_ldap = entry.single_value["cACertificate"]
            logger.debug(
                "CA cert from LDAP on %s:\n%r",
                host,
                cert_from_ldap.public_bytes(x509.Encoding.PEM),
            )
        ...
        for host in self.get_all_hosts():
            ...
            logger.debug(
                "%s:/etc/ipa/ca.crt contents:\n%s", host, remote_cacrt.decode("utf-8")
            )
            cacrt = x509.load_unknown_x509_certificate(remote_cacrt)
            logger.debug(
                "%s: Decoded /etc/ipa/ca.crt:\n%r",
                host,
...
class TestServerInstall(CALessBase):
    ...
    def test_nonexistent_http_pkcs12_file(self):
        ...
        result = self.install_server(
            http_pkcs12="does_not_exist", http_pkcs12_exists=False
        )
        assert_error(
            result, "Failed to open %s/does_not_exist" % self.master.config.test_dir
        )
    ...
    def test_nonexistent_ds_pkcs12_file(self):
        ...
        result = self.install_server(
            dirsrv_pkcs12="does_not_exist", dirsrv_pkcs12_exists=False
        )
        assert_error(
            result, "Failed to open %s/does_not_exist" % self.master.config.test_dir
        )
    ...
    def test_incorect_http_pin(self):
        ...
        assert_error(
            result,
            "incorrect password for pkcs#12 file %s"
    ...
    def test_invalid_http_cn(self):
        ...
        result = self.install_server(http_pkcs12="http.p12", dirsrv_pkcs12="dirsrv.p12")
        assert_error(
            result,
    ...
    def test_http_bad_usage(self):
        ...
        assert_error(
            ...
            "valid: {err}".format(dir=self.master.config.test_dir, err=BAD_USAGE_MSG),
        )
    ...
    def test_http_intermediate_ca(self):
        ...
        assert_error(
            result,
            "Apache Server SSL certificate and"
    ...
    def test_interactive_missing_http_pkcs_password(self):
        ...
        result = self.install_server(
            http_pin=None, unattended=False, stdin_text=stdin_text
        )
        assert result.returncode == 0
        self.verify_installation()
        assert (
            "Enter Apache Server private key unlock password" in result.stdout_text
        ), result.stdout_text
    ...
    def test_interactive_missing_ds_pkcs_password(self):
        ...
        result = self.install_server(
            dirsrv_pin=None, unattended=False, stdin_text=stdin_text
        )
        assert result.returncode == 0
        self.verify_installation()
        assert (
            "Enter Directory Server private key unlock password" in result.stdout_text
        ), result.stdout_text
    ...
    def test_no_http_password(self):
        ...
        result = self.install_server(
            http_pkcs12="http.p12", dirsrv_pkcs12="dirsrv.p12", http_pin=""
        )
        assert result.returncode == 0
        self.verify_installation()
    ...
    def test_no_ds_password(self):
        ...
        result = self.install_server(
            http_pkcs12="http.p12", dirsrv_pkcs12="dirsrv.p12", dirsrv_pin=""
        )
        assert result.returncode == 0
        self.verify_installation()
...
class TestReplicaInstall(CALessBase):
    ...
    def test_no_certs(self):
        ...
        result = self.prepare_replica(
            http_pkcs12_exists=False, dirsrv_pkcs12_exists=False
        )
        assert_error(
            result,
    ...
    def test_nonexistent_http_pkcs12_file(self):
        ...
        result = self.prepare_replica(
            dirsrv_pkcs12="does_not_exist", http_pkcs12="http.p12"
        )
        assert_error(
            result, "Failed to open %s/does_not_exist" % self.master.config.test_dir
        )
    ...
    def test_nonexistent_ds_pkcs12_file(self):
        ...
        result = self.prepare_replica(
            http_pkcs12="does_not_exist", dirsrv_pkcs12="dirsrv.p12"
        )
        assert_error(
            result, "Failed to open %s/does_not_exist" % self.master.config.test_dir
        )
    ...
    def test_incorect_http_pin(self):
        ...
        assert_error(
            result,
            "incorrect password for pkcs#12 file %s"
            % os.path.join(self.replicas[0].config.test_dir, "replica.p12"),
        )
    ...
    def test_http_unknown_ca(self):
        ...
        result = self.prepare_replica(
            http_pkcs12="http.p12", dirsrv_pkcs12="dirsrv.p12"
        )
        assert_error(
            result,
            "Apache Server SSL certificate and"
    ...
    def test_interactive_missing_http_pkcs_password(self):
        ...
        stdin_text = get_replica_prepare_stdin(cert_passwords=[self.cert_password])
...
class TestIPACommands(CALessBase):
    ...
    def host(self):
        ...
        try:
            ...
            self.master.run_command(
                ["ipa", "host-del", self.test_hostname], raiseonerr=False
            )
    ...
    def test_service_mod_doesnt_revoke(self):
        ...
        with self.service():
            self.master.run_command(
                ["ipa", "service-mod", self.test_service, "--certificate="]
            )
    ...
    def test_service_disable_doesnt_revoke(self):
        ...
        with self.service():
            result = self.master.run_command(
                ["ipa", "service-disable", self.test_service], raiseonerr=False
            )
            assert result.returncode == 0, (
...
class TestCertInstall(CALessBase):
    ...
    def certinstall(
        ...
        if not args:
            ...
            if pin is not None:
                args += ["--pin", pin]
        return self.master.run_command(args, raiseonerr=False, stdin_text=stdin_text)

    def test_nonexistent_http_pkcs12_file(self):
        "Install new HTTP certificate from non-existent PKCS#12 file"

        result = self.certinstall(
            "w", filename="does_not_exist", pin="none", cert_exists=False
        )
    ...
    def test_nonexistent_ds_pkcs12_file(self):
        ...
        result = self.certinstall(
            "d", filename="does_not_exist", pin="none", cert_exists=False
        )
        assert_error(result, "Failed to open does_not_exist")
    ...
    def _test_expired_service_cert(self, w_or_d):
        ...
        pattern = re.compile(
            r"The server certificate in server\.p12 is not valid: " ".*has expired"
        )
        assert_error(result, pattern)
    ...
    def test_http_bad_usage(self):
        ...
        assert_error(
            result,
            "The server certificate in server.p12 is not valid: {err}".format(
                err=BAD_USAGE_MSG
            ),
        )
    ...
    def test_self_signed(self):
        ...
        assert_error(result, "The full certificate chain is not present in server.p12")
    ...
    def test_interactive_missing_http_pkcs_password(self):
        ...
        result = self.certinstall(
            "w", "ca1/server", pin=None, stdin_text=self.cert_password + "\n"
        )
        assert result.returncode == 0
    ...
    def test_interactive_missing_ds_pkcs_password(self):
        ...
        result = self.certinstall(
            "d", "ca1/server", pin=None, stdin_text=self.cert_password + "\n"
        )
        assert result.returncode == 0
    ...
    def test_ds_old_options(self):
        ...
        result = self.certinstall("d", "ca1/server", args=args, stdin_text=stdin_text)
    ...
    def test_anon_pkinit_with_external_CA(self):
        ...
        result = self.master.run_command(
            ["ipa-cacert-manage", "install", os.path.join(test_dir, self.ca2_crt)]
        )
        assert result.returncode == 0
        result = self.master.run_command(["ipa-certupdate"])
        assert result.returncode == 0
        result = self.certinstall("k", "ca2/server-kdc", filename=self.ca2_kdc_crt)
        assert result.returncode == 0
...
class TestPKINIT(CALessBase):
    ...
    def install(cls, mh):
        ...
        cls.prepare_cacert("ca1")
        result = cls.install_server(pkinit_pkcs12_exists=True, pkinit_pin=_DEFAULT)
        assert result.returncode == 0
    ...
    def test_server_replica_install_pkinit(self):
        ...
        result = self.prepare_replica(pkinit_pkcs12_exists=True, pkinit_pin=_DEFAULT)
        assert result.returncode == 0
        self.verify_installation()
...
class TestServerReplicaCALessToCAFull(CALessBase):
    ...
    def test_server_ipa_ca_install(self):
        ...
        assert (
            "Subject DN: CN=Certificate Authority,O={}".format(self.master.domain.realm)
            in ca_show.stdout_text
        )
