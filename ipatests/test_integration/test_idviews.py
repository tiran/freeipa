...
class TestCertsInIDOverrides(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        master.run_command(["dnf", "install", "-y", "sssd-dbus"], raiseonerr=False)
        ...
        master.run_command("sed -i '/^services/ s/$/, ifp/' %s" % paths.SSSD_CONF)
        master.run_command(
        ...
        tasks.establish_trust_with_ad(
            cls.master, cls.ad_domain, extra_args=["--range-type", "ipa-ad-trust"]
        )
        ...
        tasks.run_certutil(
            ...
            [
                "-S",
                "-s",
                "cn=%s,dc=ad,dc=test" % cls.adcert1,
                "-n",
                cls.adcert1,
                "-x",
                "-t",
        ...
        tasks.run_certutil(
            ...
            [
                "-S",
                "-s",
                "cn=%s,dc=ad,dc=test" % cls.adcert2,
                "-n",
                cls.adcert2,
                "-x",
                "-t",
        ...
        res = tasks.run_certutil(master, ["-L", "-n", cls.adcert1, "-a"], cls.reqdir)
        master.put_file_contents(
            os.path.join(master.config.test_dir, cls.adcert1_file), res.stdout_text
        )
        res = tasks.run_certutil(master, ["-L", "-n", cls.adcert2, "-a"], cls.reqdir)
        master.put_file_contents(
            os.path.join(master.config.test_dir, cls.adcert2_file), res.stdout_text
        )
    ...
    def test_certs_in_idoverrides_ad_users(self):
        ...
        """
        master = self.master
        master.run_command(["ipa", "idoverrideuser-add", self.adview, self.aduser])
        master.run_command(
            [
                "ipa",
        ...
        result = master.run_command(
            ["ipa", "idoverrideuser-show", self.adview, self.aduser]
        )
        assert (
            self.cert1_base64 in result.stdout_text
...
EOF
    ...
    def install(cls, mh):
        ...
        master.run_command("ipa service-add %s --force" % cls.replica_service_name)
        master.run_command(
            "ipa service-add-host %s --hosts %s" % (cls.service_name, cls.replica)
        )
        master.run_command('ipa caacl-add %s --desc "test"' % cls.caacl)
        master.run_command(
            "ipa caacl-add-host %s --hosts %s" % (cls.caacl, cls.replica)
        )
        master.run_command(
            "ipa caacl-add-service %s --services" " svc/`hostname`" % cls.caacl
        )
        master.run_command(
            "ipa-getkeytab -p host/%s@%s -k %s"
    ...
    def test_rules_with_service_principals(self):
        result = self.master.run_command(
            [
                ...
                self.csr,
                "--principal",
                "svc/%s@%s" % (self.replica, self.master.domain.realm),
                "--profile-id",
                self.service_certprofile,
            ],
            raiseonerr=False,
        )
        assert result.returncode == 0, "Failed to add a cert to custom certprofile"
...
class TestIDViews(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        tasks.group_add(master, cls.group1, extra_args=["--gid", str(cls.group1_gid)])
        ...
        master.run_command(
            [
                ...
                cls.user1,
                "--uid",
                str(cls.user1_uid_override),
                "--gid",
                str(cls.user1_gid_override),
                "--homedir",
                "/special-home/{}".format(cls.user1),
                "--shell",
                "/bin/special",
            ]
        )
    ...
    def test_useroverride_original_uid(self):
        ...
        result = self.clients[0].run_command(["getent", "passwd", str(self.user1_uid)])
        expected = "{}:*:{}:{}".format(
            self.user1, self.user1_uid_override, self.user1_gid_override
        )
        assert expected in result.stdout_text
    ...
    def test_anchor_groupname(self):
        result = self.master.run_command(
            ["ipa", "idoverridegroup-find", self.idview, "--anchor", self.group1]
        )
