...
class BaseTestTrust(IntegrationTest):
    ...
    def install(cls, mh):
        if not cls.master.transport.file_exists("/usr/bin/rpcclient"):
            raise pytest.skip(
                "Package samba-client not available "
        ...
        cls.srv_gc_record_name = "_ldap._tcp.Default-First-Site-Name._sites.gc._msdcs"
    ...
    def check_sid_generation(cls):
        ...
        sid_regex = "S-1-5-21-%(idauth)s-%(idauth)s-%(idauth)s" % dict(
            idauth=_sid_identifier_authority
        )
        stdout_re = re.escape("  ipaNTSecurityIdentifier: ") + sid_regex

        tasks.run_repeatedly(
            cls.master, command, test=lambda x: re.search(stdout_re, x)
        )
    ...
    def check_range_properties(self, realm, expected_type, expected_size):
        ...
        result = self.master.run_command(
            ["ipa", "idrange-show", range_name, "--all", "--raw"]
        )
...
class TestTrust(BaseTestTrust):
    ...
    def test_establish_nonposix_trust(self):
        ...
        tasks.establish_trust_with_ad(
            self.master, self.ad_domain, extra_args=["--range-type", "ipa-ad-trust"]
        )

    def test_trustdomains_found_in_nonposix_trust(self):
        self.check_trustdomains(self.ad_domain, [self.ad_domain, self.ad_subdomain])
    ...
    def test_user_gid_uid_resolution_in_nonposix_trust(self):
        ...
        assert re.search(testuser_regex, result.stdout_text), result.stdout_text
    ...
    def test_ipa_commands_run_as_aduser(self):
        ...
        tasks.kinit_as_user(self.master, ad_admin, self.master.config.ad_admin_password)
        err_string = (
    ...
    def test_ipauser_authentication_with_nonposix_trust(self):
        ...
        self.master.run_command(
            ["ipa", "user-add", ipauser, "--first=Test", "--last=User", "--password"],
            stdin_text=original_passwd,
        )

        # change password for the user to be able to kinit
        tasks.ldappasswd_user_change(ipauser, original_passwd, new_passwd, self.master)
    ...
    def test_upn_in_nonposix_trust(self):
        ...
        result = self.master.run_command(
            ["ipa", "trust-show", self.ad_domain, "--all", "--raw"]
        )

        assert (
            "ipantadditionalsuffixes: {}".format(self.upn_suffix) in result.stdout_text
        )

    def test_upn_user_resolution_in_nonposix_trust(self):
        """Check that user with UPN can be resolved"""
        result = self.master.run_command(["getent", "passwd", self.upn_principal])

        # result will contain AD domain, not UPN
        upnuser_regex = r"^{}@{}:\*:(\d+):(\d+):{}:/home/{}/{}:{}$".format(
            self.upn_username,
            self.ad_domain,
            self.upn_name,
            self.ad_domain,
    ...
    def test_upn_user_authentication_in_nonposix_trust(self):
        ...
        self.master.run_command(
            ["kinit", "-C", "-E", self.upn_principal], stdin_text=self.upn_password
        )
    ...
    def test_range_properties_in_posix_trust(self):
        """Check the properties of the created range"""
        self.check_range_properties(self.ad_domain, "ipa-ad-trust-posix", 200000)
    ...
    def test_user_uid_gid_resolution_in_posix_trust(self):
        ...
        testuser_stdout = (
            "testuser@%s:*:10042:10047:"
            "Test User:/home/%s/testuser:%s"
            % (self.ad_domain, self.ad_domain, self.default_shell,)
        )
    ...
    def test_user_without_posix_attributes_not_visible(self):
        ...
        result = self.master.run_command(
            ["getent", "passwd", nonposixuser], raiseonerr=False
        )
    ...
    def test_override_homedir(self):
        ...
        try:
            ...
            with tasks.remote_sssd_config(self.master) as sssd_conf:
                sssd_conf.edit_domain(self.master.domain, "subdomain_homedir", "%o")
    ...
    def test_extdom_plugin(self):
        ...
        """
        extdom_dn = DN(("cn", "ipa_extdom_extop"), ("cn", "plugins"), ("cn", "config"))

        client = self.clients[0]
        tasks.backup_file(self.master, paths.SSSD_CONF)
        log_file = "{0}/sssd_{1}.log".format(paths.VAR_LOG_SSSD_DIR, client.domain.name)
    ...
    def test_trustdomains_found_in_external_subdomain_trust(self):
        self.check_trustdomains(self.ad_subdomain, [self.ad_subdomain])
    ...
    def test_establish_nonexternal_subdomain_trust(self):
        ...
        try:
            ...
            assert result != 0
            assert (
                "Domain '{0}' is not a root domain".format(self.ad_subdomain)
    ...
    def test_trustdomains_found_in_external_treedomain_trust(self):
        self.check_trustdomains(self.ad_treedomain, [self.ad_treedomain])
    ...
    def test_establish_nonexternal_treedomain_trust(self):
        ...
        try:
            ...
            assert result != 0
            assert (
                "Domain '{0}' is not a root domain".format(self.ad_treedomain)
                in result.stderr_text
            )
        finally:
            tasks.unconfigure_dns_for_trust(self.master, self.ad, self.tree_ad)
    ...
    def test_trustdomains_found_in_forest_trust_with_shared_secret(self):
        result = self.master.run_command(
            ["ipa", "trust-fetch-domains", self.ad.domain.name], raiseonerr=False
        )
        assert result.returncode == 1
        self.check_trustdomains(self.ad_domain, [self.ad_domain, self.ad_subdomain])
    ...
    def test_remove_forest_trust_with_shared_secret(self):
        ps_cmd = (
            "[System.DirectoryServices.ActiveDirectory.Forest]"
            "::getCurrentForest()"
            '.DeleteLocalSideOfTrustRelationship("{}")'.format(self.master.domain.name)
        )
        self.ad.run_command(["powershell", "-c", ps_cmd])
    ...
    def test_server_option_with_unreachable_ad(self):
        ...
        named_conf = self.master.get_file_contents(paths.NAMED_CONF, encoding="utf-8")
        ...
        try:
            ...
            assert result.returncode == 1
            assert (
                'CIFS server communication error: code "3221225653", '
                'message "{Device Timeout}' in result.stderr_text
            )

            # Check that trust is successfully established with --server option
            tasks.establish_trust_with_ad(
                self.master, self.ad_domain, extra_args=["--server", self.ad.hostname]
            )
            ...
            assert result.returncode == 1
            assert "Fetching domains from trusted forest failed" in result.stderr_text
            ...
            assert result.returncode == 1
            assert "List of trust domains successfully refreshed" in result.stdout_text
        finally:
            self.remove_trust(self.ad)
            tasks.restore_files(self.master)
