...
class TestKerberosFlags(IntegrationTest):
    ...
    def test_try_to_set_flag_using_unexpected_values(self):
        ...
        for v in invalid_values:
            self.mod_object_cli("service", http_service, trusted=v, expect_fail=True)
    ...
    def mod_object_cli(self, object_type, object_id, trusted, expect_fail=False):
    ...
    def mod_service_kadmin_local(self, service, trusted):
        ...
        stdin_text = "\n".join(
            ["modify_principal %sok_as_delegate %s" % (sign, service), "q", ""]
        )
    ...
    def getkeytab(self, service, keytab):
        result = self.master.run_command(
            ["ipa-getkeytab", "-s", self.master.hostname, "-p", service, "-k", keytab]
        )
