...
def p11(request, token_path):
    ...
    def fin():
        try:
            ...
            subprocess.call(
                [SOFTHSM2_UTIL, "--delete-token", "--label", "test"], cwd=token_path
            )
...
class test_p11helper:
    def test_generate_master_key(self, p11):
        assert p11.generate_master_key(
            master_key_label,
            master_key_id,
            key_length=16,
            cka_wrap=True,
    ...
    def test_search_for_master_key(self, p11):
        master_key = p11.find_keys(
            _ipap11helper.KEY_CLASS_SECRET_KEY, label=master_key_label, id=master_key_id
        )
        assert len(master_key) == 1, "The master key should exist."

    def test_generate_replica_key_pair(self, p11):
        assert p11.generate_replica_key_pair(
            replica1_key_label, replica1_key_id, pub_cka_wrap=True, priv_cka_unwrap=True
        )

    def test_find_key(self, p11):
        rep1_pub = p11.find_keys(
            _ipap11helper.KEY_CLASS_PUBLIC_KEY, label=replica1_key_label, cka_wrap=True
        )
        assert (
            len(rep1_pub) == 1
        ), "replica key pair has to contain " "1 pub key instead of %s" % len(rep1_pub)

        rep1_priv = p11.find_keys(
            _ipap11helper.KEY_CLASS_PRIVATE_KEY,
            label=replica1_key_label,
            cka_unwrap=True,
        )
        assert len(rep1_priv) == 1, (
            "replica key pair has to contain 1 "
            "private key instead of %s" % len(rep1_priv)
        )
    ...
    def test_find_key_on_nonexistent_key_pair(self, p11):
        test_list = p11.find_keys(
            _ipap11helper.KEY_CLASS_PUBLIC_KEY, label=replica_non_existent_label
        )
        assert len(test_list) == 0, (
            "list should be empty because label "
            "'%s' should not exist" % replica_non_existent_label
        )
    ...
    def test_export_import_of_public_key(self, p11, token_path):
        ...
        rep1_pub_import = p11.import_public_key(
            replica1_import_label, replica1_import_id, pub, cka_wrap=True
        )
        ...
        rep1_modulus_import = p11.get_attribute(
            rep1_pub_import, _ipap11helper.CKA_MODULUS
        )
    ...
    def test_wrap_unwrap_key_by_master_key_with_AES(self, p11, token_path):
        ...
        rep2_priv = p11.find_keys(
            _ipap11helper.KEY_CLASS_PRIVATE_KEY,
            label=replica2_key_label,
            cka_unwrap=True,
        )[0]
    ...
    def test_wrap_unwrap_key_by_master_key_with_RSA_PKCS(self, p11):
        ...
        rep2_pub = p11.find_keys(
            _ipap11helper.KEY_CLASS_PUBLIC_KEY, label=replica2_key_label, cka_wrap=True
        )[0]
        rep2_priv = p11.find_keys(
            _ipap11helper.KEY_CLASS_PRIVATE_KEY,
            label=replica2_key_label,
        ...
        wrapped = p11.export_wrapped_key(
            master_key, rep2_pub, _ipap11helper.MECH_RSA_PKCS
        )
        assert wrapped

        log.debug(
            "wrapped key MECH_RSA_PKCS (secret master wrapped by pub " "key): %s",
            hexlify(wrapped),
        )
        assert p11.import_wrapped_secret_key(
            u"test_import_wrapped",
            "2",
            wrapped,
            rep2_priv,
    ...
    def test_wrap_unwrap_by_master_key_with_RSA_PKCS_OAEP(self, p11):
        ...
        wrapped = p11.export_wrapped_key(
            master_key, rep2_pub, _ipap11helper.MECH_RSA_PKCS_OAEP
        )
        assert wrapped

        log.debug(
            "wrapped key MECH_RSA_PKCS_OAEP (secret master wrapped by " "pub key): %s",
            hexlify(wrapped),
        )
    ...
    def test_set_attribute_on_object(self, p11):
        ...
        assert (
            p11.get_attribute(rep1_pub, _ipap11helper.CKA_LABEL) == test_label
    ...
    def test_do_not_generate_identical_master_keys(self, p11):
        with pytest.raises(_ipap11helper.DuplicationError):
            p11.generate_master_key(master_key_label, master_key_id, key_length=16)

        master_key = p11.find_keys(
            _ipap11helper.KEY_CLASS_SECRET_KEY, label=master_key_label
        )
        assert len(master_key) == 1, (
            "There shouldn't be multiple keys " "with the same label."
        )
    ...
    def test_delete_key(self, p11):
        ...
        rep1_pub = p11.find_keys(
            _ipap11helper.KEY_CLASS_PUBLIC_KEY, label=replica1_new_label, cka_wrap=True
        )[0]
        rep2_priv = p11.find_keys(
            _ipap11helper.KEY_CLASS_PRIVATE_KEY,
            label=replica2_key_label,
        ...
        assert len(rep1_pub) == 0, (
            "The public key of replica1 pair should " "be deleted."
        )
        rep2_priv = p11.find_keys(
            _ipap11helper.KEY_CLASS_PRIVATE_KEY,
            label=replica2_key_label,
        ...
        assert len(rep2_priv) == 0, (
            "The private key of replica2 pair should" " be deleted."
        )
