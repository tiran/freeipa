...
master_data = {
    "ca-dns-dnssec-keymaster-pkinit-server": {
        "services": {
            "CA": {"enabled": True,},
            "DNS": {"enabled": True,},
            "DNSKeySync": {"enabled": True,},
            "DNSSEC": {"enabled": True, "config": ["DNSSecKeyMaster"]},
            "KDC": {"enabled": True, "config": ["pkinitEnabled"]},
        },
        "expected_roles": {"enabled": ["IPA master", "CA server", "DNS server"]},
        "expected_attributes": {
            "DNS server": "dnssec_key_master_server",
            "IPA master": "pkinit_server_server",
        },
    },
    "ca-kra-renewal-master-pkinit-server": {
        "services": {
            "CA": {"enabled": True, "config": ["caRenewalMaster"]},
            "KRA": {"enabled": True,},
            "KDC": {"enabled": True, "config": ["pkinitEnabled"]},
        },
        "expected_roles": {"enabled": ["IPA master", "CA server", "KRA server"]},
        "expected_attributes": {
            "CA server": "ca_renewal_master_server",
            "IPA master": "pkinit_server_server",
        },
    },
    "dns-trust-agent": {
        "services": {"DNS": {"enabled": True,}, "DNSKeySync": {"enabled": True,}},
        "attributes": {_adtrust_agents: {"member": ["host"]}},
        "expected_roles": {"enabled": ["IPA master", "DNS server", "AD trust agent"]},
    },
    "trust-agent": {
        "attributes": {_adtrust_agents: {"member": ["host"]}},
        "expected_roles": {"enabled": ["IPA master", "AD trust agent"]},
    },
    "trust-controller-dns": {
        "services": {
            "ADTRUST": {"enabled": True,},
            "DNS": {"enabled": True,},
            "DNSKeySync": {"enabled": True,},
        },
        "attributes": {_adtrust_agents: {"member": ["host", "cifs"]}},
        "expected_roles": {
            "enabled": [
                "IPA master",
                "AD trust agent",
    ...
    "trust-controller-ca": {
        "services": {"ADTRUST": {"enabled": True,}, "CA": {"enabled": True,},},
        "attributes": {_adtrust_agents: {"member": ["host", "cifs"]}},
        "expected_roles": {
            "enabled": [
                "IPA master",
    ...
    "configured-ca": {
        "services": {"CA": {"enabled": False,},},
        "expected_roles": {"enabled": ["IPA master"], "configured": ["CA server"]},
    },
    "configured-dns": {
        "services": {"DNS": {"enabled": False,}, "DNSKeySync": {"enabled": False,}},
        "expected_roles": {"enabled": ["IPA master"], "configured": ["DNS server"]},
    },
    "mixed-state-dns": {
        "services": {"DNS": {"enabled": False}, "DNSKeySync": {"enabled": True}},
        "expected_roles": {"enabled": ["IPA master"], "configured": ["DNS server"]},
    },
}
...
class MockMasterTopology:
    ...
    def __init__(self, api_instance, domain_data):
        ...
        self.masters_base = DN(self.api.env.container_masters, self.api.env.basedn)
    ...
    def iter_domain_data(self):
        MasterData = namedtuple("MasterData", ["dn", "fqdn", "services", "attrs"])
    ...
    def _add_service_entry(self, service, fqdn):
        return self.api.Command.service_add("/".join([service, fqdn]), force=True)
    ...
    def _remove_svc_master_entries(self, master_dn):
        try:
            entries = self.ldap.get_entries(master_dn, ldap.SCOPE_SUBTREE)
        except errors.NotFound:
            return
    ...
    def _remove_members(self, dn, fqdn, member_attrs):
        ...
        for a in member_attrs:
            ...
            if a == "host":
                ...
                try:
                    value.remove(
                        str(self.api.Object.service.get_dn("/".join([a, fqdn])))
                    )
                except KeyError:
                    pass
    ...
    def _remove_test_host_attrs(self):
        ...
        for attr_name in ("caRenewalMaster", "dnssecKeyMaster", "pkinitEnabled"):
            try:
    ...
    def setup_data(self):
        for master_data in self.iter_domain_data():
            ...
            self.ldap.add_entry(
                _make_master_entry(
                    self.ldap, master_data.dn, ca="CA" in master_data.services
                )
            )
            ...
            for entry_dn, attrs in master_data.attrs.items():
                if "member" in attrs:
                    self._add_members(entry_dn, master_data.fqdn, attrs["member"])
            ...
            for entry_dn, attrs in master_data.attrs.items():
                if "member" in attrs:
                    self._remove_members(
                        entry_dn, master_data.fqdn, attrs["member"],
                    )
...
def mock_api(request):
    ...
    test_api.bootstrap(in_server=True, ldap_uri=api.env.ldap_uri, confdir=paths.ETC_IPA)
    test_api.finalize()
...
@pytest.fixture(
    params=list(enabled_role_iter(master_data)),
    ids=[
        "role: {}, master: {}, enabled".format(role, m)
...
@pytest.fixture(
    params=list(provided_role_iter(master_data)),
    ids=[
        "{}: {}".format(m, ", ".join(roles))
        for m, roles in provided_role_iter(master_data)
    ],
)
...
@pytest.fixture(
    params=list(configured_role_iter(master_data)),
    ids=[
        "role: {}, master: {}, configured".format(role, m)
...
@pytest.fixture(
    params=list(role_provider_iter(master_data)),
    ids=[
        "{} providers".format(role_name)
        for role_name, _m in role_provider_iter(master_data)
    ],
)
...
@pytest.fixture(
    params=list(attribute_masters_iter(master_data)),
    ids=[
        "{} of {}: {}".format(attr, role, m)
        for m, role, attr in attribute_masters_iter(master_data)
    ],
)
...
@pytest.fixture(
    params=list(dns_servers_iter(master_data)), ids=list(dns_servers_iter(master_data))
)
...
class TestServerRoleStatusRetrieval:
    ...
    def find_role(self, role_name, mock_api, mock_masters, master=None):
        ...
        result = mock_api.Backend.serverroles.server_role_search(
            server_server=hostname, role_servrole=role_name
        )

        return [
            r
            for r in result
            if r[u"server_server"] not in mock_masters.existing_masters
        ]
    ...
    def get_masters_with_enabled_role(self, role_name, mock_api, mock_masters):
        ...
        return sorted(
            ...
            and r[u"server_server"] not in mock_masters.existing_masters
        )

    def test_listing_of_enabled_role(self, mock_api, mock_masters, enabled_role):
    ...
    def test_listing_of_configured_role(self, mock_api, mock_masters, configured_role):
        master, role_name = configured_role
        result = self.retrieve_role(master, role_name, mock_api, mock_masters)

        assert result[0][u"status"] == u"configured"

    def test_role_providers(self, mock_api, mock_masters, role_providers):
    ...
    def test_provided_roles_on_master(self, mock_api, mock_masters, provided_roles):
        master, expected_roles = provided_roles
        expected_roles.sort()
        actual_roles = self.get_enabled_roles_on_master(master, mock_api, mock_masters)
    ...
    def test_unknown_role_status_raises_notfound(self, mock_api, mock_masters):
        ...
        with pytest.raises(errors.NotFound):
            mock_api.Backend.serverroles.server_role_retrieve(fqdn, unknown_role)

    def test_no_servrole_queries_all_roles_on_server(self, mock_api, mock_masters):
        master_name = "ca-dns-dnssec-keymaster-pkinit-server"
        enabled_roles = master_data[master_name]["expected_roles"]["enabled"]
        result = self.find_role(None, mock_api, mock_masters, master=master_name)
    ...
    def test_invalid_substring_search_returns_nothing(self, mock_api, mock_masters):
        invalid_substr = "fwfgbb"

        assert not self.find_role(
            invalid_substr,
            mock_api,
            mock_masters,
            "ca-dns-dnssec-keymaster-pkinit-server",
...
class TestServerAttributes:
    def config_retrieve(self, assoc_role_name, mock_api):
        return mock_api.Backend.serverroles.config_retrieve(assoc_role_name)

    def config_update(self, mock_api, **attrs_values):
        return mock_api.Backend.serverroles.config_update(**attrs_values)

    def test_attribute_master(self, mock_api, mock_masters, attribute_providers):
        master, assoc_role, attr_name = attribute_providers
        fqdn = mock_masters.get_fqdn(master)
        actual_attr_masters = self.config_retrieve(assoc_role, mock_api)[attr_name]
    ...
    def test_set_attribute_on_the_same_provider_raises_emptymodlist(
        ...
        existing_renewal_master = self.config_retrieve(role_name, mock_api)[attr_name]

        with pytest.raises(errors.EmptyModlist):
            self.config_update(mock_api, **{attr_name: existing_renewal_master})
    ...
    def test_set_ca_renewal_master_on_other_ca_and_back(self, mock_api, mock_masters):
        attr_name = "ca_renewal_master_server"
        role_name = "CA server"
        original_renewal_master = self.config_retrieve(role_name, mock_api)[attr_name]
        ...
        for host in (other_ca_server, original_renewal_master):
            ...
            assert self.config_retrieve(role_name, mock_api)[attr_name] == host
