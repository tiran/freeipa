...
def gpgkey(request, tempdir):
    ...
    def fin():
        ...
        subprocess.run(
            [paths.GPG_CONF, "--kill", "all"], check=True, env=env,
        )
    ...
    cmd = "PRESET_PASSPHRASE {} -1 {}".format(
        keygrip, binascii.hexlify(passphrase.encode("utf-8")).decode("utf-8")
    )
    subprocess.check_call(
        [paths.GPG_CONNECT_AGENT, cmd, "/bye"], env=env, stdout=devnull, stderr=devnull
    )
