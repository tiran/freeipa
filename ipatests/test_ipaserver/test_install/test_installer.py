...
class InstallerTestBase(six.with_metaclass(ABCMeta, object)):
    ...
    def installer_setup(self, request):
        ...
        assert getattr(
            cls.tested_cls, "option_parser", False
        ), "Unable to generate option parser for {}".format(cls.tested_cls.__name__)
    ...
    def _populate_opts_dict(cls):
        ...
        if not getattr(cls.tested_cls, "option_parser", False):
            raise RuntimeError(
                "You need to create the parser of the tested " "class first."
            )
    ...
    def all_option_permutations(self, *key_val):
        ...
        if len(key_val) == 0:
            ...
            val = key_val[0].value
            return [
                "{opt} {val}".format(opt=opt, val=val)
        ...
        for p in permutation:
            ...
            for i, kv in enumerate(key_val):
                opt_vals.append("{opt} {val}".format(opt=p[i], val=kv.value))
            ret.append(opt_vals)
        return ret
...
class TestReplicaInstaller(InstallerTestBase):
    ...
    PASSWORD = Keyval("auto_password", "c3ca2246bcf309d1b636581ce429da3522a8aec4")
    ...
    def test_password_option_DL1(self):
        ...
        for adm_password_opt in self.all_option_permutations(self.ADMIN_PASSWORD):
        ...
        for passwd_opt, principal_opt in self.all_option_permutations(
            self.PASSWORD, self.PRINCIPAL
        ):
        ...
        # again an OTP
        for adm_password_opt, passwd_opt, principal_opt in self.all_option_permutations(
            self.ADMIN_PASSWORD, self.PASSWORD, self.PRINCIPAL
        ):
            ic = self.get_installer_instance(
                self.combine_options(adm_password_opt, passwd_opt, principal_opt)
            )
            assert_equal(ic.password, self.PASSWORD.value)
            assert_equal(ic.principal, self.PRINCIPAL.value)
            assert_equal(ic.admin_password, self.ADMIN_PASSWORD.value)
