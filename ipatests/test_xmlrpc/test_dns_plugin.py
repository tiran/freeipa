...
zone1_permission_dn = DN(
    ("cn", zone1_permission), api.env.container_permission, api.env.basedn
)
zone1_txtrec_dn = DN(("idnsname", "_kerberos"), zone1_dn)

zone1_sub = u"sub.%s" % zone1_absolute
zone1_sub_dnsname = DNSName(zone1_sub)
zone1_sub_dn = DN(("idnsname", zone1_sub), api.env.container_dns, api.env.basedn)

zone1_sub_fw = u"fw.%s" % zone1_sub
zone1_sub_fw_dnsname = DNSName(zone1_sub_fw)
zone1_sub_fw_dn = DN(("idnsname", zone1_sub_fw), api.env.container_dns, api.env.basedn)

zone1_sub2_fw = u"fw.sub2.%s" % zone1_sub
zone1_sub2_fw_dnsname = DNSName(zone1_sub2_fw)
zone1_sub2_fw_dn = DN(
    ("idnsname", zone1_sub2_fw), api.env.container_dns, api.env.basedn
)
...
zone6b_absolute_dn = DN(
    ("idnsname", zone6b_absolute), api.env.container_dns, api.env.basedn
)
...
zone6_absolute_dn = DN(
    ("idnsname", zone6_absolute), api.env.container_dns, api.env.basedn
)
...
revzone3_classless1_dn = DN(
    ("idnsname", revzone3_classless1), api.env.container_dns, api.env.basedn
)
...
revzone3_classless2_dn = DN(
    ("idnsname", revzone3_classless2), api.env.container_dns, api.env.basedn
)

revzone3_classless2_permission = u"Manage DNS zone %s" % revzone3_classless2
revzone3_classless2_permission_dn = DN(
    ("cn", revzone3_classless2_permission), api.env.container_permission, api.env.basedn
)
...
tlsarec_err1 = (
    u"300 0 1 d2abde240d7cd3ee6b4b28c54df034b97983a1d16e8a410e4561cb106618e971"
)
tlsarec_err2 = (
    u"0 300 1 d2abde240d7cd3ee6b4b28c54df034b97983a1d16e8a410e4561cb106618e971"
)
tlsarec_err3 = (
    u"0 0 300 d2abde240d7cd3ee6b4b28c54df034b97983a1d16e8a410e4561cb106618e971"
)
...
idnzone1_dn = DN(
    ("idnsname", idnzone1_punycoded), api.env.container_dns, api.env.basedn
)
...
idnzone1_permission_dn = DN(
    ("cn", idnzone1_permission), api.env.container_permission, api.env.basedn
)
idnres1 = u"sk\xfa\u0161ka"
...
fwzone1_permission_dn = DN(
    ("cn", fwzone1_permission), api.env.container_permission, api.env.basedn
)
...
zone_findtest_master_dn = DN(
    ("idnsname", zone_findtest_master), api.env.container_dns, api.env.basedn
)
...
zone_findtest_forward_dn = DN(
    ("idnsname", zone_findtest_forward), api.env.container_dns, api.env.basedn
)
...
zone_root_permission_dn = DN(
    ("cn", zone_root_permission), api.env.container_permission, api.env.basedn
)
...
class test_dns(Declarative):
    ...
    def dns_setup(self, declarative_setup):
        ...
        if get_nameservers_error is not None:
            pytest.skip(
                "unable to get list of nameservers (%s)" % get_nameservers_error
            )

        try:
            api.Command["dnszone_add"](
                zone1, idnssoarname=zone1_rname,
            )
    ...
    cleanup_commands = [
        ...
        (
            "permission_del",
            [zone1_permission, idnzone1_permission, revzone3_classless2_permission],
            {"force": True},
        ),
    ]
    ...
    tests = [
        dict(
            ...
            expected=errors.NotFound(reason=u"%s: DNS zone not found" % zone1_absolute),
        ),
        dict(
            desc="Try to retrieve non-existent IDN zone %r" % idnzone1,
            command=("dnszone_show", [idnzone1], {}),
            expected=errors.NotFound(reason=u"%s: DNS zone not found" % idnzone1),
        ),
        dict(
        ...
        dict(
            ...
            command=("dnszone_add", [zone1], {"idnssoarname": zone1_rname,}),
            expected={
        ...
        dict(
            ...
            command=(
                "dnszone_add",
                [zone2],
                {"idnssoamname": zone2_ns, "idnssoarname": zone2_rname,},
            ),
            expected=errors.NotFound(
                reason="Nameserver '%s' does not have a corresponding A/AAAA record"
                % (zone2_ns)
            ),
        ),
        dict(
            ...
            command=(
                "dnszone_add",
                [zone2],
                {"idnssoamname": zone2_ns, "idnssoarname": zone2_rname, "force": True,},
            ),
            expected={
        ...
        dict(  # https://fedorahosted.org/freeipa/ticket/4268
            ...
            command=("dnszone_add", [zone5], {"idnssoarname": zone5_rname,}),
            expected={
                "value": zone5_dnsname,
                "summary": None,
                "result": {
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="nsec3param_rec", error=u"algorithm value: allowed interval 0-255"
            ),
        ),
        dict(
            desc="Try to add invalid NSEC3PARAM record to zone %s" % (zone1),
            ...
            expected=errors.ValidationError(
                name="nsec3param_rec", error=u"flags value: allowed interval 0-255"
            ),
        ),
        dict(
            desc="Try to add invalid NSEC3PARAM record to zone %s" % (zone1),
        ...
        dict(
            ...
            command=(
                "dnszone_add",
                [revzone1],
                {"idnssoamname": u"ns", "idnssoarname": zone1_rname,},
            ),
            expected=errors.ValidationError(
                name="name-server",
        ...
        dict(
            ...
            command=("dnszone_add", [revzone1], {"idnssoarname": zone1_rname,}),
            expected={
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="del_all", error=u"Zone record '@' cannot be deleted"
            ),
        ),
        dict(
        ...
        dict(
            ...
            expected={
                ...
                "result": [
                    ...
                    {"dn": name1_dn, "idnsname": [name1_dnsname], "arecord": [arec2],},
                ],
            },
        ),
        dict(
        ...
        dict(
            ...
            expected={
                "value": [name1_dnsname],
                "summary": None,
                "result": {"idnsname": [name1_dnsname], "arecord": [arec3],},
            },
        ),
        dict(
        ...
        dict(
            ...
            command=(
                "dnsrecord_mod",
                [zone1, u"_foo._tcp"],
                {"srv_part_priority": 1, "srvrecord": [u"0 100 1234 %s" % absnxname]},
            ),
            expected=errors.AttrValueNotFound(
                attr="SRV record", value=u"0 100 1234 %s" % absnxname
            ),
        ),
        dict(
            ...
            expected=errors.ValidationError(
                name="srv_priority", error=u"can be at most 65535"
            ),
        ),
        dict(
            ...
            command=(
                "dnsrecord_mod",
                [zone1, u"_foo._tcp"],
                {"srv_part_priority": 1, "srvrecord": [u"0 100 1234 %s" % zone1_ns]},
            ),
            expected={
                "value": DNSName(u"_foo._tcp"),
                "summary": None,
        ...
        dict(
            ...
            command=(
                "dnsrecord_add",
                [zone1, u"@"],
                {"locrecord": u"91 11 42.4 N 16 36 29.6 E 227.64"},
            ),
            expected=errors.ValidationError(name="lat_deg", error=u"can be at most 90"),
        ),
        dict(
            ...
            command=(
                "dnsrecord_add",
                [zone1, u"@"],
                {"locrecord": u"49 11 42.4 N 16 36 29.6 E 227.64m 10m 10.0m 0.1"},
            ),
            expected={
                "value": _dns_zone_record,
                "summary": None,
                "result": {
                    ...
                    "nsrecord": nameservers,
                    "locrecord": [
                        u"49 11 42.400 N 16 36 29.600 E 227.64 10.00 10.00 0.10"
                    ],
                },
            },
        ),
        ...
        dict(
            ...
            command=(
                "dnsrecord_add",
                [zone1, cname],
                {"cnamerecord": [u"1.%s" % absnxname, u"2.%s" % absnxname]},
            ),
            expected=errors.ValidationError(
                name="cnamerecord",
        ...
        dict(
            ...
            expected={
                "value": cname_dnsname,
                "summary": None,
                "result": {"idnsname": [cname_dnsname], "arecord": [arec1],},
            },
        ),
        dict(
            ...
            command=(
                "dnsrecord_add",
                [zone1, name1],
                {"dnamerecord": [u"foo-1.%s" % absnxname, u"foo-2.%s" % absnxname]},
            ),
            expected=errors.ValidationError(
                name="dnamerecord",
        ...
        dict(
            ...
            command=(
                "dnsrecord_add",
                [zone1, dname],
                {"cnamerecord": u"foo-1.%s" % absnxname},
            ),
            expected=errors.ValidationError(
                name="cnamerecord",
                error=u"CNAME record is not allowed to coexist with any other "
        ...
        dict(
            ...
            expected=errors.NotFound(
                reason=u"Nameserver '%s' does not have a corresponding A/AAAA record"
                % absnxname
            ),
        ),
        dict(
            ...
            expected=errors.NotFound(
                reason=u"Nameserver '%s.%s.' does not "
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="rename", error=u"DNS zone root record cannot be renamed"
            ),
        ),
        dict(
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="cert_usage", error=u"can be at most 255"
            ),
        ),
        dict(
            ...
            expected=errors.ValidationError(
                name="selector", error=u"can be at most 255"
            ),
        ),
        dict(
            ...
            expected=errors.ValidationError(
                name="matching_type", error=u"can be at most 255"
            ),
        ),
        dict(
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="name_from_ip", error=u"invalid IP network format"
            ),
        ),
        dict(
            ...
            command=(
                "dnszone_add",
                [],
                {"name_from_ip": revzone1_ip, "idnssoarname": zone1_rname,},
            ),
            expected={
                "value": revzone1_dnsname,
                "summary": None,
        ...
        dict(
            ...
            command=(
                "dnszone_add",
                [],
                {"name_from_ip": revzone2_ip, "idnssoarname": zone1_rname,},
            ),
            expected={
        ...
        dict(
            ...
            expected=errors.DuplicateEntry(
                message=u"Reverse record for IP "
        ...
        dict(
            ...
            expected={
                "value": name1_dnsname,
                "summary": None,
                "result": {"idnsname": [name1_dnsname], "arecord": [revname2_ip],},
            },
        ),
        dict(
        ...
        dict(
            ...
            command=("dnszone_add_permission", [zone1], {}),
            expected=errors.DuplicateEntry(
                message=u"permission with name "
        ...
        dict(
            ...
            command=("permission_show", [zone1_permission], {}),
            expected=errors.NotFound(
                reason=u"%s: permission not found" % zone1_permission
            ),
        ),
        dict(
        ...
        dict(
            ...
            command=(
                "dnszone_add",
                [zone3],
                {"idnssoamname": u"ns", "idnssoarname": zone3_rname,},
            ),
            expected=errors.NotFound(
                reason=u"Nameserver 'ns.%s' does not have a corresponding A/AAAA record"
                % zone3_absolute
            ),
        ),
        dict(
            ...
            command=(
                "dnszone_add",
                [zone3],
                {"idnssoamname": zone3_absolute, "idnssoarname": zone3_rname,},
            ),
            expected=errors.NotFound(
                reason=u"Nameserver '%s' does not have a corresponding A/AAAA record"
                % zone3_absolute
        ...
        dict(
            ...
            command=("dnszone_add", [zone3], {"idnssoarname": zone3_rname,}),
            expected={
        ...
        dict(
            ...
            expected=dict(
                ...
                summary=u'Added system permission "%s"'
        ...
        dict(
            ...
            expected=dict(
                result=True,
                value=revzone3_classless2_permission,
                summary=u'Removed system permission "%s"'
                % revzone3_classless2_permission,
            ),
        ),
        dict(
        ...
        dict(
            ...
            command=("dnszone_add", [idnzone1], {"idnssoarname": idnzone1_rname,}),
            expected={
        ...
        dict(
            ...
            command=("dnszone_show", [idnzone1], {u"raw": True,}),
            expected={
                "value": idnzone1_dnsname,
                "summary": None,
                "result": {
                ...
                "result": [
                    {
                        ...
                        "idnsupdatepolicy": [
                            ...
                            u"* AAAA; "
                            u"grant %(realm)s krb5-self "
                            u"* SSHFP;" % dict(realm=api.env.realm)
                        ],
                    },
                ],
            },
        ...
        dict(
            ...
            command=("dnszone_find", [idnzone1_punycoded], {"raw": True,}),
            expected={
                "summary": None,
                "count": 1,
                "truncated": False,
        ...
        dict(
            ...
            command=("dnszone_add", [revidnzone1], {"idnssoarname": idnzone1_rname,}),
            expected={
        ...
        dict(
            ...
            expected={
                ...
                "truncated": False,
                "result": [
                    {"dn": idnzone1_dn, "idnsname": [_dns_zone_record],},
                    {"dn": idnres1_dn, "idnsname": [idnres1_dnsname],},
                ],
            },
        ),
        dict(
        ...
        dict(
            ...
            expected={
                ...
                "count": 1,
                "truncated": False,
                "result": [{"dn": idnres1_dn, "idnsname": [idnres1_dnsname],},],
            },
        ),
        dict(
            ...
            expected={
                ...
                "count": 1,
                "truncated": False,
                "result": [{"dn": idnres1_dn, "idnsname": [idnres1_punycoded],},],
            },
        ),
        dict(
        ...
        dict(
            ...
            expected={
                "value": [idnres1_dnsname],
                "summary": None,
                "result": {"idnsname": [idnres1_dnsname], "arecord": [u"10.10.0.1"],},
            },
        ),
        dict(
        ...
        dict(
            ...
            command=(
                "dnsrecord_add",
                [idnzone1, idnzone1],
                {"mxrecord": u"10 %s" % idnzone1_mname},
            ),
            expected={
                "value": idnzone1_dnsname,
                "summary": None,
        ...
        dict(
            ...
            command=(
                ...
                {"afsdb_part_subtype": 0, "afsdb_part_hostname": idnzone1_mname},
            ),
            expected={
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="idnsname",
                error=(
                    u"owner of DNAME, DS, NS records "
...
class test_root_zone(Declarative):
    ...
    cleanup_commands = [
        ("dnszone_del", [zone_root,], {"continue": True}),
    ...
    tests = [
        dict(
            ...
            command=(
                "dnszone_add",
                [zone_root],
                {"idnssoarname": zone_root_rname, "skip_overlap_check": True},
            ),
            expected={
        ...
        dict(
            ...
            command=("dnszone_add_permission", [zone_root], {}),
            expected=dict(
                result=True,
...
class test_forward_zones(Declarative):
    ...
    cleanup_commands = [
        (
            "dnsforwardzone_del",
            [zone_fw_wildcard, fwzone1, fwzone2, fwzone3],
            {"continue": True},
        ),
    ...
    tests = [
        dict(
            ...
            expected={"summary": None, "count": 0, "truncated": False, "result": [],},
        ),
        dict(
        ...
        dict(
            ...
            command=("dnsforwardzone_add", [u""], {}),
            expected=errors.RequirementError(name="name"),
        ),
        dict(
            desc="Try to create forward zone %r with invalid name"
            % "invalid..name.fwzone.test.",
            command=("dnsforwardzone_add", [u"invalid..name.fwzone.test.",], {}),
            expected=errors.ConversionError(name="name", error=u"empty DNS label"),
        ),
        dict(
            ...
            expected=errors.ValidationError(
                name="idnsforwarders", error=u"Please specify forwarders."
            ),
        ),
        dict(
            desc='Try to create forward zone %r without forwarders with "only" policy'
            % fwzone1,
            command=("dnsforwardzone_add", [fwzone1], {"idnsforwardpolicy": u"only"}),
            expected=errors.ValidationError(
                name="idnsforwarders", error=u"Please specify forwarders."
            ),
        ),
        dict(
            ...
            % fwzone1,
            command=("dnsforwardzone_add", [fwzone1], {"idnsforwardpolicy": u"first"}),
            expected=errors.ValidationError(
                name="idnsforwarders", error=u"Please specify forwarders."
            ),
        ),
        dict(
            ...
            command=(
                "dnsforwardzone_add",
                [fwzone1],
                {"idnsforwardpolicy": u"only", "idnsforwarders": [u"127.0.0.999",]},
            ),
            expected=errors.ValidationError(
                name="forwarder", error=u"invalid IP address format"
            ),
        ),
        dict(
            ...
            command=(
                "dnsforwardzone_add",
                [fwzone1],
                {"idnsforwardpolicy": u"first", "idnsforwarders": [u"127.0.0.999",]},
            ),
            expected=errors.ValidationError(
                name="forwarder", error=u"invalid IP address format"
            ),
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="forward_policy", error=u"must be one of 'only', 'first', 'none'"
            ),
        ),
        dict(
            desc='Create forward zone %r without forwarders with "none" policy'
            % fwzone1,
            command=("dnsforwardzone_add", [fwzone1], {"idnsforwardpolicy": u"none"}),
            expected={
        ...
        dict(
            ...
            % fwzone2,
            command=("dnsforwardzone_add", [fwzone2], {"idnsforwarders": [forwarder1]}),
            expected={
                ...
                u"messages": (
                    {
                        u"message": lambda x: x.startswith(
                            u"DNS server %s: query '%s SOA':" % (forwarder1, fwzone2)
                        ),
                        u"code": 13006,
                        u"type": u"warning",
                        u"name": u"DNSServerValidationWarning",
        ...
        dict(
            ...
            command=("dnsforwardzone_mod", [fwzone3], {"idnsforwarders": forwarder1,}),
            expected={
                "value": fwzone3_dnsname,
                "summary": None,
                "messages": lambda x: True,  # fake forwarders - ignore message
        ...
        dict(
            ...
            command=(
                "dnsforwardzone_mod",
                [fwzone1],
                {"idnsforwardpolicy": u"none", "idnsforwarders": [forwarder3],},
            ),
            expected={
                "value": fwzone1_dnsname,
                "summary": None,
        ...
        dict(
            ...
            command=("dnsforwardzone_mod", [fwzone2], {"idnsforwardpolicy": u"none",}),
            expected={
                "value": fwzone2_dnsname,
                "summary": None,
                "result": {
        ...
        dict(
            ...
            % fwzone2,
            command=("dnsforwardzone_mod", [fwzone2], {"idnsforwardpolicy": u"only",}),
            expected={
                "value": fwzone2_dnsname,
                "summary": None,
                "result": {
        ...
        dict(
            ...
            command=(
                "dnsforwardzone_mod",
                [fwzone1],
                {"idnsforwardpolicy": u"first", "idnsforwarders": [forwarder3],},
            ),
            expected={
                "value": fwzone1_dnsname,
                "summary": None,
        ...
        dict(
            ...
            command=(
                "dnsforwardzone_mod",
                [fwzone1],
                {"idnsforwardpolicy": u"none", "idnsforwarders": [],},
            ),
            expected={
                "value": fwzone1_dnsname,
                "summary": None,
        ...
        dict(
            ...
            command=("dnsforwardzone_mod", [fwzone1], {"idnsforwardpolicy": u"first",}),
            expected={
                "value": fwzone1_dnsname,
                "summary": None,
                "result": {
        ...
        dict(
            ...
            command=("dnsforwardzone_mod", [fwzone1], {"idnsforwardpolicy": u"only",}),
            expected={
                "value": fwzone1_dnsname,
                "summary": None,
                "result": {
        ...
        dict(
            ...
            % fwzone2,
            command=("dnsforwardzone_mod", [fwzone2], {"idnsforwarders": [],}),
            expected=errors.ValidationError(
                name="idnsforwarders", error=u"Please specify forwarders."
            ),
        ),
        dict(
            ...
            % fwzone3,
            command=("dnsforwardzone_mod", [fwzone3], {"idnsforwarders": [],}),
            expected=errors.ValidationError(
                name="idnsforwarders", error=u"Please specify forwarders."
            ),
        ),
        ...
        dict(
            ...
            expected={
                ...
                "truncated": False,
                "result": [
                    {"dn": fwzone1_dn, "idnsname": [fwzone1_dnsname],},
                    {"dn": fwzone2_dn, "idnsname": [fwzone2_dnsname],},
                    {"dn": fwzone3_dn, "idnsname": [fwzone3_dnsname],},
                ],
            },
        ),
        dict(
        ...
        dict(
            ...
            expected={
                ...
                "result": {"failed": [nonexistent_fwzone_dnsname],},
            },
        ),
        dict(
            ...
            expected=errors.NotFound(
                reason=u"%s: DNS forward zone not found" % absnxname
            ),
        ),
        dict(
        ...
        dict(
            ...
            command=("permission_show", [fwzone1_permission], {}),
            expected=errors.NotFound(
                reason=u"%s: permission not found" % fwzone1_permission
            ),
        ),
        dict(
...
class test_forward_master_zones_mutual_exlusion(Declarative):
    ...
    cleanup_commands = [
        ...
        ("dnsforwardzone_del", [fwzone1, zone_findtest_forward], {"continue": True}),
        ("permission_del", [fwzone1_permission,], {"force": True}),
    ]
    ...
    tests = [
        ...
        dict(
            desc="Try to create duplicate zone which is already forward zone %r"
            % fwzone1,
            command=("dnszone_add", [fwzone1], {"idnssoarname": zone1_rname,}),
            expected=errors.DuplicateEntry(
                message=u"Only one zone type is allowed per zone name"
            ),
        ),
        dict(
            desc="Try to create duplicate forward zone which is already master zone %r"
            % zone1,
            command=("dnsforwardzone_add", [zone1], {"idnsforwardpolicy": u"none",}),
            expected=errors.DuplicateEntry(
                message=u"Only one zone type is allowed per zone name"
            ),
        ),
        dict(
            ...
            command=("dnszone_mod", [fwzone1], {"idnssoarname": zone1_rname,}),
            expected=errors.NotFound(reason=u"%s: DNS zone not found" % fwzone1),
        ),
        dict(
            desc="Try to modify master zone %r using dnsforwardzone-mod" % zone1,
            command=("dnsforwardzone_mod", [zone1], {"idnsforwardpolicy": u"none",}),
            expected=errors.NotFound(
        ...
        dict(
            desc="Try to add per-zone permission for forward zone %r using dnszone-add-permission"
            % fwzone1,
            command=("dnszone_add_permission", [fwzone1], {}),
            expected=errors.NotFound(reason=u"%s: DNS zone not found" % fwzone1),
        ),
        dict(
            desc="Try to add per-zone permission for master zone %r using dnsforwardzone-add-permission"
            % zone1,
        ...
        dict(
            desc="Try to remove per-zone permission for forward zone %r using dnszone-remove-permission"
            % fwzone1,
            command=("dnszone_remove_permission", [fwzone1], {}),
            expected=errors.NotFound(reason=u"%s: DNS zone not found" % fwzone1),
        ),
        dict(
            desc="Try to remove per-zone permission for master zone %r using dnsforwardzone-remove-permission"
            % zone1,
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="dnszoneidnsname", error=(u"only master zones can contain records")
            ),
        ),
        dict(
...
class test_forwardzone_delegation_warnings(Declarative):
    ...
    cleanup_commands = [
        ("dnsforwardzone_del", [zone1_sub_fw, zone1_sub2_fw], {"continue": True}),
        ("dnszone_del", [zone1, zone1_sub], {"continue": True}),
    ]
    ...
    tests = [
        ...
        dict(
            desc="Create zone %r (expected warning for %r)" % (zone1, zone1_sub_fw),
        ...
        dict(
            desc="Create zone %r (expected warning for %r)" % (zone1_sub, zone1_sub_fw),
        ...
        dict(
            desc="Enable zone %r (expected warning for %r)" % (zone1_sub, zone1_sub_fw),
        ...
        dict(
            ...
            command=("dnsrecord_add", [zone1_sub, u"fw"], {"nsrecord": self_server_ns}),
            expected={
                "value": DNSName(u"fw"),
                "summary": None,
                "result": {
        ...
        dict(
            ...
            "(expected warning for %r)" % (zone1_sub_fw, zone1_sub, zone1_sub_fw),
            command=("dnsrecord_del", [zone1_sub, u"fw"], {"del_all": True}),
            expected={
                "value": [DNSName(u"fw")],
        ...
        dict(
            ...
            command=("dnsrecord_mod", [zone1, u"fw.sub2.sub"], {"nsrecord": None}),
            expected={
                "value": DNSName(u"fw.sub2.sub"),
...
class test_dns_soa(Declarative):
    ...
    tests = [
        dict(
            ...
            expected=errors.NotFound(reason=u"%s: DNS zone not found" % zone6_absolute),
        ),
        dict(
            desc="Create zone %r" % zone6b,
            command=("dnszone_add", [zone6b], {"idnssoarname": zone6b_rname,}),
            expected={
        ...
        dict(
            ...
            command=("dnsrecord_add", [zone6b, zone6b_ns], {"arecord": zone6b_ip}),
            expected={
        ...
        dict(
            ...
            command=("dnszone_mod", [zone6], {"idnssoamname": zone6b_ns,}),
            expected={
                "value": zone6_absolute_dnsname,
                "summary": None,
                "result": {
        ...
        dict(
            ...
            command=("dnsrecord_add", [zone6, zone6_ns], {"arecord": zone6b_ip}),
            expected={
        ...
        dict(
            ...
            command=("dnszone_mod", [zone6], {"idnssoamname": zone6_ns_relative,}),
            expected={
                "value": zone6_absolute_dnsname,
                "summary": None,
                "result": {
        ...
        dict(
            ...
            command=(
                "dnszone_mod",
                [zone6],
                {"idnssoamname": zone6_unresolvable_ns, "force": True,},
            ),
            expected={
                "value": zone6_absolute_dnsname,
                "summary": None,
        ...
        dict(
            ...
            command=(
                "dnszone_mod",
                [zone6],
                {"idnssoamname": zone6_unresolvable_ns_relative, "force": True,},
            ),
            expected={
                "value": zone6_absolute_dnsname,
                "summary": None,
        ...
        dict(
            ...
            expected=errors.ConversionError(
                name="admin_email", error=u"empty DNS label"
            ),
        ),
        dict(
            ...
            expected=errors.ConversionError(
                name="name_server", error=u"empty DNS label"
            ),
        ),
        dict(
            desc="Updating a zone - %r - with unresolvable name-server - %r"
            % (zone6, zone6_unresolvable_ns),
            command=("dnszone_mod", [zone6], {"idnssoamname": zone6_unresolvable_ns,}),
            expected=errors.NotFound(
                reason=u"Nameserver '%s' does not have a corresponding "
                u"A/AAAA record" % zone6_unresolvable_ns_dnsname,
            ),
        ),
        dict(
        ...
        dict(
            ...
            command=("dnszone_mod", [zone6], {"idnssoamname": "",}),
            expected=errors.ValidationError(name="name_server", error=u"is required"),
        ),
        dict(
        ...
        dict(
            desc="Adding a zone - %r - with name-server %r" % (zone6, zone6_ns_dnsname),
            command=("dnszone_add", [zone6], {"idnssoamname": zone6b_ns,}),
            expected={
                "value": zone6_absolute_dnsname,
                "summary": None,
                "result": {
        ...
        dict(
            ...
            % (zone6, zone6_unresolvable_ns),
            command=("dnszone_add", [zone6], {"idnssoamname": zone6_unresolvable_ns,}),
            expected=errors.NotFound(
                reason=u"Nameserver '%s' does not have a corresponding "
                u"A/AAAA record" % zone6_unresolvable_ns_dnsname,
            ),
        ...
        dict(
            ...
            "relative name-server - %r" % (zone6, zone6_unresolvable_ns_relative),
            command=(
                "dnszone_add",
                [zone6],
                {"idnssoamname": zone6_unresolvable_ns_relative,},
...
class test_dns_type_uri(test_dns):
    ...
    uri_raw_value = u'{0} {1} "{2}"'.format(uri_priority, uri_weight, uri_target)
    tests = [
        dict(
        ...
        dict(
            ...
            expected=errors.AttrValueNotFound(
                attr="URI record", value=uri_raw_value.upper()
            ),
        ),
        dict(
