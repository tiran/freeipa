...
def manager1(request, xmlrpc_setup):
    ...
    tracker = UserTracker(
        name=u"mscott", sn=u"Manager1", givenname=u"Automember test manager user1"
    )
...
def group1(request, xmlrpc_setup):
    tracker = GroupTracker(name=u"tgroup1", description=u"Automember test group1")
    return tracker.make_fixture(request)
...
def defaultgroup1(request, xmlrpc_setup):
    tracker = GroupTracker(
        name=u"defaultgroup1", description=u"Automember test defaultgroup1"
    )
    return tracker.make_fixture(request)
...
def hostgroup1(request, xmlrpc_setup):
    tracker = HostGroupTracker(
        name=u"thostgroup1", description=u"Automember test hostgroup1"
    )
    return tracker.make_fixture(request)
...
def hostgroup2(request, xmlrpc_setup):
    tracker = HostGroupTracker(
        name=u"thostgroup2", description=u"Automember test hostgroup2"
    )
    return tracker.make_fixture(request)
...
def hostgroup3(request, xmlrpc_setup):
    tracker = HostGroupTracker(
        name=u"thostgroup3", description=u"Automember test hostgroup3"
    )
    return tracker.make_fixture(request)
...
def hostgroup4(request, xmlrpc_setup):
    tracker = HostGroupTracker(
        name=u"thostgroup4", description=u"Automember test hostgroup4"
    )
    return tracker.make_fixture(request)
...
def defaulthostgroup1(request, xmlrpc_setup):
    tracker = HostGroupTracker(
        name=u"defaulthostgroup1", description=u"Automember test" "defaulthostgroup1"
    )
    return tracker.make_fixture(request)
...
class TestNonexistentAutomember(XMLRPC_test):
    def test_create_with_nonexistent_group(self, automember_group, group1):
        ...
        with raises_exact(errors.NotFound(reason=u'group "%s" not found' % group1.cn)):
            command()
    ...
    def test_delete_with_nonexistent_group(self, automember_group, group1):
        ...
        with raises_exact(
            errors.NotFound(reason=u"%s: Automember rule not found" % group1.cn)
        ):
            command()

    def test_create_with_nonexistent_hostgroup(self, automember_hostgroup, hostgroup1):
        """ Try to add a rule with non-existent group """
        hostgroup1.ensure_missing()
        command = automember_hostgroup.make_create_command()
        with raises_exact(
            errors.NotFound(reason=u'hostgroup "%s" not found' % hostgroup1.cn)
        ):
            command()

    def test_delete_with_nonexistent_hostgroup(self, automember_hostgroup, hostgroup1):
        """ Try to delete a rule with non-existent group """
        hostgroup1.ensure_missing()
        command = automember_hostgroup.make_delete_command()
        with raises_exact(
            errors.NotFound(reason=u"%s: Automember rule not found" % hostgroup1.cn)
        ):
            command()
...
class TestCRUDFOnAutomember(XMLRPC_test):
    ...
    def test_basic_ops_on_hostgroup_automember(self, automember_hostgroup, hostgroup1):
        """ Test create, retrieve, find, update,
...
def set_automember_process_modify_ops(value):
    ...
    ldap = ldap2(api)
    ldap.connect()
    plugin_entry = ldap.get_entry(DN("cn=Auto Membership Plugin,cn=plugins,cn=config"))
...
def wait_automember_rebuild():
    ...
    try:
        task_entries, unused = ldap.find_entries(
            base_dn=REBUILD_TASK_CONTAINER, filter="(&(!(nstaskexitcode=0))(scope=*))"
        )
...
class TestAutomemberRebuildHostMembership(XMLRPC_test):
    def test_create_deps_for_rebuilding_hostgroups(
        self, hostgroup1, host1, automember_hostgroup
    ):
        ...
        automember_hostgroup.add_condition(
            key=u"fqdn", type=u"hostgroup", inclusiveregex=[hostgroup_include_regex]
        )
        hostgroup1.retrieve()

    def test_rebuild_membership_hostgroups(
        self, automember_hostgroup, hostgroup1, host1
    ):
    ...
    def test_rebuild_membership_for_host(self, host1, automember_hostgroup, hostgroup1):
    ...
    def test_delete_deps_for_rebuilding_hostgroups(
        self, host1, hostgroup1, automember_hostgroup
    ):
...
class TestAutomemberRebuildGroupMembership(XMLRPC_test):
    def test_create_deps_for_rebuilding_groups(
        self, group1, manager1, user1, automember_group
    ):
    ...
    def test_rebuild_membership_for_user(self, user1, automember_group, group1):
        ...
        command = automember_group.make_rebuild_command(users=user1.name, no_wait=True)
        result = command()
    ...
    def test_delete_deps_for_rebuilding_groups(
        self, user1, manager1, group1, automember_group
    ):
        """ Delete dependences for this class of tests in desired order """
        user1.delete()
        manager1.delete()
...
class TestAutomemberRebuildMembershipIncorrectly(XMLRPC_test):
    ...
    def test_rebuild_membership_user_hosts(self, automember_hostgroup, user1, host1):
        """ Try to issue rebuild membership command with --users and --hosts
        together """
        command = automember_hostgroup.make_rebuild_command(
            users=user1.name, hosts=host1.fqdn
        )
        with raises_exact(
            errors.MutuallyExclusiveError(reason=u"users and hosts cannot both be set")
        ):
            command()

    def test_rebuild_membership_users_hostgroup(self, automember_hostgroup, user1):
        """ Try to issue rebuild membership command with type --hosts and
        users specified """
        command = automember_hostgroup.make_rebuild_command(
            users=user1.name, type=u"hostgroup"
        )
        with raises_exact(
            errors.MutuallyExclusiveError(
    ...
    def test_rebuild_membership_hosts_group(self, automember_hostgroup, user1, host1):
        """ Try to issue rebuild membership command with type --users and
        hosts specified """
        command = automember_hostgroup.make_rebuild_command(
            hosts=host1.fqdn, type=u"group"
        )
        with raises_exact(
            errors.MutuallyExclusiveError(
...
class TestMultipleAutomemberConditions(XMLRPC_test):
    ...
    def test_create_duplicate_automember_condition(
        self, automember_hostgroup, hostgroup1
    ):
    ...
    def test_create_additional_automember_conditions(self, automember_hostgroup):
        ...
        expected = dict(
            ...
            failed=dict(
                failed=dict(
                    automemberinclusiveregex=tuple(), automemberexclusiveregex=tuple(),
                )
            ),
            result=dict(
                ...
                automembertargetgroup=[
                    automember_hostgroup.attrs["automembertargetgroup"][0]
                ],
                automemberinclusiveregex=[
                ...
                automemberexclusiveregex=[
                    u"fqdn=%s" % hostgroup_exclude_regex2,
    ...
    def test_create_set_of_hostgroup_automembers(
        self, automember_hostgroup2, automember_hostgroup3, automember_hostgroup4
    ):
        """ Create three more hostgroup automembers """
        automember_hostgroup2.ensure_exists()
        automember_hostgroup2.add_condition(
            key=u"fqdn", type=u"hostgroup", inclusiveregex=[hostgroup_exclude_regex]
        )
        automember_hostgroup3.ensure_exists()
        automember_hostgroup3.add_condition(
            key=u"fqdn", type=u"hostgroup", inclusiveregex=[hostgroup_exclude_regex2]
        )
        automember_hostgroup4.ensure_exists()
        automember_hostgroup4.add_condition(
            key=u"fqdn", type=u"hostgroup", inclusiveregex=[hostgroup_exclude_regex3]
        )
    ...
    def test_set_default_group_for_automembers(self, defaultgroup1):
        ...
        result = api.Command["automember_default_group_set"](
            type=u"group", automemberdefaultgroup=defaultgroup1.cn
        )
        ...
        result = api.Command["automember_default_group_show"](type=u"group",)
    ...
    def test_set_default_hostgroup_for_automembers(self, defaulthostgroup1):
        ...
        result = api.Command["automember_default_group_set"](
            type=u"hostgroup", automemberdefaultgroup=defaulthostgroup1.cn
        )
        ...
        result = api.Command["automember_default_group_show"](type=u"hostgroup",)
    ...
    def test_create_deps_under_new_conditions(
        ...
        host1.track_create()
        host1.attrs.update(
            memberofindirect_netgroup=[hostgroup1.cn],
        ...
        host2.track_create()
        host2.attrs.update(
            memberof_hostgroup=[defaulthostgroup1.cn],
        ...
        host3.track_create()
        host3.attrs.update(
            memberofindirect_netgroup=[hostgroup2.cn],
        ...
        host4.track_create()
        host4.attrs.update(
            memberofindirect_netgroup=[hostgroup3.cn],
        ...
        host5.track_create()
        host5.attrs.update(
            memberofindirect_netgroup=[hostgroup4.cn],
    ...
    def test_rebuild_membership_for_one_host(self, automember_hostgroup, host1):
        """ Rebuild hostgroup automember membership for one host """
        command = automember_hostgroup.make_rebuild_command(
            type=u"hostgroup", hosts=host1.fqdn
        )
        result = command()
        automember_hostgroup.check_rebuild(result)
    ...
    def test_rebuild_membership_for_one_user(self, automember_group, user1):
        ...
        command = automember_group.make_rebuild_command(type=u"group", users=user1.name)
        result = command()
        automember_group.check_rebuild(result)

    def test_rebuild_membership_with_invalid_hosts_in_hosts(self, automember_hostgroup):
        """ Try to rebuild membership with invalid host in --hosts """
        command = automember_hostgroup.make_rebuild_command(hosts=fqdn_does_not_exist)
        with raises_exact(
            errors.NotFound(reason=u"%s: host not found" % fqdn_does_not_exist)
        ):
            command()

    def test_rebuild_membership_with_invalid_user_in_users(self, automember_group):
        """ Try to rebuild membership with invalid user in --users """
        command = automember_group.make_rebuild_command(users=user_does_not_exist)
        with raises_exact(
            errors.NotFound(reason=u"%s: user not found" % user_does_not_exist)
        ):
            command()

    def test_reset_automember_default_groups(
        self, defaultgroup1, user1, defaulthostgroup1, manager1
    ):
        """ Reset automember group defaults """
        manager1.delete()
        user1.delete()
        result = api.Command["automember_default_group_remove"](type=u"group",)
        ...
        assert_deepequal(
            dict(
                ...
                value=u"group",
                summary=u"Removed default (fallback) group" ' for automember "group"',
            ),
            result,
        )

        result = api.Command["automember_default_group_remove"](type=u"hostgroup",)
...
class TestAutomemberFindOrphans(XMLRPC_test):
    def test_create_deps_for_find_orphans(
        self, hostgroup1, host1, automember_hostgroup
    ):
        """ Create host, hostgroup, and automember tracker for this class
        of tests. """
    ...
    def test_find_orphan_automember_rules(self, hostgroup1):
        ...
        if expected_failure != rebuild_failure:
            pytest.fail(
                "unexpected result for automember_rebuild with "
        ...
        result = api.Command["automember_find_orphans"](type=u"hostgroup", remove=True)
        assert result["count"] == 1
