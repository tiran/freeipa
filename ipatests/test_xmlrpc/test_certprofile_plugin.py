...
IPA_CERT_SUBJ_BASE = api.Command.config_show()["result"]["ipacertificatesubjectbase"][0]
...
CA_IPA_SERVICE_XML_TEMPLATE = os.path.join(BASE_DIR, "data/caIPAserviceCert.xml.tmpl")
...
def user_profile(request, xmlrpc_setup):
    ...
    tracker = CertprofileTracker(
        name, store=True, desc=u"Storing copy of a profile", profile=profile_path
    )
...
def malformed(request, xmlrpc_setup):
    ...
    tracker = CertprofileTracker(
        name, store=True, desc=u"malformed profile", profile=profile_path
    )
...
def xmlprofile(request, xmlrpc_setup):
    ...
    tracker = CertprofileTracker(
        name, store=True, desc=u"xml format profile", profile=profile_path
    )
...
class TestDefaultProfile(XMLRPC_test):
    ...
    def test_try_rename_by_setattr(self, default_profile):
        command = default_profile.make_update_command(updates=dict(setattr=u"cn=bogus"))
...
class TestProfileCRUD(XMLRPC_test):
    def test_create_duplicate(self, user_profile):
        ...
        with raises_exact(errors.DuplicateEntry(message=msg.format(user_profile.name))):
            command()
    ...
    def test_update_store(self, user_profile):
        user_profile.update(
            dict(ipacertprofilestoreissued=False),
            expected_updates=dict(ipacertprofilestoreissued=[u"FALSE"]),
        )

    def test_update_description(self, user_profile):
        new_desc = u"new description"
        user_profile.update(
            dict(description=new_desc), expected_updates=dict(description=[new_desc])
        )
    ...
    def test_update_by_malformed_profile(self, user_profile):
        ...
        command = user_profile.make_update_command(dict(file=unicode(profile_content)))
    ...
    def test_try_rename_by_setattr(self, user_profile):
        user_profile.ensure_exists()
        command = user_profile.make_update_command(updates=dict(setattr=u"cn=bogus"))
