...
class TestAttrOnUser(XMLRPC_test):
    def test_add_user_with_singlevalue_addattr(self):
        ...
        user = UserTracker(
            name=u"user", givenname=u"Test", sn=u"User1", addattr=u"sn=User2"
        )
    ...
    def test_add_another_mail_user(self, user):
        ...
        user.update(dict(addattr="mail=" + update), dict(addattr=""))
    ...
    def test_add_two_phone_numbers_at_once_user(self, user):
        ...
        user.update(
            dict(
                setattr=u"telephoneNumber=" + update1,
    ...
    def test_go_from_two_phone_numbers_to_one(self, user):
        ...
        user.ensure_exists()
        user.update(
            dict(setattr=u"telephoneNumber=" + update),
    ...
    def test_add_two_more_phone_numbers(self, user):
        ...
        user.update(
            dict(
                addattr=(u"telephoneNumber=" + update1, u"telephoneNumber=" + update2)
            ),
            dict(addattr=""),
        )
    ...
    def test_delete_the_number_again(self, user):
        ...
        user.ensure_exists()
        update = u"301-555-1212"
        command = user.make_update_command(dict(delattr=u"telephoneNumber=" + update))
        with raises_exact(
            errors.AttrValueNotFound(attr=u"telephonenumber", value=update)
        ):
            command()
    ...
    def test_set_and_delete_a_phone_number(self, user):
        ...
        user.update(
            dict(
                setattr=(u"telephoneNumber=" + update1, u"telephoneNumber=" + update2),
                delattr=u"telephoneNumber=" + update1,
            ),
    ...
    def test_set_givenname_with_setattr_in_user(self, user):
        ...
        user.ensure_exists()
        user.update(
            dict(setattr=u"givenname=Finkle"), dict(givenname=[u"Finkle"], setattr="")
        )
    ...
    def test_lock_user_using_setattr(self, user):
        ...
        user.ensure_exists()
        user.update(
            dict(setattr=u"nsaccountlock=TrUe"), dict(nsaccountlock=True, setattr="")
        )
    ...
    def test_unlock_user_using_addattr_delattr(self, user):
        ...
        user.ensure_exists()
        user.update(
            dict(addattr=u"nsaccountlock=FaLsE", delattr=u"nsaccountlock=TRUE"),
...
class TestAttrOnConfigs(XMLRPC_test):
    def test_add_new_group_search_fields_config_entry(self, user):
        ...
        with raises_exact(errors.OnlyOneValueAllowed(attr="ipagroupsearchfields")):
            command()
    ...
    def test_add_a_new_cert_subject_base_config_entry(self, user):
        ...
        with raises_exact(
            errors.ValidationError(
                name="ipacertificatesubjectbase", error="attribute is not configurable"
            )
        ):
            command()
    ...
    def test_delete_required_config_entry(self, user):
        ...
        with raises_exact(errors.RequirementError(name="searchrecordslimit")):
            command()

    def test_set_nonexistent_attribute(self, user):
        """ Try setting a nonexistent attribute """
        command = user.make_command("config_mod", **dict(setattr=u"invalid_attr=false"))
        with raises_exact(
            errors.ObjectclassViolation(info='attribute "invalid_attr" not allowed')
        ):
            command()
    ...
    def test_set_outofrange_krbpwdmaxfailure(self, user):
        ...
        with raises_exact(
            errors.ValidationError(name="krbpwdmaxfailure", error="must be at least 0")
        ):
            command()

    def test_set_outofrange_maxfail(self, user):
        """ Try setting out-of-range maxfail """
        command = user.make_command("pwpolicy_mod", **dict(krbpwdmaxfailure=u"-1"))
        with raises_exact(
            errors.ValidationError(name="maxfail", error="must be at least 0")
        ):
            command()
    ...
    def test_set_nonnumeric_krbpwdmaxfailure(self, user):
        ...
        with raises_exact(
            errors.ConversionError(name="krbpwdmaxfailure", error="must be an integer")
        ):
            command()

    def test_set_nonnumeric_maxfail(self, user):
        """ Try setting non-numeric maxfail """
        command = user.make_command("pwpolicy_mod", **dict(krbpwdmaxfailure=u"abc"))
        with raises_exact(
            errors.ConversionError(name="maxfail", error="must be an integer")
        ):
            command()

    def test_delete_bogus_attribute(self, user):
        """ Try deleting bogus attribute """
        command = user.make_command("config_mod", **dict(delattr=u"bogusattribute=xyz"))
        with raises_exact(
            errors.ValidationError(
                name="bogusattribute", error="No such attribute on this entry"
            )
        ):
            command()
    ...
    def test_delete_empty_attribute(self, user):
        ...
        command = user.make_command(
            "config_mod", **dict(delattr=u"ipaCustomFields=See Also,seealso,false")
        )
        with raises_exact(
            errors.ValidationError(
                name="ipacustomfields", error="No such attribute on this entry"
            )
        ):
            command()
    ...
    def test_set_and_del_value_and_del_missing_one(self, user):
        ...
        with raises_exact(
            errors.AttrValueNotFound(attr="ipacustomfields", value="Country,c,false")
        ):
            command()
    ...
    def test_delete_an_operational_attribute_with_delattr(self, user):
        ...
        command = user.make_command(
            "config_mod", **dict(delattr=u"creatorsName=cn=directory manager")
        )
        with raises_exact(
            errors.DatabaseError(desc="Server is unwilling to perform", info="")
        ):
            command()
