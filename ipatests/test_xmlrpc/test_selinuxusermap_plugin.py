...
from ipatests.test_xmlrpc.xmlrpc_test import Declarative, fuzzy_digits, fuzzy_uuid
...
INVALID_MCS = "Invalid MCS value, must match {}, where max category {}".format(
    platformconstants.SELINUX_MCS_REGEX, platformconstants.SELINUX_MCS_MAX
)

INVALID_MLS = "Invalid MLS value, must match {}, where max level {}".format(
    platformconstants.SELINUX_MLS_REGEX, platformconstants.SELINUX_MLS_MAX
)

user1 = u"tuser1"
group1 = u"testgroup1"
host1 = u"testhost1.%s" % api.env.domain
hostdn1 = DN(("fqdn", host1), ("cn", "computers"), ("cn", "accounts"), api.env.basedn)
...
class test_selinuxusermap(Declarative):
    ...
    tests = [
        ...
        dict(
            ...
            command=("selinuxusermap_add", [rule1], dict(ipaselinuxuser=selinuxuser1)),
            expected=dict(
                value=rule1,
        ...
        dict(
            ...
            expected=errors.DuplicateEntry(
                message=u"SELinux User Map rule "
        ...
        dict(
            ...
            command=("selinuxusermap_mod", [rule1], dict(ipaselinuxuser=selinuxuser2)),
            expected=dict(
                result=dict(
                    cn=[rule1], ipaselinuxuser=[selinuxuser2], ipaenabledflag=[u"TRUE"],
                ),
        ...
        dict(
            ...
            expected=dict(
                failed=dict(
                    memberuser=dict(group=[], user=[(u"notfound", u"no such entry")])
                ),
                completed=0,
                result=dict(
                    cn=[rule1],
        ...
        dict(
            ...
            expected=dict(
                failed=dict(
                    memberuser=dict(
                        group=[], user=[(u"notfound", u"This entry is not a member")]
                    )
                ),
                completed=0,
                result=dict(
        ...
        dict(
            ...
            command=("selinuxusermap_mod", [rule1], dict(seealso=u"notfound")),
            expected=errors.NotFound(reason=u"HBAC rule notfound not found"),
        ),
        dict(
        ...
        dict(
            ...
            expected=errors.DependentEntry(
                key=hbacrule1, label=u"SELinux User Map", dependent=rule1
            ),
        ),
        ...
        dict(
            ...
            expected=errors.NotFound(
                reason=u"SELinux user notfound:s0:c0 "
                + u"not found in ordering list (in config)"
            ),
        ),
        dict(
            desc="Create rule with invalid user bad+user",
            command=("selinuxusermap_add", [rule1], dict(ipaselinuxuser=u"bad+user")),
        ...
        dict(
            ...
            command=(
                ...
                [rule1],
                dict(
                    ipaselinuxuser=u"foo:s{}".format(
                        platformconstants.SELINUX_MLS_MAX + 1
                    )
                ),
            ),
            expected=errors.ValidationError(name="selinuxuser", error=INVALID_MLS),
        ),
        dict(
            desc="Create rule with invalid MCS foo:s0:p88",
            command=("selinuxusermap_add", [rule1], dict(ipaselinuxuser=u"foo:s0:p88")),
            expected=errors.ValidationError(name="selinuxuser", error=INVALID_MCS),
        ),
        dict(
            ...
            command=(
                ...
                [rule1],
                dict(
                    ipaselinuxuser=u"foo:s0-s0:c0.c{}".format(
                        platformconstants.SELINUX_MCS_MAX + 1
                    )
                ),
            ),
        ...
        dict(
            ...
            expected=errors.ValidationError(name="ipaselinuxuser", error=INVALID_MLS),
        ),
        dict(
            ...
            command=(
                ...
                [rule1],
                dict(
                    ipaselinuxuser=selinuxuser1, seealso=hbacrule1, usercategory=u"all"
                ),
            ),
            expected=errors.MutuallyExclusiveError(
                reason=u"HBAC rule and local members cannot both be set"
        ...
        dict(
            ...
            command=(
                ...
                [rule1],
                dict(
                    ipaselinuxuser=selinuxuser1, seealso=hbacrule1, hostcategory=u"all"
                ),
            ),
            expected=errors.MutuallyExclusiveError(
                reason=u"HBAC rule and local members cannot both be set"
        ...
        dict(
            ...
            command=("selinuxusermap_mod", [rule1], dict(usercategory=u"all")),
            expected=errors.MutuallyExclusiveError(
                reason=u"HBAC rule and local members cannot both be set"
            ),
        ),
        dict(
            ...
            command=("selinuxusermap_mod", [rule1], dict(hostcategory=u"all")),
            expected=errors.MutuallyExclusiveError(
                reason=u"HBAC rule and local members cannot both be set"
            ),
        ),
        dict(
            ...
            command=("selinuxusermap_mod", [rule1], dict(setattr=u"usercategory=all")),
            expected=errors.MutuallyExclusiveError(
                reason=u"HBAC rule and local members cannot both be set"
            ),
        ),
        dict(
            ...
            command=("selinuxusermap_mod", [rule1], dict(setattr=u"hostcategory=all")),
            expected=errors.MutuallyExclusiveError(
                reason=u"HBAC rule and local members cannot both be set"
            ),
        ),
        ...
        dict(
            ...
            command=(
                "selinuxusermap_mod",
                [rule1],
                dict(seealso=hbacrule1, usercategory=u"all", hostcategory=u"all"),
            ),
            expected=errors.MutuallyExclusiveError(
                reason=u"HBAC rule and local members cannot both be set"
            ),
        ...
        dict(
            desc="Create rule %r with " "--setattr=seealso=<allow_all rule DN>" % rule1,
            command=(
                "selinuxusermap_add",
                [rule1],
                dict(
