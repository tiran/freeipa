...
class CertmapruleTracker(Tracker, EnableTracker):
    ...
    def __init__(
        ...
        super(CertmapruleTracker, self).__init__(default_version=default_version)

        self.dn = DN(
            (u"cn", cn,), self.api.env.container_certmaprules, self.api.env.basedn
        )
        self.options = {
    ...
    def make_retrieve_command(self, all=False, raw=False):
        return self.make_command("certmaprule_show", self.name, all=all, raw=raw)
...
class CertmapconfigTracker(ConfigurationTracker):
    ...
    def __init__(self, default_version=None):
        super(CertmapconfigTracker, self).__init__(default_version=default_version)
    ...
    def check_retrieve(self, result, all=False, raw=False):
        if all:
            ...
            expected = self.filter_attrs(self.retrieve_keys)
        assert_deepequal(dict(value=None, summary=None, result=expected,), result)
