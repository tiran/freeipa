...
class UserTracker(CertmapdataMixin, KerberosAliasMixin, Tracker):
    ...
    retrieve_keys = {
        ...
        u"ipauserauthtype",
        u"userclass",
        u"ipatokenradiusconfiglink",
        u"ipatokenradiususername",
        u"krbprincipalexpiration",
        u"usercertificate;binary",
        u"has_keytab",
        u"has_password",
        u"memberof_group",
        u"sshpubkeyfp",
    ...
    retrieve_preserved_keys = (retrieve_keys - {u"memberof_group"}) | {u"preserved"}
    ...
    def __init__(self, name=None, givenname=None, sn=None, **kwargs):
        ...
        if not (isinstance(givenname, str) and givenname):
            raise ValueError("Invalid first name provided: {!r}".format(givenname))
    ...
    def make_create_command(self, force=None):
        ...
        if self.uid is not None:
            ...
            return self.make_command(
                "user_add", givenname=self.givenname, sn=self.sn, **self.kwargs
            )
    ...
    def make_delete_command(self, no_preserve=True, preserve=False):
        ...
        if preserve and not no_preserve:
            ...
            # attributes
            self.attrs[u"dn"] = DN(
                ("uid", self.uid), api.env.container_deleteuser, api.env.basedn
            )
            self.attrs[u"objectclass"] = objectclasses.user_base

        return self.make_command(
            "user_del", self.uid, no_preserve=no_preserve, preserve=preserve
        )
        ...
        for key in self.kwargs:
            if key == u"krbprincipalname":
                try:
                    ...
                    self.attrs[key] = [
                        u"%s@%s" % ((self.kwargs[key].lower(), self.api.env.realm))
                    ]
            else:
    ...
    def check_retrieve(self, result, all=False, raw=False):
        ...
        assert_deepequal(dict(value=self.uid, summary=None, result=expected,), result)

    def check_find(
        self, result, all=False, pkey_only=False, raw=False, expected_override=None
    ):
        ...
        assert_deepequal(
            dict(
                count=1, truncated=False, summary=u"1 user matched", result=[expected],
            ),
            result,
        )
    ...
    def check_find_nomatch(self, result):
        ...
        assert_deepequal(
            dict(count=0, truncated=False, summary=u"0 users matched", result=[],),
            result,
        )
    ...
    def create_from_staged(self, stageduser):
        ...
        self.attrs[u"mepmanagedentry"] = [
            u"cn=%s,%s,%s" % (self.uid, api.env.container_group, api.env.basedn)
        ]
    ...
    def enable(self):
        ...
        if self.attrs["nsaccountlock"] is True or self.attrs["nsaccountlock"] == [
            u"true"
        ]:
    ...
    def disable(self):
        ...
        if self.attrs["nsaccountlock"] is False or self.attrs["nsaccountlock"] == [
            u"false"
        ]:
    ...
    def check_attr_preservation(self, expected):
        ...
        if u"memberof_group" not in result[u"result"] or result[u"result"][
            u"memberof_group"
        ] != (u"ipausers",):
            assert False
    ...
    def make_fixture_restore(self, request):
        ...
        def finish():
            with raises_exact(errors.NotFound(reason=u"%s: user not found" % self.uid)):
                del_command()
    ...
    def make_admin(self, admin_group=u"admins"):
        ...
        assert_deepequal(
            dict(
                completed=1,
                failed=dict(member=dict(group=tuple(), user=tuple(), service=tuple())),
                result={
