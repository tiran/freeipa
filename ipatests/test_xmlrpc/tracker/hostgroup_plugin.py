...
class HostGroupTracker(Tracker):
    ...
    def __init__(self, name, description=u"HostGroup desc"):
        ...
        self.dn = DN(
            ("cn", self.cn), ("cn", "hostgroups"), ("cn", "accounts"), api.env.basedn
        )

    def make_create_command(self, force=True, *args, **kwargs):
        """ Make function that creates a hostgroup using 'hostgroup-add' """
        return self.make_command(
            "hostgroup_add", self.cn, description=self.description, *args, **kwargs
        )
    ...
    def add_member(self, options):
        ...
        if u"host" in options:
            try:
                self.attrs[u"member_host"] = self.attrs[u"member_host"] + [
                    options[u"host"]
                ]
            except KeyError:
            ...
            try:
                self.attrs[u"member_hostgroup"] = self.attrs[u"member_hostgroup"] + [
                    options[u"hostgroup"]
                ]
            except KeyError:
