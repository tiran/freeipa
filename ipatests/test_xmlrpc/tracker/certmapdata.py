...
class CertmapdataMixin:
    certmapdata_options = {u"issuer", u"subject", u"certificate", u"ipacertmapdata"}
    ...
    def _data_from_options(self, **options):
        ...
        if {u"issuer", u"subject"} & set(options) and {
            ...
            raise MutuallyExclusiveError(
                reason=u"Mutually exclusive options " u"provided at the same time."
            )
        ...
        if {u"ipacertmapdata", u"certificate"} & set(options):
            ...
            for cert in certs:
                cert = x509.load_der_x509_certificate(
                    base64.b64decode(cert), backend=default_backend()
                )
                issuer = DN(cert.issuer).x500_text()
                subject = DN(cert.subject).x500_text()

                data.append(u"X509:<I>{i}<S>{s}".format(i=issuer, s=subject))
        else:
    ...
    def _make_add_certmap(self):
        raise NotImplementedError(
            "_make_add_certmap method must be " "implemented in instance."
        )

    def _make_remove_certmap(self):
        raise NotImplementedError(
            "_make_remove_certmap method must be " "implemented in instance."
        )
    ...
    def add_certmap(self, **kwargs):
        ...
        try:
            ...
            result = cmd(**kwargs)
            self.attrs.setdefault(u"ipacertmapdata", []).extend(expected_certmapdata)

            expected = dict(
                summary=(
                    u"Added certificate mappings to user " u'"{}"'.format(self.name)
                ),
                value=self.name,
                result=dict(uid=(self.name,),),
            )

            if self.attrs[u"ipacertmapdata"]:
                expected[u"result"][u"ipacertmapdata"] = self.attrs[u"ipacertmapdata"]
    ...
    def remove_certmap(self, **kwargs):
        ...
        try:
            ...
            expected = dict(
                summary=(
                    u"Removed certificate mappings from user " u'"{}"'.format(self.name)
                ),
                value=self.name,
                result=dict(uid=(self.name,),),
            )
