...
class HostTracker(KerberosAliasMixin, Tracker):
    ...
    retrieve_all_keys = retrieve_keys | {
        ...
        u"ipakrbrequirespreauth",
        u"ipauniqueid",
        u"krbcanonicalname",
        u"managing_host",
        u"objectclass",
        u"serverhostname",
        u"ipakrboktoauthasdelegate",
        u"krbpwdpolicyreference",
    }
    create_keys = retrieve_keys | {"objectclass", "ipauniqueid", "randompassword"}
    ...
    def __init__(self, name, fqdn=None, default_version=None):
        ...
        self.dn = DN(
            ("fqdn", self.fqdn), "cn=computers", "cn=accounts", self.api.env.basedn
        )
    ...
    def check_find(self, result, all=False, raw=False):
        ...
        assert_deepequal(
            dict(
                count=1, truncated=False, summary=u"1 host matched", result=[expected],
            ),
            result,
        )
    ...
    def add_finalizer_certcleanup(self, request):
        ...
        cleanup_command = self.make_update_command(updates={"usercertificate": ""})
