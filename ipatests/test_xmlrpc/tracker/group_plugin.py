...
class GroupTracker(Tracker):
    ...
    def make_create_command(self, nonposix=False, external=False, *args, **kwargs):
    ...
    def make_add_member_manager_command(self, options={}):
        return self.make_command("group_add_member_manager", self.cn, **options)

    def make_remove_member_manager_command(self, options={}):
        return self.make_command("group_remove_member_manager", self.cn, **options)
    ...
    def add_member(self, options):
        ...
        if u"user" in options:
            try:
                self.attrs[u"member_user"] = self.attrs[u"member_user"] + [
                    options[u"user"]
                ]
            except KeyError:
            ...
            try:
                self.attrs[u"member_group"] = self.attrs[u"member_group"] + [
                    options[u"group"]
                ]
            except KeyError:
            ...
            try:
                self.attrs[u"member_service"] = self.attrs[u"member_service"] + [
                    options[u"service"]
                ]
            except KeyError:
    ...
    def check_find(self, result, all=False, raw=False):
        ...
        assert_deepequal(
            dict(
                count=1, truncated=False, summary=u"1 group matched", result=[expected],
            ),
            result,
        )
    ...
    def check_detach(self, result):
        ...
        assert_deepequal(
            dict(
                value=self.cn,
                summary=u'Detached group "%s" from user "%s"' % (self.cn, self.cn),
                result=True,
            ),
            result,
        )
