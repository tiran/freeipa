...
class CATracker(Tracker):
    ...
    def __init__(self, name, subject, desc=u"Test generated CA", default_version=None):
        super(CATracker, self).__init__(default_version=default_version)
        self.attrs = {}
        self.ipasubjectdn = subject
        self.description = desc

        self.dn = DN(("cn", name), self.api.env.container_ca, self.api.env.basedn)
    ...
    def make_retrieve_command(self, all=False, raw=False, **options):
        ...
        return self.make_command("ca_show", self.name, all=all, raw=raw, **options)
    ...
    def check_find(self, result, all=False, raw=False):
        ...
        assert_deepequal(
            dict(count=1, truncated=False, summary=u"1 CA matched", result=[expected]),
            result,
        )
