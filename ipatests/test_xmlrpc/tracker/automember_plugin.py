...
class AutomemberTracker(Tracker):
    ...
    retrieve_keys = {
        ...
        u"automemberinclusiveregex",
        u"automemberexclusiveregex",
    ...
    def make_delete_command(self):
        ...
        return self.make_command(
            "automember_del", self.cn, **dict(type=self.membertype)
        )
    ...
    def make_find_command(self, *args, **kwargs):
        ...
        return self.make_command("automember_find", self.cn, type=self.membertype)

    def make_update_command(self, updates):
        """ Make function that updates an automember using 'automember-mod' """
        return self.make_command(
            "automember_mod", self.cn, type=self.membertype, **updates
        )
    ...
    def make_remove_member_command(self, options={}):
        ...
        return self.make_command("automember_remove_member", self.cn, **options)
    ...
    def make_add_condition_command(self, *args, **kwargs):
        ...
        return self.make_command("automember_add_condition", self.cn, *args, **kwargs)
    ...
    def track_create(self):
        ...
        self.attrs = dict(
            dn=self.dn,
            mepmanagedentry=[
                DN(("cn", self.cn), ("cn", "ng"), ("cn", "alt"), api.env.basedn)
            ],
            cn=[self.cn],
            description=[self.description],
    ...
    def add_member(self, options):
        ...
        if u"group" in options:
            try:
                self.attrs[u"group"] = self.attrs[u"group"] + [options[u"group"]]
            except KeyError:
            ...
            try:
                self.attrs[u"hostgroup"] = self.attrs[u"hostgroup"] + [
                    options[u"hostgroup"]
                ]
            except KeyError:
    ...
    def add_condition(self, key, type, inclusiveregex):
        ...
        self.attrs["automemberinclusiveregex"] = [u"%s=%s" % (key, inclusiveregex[0])]
        result = command()
        self.check_add_condition(result)

    def rebuild(self, no_wait=False):
        """ Rebuild automember conditions and check for result """
        command = self.make_rebuild_command(type=self.membertype, no_wait=no_wait)
        result = command()
    ...
    def check_rebuild(self, result, no_wait=False):
        ...
        if no_wait is False:
            assert_deepequal(
                dict(value=None, result=dict(), summary=fuzzy_automember_message),
                result,
            )
        else:
            assert_deepequal(
    ...
    def check_find(self, result, all=False, raw=False):
        ...
        assert_deepequal(
            dict(
                count=1, truncated=False, summary=u"1 rules matched", result=[expected],
            ),
            result,
        )
