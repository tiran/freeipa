...
def admins(request, xmlrpc_setup):
    ...
    tracker = GroupTracker(name=u"admins", description=u"Account administrators group")
    tracker.exists = True
    tracker.track_create()
...
class TestGroup(XMLRPC_test):
    ...
    def test_rename(self, group):
        ...
        command = group.make_command(
            "group_mod", *[group.cn], **dict(setattr=u"cn=%s" % renamedgroup1)
        )
        result = command()
        ...
        command = group.make_command(
            "group_mod", *[group.cn], **dict(setattr=u"cn=%s" % origname)
        )
        result = command()
    ...
    def test_create_with_name_starting_with_numeric(self):
        ...
        testgroup = GroupTracker(
            name=u"1234group", description=u"Group name starting with numeric chars",
        )
        testgroup.create()
        testgroup.delete()
    ...
    def test_create_with_numeric_only_group_name(self):
        ...
        testgroup = GroupTracker(name=u"1234", description=u"Numeric only group name",)
        with raises_exact(
            errors.ValidationError(
                name="group_name",
                error=u"may only include letters, numbers, _, -, . and $",
...
class TestFindGroup(XMLRPC_test):
    ...
    def test_search_for_all_posix(self, group, group2):
        ...
        command = group.make_command("group_find", **dict(posix=True, all=True))
        result = command()
        assert_deepequal(
            dict(
                ...
                result=[
                    {
                        ...
                        "cn": [u"admins"],
                        "description": [u"Account administrators group"],
                        "objectclass": fuzzy_set_ci(
                            add_oc(objectclasses.posixgroup, u"ipantgroupattrs")
                        ),
...
class TestNonexistentGroup(XMLRPC_test):
    def test_retrieve_nonexistent(self, group):
        ...
        with raises_exact(errors.NotFound(reason=u"%s: group not found" % group.cn)):
            command()

    def test_update_nonexistent(self, group):
        """ Try to update a non-existent group """
        group.ensure_missing()
        command = group.make_update_command(updates=dict(description=u"hey"))
        with raises_exact(errors.NotFound(reason=u"%s: group not found" % group.cn)):
            command()
...
class TestNonposixGroup(XMLRPC_test):
    ...
    def test_search_for_all_nonposix(self, group):
        ...
        command = group.make_command("group_find", **dict(nonposix=True, all=True))
        result = command()
        assert_deepequal(
            dict(
...
class TestExternalGroup(XMLRPC_test):
    ...
    def test_search_for_external(self, group):
        ...
        command = group.make_command("group_find", **dict(external=True, all=True))
        result = command()
    ...
    def test_add_external_member_to_external(self, group):
        ...
        command = group.make_command(
            "group_add_member", *[group.cn], **dict(ipaexternalmember=external_sid1)
        )
        try:
            command()
...
class TestValidation(XMLRPC_test):
    ...
    def test_validation_disabled_on_delete(self, group):
        ...
        with raises_exact(
            errors.NotFound(reason=u"%s: group not found" % invalidgroup1)
        ):
            command()
...
class TestManagedGroups(XMLRPC_test):
    ...
    def test_delete_detached_managed(self, managed_group, user):
        ...
        with raises_exact(
            errors.NotFound(reason=u"%s: group not found" % managed_group.cn)
        ):
            command()
        user.ensure_missing()
    ...
    def test_verify_managed_missing_for_user_without_upg(self, user_npg2):
        ...
        with raises_exact(
            errors.NotFound(reason=u"%s: group not found" % user_npg2.uid)
        ):
            command()
...
class TestAdminGroup(XMLRPC_test):
    def test_remove_admin_from_admins(self, admins):
        ...
        command = admins.make_remove_member_command(dict(user=u"admin"))
        with raises_exact(
            errors.LastMemberError(key=u"admin", label=u"group", container=admins.cn)
        ):
            command()
    ...
    def test_remove_all_admins_from_admins(self, admins, user):
        ...
        command = admins.make_remove_member_command(dict(user=[u"admin", user.uid]))
        with raises_exact(
            errors.LastMemberError(key=u"admin", label=u"group", container=admins.cn)
        ):
            command()
    ...
    def test_delete_admins(self, admins):
        ...
        with raises_exact(
            errors.ProtectedEntryError(
                label=u"group", key=admins.cn, reason="privileged group"
            )
        ):
            command()
    ...
    def test_rename_admins(self, admins):
        ...
        command = admins.make_command(
            "group_mod", *[admins.cn], **dict(rename=renamedgroup1)
        )
        with raises_exact(
            errors.ProtectedEntryError(
                label=u"group", key=admins.cn, reason="Cannot be renamed"
            )
        ):
            command()
    ...
    def test_rename_admins_using_setattr(self, admins):
        ...
        command = admins.make_command(
            "group_mod", *[admins.cn], **dict(setattr=u"cn=%s" % renamedgroup1)
        )
        with raises_exact(
            errors.ProtectedEntryError(
                label=u"group", key=admins.cn, reason="Cannot be renamed"
    ...
    def test_update_admins_to_support_external_membership(self, admins):
        ...
        command = admins.make_command("group_mod", *[admins.cn], **dict(external=True))
        with raises_exact(
            errors.ProtectedEntryError(
                label=u"group",
                key=admins.cn,
...
class TestTrustAdminGroup(XMLRPC_test):
    def test_delete_trust_admins(self, trustadmins):
        ...
        with raises_exact(
            errors.ProtectedEntryError(
                label=u"group", key=trustadmins.cn, reason="privileged group"
            )
        ):
            command()
    ...
    def test_rename_trust_admins(self, trustadmins):
        ...
        command = trustadmins.make_command(
            "group_mod", *[trustadmins.cn], **dict(rename=renamedgroup1)
        )
        with raises_exact(
            errors.ProtectedEntryError(
                label=u"group", key=trustadmins.cn, reason="Cannot be renamed"
            )
        ):
            command()
    ...
    def test_rename_trust_admins_using_setattr(self, trustadmins):
        ...
        command = trustadmins.make_command(
            "group_mod", *[trustadmins.cn], **dict(setattr=u"cn=%s" % renamedgroup1)
        )
        with raises_exact(
            errors.ProtectedEntryError(
                label=u"group", key=trustadmins.cn, reason="Cannot be renamed"
    ...
    def test_update_trust_admins_to_support_external_membership(self, trustadmins):
        """ Try to modify the 'trust admins' group to
            support external membership """
        command = trustadmins.make_command(
            "group_mod", *[trustadmins.cn], **dict(external=True)
        )
        with raises_exact(
            errors.ProtectedEntryError(
                label=u"group",
