...
class test_service(Declarative):
    ...
    tests = [
        dict(
            ...
            expected=errors.NotFound(reason=u"%s: service not found" % service1),
        ),
        dict(
        ...
        dict(
            ...
            expected=errors.NotFound(reason=u"%s: service not found" % badservice),
        ),
        dict(
            desc="Create %r" % fqdn1,
            command=(
        ...
        dict(
            ...
            expected=dict(
                ...
                result=dict(
                    ...
                    krbcanonicalname=[u"host/%s@%s" % (fqdn3.lower(), api.env.realm)],
                    objectclass=objectclasses.host,
                    ipauniqueid=[fuzzy_uuid],
        ...
        dict(
            ...
            command=("service_allow_create_keytab", [service1], dict(user=u"admin"),),
            expected=dict(
                completed=1,
                failed=dict(
        ...
        dict(
            ...
            expected=dict(
                ...
                result=dict(
                    ...
                    ipakrbprincipalalias=[service1],
                    krbcanonicalname=[service1],
                    objectclass=objectclasses.service + [u"ipaallowedoperations"],
                    ipauniqueid=[fuzzy_uuid],
                    managedby_host=[fqdn1],
                    has_keytab=False,
                    ipakrbrequirespreauth=True,
        ...
        dict(
            ...
            expected=dict(
                failed=dict(
                    managedby=dict(host=[(u"notfound", u"This entry is not a member")])
                ),
                completed=0,
                result=dict(
                    dn=service1dn,
        ...
        dict(
            ...
            command=("service_mod", [service1], dict(ipakrbauthzdata=[u"MS-PAC"])),
            expected=dict(
                value=service1,
                summary=u'Modified service "%s"' % service1,
                result=dict(
        ...
        dict(
            ...
            command=("service_mod", [badservice], dict(usercertificate=servercert)),
            expected=errors.NotFound(reason=u"%s: service not found" % badservice),
        ),
        dict(
            desc="Try to delete non-existent %r" % service1,
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="canonical_principal", error="Service principal is required"
            ),
        ),
        dict(
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name="principal", error="This principal is required by the IPA master"
            ),
        ),
        dict(
...
class test_service_in_role(Declarative):
    ...
    tests = [
        ...
        dict(
            ...
            command=("role_add_member", [role1], dict(service=service1_no_realm)),
            expected=dict(
                failed=dict(
                    member=dict(host=[], group=[], hostgroup=[], service=[], user=[],),
                ),
                completed=1,
                result=dict(
                    dn=role1_dn,
...
class test_service_allowed_to(Declarative):
    ...
    tests = [
        ...
        dict(
            ...
            command=("group_add_member", [group1], dict(service=[d_service_no_realm])),
            expected=dict(
                completed=1,
                failed=dict(member=dict(group=[], service=[], user=[])),
                result=dict(
                    cn=[group1],
                    gidnumber=[fuzzy_digits],
                    dn=group1_dn,
        ...
        dict(
            ...
            command=("service_allow_retrieve_keytab", [service1], dict(user=user1)),
            expected=dict(
                failed=dict(
        ...
        dict(
            ...
            command=(
                ...
                [service1],
                dict(group=[group1, group2], host=[fqdn1], hostgroup=[hostgroup1]),
            ),
            expected=dict(
                failed=dict(
                    ipaallowedtoperform_read_keys=dict(
        ...
        dict(
            ...
            command=(
                ...
                [service1],
                dict(
                    group=[group1], user=[user1], host=[fqdn1], hostgroup=[hostgroup1]
                ),
            ),
            expected=dict(
                failed=dict(
        ...
        dict(
            ...
            command=("service_disallow_create_keytab", [service1], dict(user=[user2])),
            expected=dict(
                failed=dict(
                    ipaallowedtoperform_write_keys=dict(
                        group=[],
        ...
        dict(
            ...
            command=("service_mod", [service1_no_realm], dict(ipakrbokasdelegate=True)),
            expected=dict(
                value=service1,
                summary=u'Modified service "%s"' % service1,
                result=dict(
...
def indicators_service(request):
    tracker = ServiceTracker(
        name=u"SRV1", host_fqdn=fqdn1, options={u"krbprincipalauthind": u"otp"}
    )
    return tracker.make_fixture(request)
...
class TestAuthenticationIndicators(XMLRPC_test):
    ...
    def test_adding_all_indicators(self, indicators_host, indicators_service):
        ...
        indicators_service.update(
            updates={
                u"krbprincipalauthind": [u"otp", u"radius", u"pkinit", u"hardened"]
            }
        )
...
def managed_service(request):
    tracker = ServiceTracker(name=u"managed-service", host_fqdn=fqdn2)
    return tracker.make_fixture(request)
