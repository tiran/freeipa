...
hostgroup1_dn = DN(
    ("cn", hostgroup1), ("cn", "hostgroups"), ("cn", "accounts"), api.env.basedn
)

host_cert = get_testcert(
    DN(("CN", api.env.host), subject_base()),
...
def lab_host(request, xmlrpc_setup):
    name = u"testhost1"
    tracker = HostTracker(name=name, fqdn=u"%s.lab.%s" % (name, api.env.domain))
    return tracker.make_fixture(request)
...
def this_host(request, xmlrpc_setup):
    ...
    tracker = HostTracker(name=api.env.host.partition(".")[0], fqdn=api.env.host)
    tracker.exists = True
...
class TestNonexistentHost(XMLRPC_test):
    def test_retrieve_nonexistent(self, host):
        ...
        with raises_exact(errors.NotFound(reason=u"%s: host not found" % host.fqdn)):
            command()
...
class TestCRUD(XMLRPC_test):
    ...
    def test_update_simple(self, host):
        host.update(
            dict(description=u"Updated host 1", usercertificate=host_cert),
        ...
        command = host.make_find_command(fqdn=host.fqdn, usercertificate=host_cert)
        res = command()["result"]
        assert len(res) == 1
    ...
    def test_host_find_pkey_only(self, host5):
        ...
        host5.update(
            ...
            expected_updates=dict(
                ...
                has_keytab=False,
                has_password=False,
                krbprincipalname=["host/%s@%s" % (host5.fqdn, api.env.realm)],
                krbcanonicalname=["host/%s@%s" % (host5.fqdn, api.env.realm)],
                managedby_host=[host5.fqdn],
                sshpubkeyfp=[sshpubkeyfp],
            ),
        )
        command = host5.make_find_command(fqdn=host5.fqdn, pkey_only=True)
    ...
    def test_add_mac_address(self, host):
        host.update(
            dict(macaddress=u"00:50:56:30:F6:5F"),
            expected_updates=dict(macaddress=[u"00:50:56:30:F6:5F"]),
        )
        host.retrieve()

    def test_add_mac_addresses(self, host):
        host.update(dict(macaddress=[u"00:50:56:30:F6:5F", u"00:50:56:2C:8D:82"]))
        host.retrieve()

    def test_try_illegal_mac(self, host):
        command = host.make_update_command(updates=dict(macaddress=[u"xx"]))
        with raises_exact(
    ...
    def test_add_ssh_pubkey(self, host):
        host.update(
            dict(ipasshpubkey=[sshpubkey]),
            expected_updates=dict(ipasshpubkey=[sshpubkey], sshpubkeyfp=[sshpubkeyfp],),
        )
        host.retrieve()
    ...
    def test_try_illegal_ssh_pubkey(self, host):
        ...
        with raises_exact(
            errors.ValidationError(name="sshpubkey", error=u"options are not allowed")
        ):
            command()
    ...
    def modify_config_maxhostname(host_tracker, value):
        try:
            command = host_tracker.make_command(
                "config_mod", **dict(setattr=u"ipamaxhostnamelength={}".format(value))
            )
            command()
    ...
    def generate_hostname(total_length, label_len=5):
        ...
        if total_length < 9:
            raise ArithmeticError(
                "Total DNS length in theses tests" "must be at least 9"
            )
    ...
    def test_try_hostname_length_above_maxhostnamelimit(self):
        ...
        testhost = HostTracker(
            name=u"testhost",
            fqdn=u"{}".format(self.generate_hostname(MAXHOSTNAMELEN + 1)),
        )
        self.modify_config_maxhostname(testhost, MAXHOSTNAMELEN)
        with raises_exact(
            errors.ValidationError(
                name=u"hostname",
                error=u"can be at most {} characters".format(MAXHOSTNAMELEN),
            )
        ):
            testhost.create()
    ...
    def test_try_hostname_length_below_maximum(self):
        ...
        testhost = HostTracker(
            name=u"testhost", fqdn=u"{}".format(self.generate_hostname(valid_length))
        )
        self.modify_config_maxhostname(testhost, MAXHOSTNAMELEN)
        testhost.create()
        testhost.ensure_missing()
    ...
    def test_raise_limit_above_and_try_hostname_len_above_limit(self):
        ...
        testhost = HostTracker(
            name=u"testhost",
            fqdn=u"{}".format(self.generate_hostname(MAXHOSTNAMELEN * 3)),
        )
        self.modify_config_maxhostname(testhost, MAXHOSTNAMELEN * 2)
        with raises_exact(
            errors.ValidationError(
                name="hostname",
                error=u"can be at most {} characters".format(MAXHOSTNAMELEN * 2),
            )
        ):
            testhost.create()
            testhost.ensure_missing()
...
class TestHostWithService(XMLRPC_test):
    ...
    def test_host_with_service(self, host):
        ...
        service1dn = DN(
            ("krbprincipalname", service1.lower()),
            ("cn", "services"),
            ("cn", "accounts"),
            host.api.env.basedn,
        )
        ...
        try:
            ...
            assert_deepequal(
                dict(
                    count=0, truncated=False, summary=u"0 services matched", result=[],
                ),
                result,
            )
        finally:
...
class TestManagedHosts(XMLRPC_test):
    ...
    def add_managed_host(self, manager, underling):
        command = manager.make_command(
            "host_add_managedby", underling.fqdn, host=manager.fqdn
        )
        result = command()
    ...
    def search_man_noman_hosts(self, host, noman_host):
        command = host.make_find_command(
            host.fqdn, man_host=host.fqdn, not_man_host=noman_host.fqdn
        )
        result = command()
        assert_deepequal(
            dict(
    ...
    def search_man_hosts(self, host1, host2):
        ...
        result = command()
        assert_deepequal(
            dict(count=0, truncated=False, summary=u"0 hosts matched", result=[],),
            result,
        )

    def remove_man_hosts(self, manager, underling):
        command = manager.make_command(
            "host_remove_managedby", underling.fqdn, host=manager.fqdn
        )
        result = command()
...
class TestValidation(XMLRPC_test):
    ...
    def test_try_validate_update(self, invalid_host):
        ...
        with raises_exact(
            errors.NotFound(reason=u"%s: host not found" % invalid_host.fqdn)
        ):
            command()
...
class TestHostFalsePwdChange(XMLRPC_test):
    def test_join_host(self, host, keytabname):
        ...
        if not os.path.isfile(paths.SBIN_IPA_JOIN):
            pytest.skip(
                "Command '%s' not found. "
                "The test must not run remotely." % paths.SBIN_IPA_JOIN
            )
        ...
        objclass = list(
            set(host.attrs["objectclass"]) - {u"krbprincipal", u"krbprincipalaux"}
        )
        ...
        command = host.make_update_command(updates=dict(userpassword=u"pass_123"))
...
def dns_setup_nonameserver(host4):
    ...
    try:
        host4.run_command("dnsserver_mod", api.env.host, idnsforwardpolicy=u"none")
    except errors.EmptyModlist:
        pass
    ...
    try:
        ...
        try:
            host4.run_command(
                "dnsserver_mod", api.env.host, idnsforwardpolicy=current_fwd_pol
            )
        except errors.EmptyModlist:
            pass
...
def dns_setup(host):
    try:
        host.run_command(
            "dnszone_del", dnszone, revzone, revipv6zone, **{"continue": True}
        )
    except (errors.NotFound, errors.EmptyModlist):
        pass
    ...
    try:
        ...
        host.run_command("dnszone_add", revipv6zone, idnssoarname=dnszone_rname)
        yield
    finally:
        try:
            host.run_command(
...
class TestHostDNS(XMLRPC_test):
    def test_add_ipv6only_host(self, dns_setup, ipv6only_host):
        ipv6only_host.run_command(
            "dnsrecord_add", dnszone, ipv6only_host.shortname, aaaarecord=aaaarec
        )
        try:
            ipv6only_host.create(force=False)
        finally:
            ipv6only_host.run_command(
                "dnsrecord_del", dnszone, ipv6only_host.shortname, aaaarecord=aaaarec
            )

    def test_add_ipv4only_host(self, dns_setup, ipv4only_host):
        ipv4only_host.run_command(
            "dnsrecord_add", dnszone, ipv4only_host.shortname, arecord=arec
        )
        try:
            ipv4only_host.create(force=False)
        finally:
            ipv4only_host.run_command(
                "dnsrecord_del", dnszone, ipv4only_host.shortname, arecord=arec
            )
    ...
    def test_add_ipv4_host_from_ip(self, dns_setup, ipv4_fromip_host):
        ...
        result = ipv4_fromip_host.run_command(
            "dnsrecord_show", dnszone, ipv4_fromip_host.shortname
        )
        assert_deepequal(
            dict(
        ...
        result = ipv4_fromip_host.run_command(
            "dnsrecord_show", revzone, ipv4_fromip_ptr
        )
        assert_deepequal(
            dict(
    ...
    def test_add_ipv6_host_from_ip(self, dns_setup, ipv6_fromip_host):
        ...
        result = ipv6_fromip_host.run_command(
            "dnsrecord_show", dnszone, ipv6_fromip_host.shortname
        )
        assert_deepequal(
            dict(
        ...
        result = ipv6_fromip_host.run_command(
            "dnsrecord_show", revipv6zone, ipv6_fromip_ptr
        )
        assert_deepequal(
            dict(
...
def allowedto_context(request, host3):
    def cleanup():
        try:
            ...
            pass
        try:
            host3.run_command("group_del", group1, group2, **{"continue": True})
        except errors.NotFound:
            pass
        try:
    ...
    host3.run_command("hostgroup_add", hostgroup1, description=u"Test hostgroup 1")
...
class TestHostAllowedTo(XMLRPC_test):
    def test_user_allow_retrieve_keytab(self, allowedto_context, host):
        host.ensure_exists()
        result = host.run_command("host_allow_retrieve_keytab", host.fqdn, user=user1)
    ...
    def test_group_allow_retrieve_keytab(self, allowedto_context, host, host3):
        ...
        result = host.run_command(
            "host_disallow_retrieve_keytab", host.fqdn, user=[user2]
        )
        assert_deepequal(
            dict(
                failed=dict(
        ...
        result = host.run_command(
            "host_disallow_retrieve_keytab", host.fqdn, group=[group2]
        )
    ...
    def test_allow_create(self, allowedto_context, host, host3):
        ...
        result = host.run_command(
            "host_disallow_create_keytab", host.fqdn, user=[user2]
        )
        assert_deepequal(
            dict(
                failed=dict(
        ...
        # Disallow one of the existing allowed groups
        result = host.run_command(
            "host_disallow_create_keytab", host.fqdn, group=[group2]
        )
    ...
    def test_host_mod(self, host):
        ...
        host.update(
            updates=dict(description=u"desc"),
