...
dnszone_1_dn = DN(
    ("idnsname", dnszone_1_absolute), api.env.container_dns, api.env.basedn
)
...
dnszone_2_dn = DN(
    ("idnsname", dnszone_2_absolute), api.env.container_dns, api.env.basedn
)
...
class test_dns_realmdomains_integration(Declarative):
    ...
    tests = [
        dict(
            desc="Check realmdomain and TXT record get created " "during dnszone_add",
            command=("dnszone_add", [dnszone_1], {"idnssoarname": idnssoarname,}),
            expected={
        ...
        dict(
            desc="Check realmdomain and TXT record get deleted " "during dnszone_del",
