...
def generate_user_csr(username, domain=None):
    csr_values = dict(ipadomain=domain if domain else api.env.domain, username=username)
...
def smime_profile(request, xmlrpc_setup):
    profile_path = prepare_config(
        SMIME_PROFILE_TEMPLATE, dict(ipadomain=api.env.domain, iparealm=api.env.realm)
    )

    tracker = CertprofileTracker(
        u"smime", store=True, desc=u"S/MIME certificate profile", profile=profile_path
    )
...
def smime_user(request, xmlrpc_setup):
    ...
    api.Command.user_add(
        uid=username, givenname=u"Alice", sn=u"SMIME", userpassword=SMIME_USER_INIT_PW
    )
...
class TestCertSignMIME(XMLRPC_test):
    ...
    def test_sign_smime_csr(self, smime_profile, smime_user):
        ...
        with change_principal(smime_user, SMIME_USER_PW):
            api.Command.cert_request(
                csr, principal=smime_user, profile_id=smime_profile.name
            )
    ...
    def test_sign_smime_csr_full_principal(self, smime_profile, smime_user):
        ...
        with change_principal(smime_user, SMIME_USER_PW):
            api.Command.cert_request(
                csr, principal=smime_user_principal, profile_id=smime_profile.name
            )
...
class TestSignWithDisabledACL(XMLRPC_test):
    ...
    def test_signing_with_disabled_acl(self, smime_acl, smime_profile, smime_user):
        csr = generate_user_csr(smime_user)
        with change_principal(smime_user, SMIME_USER_PW):
            with pytest.raises(errors.ACIError):
                api.Command.cert_request(
                    csr, profile_id=smime_profile.name, principal=smime_user
                )

    def test_admin_overrides_disabled_acl(self, smime_acl, smime_profile, smime_user):
        csr = generate_user_csr(smime_user)
        api.Command.cert_request(
            csr, profile_id=smime_profile.name, principal=smime_user
        )
...
class TestSignWithoutGroupMembership(XMLRPC_test):
    ...
    def test_signing_with_non_member_principal(
        self, smime_acl, smime_profile, smime_user
    ):
    ...
    def test_admin_overrides_group_membership(
        self, smime_acl, smime_profile, smime_user
    ):
        csr = generate_user_csr(smime_user)
        api.Command.cert_request(
...
class TestCertSignMIMEwithSubCA(XMLRPC_test):
    ...
    def test_sign_smime_csr_fallback_to_default_cert_profile(
        ...
        with pytest.raises(errors.ACIError):
            with change_principal(smime_user, SMIME_USER_PW):
                api.Command.cert_request(
                    csr, principal=smime_user_principal, cacn=smime_signing_ca.name
                )
    ...
    csr = (
        ...
        .add_extension(
            x509.SubjectAlternativeName(
                [x509.DNSName(santest_host_1.name), x509.DNSName(santest_host_2.name)]
            ),
            False,
        )
...
class SubjectAltNameOneServiceBase(XMLRPC_test):
    ...
    def test_prepare_caacl_services(self, santest_subca_acl, santest_service_host_1):
...
class TestNoMatchForSubjectAltNameDnsName(SubjectAltNameOneServiceBase):
    ...
    def test_request_cert_with_not_allowed_SAN(
        self, santest_subca, santest_host_1, santest_service_host_1, santest_csr
    ):
...
class TestPrincipalAliasForSubjectAltNameDnsName(SubjectAltNameOneServiceBase):
    ...
    def test_add_principal_alias(self, santest_service_host_1, santest_service_host_2):
        api.Command.service_add_principal(
            santest_service_host_1.name, santest_service_host_2.name
        )
...
class TestSignServiceCertManagedByMultipleHosts(CAACLEnforcementOnCertBase):
    ...
    def test_make_service_managed_by_each_host(
        self,
        santest_host_1,
        santest_service_host_1,
        santest_host_2,
    ...
    def test_extend_the_ca_acl(
        self, santest_subca_acl, santest_host_2, santest_service_host_2
    ):
...
class TestManagedByACIOnCertRequest(CAACLEnforcementOnCertBase):
    ...
    def test_update_the_caacl(
        self, santest_subca_acl, santest_host_2, santest_service_host_2
    ):
        santest_subca_acl.add_host(santest_host_2.name)
        santest_subca_acl.add_service(santest_service_host_2.name)

    def test_issuing_service_cert_by_unrelated_host(
        self,
        santest_subca,
        santest_host_1,
        santest_host_2,
    ...
    def test_issuing_service_cert_by_related_host(
        self,
        santest_subca,
        santest_host_1,
        santest_host_2,
