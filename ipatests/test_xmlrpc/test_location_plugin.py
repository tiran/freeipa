...
class TestNonexistentIPALocation(XMLRPC_test):
    def test_retrieve_nonexistent(self, location):
        ...
        with raises_exact(
            errors.NotFound(reason=u"%s: location not found" % location.idnsname)
        ):
            command()

    def test_update_nonexistent(self, location):
        location.ensure_missing()
        command = location.make_update_command(updates=dict(description=u"Nope"))
        with raises_exact(
            errors.NotFound(reason=u"%s: location not found" % location.idnsname)
        ):
            command()
...
class TestInvalidIPALocations(XMLRPC_test):
    def test_invalid_name(self, location_invalid):
        ...
        with raises_exact(
            errors.ConversionError(name=u"name", error=u"empty DNS label")
        ):
            command()
    ...
    def test_invalid_absolute(self, location_absolute):
        ...
        with raises_exact(
            errors.ValidationError(name=u"name", error=u"must be relative")
        ):
            command()
...
class TestCRUD(XMLRPC_test):
    def test_create_duplicate(self, location):
        ...
        with raises_exact(
            errors.DuplicateEntry(
                message=u'location with name "%s" already exists' % location.idnsname
            )
        ):
            command()
    ...
    def test_update_simple(self, location):
        location.update(
            dict(description=u"Updated description",),
            expected_updates=dict(description=[u"Updated description"],),
        )
        location.retrieve()
...
class TestLocationsServer(XMLRPC_test):
    messages = [
        {
            u"data": {
                u"service": knownservices.named.systemd_name,
                u"server": api.env.host,
            },
            u"message": (
                u"Service %s requires restart "
                u"on IPA server %s to apply configuration "
                u"changes." % (knownservices.named.systemd_name, api.env.host)
            ),
            u"code": 13025,
            u"type": u"warning",
    ...
    def test_add_location_to_server(self, location, server):
        ...
        server.update(
            ...
            expected_updates={
                u"ipalocation_location": [location.idnsname_obj],
