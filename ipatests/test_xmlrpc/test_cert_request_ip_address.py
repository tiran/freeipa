...
def ipv4_ptr(host, ipv4_revzone):
    yield from _record_setup(host, ipv4_revzone, ipv4_revrec_s, ptrrecord=host_ptr)


@pytest.fixture(scope="class")
def ipv6_ptr(host, ipv6_revzone):
    yield from _record_setup(host, ipv6_revzone, ipv6_revrec_s, ptrrecord=host_ptr)


@pytest.fixture(scope="class")
def ipv4_a(host):
    yield from _record_setup(host, api.env.domain, "iptest", arecord=ipv4_address)


@pytest.fixture(scope="class")
def ipv6_aaaa(host):
    yield from _record_setup(host, api.env.domain, "iptest", aaaarecord=ipv6_address)
...
def other_forward_records(host):
    ...
    yield from _record_setup(
        host, api.env.domain, "other", arecord=ipv4_address, aaaarecord=ipv6_address
    )


@pytest.fixture(scope="function")
def ipv4_ptr_other(host, ipv4_revzone):
    yield from _record_setup(host, ipv4_revzone, ipv4_revrec_s, ptrrecord=other_ptr)


@pytest.fixture(scope="class")
def cname1(host):
    yield from _record_setup(host, api.env.domain, "cname1", cnamerecord="iptest")


@pytest.fixture(scope="class")
def cname2(host):
    yield from _record_setup(host, api.env.domain, "cname2", cnamerecord="cname1")
...
csr_ipv4 = csr(
    [x509.DNSName(host_fqdn), x509.IPAddress(ipaddress.ip_address(ipv4_address)),]
)
csr_ipv6 = csr(
    [x509.DNSName(host_fqdn), x509.IPAddress(ipaddress.ip_address(ipv6_address)),]
)
csr_ipv4_ipv6 = csr(
    [
...
csr_no_dnsname = csr([x509.IPAddress(ipaddress.ip_address(ipv4_address)),])
csr_alice = csr(
    [x509.DNSName(host_fqdn), x509.IPAddress(ipaddress.ip_address(ipv4_address)),],
    cn="alice",
)
...
class TestIPAddressSANIssuance(XMLRPC_test):
    ...
    def test_failure_extra_ip(self, host, ipv4_a, ipv4_ptr, csr_extra_ipv4):
        with pytest.raises(errors.ValidationError, match=PAT_FWD):
            host.run_command("cert_request", csr_extra_ipv4, principal=host_princ)

    def test_failure_no_dnsname(self, host, ipv4_a, ipv4_ptr, csr_no_dnsname):
        with pytest.raises(errors.ValidationError, match=PAT_FWD):
            host.run_command("cert_request", csr_no_dnsname, principal=host_princ)

    def test_failure_user_princ(self, host, ipv4_a, ipv4_ptr, csr_alice, user_alice):
        user_alice.ensure_exists()
        with pytest.raises(errors.ValidationError, match=PAT_USER):
            host.run_command("cert_request", csr_alice, principal=user_alice.uid)
...
class TestIPAddressSANMissingARecord(XMLRPC_test):
    ...
    def test_issuance_ipv4(self, host, ipv6_aaaa, ipv6_ptr, ipv4_ptr, csr_ipv4):
        """Issuing with IPv4 address fails."""
        with pytest.raises(errors.ValidationError, match=PAT_FWD):
            host.run_command("cert_request", csr_ipv4, principal=host_princ)

    def test_issuance_ipv6(self, host, ipv6_aaaa, ipv6_ptr, ipv4_ptr, csr_ipv6):
...
class TestIPAddressSANMissingAAAARecord(XMLRPC_test):
    ...
    def test_issuance_ipv4(self, host, ipv4_a, ipv6_ptr, ipv4_ptr, csr_ipv4):
        """Issuing with IPv4 address suceeds."""
        host.run_command("cert_request", csr_ipv4, principal=host_princ)

    def test_issuance_ipv6(self, host, ipv4_a, ipv6_ptr, ipv4_ptr, csr_ipv6):
        """Issuing with IPv6 address fails."""
        with pytest.raises(errors.ValidationError, match=PAT_FWD):
            host.run_command("cert_request", csr_ipv6, principal=host_princ)

    def test_issuance_ipv4_ipv6(self, host, ipv4_a, ipv4_ptr, ipv6_ptr, csr_ipv4_ipv6):
        """Issuing with IPv4 *and* IPv6 address fails."""
        with pytest.raises(errors.ValidationError, match=PAT_FWD):
            host.run_command("cert_request", csr_ipv4_ipv6, principal=host_princ)
...
class TestIPAddressSANMissingIPv4Ptr(XMLRPC_test):
    ...
    def test_issuance_ipv4(self, host, ipv4_a, ipv6_aaaa, ipv6_ptr, csr_ipv4):
        """Issuing with IPv4 address fails."""
        with pytest.raises(errors.ValidationError, match=PAT_REV):
            host.run_command("cert_request", csr_ipv4, principal=host_princ)

    def test_issuance_ipv6(self, host, ipv4_a, ipv6_aaaa, ipv6_ptr, csr_ipv6):
        """Issuing with IPv6 address succeeds."""
        host.run_command("cert_request", csr_ipv6, principal=host_princ)

    def test_issuance_ipv4_ipv6(self, host, ipv4_a, ipv6_aaaa, ipv6_ptr, csr_ipv4_ipv6):
        """Issuing with IPv4 *and* IPv6 address fails."""
        with pytest.raises(errors.ValidationError, match=PAT_REV):
            host.run_command("cert_request", csr_ipv4_ipv6, principal=host_princ)
...
class TestIPAddressSANMissingIPv6Ptr(XMLRPC_test):
    ...
    def test_issuance_ipv4(self, host, ipv4_a, ipv6_aaaa, ipv4_ptr, csr_ipv4):
        """Issuing with IPv4 address succeeds."""
        host.run_command("cert_request", csr_ipv4, principal=host_princ)

    def test_issuance_ipv6(self, host, ipv4_a, ipv6_aaaa, ipv4_ptr, csr_ipv6):
        """Issuing with IPv6 address fails."""
        with pytest.raises(errors.ValidationError, match=PAT_REV):
            host.run_command("cert_request", csr_ipv6, principal=host_princ)

    def test_issuance_ipv4_ipv6(self, host, ipv4_a, ipv6_aaaa, ipv4_ptr, csr_ipv4_ipv6):
        """Issuing with IPv4 *and* IPv6 address fails."""
        with pytest.raises(errors.ValidationError, match=PAT_REV):
            host.run_command("cert_request", csr_ipv4_ipv6, principal=host_princ)
...
class TestIPAddressPTRLoopback(XMLRPC_test):
    ...
    def test_failure(self, host, ipv4_a, ipv4_ptr_other, csr_iptest_other):
        ...
        with pytest.raises(errors.ValidationError, match=PAT_LOOP):
            host.run_command("cert_request", csr_iptest_other, principal=host_princ)
