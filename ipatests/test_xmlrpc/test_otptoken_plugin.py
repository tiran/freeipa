...
def user(request):
    tracker = UserTracker(
        name=u"user_for_otp_test", givenname=u"Test", sn=u"User for OTP"
    )
...
class TestDeleteLastOtpToken(XMLRPC_test):
    ...
    def test_delete(self, globalCfg, userCfg, allowDelLast, user):
        ...
        try:
            ...
            unlock_principal_password(user.name, user_password, user_password)
            ...
            with change_principal(user.name, user_password):
                api.Command.otptoken_add(
                    u"lastotp", description=u"last otp", ipatokenowner=user.name
                )
                if allowDelLast:
