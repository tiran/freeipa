...
_shell_substitution_regexp = re.compile(r"\$(\s*)([({]?)(\s*)\w+(\s*)([)}]?)", ASCII)
...
def validate_substitutions_match(s1, s2, s1_name="string1", s2_name="string2"):
    ...
    * $(foo)
    ...
    if missing1:
        errors.append(
            "The following substitutions are absent in %s: %s"
    ...
    if pedantic:
        ...
        for sub in common:
            if subs1[sub] != subs2[sub]:
                errors.append(
                    "unequal occurances of '%s', %s has %d occurances, %s has %d occurances"
...
def validate_substitution_syntax(s, s_name="string"):
    ...
    * $(foo)
    ...
    for match in _python_substitution_regexp.finditer(s):
        if match.group(1):
            errors.append(
                "%s has whitespace between %% and key in '%s'"
                % (s_name, match.group(0))
            )
        if match.group(2) or match.group(3):
            errors.append(
                "%s has whitespace next to key in '%s'" % (s_name, match.group(0))
            )
        if match.group(4):
            errors.append(
                "%s has whitespace between key and format character in '%s'"
                % (s_name, match.group(0))
            )
        if not match.group(5):
            errors.append(
                "%s has no format character in '%s'" % (s_name, match.group(0))
            )
    ...
    for match in _shell_substitution_regexp.finditer(s):
        if match.group(1):
            errors.append(
                "%s has whitespace between $ and variable in '%s'"
                % (s_name, match.group(0))
            )
        if match.group(3) or (match.group(4) and match.group(5)):
            errors.append(
                "%s has whitespace next to variable in '%s'" % (s_name, match.group(0))
            )
        ...
        if beg_delimiter is not None or end_delimiter is not None:
            if matched_delimiters[beg_delimiter] != end_delimiter:
                errors.append(
                    "%s variable delimiters do not match in '%s', begin delimiter='%s' end delimiter='%s'"
...
def validate_positional_substitutions(s, prog_langs, s_name="string"):
    ...
    if n_fmts > 1:
        for fmt_parts in fmts:
            ...
            if width == "*":
                errors.append(
                    "Error: * width arg in format '%s should be indexed" % fmt
                )

            if fmt_arg is None:
                if "c" in prog_langs:
                    errors.append(
                        "%s format '%s' is positional, should use indexed argument"
                        % (s_name, fmt)
                    )
                else:
                    errors.append(
                        "%s format '%s' is positional, should use keyword substitution"
                        % (s_name, fmt)
                    )
    ...
    * validate_positional_substitutions()
    ...
    def emit_messages():
        if n_warnings:
            ...
            warning_lines.insert(
                1, "%d validation warnings in %s" % (n_warnings, file_path)
            )
            print("\n".join(warning_lines))
    ...
    if validation_mode == "po":
        ...
        if match:
            ...
            error_lines.append(entry_seperator)
            error_lines.append(
                "%s: does not specify integer nplurals in Plural-Forms header"
                % file_path
            )
            n_errors += 1
    ...
    for entry in po:
        ...
        if validation_mode == "pot":
            ...
            if have_msgid:
                errors = validate_positional_substitutions(
                    entry.msgid, prog_langs, "msgid"
                )
                entry_errors.extend(errors)
            if have_msgid_plural:
                errors = validate_positional_substitutions(
                    entry.msgid_plural, prog_langs, "msgid_plural"
                )
                entry_errors.extend(errors)
        elif validation_mode == "po":
            if have_msgid:
                if have_msgstr:
                    errors = validate_substitutions_match(
                        entry.msgid, entry.msgstr, "msgid", "msgstr"
                    )
                    entry_errors.extend(errors)
                ...
                if have_msgid_plural and have_msgstr:
                    ...
                    if n_plural_forms != n_plurals:
                        entry_errors.append(
                            "%d plural forms specified, but this entry has %d plurals"
                            % (n_plural_forms, n_plurals)
                        )
        ...
        if pedantic:
            ...
            if have_msgid_plural:
                errors = validate_substitution_syntax(
                    entry.msgid_plural, "msgid_plural"
                )
                entry_warnings.extend(errors)

                errors = validate_substitutions_match(
                    entry.msgid, entry.msgid_plural, "msgid", "msgid_plural"
                )
                entry_warnings.extend(errors)
        ...
        if entry_warnings:
            ...
            warning_lines.append(
                "locations: %s"
                % (", ".join(["%s:%d" % (x[0], int(x[1])) for x in entry.occurrences]))
            )
...
def validate_unicode_edit(msgid, msgstr):
    ...
    if msgstr[0] != prefix:
        raise ValueError(
            'First char in translated string "%s" not equal to prefix "%s"'
    ...
    if msgstr[-1] != suffix:
        raise ValueError(
            'Last char in translated string "%s" not equal to suffix "%s"'
    ...
    if msgstr[1:-1] != msgid:
        raise ValueError(
            'Translated string "%s" minus the first & last character is not equal to msgid "%s"'
            % (msgstr.encode("utf-8"), msgid)
        )

    if verbose:
        msg = 'Success: message string "%s" maps to translated string "%s"' % (
            msgid,
            msgstr,
        )
...
def test_translations(po_file, lang, domain, locale_dir):
    ...
    os.environ.update({locale_env: lang for locale_env in locale_envs})
...
def po_file_iterate(po_file, get_msgstr, get_msgstr_plural):
    ...
    if n_fail:
        print(
            "ERROR: %d failures out of %d translations" % (n_fail, n_entries),
            file=sys.stderr,
        )
        return 1

    print(
        "%d translations in %d messages successfully tested"
        % (n_translations, n_entries)
    )
    return 0
...
def main():
    ...
    mode_group = optparse.OptionGroup(
        parser, "Operational Mode", "You must select one these modes to run in"
    )
    ...
    param_group = optparse.OptionGroup(
        parser,
        "Run Time Parameters",
    ...
    if options.mode in ("validate_pot", "validate_po"):
        if options.mode == "validate_pot":
            ...
            print(
                'ERROR: unknown validation mode "%s"' % (options.mode), file=sys.stderr
            )
            return 1
        ...
        for f in files:
            ...
            print(
                "%s: %d entries, %d msgid, %d msgstr, %d warnings %d errors"
                % (
                    f,
                    result.n_entries,
                    result.n_msgids,
