...
class test_pwpolicy(UI_driver):
    ...
    def test_misc(self):
        ...
        self.add_record(
            group.ENTITY,
            [group.DATA, group.DATA2, group.DATA3, group.DATA_SPECIAL_CHAR_GROUP],
        )
        ...
        self.add_record(pwpolicy.ENTITY, pwpolicy.DATA, dialog_btn="add_and_edit")
        ...
        self.add_record(pwpolicy.ENTITY, pwpolicy.DATA_SPECIAL_CHAR, delete=True)
    ...
    def test_negative_value(self):
        ...
        self.add_record(pwpolicy.ENTITY, pwpolicy.DATA1, dialog_btn="add_and_edit")

        for field in FIELDS:
            # bigger than max value
            # verifying if field value is more then 20000
            if field == "krbmaxpwdlife":
                self.check_expected_error(
                    field, "Maximum value is 20000", maximum_value
                )
            # verifying if field value is more then 5
            elif field == "krbpwdmindiffchars":
                self.check_expected_error(field, "Maximum value is 5", maximum_value)
            # verifying if field value is more then 2147483647
            else:
                self.check_expected_error(
                    field, "Maximum value is 2147483647", maximum_value
                )

            # string used instead of integer
            self.check_expected_error(field, non_interger_expected_error, non_integer)

            # smaller than max value
            self.check_expected_error(
                field, minimum_value_expected_error, minimum_value
            )
        self.navigate_to_entity(pwpolicy.ENTITY)
        ...
        self.assert_field_validation(non_interger_expected_error, field=field_priority)
        #  lower bound of data range
        self.fill_input(field_priority, minimum_value)
        self.assert_field_validation(minimum_value_expected_error, field=field_priority)
        # upper bound of data range
        self.fill_input(field_priority, maximum_value)
        self.assert_field_validation(
            expect_error="Maximum value is" " 2147483647", field=field_priority
        )
        self.close_all_dialogs()
    ...
    def test_verify_measurement_unit(self):
        ...
        krbpwdfailurecountinterval = self.get_text(
            'label[name="krbpwdfailurecountinterval"]'
        )
        krbpwdlockoutduration = self.get_text('label[name="krbpwdlockoutduration"]')
