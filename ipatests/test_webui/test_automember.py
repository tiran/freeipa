...
USER_GROUP_DATA = {
    ...
    "add": [("combobox", "cn", USER_GROUP_PKEY),],
    "mod": [("textarea", "description", "user group rule description"),],
}

HOST_GROUP_DATA = {
    "pkey": hostgroup.PKEY,
    "add": [("combobox", "cn", hostgroup.PKEY),],
    "mod": [("textarea", "description", "host group rule description"),],
}
...
SEARCH_CASES = {
    ...
    "negative": ["searc-123", "321", "search 123", "search Short", "description!",],
}
...
class TestAutomember(UI_driver):
    ...
    AUTOMEMBER_RULE_EXISTS_ERROR = 'Automember rule with name "{}" already exists'
    ...
    def add_user_group_rules(self, *pkeys, **kwargs):
        ...
        self.add_record(
            ENTITY,
            [{"pkey": pkey, "add": [("combobox", "cn", pkey)],} for pkey in pkeys],
            facet="searchgroup",
            **kwargs
        )
    ...
    def delete_user_group_rules(self, *pkeys):
        self.delete(ENTITY, [{"pkey": pkey} for pkey in pkeys], facet="searchgroup")

    def delete_host_groups(self, *pkeys):
        self.delete("hostgroup", [{"pkey": pkey} for pkey in pkeys])

    def delete_host_group_rules(self, *pkeys):
        self.delete(ENTITY, [{"pkey": pkey} for pkey in pkeys], facet="searchhostgroup")
    ...
    def open_new_condition_dialog(self, condition_type):
        table = self.find_by_selector(
            "table[name='automember{}regex'].table".format(condition_type), strict=True
        )
    ...
    def test_search_user_group_rule(self):
        ...
        self.mod_record(
            ENTITY,
            {"mod": [("textarea", "description", SEARCH_CASES["description"]),]},
            facet="usergrouprule",
        )
