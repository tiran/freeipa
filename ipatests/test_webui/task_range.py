...
class range_tasks(UI_driver):
    ...
    BASE_RANGE_OVERLAPS_ERROR = (
        "Constraint violation: " "New base range overlaps with existing base range."
    )
    ...
    def get_data(self, pkey=None, form_data=None, **kwargs):
        ...
        data = {
            ...
            "mod": [("textbox", "ipaidrangesize", str(form_data.size + 1)),],
        }
        return data

    def get_add_form_data(
        self, pkey, range_type=LOCAL_ID_RANGE, size=50, domain=None, **kwargs
    ):
        """
...
class RangeAddFormData:
    ...
    def serialize(self):
        ...
        if self.domain:
            serialized.append(("textbox", "ipanttrusteddomainname", self.domain))
        else:
            serialized.append(
                ("textbox", "ipasecondarybaserid", str(self.secondary_base_rid))
            )
