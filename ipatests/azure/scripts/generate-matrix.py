...
parser.add_argument(
    "max_azure_env_jobs", type=int, help="maximum number of Docker envs within VM"
)
...
with open(args.azure_template) as f:
    ...
    for vm in data["vms"]:
        ...
        for job_id, vm_job in enumerate(vm_jobs, 1):
            ...
            jobs[f"ipa_tests_to_ignore_{job_id}"] = " ".join(vm_job.get("ignore", ""))
            jobs[f"ipa_tests_type_{job_id}"] = vm_job.get("type", "integration")
