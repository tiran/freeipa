...
class RedHatTaskNamespace(BaseTaskNamespace):
    ...
    def check_selinux_status(self, restorecon=paths.RESTORECON):
        ...
        if not os.path.exists(restorecon):
            raise RuntimeError(
                "SELinux is enabled but %s does not exist.\n"
    ...
    def detect_container(self):
        ...
        try:
            output = subprocess.check_output(
                [paths.SYSTEMD_DETECT_VIRT, "--container"], stderr=subprocess.STDOUT
            )
    ...
    def reload_systemwide_ca_store(self):
        try:
            ...
            logger.error("Could not update systemwide CA trust database: %s", e)
            return False
        else:
        ...
        with f:
            ...
            for cert, nickname, trusted, _ext_key_usage in ca_certs:
                try:
                    ...
                    logger.error('Failed to decode certificate "%s"', nickname)
                    raise
                ...
                if (
                    cert.extended_key_usage is not None
                    ...
                    try:
                        ...
                        logger.error(
                            'Failed to encode extended key usage for "%s"', nickname
                        )
                        raise
                    ...
                    obj = (
                        ...
                        'value: "%(value)s"\n\n'
                        % dict(
                            label=label, public_key_info=public_key_info, value=value
                        )
                    )
                    f.write(obj)
    ...
    def set_selinux_booleans(self, required_settings, backup_func=None):
        ...
        for setting, state in required_settings.items():
            if state is None:
                continue
            try:
                result = ipautil.run([paths.GETSEBOOL, setting], capture_output=True)
        ...
        if failed_vars:
            raise ipapython.errors.SetseboolError(
                failed=failed_vars, command=" ".join(get_setsebool_args(failed_vars))
            )
    ...
    def configure_httpd_wsgi_conf(self):
        ...
        ipautil.copy_template_file(
            os.path.join(paths.USR_SHARE_IPA_DIR, "ipa-httpd-wsgi.conf.template"),
            conf,
    ...
    def remove_httpd_service_ipa_conf(self):
        ...
        try:
            ...
            if e.errno == errno.ENOENT:
                ...
                logger.error(
                    "Error removing %s: %s", paths.SYSTEMD_SYSTEM_HTTPD_IPA_CONF, e
                )
            return
    ...
    def configure_httpd_protocol(self):
        ...
        directivesetter.set_directive(paths.HTTPD_SSL_CONF, "SSLProtocol", None, False)
    ...
    def setup_httpd_logging(self):
        directivesetter.set_directive(
            paths.HTTPD_SSL_CONF, "ErrorLog", "logs/error_log", False
        )
        directivesetter.set_directive(
            paths.HTTPD_SSL_CONF, "TransferLog", "logs/access_log", False
        )
    ...
    def configure_dns_resolver(self, nameservers, searchdomains, fstore=None):
        ...
        if nm.is_enabled():
            logger.debug(
                "Network Manager is enabled, write %s", paths.NETWORK_MANAGER_IPA_CONF
            )
            ...
            cfg = NM_IPA_CONF.format(
                servers=",".join(nameservers), searches=",".join(searchdomains)
            )
            ...
            logger.debug(
                "Network Manager is not enabled, write %s directly.", paths.RESOLV_CONF
            )
            cfg = [
                "# auto-generated by IPA installer",
    ...
    def configure_pkcs11_modules(self, fstore):
        ...
        for name, module, disabled_in in PKCS11_MODULES:
            filename = os.path.join(
                paths.ETC_PKCS11_MODULES_DIR, "{}.module".format(name)
            )
    ...
    def get_pkcs11_modules(self):
        ...
        """
        return tuple(
            os.path.join(paths.ETC_PKCS11_MODULES_DIR, "{}.module".format(name))
    ...
    def enable_ldap_automount(self, statestore):
        ...
        authselect_cmd = [paths.AUTHSELECT, "enable-feature", "with-custom-automount"]
        ipautil.run(authselect_cmd)
    ...
    def disable_ldap_automount(self, statestore):
        ...
        authselect_cmd = [paths.AUTHSELECT, "disable-feature", "with-custom-automount"]
        ipautil.run(authselect_cmd)
