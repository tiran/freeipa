...
class DebianSysvService(base_services.PlatformService):
    ...
    def stop(self, instance_name="", capture_output=True):
        ipautil.run(
            [paths.SBIN_SERVICE, self.service_name, "stop", instance_name],
    ...
    def start(self, instance_name="", capture_output=True, wait=True):
        ipautil.run(
            [paths.SBIN_SERVICE, self.service_name, "start", instance_name],
            capture_output=capture_output,
        )
    ...
    def restart(self, instance_name="", capture_output=True, wait=True):
        ipautil.run(
            [paths.SBIN_SERVICE, self.service_name, "restart", instance_name],
            capture_output=capture_output,
        )
        if wait and self.is_running(instance_name):
            self.__wait_for_open_ports(instance_name)
    ...
    def is_running(self, instance_name="", wait=True):
        ...
        try:
            result = ipautil.run(
                [paths.SBIN_SERVICE, self.service_name, "status", instance_name],
                capture_output=True,
            )
            sout = result.output
